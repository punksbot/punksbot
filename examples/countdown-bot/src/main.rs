...
const BOT_ABOUT: &str =
    "A tiny non-AI Punks reference bot that replies to !countdown and countdown-style !fib.";
...
impl Config {
    fn from_env() -> Result<Self> {
        let relay_url =
            std::env::var("PUNKS_RELAY_URL").unwrap_or_else(|_| DEFAULT_RELAY_URL.to_string());
        let channel_id = required_env("PUNKS_CHANNEL_ID")?;
        let bot_keys = Keys::parse(&required_env("PUNKS_BOT_PRIVATE_KEY")?)
            .context("PUNKS_BOT_PRIVATE_KEY must be an nsec or hex private key")?;

        let auth_mode =
            std::env::var("PUNKS_BOT_AUTH_MODE").unwrap_or_else(|_| "standalone".to_string());
        let owner_auth_tag = match auth_mode.as_str() {
            "standalone" => None,
            "owner-attested" => {
                let tag_json = match std::env::var("PUNKS_AUTH_TAG") {
                    Ok(value) if !value.trim().is_empty() => value,
                    _ => {
                        let owner_keys = Keys::parse(&required_env("PUNKS_OWNER_PRIVATE_KEY")?)
                            .context("PUNKS_OWNER_PRIVATE_KEY must be an nsec or hex private key")?;
                        punks_sdk::nip_oa::compute_auth_tag(&owner_keys, &bot_keys.public_key(), "")?
                    }
                };

                let owner = punks_sdk::nip_oa::verify_auth_tag(&tag_json, &bot_keys.public_key())
                    .context("PUNKS_AUTH_TAG is not valid for PUNKS_BOT_PRIVATE_KEY")?;
                eprintln!("owner-attested auth tag verified; owner={}", owner.to_hex());
                Some(punks_sdk::nip_oa::parse_auth_tag(&tag_json)?)
            }
            other => {
                bail!("PUNKS_BOT_AUTH_MODE must be 'standalone' or 'owner-attested', got {other:?}")
            }
        };
...
async fn publish_profile(ws: &mut Ws, config: &Config) -> Result<()> {
    let builder = punks_sdk::builders::build_profile(
...
async fn maybe_reply(
    ...
    let builder = punks_sdk::builders::build_message(
