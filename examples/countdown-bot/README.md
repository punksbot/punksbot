...
A tiny non-AI Punks bot example.

The bot is deliberately boring and algorithmic: it listens to one Punks channel
...
It demonstrates that Punks participants do not have to be LLM agents. Any
...
members list and in Punks's mention autocomplete.
...
```bash
PUNKS_RELAY_URL=ws://localhost:3000 \
PUNKS_CHANNEL_ID=<channel-uuid> \
PUNKS_BOT_PRIVATE_KEY=<bot-nsec-or-hex-secret> \
PUNKS_BOT_AUTH_MODE=standalone \
...
relay. This reuses the same owner-attestation credential path that Punks agents
...
PUNKS_CHANNEL_ID=<channel-uuid> \
PUNKS_BOT_PRIVATE_KEY=<bot-nsec-or-hex-secret> \
PUNKS_OWNER_PRIVATE_KEY=<owner-or-agent-nsec-or-hex-secret> \
PUNKS_BOT_AUTH_MODE=owner-attested \
cargo run --manifest-path examples/countdown-bot/Cargo.toml
```
...
```bash
PUNKS_AUTH_TAG='["auth","<owner-pubkey>","","<sig>"]' \
PUNKS_BOT_AUTH_MODE=owner-attested \
# plus PUNKS_RELAY_URL, PUNKS_CHANNEL_ID, PUNKS_BOT_PRIVATE_KEY
cargo run --manifest-path examples/countdown-bot/Cargo.toml
```

Relay requirements for this path:

- `PUNKS_REQUIRE_RELAY_MEMBERSHIP=true` on closed relays.
- `PUNKS_ALLOW_NIP_OA_AUTH=true` so owner-attested non-member bot keys can be
  admitted.
...
- Mention commands require both text like `@Countdown Bot fib 8` and a `p` tag
  for the bot pubkey. The Punks UI adds that tag when the bot is selected from
