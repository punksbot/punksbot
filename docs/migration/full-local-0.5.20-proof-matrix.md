...
| 14 | Premier Bot Punks | runtime ACP et `managed_agents` | Honey puis Fizz démarrés, relus `running`, puis arrêtés proprement dans leurs Workspaces | Chaque clé exacte a été réparée comme membre Workspace `bot` avant spawn ; isolation par URL couverte en TDD | Processus `punks-acp` vivants sans nouvelle erreur de harness ; le journal Fizz conserve l’ancien échec puis un nouveau start réussi | Après relaunch, Honey reste arrêté et les adhésions `bot` persistent ; Fizz finit `Stopped/Offline` | PARTIELLE — Message/réaction, Admission, budgets et Reçu restent à prouver |
...
2. Rejouer Pollen avec un modèle réel ; l’adhésion Workspace des corps locaux
   est réparée et Honey/Fizz démarrent puis s’arrêtent sans nouvelle erreur d’auth.
