...
| 12 | Conversations directes fiables | `local_authority_dms` | DM privé Local Punk ↔ `Codex Proof` créé, plusieurs prompts et réponses réelles échangés | Conversation UUID `fe4a4503…82bf` et trois réponses Agent kind 9 signées/reliées à leurs prompts exacts | `OK`, `Oui, je suis là.` et `Non. Qu’est-ce qui te fait penser ça ?` rendus dans leurs Fils | DM et prompts antérieurs ont survécu aux relaunchs ; relecture des trois réponses après relaunch reste à faire | PARTIELLE — DM humain 2–9 participants, changement d’ensemble et masquage Attention restent à prouver |
...
| 14 | Premier Bot Punks | runtime ACP et `managed_agents` | Honey/Fizz/Pollen exercés ; `Codex Proof` créé avec Codex ACP et a répondu sur trois tours réels | Clé Agent membre `bot`, modèle `gpt-5.6-sol`, réponses kind 9 signées et liées aux prompts | Trace `Session ready`, 145 commandes, Thinking, outils, usage tokens et réponses visibles dans les Fils | Starters persistants après relaunch ; persistance/reprise de `Codex Proof` après relaunch reste à prouver | PARTIELLE — Admission, budgets, Reçu et réaction restent à prouver |
| 15 | Références et broadcasts | autorité de contenu + mentions UI | Mention structurée `Codex Proof` insérée depuis le picker puis auto-mention activée dans le DM | Prompts portent le `p` stable de l’Installation et réveillent le bon corps Codex | Trois wakes live et trois réponses liées aux événements déclencheurs observés | Préférence d’auto-mention et réponses restent à relire après relaunch | PARTIELLE — références humaines, audiences Workspace/here et broadcast autorisé restent à prouver |
...
| 22 | Corps local d’une Installation | sidecars Punks + runtime ACP local | Starters exercés ; `Codex Proof` lance le sidecar Punks, l’adaptateur ACP et le vrai Codex app-server | Adhésion Workspace `bot` et identité `c982a1b2…2892` committées avant spawn | Arbre réel `punks-acp → codex-acp 1.7.0 → codex app-server`, session ACP et tours live observés | Les starters ont été arrêtés sans résidu ; `Codex Proof` reste volontairement online pour les essais utilisateur | PARTIELLE — bail court, révocation et changement de génération restent à prouver |
...
| 26 | Agents avancés | personas, équipes, fournisseurs et modèles locaux | Catalogue Codex `Ready`, modèles live chargés, `Codex Proof` configuré `gpt-5.6-sol` et exercé sur trois tours | Définition/instance, identité, modèle et accès owner-only distincts et persistés | ACP 1.7.0 rapporte session, contexte, 145 outils, thinking, usage et publications signées | Welcome Team persiste ; reprise de session Codex, mémoire, budgets et corps distant restent à prouver | PARTIELLE — modèle local/remote exclusif, mémoire bornée, budgets et versionnement restent à prouver |
