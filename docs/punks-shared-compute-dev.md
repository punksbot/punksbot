# Punks shared compute: local GUI verification

This runbook verifies the actual desktop path used by the built-in **Fizz** agent:

`Punks Desktop → punks-acp → punks-agent → MeshLLM SDK → local/remote compute`

It does not use a substitute agent harness.

## Before starting

Run from the `punksbot/punksbot` repository root on the mesh-enabled branch.
...
`~/.punks-dev`, and local Docker volumes; it preserves the installed Punks app's
data, production keyring, and `~/.punks`. The first dev page load also clears
...
If using `mesh-dev-fresh`, the clean window opens at **Welcome to Punks**. Join
the seeded local community before continuing:

1. Click **Join a community**.
2. Use any local name, such as **Local Punks**.
...
Stop only stale Punks/MeshLLM processes shown by that command. Do not leave a
...
take several minutes. Wait for the Punks window to open and for the terminal to
...
Punks may download the model on first use. The model picker ranks models for the
...
resolves to the bundled `punks-agent`. No API key is required.
...
**punks-agent** as the runtime, **Punks shared compute** as the LLM provider,
...
- no current Punks membership snapshot is available (admission fails closed).

## Security boundary

Punks publishes member-signed discovery notes through an ordinary relay-supported
...
tokens in the note. Current Punks membership controls which node identities are
...
`PUNKS_MESH_IROH_RELAYS` defaults to Iroh's production relay set. Set it to `0`
...
MeshLLM—not the Punks relay—carries inference over direct QUIC or its encrypted
...
is an upstream protocol change and is not claimed by the Punks-side checks above.
