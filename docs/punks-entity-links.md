...
- Slice 0 — HTTPS relay git clone URLs (`{relay-origin}/git/<pubkey>/<repo>`)
  render as Punks repository preview cards in chat
  (`desktop/src/shared/lib/linkPreview.ts`).
- Slice 1 — `punks://pr|issue|repo|project` deep links: `entityLink.ts`
...
- Slice 2 — OS-level deep links: the `repo`/`project`/`pr`/`issue` hosts in
  ...
  clicks.
- Slice 3 (create-command part) — `crates/punks-cli/src/links.rs`, `link`
...
preview card ("GitHub · PR punksbot/punksbot #4020") below the message. Those cards
...
Punks-hosted entities have no equivalent. There is **no link format at all**
for a Punks repository, project, pull request, or issue:

- The only rich deep link today is `punks://message?channel=…&id=…`
...
- `punks pr open` / `punks issues create` return raw event ids; there is no URL
  in their output and no guidance in the agent base prompt
  (`crates/punks-acp/src/base_prompt.md`) for referencing Punks work items in
  chat. Agents can only say "PR up" with a hex id.
- The relay-served web client only has `/repos/$repoId`; no PR/issue pages.

So an agent that opens a PR on a Punks-hosted repository cannot produce
anything clickable, while the same agent opening a GitHub PR gets a card for
free.

## Goals

1. A canonical, shareable link format for Punks repositories, projects, pull
   requests, and issues.
...
- Web (browser) pages for PRs/issues — the web client has no such views yet,
  so links are app-only, same as `punks://message` today.
- Cross-community links. Like `punks://message`, links are interpreted against
...
Extend the existing `punks://` scheme, mirroring `punks://message`:

```
punks://repo?owner=<pubkey-hex>&d=<repo-dtag>[&tab=<tab>]
punks://project?owner=<pubkey-hex>&d=<project-dtag>[&tab=<tab>]
punks://pr?id=<event-id-hex>&owner=<pubkey-hex>&d=<repo-dtag>
punks://issue?id=<event-id-hex>&owner=<pubkey-hex>&d=<repo-dtag>
```
...
the canonical `punks://repo?owner=…&d=…` deep link (the raw transport URL is
...
no such routes), which is why they use the `punks://` scheme above: it is
community-relative by construction, matches the established `punks://message`
...
1. **Autolinked bare URL** (`<punks://pr?…>` or bare in text): render an
   **attachment card** below the message in the existing `AttachmentGroup`,
   exactly like GitHub cards. Provider label `Punks`, type label
   `PR` / `issue` / `repo` / `project`.
2. **Explicitly labeled markdown link** (`[fix the tooltip](punks://pr?…)`):
...
Unlike GitHub (title derived from URL path only), Punks entities live on the
...
`punks://` pattern (new `SupportedLinkPreviewKind` members
`punks-pull-request`, `punks-issue`, `punks-repository`, `punks-project`), or —
...
- `punks pr open` → `{ event_id, accepted, message, link }`
- `punks issues create` → same
- `punks repos create` → link built from owner pubkey + `d`-tag
- `punks projects create` → same

The builder lives in one Rust helper (e.g. `crates/punks-cli/src/links.rs`)
...
`punks pr get` / `punks issues get` / `punks repos get` also include `link` in
their output so agents can link to existing entities, not just ones they
just created.

## Agent guidance

One addition to `crates/punks-acp/src/base_prompt.md`, next to the existing
...
0. **HTTPS clone-URL repo cards** *(done, this branch)* — recognize relay
   `/git/<pubkey>/<repo>` URLs in `linkPreview.ts`, `Punks` provider card
   with the `PunksMark` logo, href normalized to the `punks://repo` deep link
   for in-app navigation.
1. **Link core + cards** *(done, this branch)* — `entityLink.ts`, detection
   in `linkPreview.ts`, `Punks` card variant in
