...
This document specifies the protocol by which Punks Desktop delegates the
...
1. **The provider protocol** — a zero-registration plugin contract between the
   desktop and any executable named `punks-backend-<id>`: discovery, the `info`
...
3. **The Kubernetes binding** — the first conforming provider,
   `punks-backend-kubernetes`, which realizes the contract as a bare Pod
...
launcher among many**. What makes a process a live Punks agent is a keypair,
a NIP-OA auth tag, and a relay URL, handed as environment to the `punks-acp`
...
- **Agent conversational behavior.** What the agent does with events is
  governed by the ACP harness (`punks-acp`) and the NIPs it implements
...
- **Desktop** `D` — the Punks Desktop app. Holds the agent's identity (nsec in
  the OS keyring), its configuration record, and the only UI. Trusted.
- **Provider** `P` — an executable `punks-backend-<id>` on `D`'s machine.
...
- **Substrate** `S` — the remote compute environment `P` deploys into (a
  ...
  `D` never talks to `S`.
- **Agent** `A` — a `punks-acp` harness process (plus the ACP agent under it)
  running on `S`, holding the nsec it was given, connected to the relay.
- **Relay** `R` — the Punks relay. The *only* channel that connects `D` to a
...
harness spawns — `goose`, `claude-agent-acp`, `codex-acp`, `punks-agent`, or
...
1. **The agent/harness contract — binds every launcher.** A live Punks agent
   is a `punks-acp` process holding a keypair, a NIP-OA auth tag (or resolved
   ...
   restart** (I5). A bash script that exports `PUNKS_PRIVATE_KEY`,
   `PUNKS_RELAY_URL`, `PUNKS_AUTH_TAG` and execs the harness is a conforming
...
- **(I3) Presence is the status.** `D` derives a remote agent's live state
  ...
  (`PRESENCE_TTL_SECS`, `punks-pubsub/src/presence.rs:16`; the vision's
  ...
  presence-suppression knob, `PUNKS_ACP_NO_PRESENCE`, MUST join
...
`punks-backend-<id>`. The suffix after the prefix is the provider id and MUST
...
`punks-backend-kubernetes` will ask. (At `28ae6cd21` discovery records only
...
`PUNKS_PRIVATE_KEY`, `NOSTR_PRIVATE_KEY`, `PUNKS_AUTH_TAG`, `PUNKS_RELAY_URL`,
...
payload fields (`private_key_nsec` → `PUNKS_PRIVATE_KEY`/`NOSTR_PRIVATE_KEY`,
`auth_tag` → `PUNKS_AUTH_TAG`, `relay_url` → `PUNKS_RELAY_URL`); reading
...
POSIX-shaped names before merge, because a key like `PUNKS_AUTH_TAG=x`
...
"launch": {
  ...
  "policy_env":   {str: str},   // overridable behavior defaults (tier 1, below):
                                ...
                                // PUNKS_ACP_RELAY_OBSERVER, PUNKS_ACP_LAZY_POOL=true,
                                // PUNKS_ACP_SESSION_TITLE (resolved),
                                // PUNKS_ACP_TEAM_INSTRUCTIONS, PUNKS_ACP_MODEL,
                                // MCP_HOOK_SERVERS=* (mcp_hooks runtimes only)
  "owner_pubkey": str | null    // resolved workspace owner (hex) — legacy
                                // PUNKS_ACP_AGENT_OWNER fallback, non-secret
}
```
...
Claude; `PUNKS_AGENT_MODEL`/`PUNKS_AGENT_PROVIDER` for punks-agent). A fixed
`provider → PUNKS_AGENT_PROVIDER` mapping is wrong for three of the four
...
- `PUNKS_ACP_TEAM_INSTRUCTIONS` — the only truly non-reconstructible policy
  value: `effective_team_instructions` (`spawn_hash.rs:41-52`) needs the
  desktop's `TeamRecord` store, which no pod can reach.
- `PUNKS_ACP_SESSION_TITLE` — sent **resolved** (`resolve_session_title`,
...
- `PUNKS_ACP_LAZY_POOL=true` — a **deliberate pick, not a transcription**:
...
- `MCP_HOOK_SERVERS=*` when the resolved runtime has `mcp_hooks`
  (`runtime.rs:594-598`; punks-agent only at `28ae6cd21`) — gates the
  `_Stop`/`_PostCompact` hook tools.
- `PUNKS_ACP_SYSTEM_PROMPT`, `PUNKS_ACP_IDLE_TIMEOUT`,
  `PUNKS_ACP_MAX_TURN_DURATION`, `PUNKS_ACP_AGENTS` — resolved by the desktop
  from the record's `system_prompt` / `idle_timeout_seconds` /
  `max_turn_duration_seconds` / `parallelism` (each omitted when null,
  matching the local spawn's conditional emission). `PUNKS_ACP_AGENTS` is
  ...
  are **tier-1 control-plane** keys: `PUNKS_ACP_AGENTS` is in
...
`PUNKS_ACP_DEDUP` and `PUNKS_ACP_MULTIPLE_EVENT_HANDLING` are **deliberately
...
imply a divergence that does not exist. `PUNKS_MANAGED_AGENT` is likewise
...
1. **Overridable behavior defaults** — `launch.policy_env`. Most keys here
   ...
   **Exception — `PUNKS_ACP_AGENTS`:** this key IS reserved
...
3. **Authoritative** — unoverridable at every layer, written last and
   ...
   `PUNKS_ACP_AGENT_OWNER`, the inactivity bound, `PUNKS_ACP_MCP_COMMAND`,
   and `PUNKS_MANAGED_AGENT_START_NONCE`. For the nonce, the provider MUST
   set it to the attempt's **generation token** (§K8s Secrets): the harness
   stamps it into every observer lifecycle frame (`punks-acp/lib.rs:1501`),
...
- the harness and agent binaries: `launch.command` is a *name*, resolved
  against the image's own `PATH` (`PUNKS_ACP_AGENT_COMMAND`), and
  `PUNKS_ACP_MCP_COMMAND=punks-dev-mcp` likewise;
...
- `PUNKS_ACP_SETUP_PAYLOAD` is desktop-computed readiness state and MUST NOT
  appear in a remote pod.

**Owner resolution (normative):** the provider MUST have either a non-null
`auth_tag` (→ `PUNKS_AUTH_TAG`) or a non-null `launch.owner_pubkey`
(→ `PUNKS_ACP_AGENT_OWNER`) before any mutation; if both are null it MUST
refuse the deploy. Without an owner the harness cannot match `!shutdown`
(`punks-acp/src/lib.rs: resolve_agent_owner`, main-loop owner check) and the
agent answers its own stop command conversationally — §Stop would be
describing a mechanism that does not work. `PUNKS_ACP_AGENT_OWNER` is a
reserved key, so this value can only arrive as authoritative launch data,
never through user env.

**Punks shared compute (relay-mesh) is non-deployable, and this is forced,
...
**Auto-repair is fenced to Punks-authored, positively identified residue
...
management marker — `app.kubernetes.io/managed-by: punks-backend-kubernetes`
...
is substrate-operator residue survives with one qualifier: *Punks-authored*
...
`punks.block.xyz/create-intent`, written at pod create — the same shape as
...
```
--exit-after-inactivity <secs>   /   PUNKS_ACP_EXIT_AFTER_INACTIVITY
```
...
- **Mechanism**: on expiry of the bound, the harness fires the same shutdown
  ...
  **The expiry check MUST NOT depend on pool readiness.** This is a design
  ...
  `PUNKS_ACP_LAZY_POOL=true` (§Launch data) into a deadlock in I5's single
...
- **Reserved keys**: `PUNKS_ACP_EXIT_AFTER_INACTIVITY` MUST join
  ...
  door. `PUNKS_ACP_NO_PRESENCE` (`config.rs:378`) MUST join in the same
...
## The Kubernetes Binding (`punks-backend-kubernetes`)

The first conforming provider: a Rust crate in `punksbot/punksbot`, distributed as a
...
generated `punks-agents-<rand6>` as the `namespace` field's schema *default*
...
`ghcr.io/punksbot/punksbot-sprig`: Alpine base + `bash` (required by the dev-MCP
shell tool) + `git` + CA certificates + the static musl `sprig` multicall
binary with its personality links (`punks-acp`, `punks-agent`, `punks-dev-mcp`,
`rg`, `tree`, `punks`, `git-credential-nostr`, `git-sign-nostr`) + a baked
...
`ghcr.io/punksbot/punksbot-sprig@sha256:<that-digest>` — a `sha-<git-sha>` *tag*
...
**An image override MUST contain the runtime ABI** — the `punks-acp`
entrypoint and everything §Entrypoint and launch ABI requires — not merely
alternate-harness dependencies. A conforming custom image is "punks-sprig
...
exec punks-acp   # exec, not a call — punks-acp must be PID 1
```

`bash -c "setup && punks-acp"` (no `exec`) is non-conforming: bash becomes
...
| source | env var |
|---|---|
| `relay_url` | `PUNKS_RELAY_URL` |
| `private_key_nsec` | `PUNKS_PRIVATE_KEY` and `NOSTR_PRIVATE_KEY` (the git helpers read the latter) |
| `auth_tag` | `PUNKS_AUTH_TAG` (omitted when null; then `launch.owner_pubkey` → `PUNKS_ACP_AGENT_OWNER` is REQUIRED — §Launch data owner rule) |
| `launch.command` | `PUNKS_ACP_AGENT_COMMAND` — the *name*, resolved against the image's own PATH; never a forwarded host path |
| `launch.args` | `PUNKS_ACP_AGENT_ARGS`, comma-joined |
| `launch.env`, `launch.policy_env` | verbatim, at their precedence tiers |
| generation token (§K8s Secrets) | `PUNKS_MANAGED_AGENT_START_NONCE` — the lifecycle-frame correlator and the Secret generation are one identity (§Launch data tier 3) |
| `system_prompt`, `idle_timeout_seconds`, `max_turn_duration_seconds`, `parallelism` | **not mapped by the provider** — the desktop resolves these into `launch.policy_env` (`PUNKS_ACP_SYSTEM_PROMPT`, `PUNKS_ACP_IDLE_TIMEOUT`, `PUNKS_ACP_MAX_TURN_DURATION`, `PUNKS_ACP_AGENTS`). `PUNKS_ACP_AGENTS` carries the **effective** parallelism (`min(record.parallelism, harness_cap)`), is reserved (`env_vars.rs:RESERVED_ENV_KEYS`), and cannot be overridden by user env. The remaining knobs are tier-1 by local fact (written before user env); a provider that mapped the top-level copies after `launch.env` would silently defeat local overrides. The top-level fields remain as display/bookkeeping inputs only |
| `turn_timeout_seconds` | not mapped — deprecated upstream and ignored; the local spawn also does not emit it |
| `respond_to` | `PUNKS_ACP_RESPOND_TO` |
| `respond_to_allowlist` | `PUNKS_ACP_RESPOND_TO_ALLOWLIST`, comma-joined |
| — | `PUNKS_ACP_MCP_COMMAND=punks-dev-mcp` (image-local; the dev-MCP requirement) |
| `provider_config.inactivity_seconds` | `PUNKS_ACP_EXIT_AFTER_INACTIVITY` (schema default 7200; the I5 opt-in, §Auto-Stop — the config field and this env var are one knob, not two) |
...
(per-runtime vars, `provider_locked` suppression, `PUNKS_ACP_MODEL`) arrives
...
**Encoding honesty note.** `PUNKS_ACP_AGENT_ARGS` is comma-delimited by the
...
- **Naming/labeling — the exact contract** (63-char label-value limit; a hex
  ...
  - pod name: `punks-agent-<first-12-hex-of-pubkey>` — also the returned
    `agent_id`
  - label `punks.block.xyz/agent-pubkey: <first-32-hex>` — the selector key
    ...
    not decorative
  - label `app.kubernetes.io/managed-by: punks-backend-kubernetes` and label
    `punks.block.xyz/binding-version: <schema-version>` — the **management
  ...
  - annotation `punks.block.xyz/agent-pubkey-full: <full-64-hex>` —
  ...
  - annotation `punks.block.xyz/create-intent: <sha256-of-intent-template>` — the
    ...
    never-started pods
  - Secret name: `punks-agent-<first-12-hex>-<gen>`, where `<gen>` is a random
...
(`punks-agent-<first-12-hex>-<gen>`, §Pod shape). The rationale is a
...
`PUNKS_MANAGED_AGENT_START_NONCE`, so lifecycle frames and pod logs share a
...
2. It does not suppress the harness's promises on a remote agent:
   presence stays enabled (`PUNKS_ACP_NO_PRESENCE` never set — remotely,
   presence is the only signal, I3), and the inactivity knob
   (`PUNKS_ACP_EXIT_AFTER_INACTIVITY`) carries the owner's *deliberate*
...
3. **Deploy payload bypasses the launch resolver** (the prerequisite this
   ...
   policy (`PUNKS_ACP_RELAY_OBSERVER`, runtime `default_env` such as
   ...
   (`PUNKS_ACP_DEDUP`/`PUNKS_ACP_MULTIPLE_EVENT_HANDLING` are *not* on this
...
4. **The I5 reaper does not exist, and its natural home is a trap**
   (harness code prerequisite). `PUNKS_ACP_EXIT_AFTER_INACTIVITY` appears
...
| Presence publish / offline-on-exit | `crates/punks-acp/src/lib.rs` (`publish_presence`, shutdown path) |
| `!shutdown` owner check | `crates/punks-acp/src/lib.rs` (main loop) |
| Graceful shutdown path (budget enforcement *to be added* — Known Defect 7) | `crates/punks-acp/src/lib.rs` (pool shutdown, then drain / reap / presence / relay close) |
| Clean-exit exit-code contract | *to be added*: `crates/punks-acp` distinguished exit codes + pinning test (Known Defect 6; gates `OnFailure`) |
| Auto-stop flag | *to be added*: `crates/punks-acp/src/config.rs` + a pool-independent timer (NOT the `pool_ready`-gated maintenance tick — Known Defect 4) + `RESERVED_ENV_KEYS` entry |
| Kubernetes binding | *to be added*: `crates/punks-backend-kubernetes` |
...
- **G. Remote override semantics** — the spec keeps local semantics: user
  env continues to beat Punks behavior defaults remotely (three-tier
...
- **I. Never-started escape hatch** — the create-intent fingerprint
  ...
  to Punks-authored, positively identified residue** (§Deploy State Machine
...
Remote agents extend Punks's managed-agent model across a deliberately thin
