...
#
# Public Punks relay image — published as ghcr.io/punksbot/punksbot:<tag>.
#
# Builds the `punks-relay` binary (Rust 1.95) and the `punks-web` static bundle
...
# / upload-pack — see crates/punks-relay/src/api/git).
#
...
# scoping to -p punks-relay misses transitive deps and re-builds them later.
RUN cargo chef cook --release --recipe-path recipe.json
COPY . .
RUN cargo build --release --locked -p punks-relay --bin punks-relay \
                                   -p punks-admin --bin punks-admin \
                                   -p punks-pair-relay --bin punks-pair-relay
...
RUN strip target/release/punks-relay \
    && strip target/release/punks-admin \
    && strip target/release/punks-pair-relay
...
RUN pnpm install --frozen-lockfile --filter punks-web --filter punks-admin-web
COPY web/ web/
...
LABEL org.opencontainers.image.title="Punks" \
      org.opencontainers.image.description="WebSocket relay server for the Punks communications platform" \
      org.opencontainers.image.source="https://github.com/punksbot/punksbot" \
      org.opencontainers.image.url="https://github.com/punksbot/punksbot" \
      org.opencontainers.image.documentation="https://github.com/punksbot/punksbot#readme" \
...
RUN apt-get update \
    && apt-get install -y --no-install-recommends \
        ...
        openssl \
    && rm -rf /var/lib/apt/lists/* \
    && groupadd --system --gid 1000 punks \
    && useradd  --system --uid 1000 --gid 1000 --home-dir /var/lib/punks \
                --create-home --shell /usr/sbin/nologin punks

COPY --from=web-builder /build/web/dist                 /srv/punks/web
COPY --from=web-builder /build/admin-web/dist           /srv/punks/admin-web

# The invite landing page is always served from the bundled web UI. Repository
# browser routes require the separate PUNKS_SERVE_GIT_WEB_GUI=true opt-in. The
# admin bundle is inert until PUNKS_ADMIN_HOST is configured.
ENV PUNKS_WEB_DIR=/srv/punks/web \
    PUNKS_ADMIN_WEB_DIR=/srv/punks/admin-web

# 3000: app (WS + REST)  ·  8080: /_liveness, /_readiness  ·  9102: /metrics
EXPOSE 3000 8080 9102

# deploy/compose mounts a volume here; pre-created so it inherits punks:punks.
RUN mkdir -p /data/git && chown punks:punks /data/git

USER punks:punks
WORKDIR /var/lib/punks

ENTRYPOINT ["/usr/local/bin/punks-relay"]
...
COPY --from=builder /build/target/release/punks-relay /usr/local/bin/punks-relay
COPY --from=builder /build/target/release/punks-admin /usr/local/bin/punks-admin
COPY --from=builder /build/target/release/punks-pair-relay /usr/local/bin/punks-pair-relay
...
COPY --from=stripped-binaries /build/target/release/punks-relay /usr/local/bin/punks-relay
COPY --from=stripped-binaries /build/target/release/punks-admin /usr/local/bin/punks-admin
COPY --from=stripped-binaries /build/target/release/punks-pair-relay /usr/local/bin/punks-pair-relay
