...
: "${PUNKS_K8S_TEST_CONTEXT:?set the explicit disposable/local kubectl context}"
: "${PUNKS_SPRIG_IMAGE:?set an immutable image reference (name@sha256:<64 hex>)}"

if [[ ! "$PUNKS_SPRIG_IMAGE" =~ ^[^[:space:]@]+@sha256:[0-9a-fA-F]{64}$ ]]; then
    echo "error: PUNKS_SPRIG_IMAGE must be name@sha256:<64 hex>" >&2
    exit 2
fi

CONTEXT="$PUNKS_K8S_TEST_CONTEXT"
IMAGE="$PUNKS_SPRIG_IMAGE"
PULL_POLICY="${PUNKS_K8S_TEST_PULL_POLICY:-IfNotPresent}"
case "$PULL_POLICY" in
    Always|IfNotPresent|Never) ;;
    *) echo "error: invalid PUNKS_K8S_TEST_PULL_POLICY: $PULL_POLICY" >&2; exit 2 ;;
esac

MANAGED_BY="punks-backend-kubernetes"
BINDING_VERSION="v1"
NAMESPACE="punks-k8s-sprig-$(date +%s)-$RANDOM"
CREATED=0
...
cleanup() {
    ...
    binding="$(kubectl --context "$CONTEXT" get namespace "$NAMESPACE" \
        -o jsonpath='{.metadata.labels.punks\.block\.xyz/binding-version}' 2>/dev/null || true)"
    ...
    foreign="$(kubectl --context "$CONTEXT" --namespace "$NAMESPACE" get pods -o json \
        | jq '[.items[] | select(.metadata.labels["app.kubernetes.io/managed-by"] != "punks-backend-kubernetes" or .metadata.labels["punks.block.xyz/binding-version"] != "v1")] | length')"
    if [[ "$foreign" != 0 ]]; then
...
kubectl --context "$CONTEXT" label namespace "$NAMESPACE" \
    ...
    "punks.block.xyz/binding-version=$BINDING_VERSION"
...
spec:
  ...
  containers:
    - name: probe
      ...
      args:
        - 'test "\$(readlink /usr/local/bin/punks-acp)" = sprig; echo DIGEST_ABI_OK'
YAML
