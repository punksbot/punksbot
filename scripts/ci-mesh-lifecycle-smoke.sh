...
# =============================================================================
# Provisions a membership-gated punks-relay and runs the full relay-driven
# mesh lifecycle harness (crates/punks-relay/examples/mesh_relay_lifecycle_smoke.rs):
...
# CPU model, real multi-process mesh), but with the Punks relay as the control
...
if [[ "${SKIP_BUILD}" == "true" ]]; then
  ...
  cargo build --profile "${CARGO_PROFILE}" -p punks-relay -p punks-admin -p git-credential-nostr
  cargo build --profile "${CARGO_PROFILE}" -p punks-relay --example mesh_relay_lifecycle_smoke
fi

ADMIN_BIN="target/${CARGO_PROFILE}/punks-admin"
HARNESS_BIN="target/${CARGO_PROFILE}/examples/mesh_relay_lifecycle_smoke"
for bin in "${ADMIN_BIN}" "${HARNESS_BIN}" "target/${CARGO_PROFILE}/punks-relay"; do
  if [[ ! -x "${bin}" ]]; then
...
PUNKS_RELAY_PRIVATE_KEY="$(echo "${SIGNER_OUT}" | awk '/Secret key:/ {print $3}')"
if [[ -z "${RELAY_OWNER_PUBKEY}" || -z "${PUNKS_RELAY_PRIVATE_KEY}" ]]; then
  err "Failed to generate relay identities via punks-admin generate-key"
  exit 1
fi
export PUNKS_REQUIRE_RELAY_MEMBERSHIP=true
export RELAY_OWNER_PUBKEY
export PUNKS_RELAY_PRIVATE_KEY
...
cleanup() {
  log "Stopping relay..."
  if [[ -f /tmp/punks-relay.pid ]]; then
    kill "$(cat /tmp/punks-relay.pid)" 2>/dev/null || true
  fi
