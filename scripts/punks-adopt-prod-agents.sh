#!/usr/bin/env bash
#
# punks-adopt-prod-agents.sh — copy your installed (production) Punks agent
...
#   every punks-desktop-dev keychain entry, and the `_dev_migration_v1` marker.
...
#   reset wipes that Punks cannot re-derive on its own: the agent RECORDS and
...
#
# WHAT THIS DOES NOT DO (verified against punksbot/punksbot origin/main)
#   - It does NOT touch the punks-desktop-dev keychain. Agent private keys are
...
#   PUNKS_KEYCHAIN_SVC   keychain service    (default punks-desktop)
#   PUNKS_KEYCHAIN_ACCT  keychain account    (default secrets)
#
set -euo pipefail
...
PROD_DIR="$SUPPORT/xyz.block.punks.app"
DEV_DIR="$SUPPORT/xyz.block.punks.app.dev"
KEYCHAIN_SVC="${PUNKS_KEYCHAIN_SVC:-punks-desktop}"
KEYCHAIN_ACCT="${PUNKS_KEYCHAIN_ACCT:-secrets}"
...
#     `punks-desktop` for both the installed DMG
#     (/Applications/Punks.app/Contents/MacOS/punks-desktop) and dev builds
#     (target/<profile>/punks-desktop via `tauri dev`). Match that path component
#     exactly so sidecars/helpers (punks, punks-dev-mcp, punks-agent) don't
#     false-positive.
#
...
#     state) blocks.
ALLOWED_PROD_EXE="/Applications/Punks.app/Contents/MacOS/punks-desktop"
...
running_pids="$(pgrep -f '/punks-desktop( |$)' 2>/dev/null || true)"
dmg_running=0
...
if [[ ${#dev_blocking[@]} -gt 0 ]]; then
  warn "A non-installed punks-desktop process is running (dev build or unresolvable):"
...
if [[ $dmg_running -eq 1 ]]; then
  ...
  info "no running Punks process detected"
fi
...
cat <<EOF
  1. Start your dev build (\`just production\` or \`just dev\`).
     - First boot re-runs the agent-key migration → ONE prod-keychain prompt
       (copies agent:<pubkey> keys punks-desktop → punks-desktop-dev).
  ...
  2. Worktree launches: worktree-suffixed dev dirs are symlinked to the
     canonical dev dir by sync_shared_agent_data ONLY when PUNKS_SHARE_IDENTITY=1.
     If you launch from a worktree, export PUNKS_SHARE_IDENTITY=1 (and
     PUNKS_PRIVATE_KEY) or the worktree will mint its own duplicate agents.
EOF
if [[ "${PUNKS_SHARE_IDENTITY:-}" != "1" ]]; then
  say "  NOTE: PUNKS_SHARE_IDENTITY is not set in this shell — set it for worktree launches."
fi
exit 0
