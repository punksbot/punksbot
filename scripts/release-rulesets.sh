...
require_canonical_repository() {
  ...
  origin_url="$(git config --get remote.origin.url 2>/dev/null)" || \
    fail_release_ruleset "origin is required and must point to punksbot/punksbot" || return 1
  case "$origin_url" in
    git@github.com:punksbot/punksbot.git|ssh://git@github.com/punksbot/punksbot.git|https://github.com/punksbot/punksbot.git|https://github.com/punksbot/punksbot)
      ;;
    *)
      fail_release_ruleset "origin must point to canonical punksbot/punksbot, not '$origin_url'" || return 1
      ;;
  esac
}
...
require_release_tag_ruleset() {
  ...
  ruleset_endpoint="repos/punksbot/punksbot/rulesets/$RELEASE_TAG_RULESET_ID"
