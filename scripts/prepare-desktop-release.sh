...
scripts/desktop_release.py generate "$version" --base "$base_sha" --repo punksbot/punksbot
...
trap 'rm -f "$msg"' EXIT
cat >"$msg" <<EOF
chore(release): release Punks Desktop version $version

Co-authored-by: $agent_name <$agent_email>
EOF
git -c user.name='Wes' -c user.email='wesbillman@users.noreply.github.com' \
  commit -s -F "$msg"
scripts/desktop_release.py validate --candidate HEAD --version "$version" --repo punksbot/punksbot
...
cat >"$body" <<EOF
## Punks Desktop release v$version
...
EOF
if existing="$(gh pr list --repo punksbot/punksbot --head "$branch" --state open --json number --jq '.[0].number')" && [[ -n "$existing" ]]; then
  gh pr edit --repo punksbot/punksbot "$existing" --title "chore(release): release Punks Desktop version $version" --body-file "$body"
else
  gh pr create --repo punksbot/punksbot --base main --head "$branch" \
    --title "chore(release): release Punks Desktop version $version" --body-file "$body"
fi
