...
#   PUNKS_VITE_PORT, PUNKS_HMR_PORT, VITE_PORT, VITE_HMR_PORT
#   PUNKS_RELAY_PORT, PUNKS_RELAY_URL
#   PUNKS_INSTANCE_SLUG, PUNKS_WORKTREE_LABEL, VITE_DEV_BRANCH (worktrees only)
#   PUNKS_TAURI_CONFIG
#   PUNKS_PRIVATE_KEY (worktrees only, when PUNKS_SHARE_IDENTITY=1)
...
export PUNKS_VITE_PORT=$BASE_PORT
export PUNKS_HMR_PORT=$((BASE_PORT + 1))
export PUNKS_RELAY_PORT=3000
export VITE_PORT="$PUNKS_VITE_PORT"
export VITE_HMR_PORT="$PUNKS_HMR_PORT"
export PUNKS_RELAY_URL="${PUNKS_RELAY_URL:-ws://localhost:3000}"

DEV_URL="http://localhost:${PUNKS_VITE_PORT}"
if [[ "${PUNKS_RESET_WEBVIEW_STATE:-0}" == "1" ]]; then
    DEV_URL="${DEV_URL}?resetDevState=1"
fi

PUNKS_TAURI_CONFIG="{\"build\":{\"devUrl\":\"${DEV_URL}\",\"beforeDevCommand\":\"exec ./node_modules/.bin/vite --port ${PUNKS_VITE_PORT} --strictPort\"},\"identifier\":\"xyz.block.punks.app.dev\",\"productName\":\"Punks Dev\"}"
...
if git rev-parse --is-inside-work-tree &>/dev/null; then
    ...
    if [[ -n "$GIT_COMMON_DIR" && "$GIT_DIR" != "$GIT_COMMON_DIR" ]]; then
        ...
        export PUNKS_WORKTREE_LABEL="${BRANCH_NAME##*/}"
        export PUNKS_INSTANCE_SLUG=$(echo "$BRANCH_NAME" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9]/-/g' | sed 's/--*/-/g' | sed 's/^-//' | sed 's/-$//')

        # PUNKS_SHARE_IDENTITY=1: reuse the main dev checkout's Nostr key so
        ...
        if [[ "${PUNKS_SHARE_IDENTITY:-0}" == "1" ]]; then
            KEYRING_SERVICE="punks-desktop-dev"
            KEYRING_BLOB=""
            case "$(uname -s)" in
                Darwin)
            ...
            CANONICAL_KEY="$HOME/Library/Application Support/xyz.block.punks.app.dev/identity.key"
            ...
            if [[ -n "$SHARED_IDENTITY" ]]; then
                export PUNKS_PRIVATE_KEY="$SHARED_IDENTITY"
            else
                echo "⚠ PUNKS_SHARE_IDENTITY=1 but no identity found in keyring service $KEYRING_SERVICE, at $CANONICAL_KEY, or at $LEGACY_CANONICAL_KEY — run Punks from repo root first" >&2
            fi
        fi
        ...
        if swift "$GENERATE_DEV_ICON" "$BASE_ICON" "$DEV_ICON" "$PUNKS_WORKTREE_LABEL"; then
            echo "🌳 Worktree: ${PUNKS_WORKTREE_LABEL}"
            export VITE_DEV_BRANCH="$PUNKS_WORKTREE_LABEL"
            PUNKS_TAURI_CONFIG="{\"build\":{\"devUrl\":\"${DEV_URL}\",\"beforeDevCommand\":\"exec ./node_modules/.bin/vite --port ${PUNKS_VITE_PORT} --strictPort\"},\"identifier\":\"xyz.block.punks.app.dev.${PUNKS_INSTANCE_SLUG}\",\"productName\":\"Punks Dev (${PUNKS_WORKTREE_LABEL})\",\"bundle\":{\"icon\":[\"$DEV_ICON\"]}}"
        fi
    fi
fi

export PUNKS_TAURI_CONFIG
