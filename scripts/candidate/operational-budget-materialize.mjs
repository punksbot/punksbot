...
export function materializeOperationalBudgetEvidence(input) {
  if (
    ...
    typeof input.providerBundle !== "string" ||
    typeof input.output !== "string"
  ) {
    fail("exact candidate, staging and filesystem roots are required");
  }
  const provider = readProviderSources(input);
  const providerBundle = readStableEvidenceFile(
    input.providerBundle,
    "operational provider Sigstore bundle",
  );
  validateSigstoreBundleContent(providerBundle);
  const providerBundleSha256 = sha256(providerBundle);
  ...
  try {
    ...
    writeExclusive(join(output, "provider.sigstore.json"), providerBundle);
    ...
    const manifestContent = {
      ...
      provenance: {
        ...
        bundle: {
          key: `${prefix}provenance/${providerBundleSha256}.sigstore.json`,
          sha256: providerBundleSha256,
        },
      },
...
function localObjects(input, manifest, manifestBytes) {
  ...
  values.push({
    key: manifest.provenance.bundle.key,
    content: readStableEvidenceFile(
      join(input.root, "provider.sigstore.json"),
      "operational provider bundle publication",
    ),
    expectedSha256: manifest.provenance.bundle.sha256,
  });
  values.push({
...
export async function run(argv = process.argv.slice(2)) {
  ...
  if (command === "materialize") {
    ...
    const result = materializeOperationalBudgetEvidence({
      ...
      providerBundle: resolve(required("--provider-bundle")),
