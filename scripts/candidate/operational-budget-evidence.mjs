...
function verifiedProvenance(source, candidateRoot, expected) {
  if (
  ...
  const document = stableJson(path, `${expected.metric} provenance`);
  exact(
    document,
    [
      "schema",
  ...
  if (
    document.schema !== "punks.operational-metric-source.v1" ||
    ...
    !SHA256_RE.test(document.attestationSha256 ?? "") ||
...
function validateExport(document, expected) {
  exact(
    document,
    [
      "schema",
...
export function validateOperationalBudgetEvidence({
  ...
  const outbox = validateExport(outboxDocument, {
    sourceSha,
    stagingDeploymentId,
