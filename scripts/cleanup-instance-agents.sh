...
# can finish, so agent workers (goose, punks-agent, ...) it spawned in their
...
# `PUNKS_MANAGED_AGENT` env var from the shell: on macOS `pkill -f` matches only
...
#   <instance-id> is the desktop bundle identifier, e.g. `xyz.block.punks.app.dev`
#   (main checkout) or `xyz.block.punks.app.dev.my-branch` (a worktree).
