...
grep -q '^BUNDLE_IDENTIFIER = com\.punks\.punksMobile\.feature-work-1$' "$ios" \
...
grep -q '^APP_DISPLAY_NAME = Punks (Fix_Thing-2)$' "$ios" \
...
grep -Eq "^APP_DISPLAY_NAME = Punks \([A-Za-z0-9._-]+\)$" "$ios" \
...
grep -q '^BUNDLE_IDENTIFIER = com\.punks\.punksMobile$' "$release_xcconfig" \
  && pass "Release.xcconfig keeps the production bundle identifier" \
  || fail "Release.xcconfig must keep BUNDLE_IDENTIFIER = com.punks.punksMobile"
grep -q '^APP_DISPLAY_NAME = Punks$' "$release_xcconfig" \
  && pass "Release.xcconfig keeps the production display name" \
  || fail "Release.xcconfig must keep APP_DISPLAY_NAME = Punks"
...
grep -q 'resValue("string", "app_name", "Punks")' "$gradle" \
  && pass "Gradle default app_name stays Punks" \
...
if [[ "$3 $4 $5" == "shell pm list" ]]; then
  printf 'package:xyz.block.punks.mobile\n'
  printf 'package:xyz.block.punks.mobile.feature_work_1\n'
  printf 'package:xyz.block.punks.mobile.w_2fast\n'
...
if printf '%s\n' "$clean_out" | grep -Eq '(would uninstall|uninstalling).*xyz\.block\.punks\.mobile$'; then
