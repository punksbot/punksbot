...
TEST_DIR="$(mktemp -d "${TMPDIR:-/tmp}/punks-relay-key-test.XXXXXX")"
...
first_key="$(sed -n 's/^PUNKS_RELAY_PRIVATE_KEY=//p' "${ENV_FILE}")"
...
"${SCRIPT_DIR}/ensure-local-relay-key.sh" "${ENV_FILE}" >/dev/null
second_key="$(sed -n 's/^PUNKS_RELAY_PRIVATE_KEY=//p' "${ENV_FILE}")"
...
if [[ "$(grep -c '^PUNKS_RELAY_PRIVATE_KEY=' "${ENV_FILE}")" -ne 1 ]]; then
