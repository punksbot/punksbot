...
export PGUSER="${PGUSER:-punks}"
export PGPASSWORD="${PGPASSWORD:-punks_dev}"
export PGDATABASE="${PGDATABASE:-punks}"
...
elif docker exec punks-postgres psql --version >/dev/null 2>&1; then
  run_psql() {
    docker exec -i -e PGPASSWORD="${PGPASSWORD}" punks-postgres \
      psql -U "${PGUSER}" -d "${PGDATABASE}" "$@"
  }
else
  echo "error: neither psql nor punks-postgres docker psql is available" >&2
  exit 1
fi
...
upload_fixture() {
  ...
  docker exec -i punks-minio mc pipe --quiet --attr "Content-Type=${mime}" \
    "local/${PUNKS_S3_BUCKET:-punks-media}/${hash}.${extension}" < "${path}"
  printf '%s' "${sidecar}" | docker exec -i punks-minio mc pipe --quiet \
    --attr "Content-Type=application/json" \
    "local/${PUNKS_S3_BUCKET:-punks-media}/_meta/${community_id}/${hash}.json"
}

fixture_dir="$(mktemp -d "${TMPDIR:-/tmp}/punks-admin-feedback.XXXXXX")"
...
printf '%s\n' "punks feedback diagnostics" "area: composer" \
  "event: resumed_from_sleep" "result: composer_unresponsive" > "${composer_diagnostics}"
printf '%s\n' "punks feedback diagnostics" "area: workspace-switching" \
...
if ! docker exec punks-minio mc alias set local http://localhost:9000 \
  "${PUNKS_S3_ACCESS_KEY:-punks_dev}" "${PUNKS_S3_SECRET_KEY:-punks_dev_secret}" >/dev/null; then
