...
# Production state (`xyz.block.punks.app`, `~/.punks`, and `punks-desktop`) is
...
remove_bundle_state() {
  ...
  [[ -d "$base" ]] || return 0
  shopt -s nullglob
  for prefix in xyz.block.punks.app.dev xyz.block.sprout.app.dev; do
...
case "$(uname -s)" in
  Darwin)
    ...
    if command -v security >/dev/null 2>&1; then
      while security delete-generic-password -s punks-desktop-dev >/dev/null 2>&1; do :; done
...
remove_path "$HOME/.punks-dev"
remove_path "$HOME/.sprout-dev"

# A fresh dev nest must not re-import the installed app's ~/.punks contents on
# its next boot. The sentinel is the same one used by migrate_dev_nest().
mkdir -p "$HOME/.punks-dev"
: > "$HOME/.punks-dev/.dev-nest-migrated"

log "Development desktop state removed; production Punks state was not touched"
