...
load_env() {
  ...
  # Smooth the local rename path for developers with a pre-Punks .env copied
  ...
  if [[ "${DATABASE_URL:-}" == "postgres://sprout:sprout_dev@localhost:5432/sprout" ]]; then
    warn "Migrating legacy default DATABASE_URL from sprout to punks for this setup run"
    DATABASE_URL="postgres://punks:punks_dev@localhost:5432/punks"
  fi
  if [[ "${PGUSER:-}" == "sprout" ]]; then PGUSER="punks"; fi
  if [[ "${PGPASSWORD:-}" == "sprout_dev" ]]; then PGPASSWORD="punks_dev"; fi
  if [[ "${PGDATABASE:-}" == "sprout" ]]; then PGDATABASE="punks"; fi

  export DATABASE_URL="${DATABASE_URL:-postgres://punks:punks_dev@localhost:5432/punks}"
...
cleanup_legacy_sprout_containers() {
  ...
  warn "Stopping/removing legacy sprout-* dev containers so punks-* containers can bind the standard ports"
...
fail_if_local_redis_blocks_compose() {
  ...
  if docker ps --format '{{.Names}}' | grep -qx 'punks-redis'; then
    return
  fi
  local redis_pids
...
"${REPO_ROOT}/bin/cargo" run -p punks-admin -- migrate
...
echo ""
echo -e "${GREEN}=======================================================${NC}"
echo -e "${GREEN}  Punks dev environment is ready!${NC}"
echo -e "${GREEN}=======================================================${NC}"
echo ""
