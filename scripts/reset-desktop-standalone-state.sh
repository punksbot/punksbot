...
if [[ "$instance_id" != "xyz.block.punks.app.dev" && "$instance_id" != xyz.block.punks.app.dev.* ]]; then
    echo "reset-desktop-standalone-state: refusing non-dev bundle identifier: $instance_id" >&2
    exit 1
fi
if [[ "$keyring_service" != "punks-desktop-dev" && "$keyring_service" != punks-desktop-dev.* ]]; then
...
case "${PUNKS_TEST_PLATFORM:-$(uname -s)}" in
    Darwin)
