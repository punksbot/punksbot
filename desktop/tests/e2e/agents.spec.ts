...
async function countCommandInvocations(
  ...
  return page.evaluate(
    (targetCommand) =>
      ...
      ).__PUNKS_E2E_COMMANDS__?.filter((invoked) => invoked === targetCommand)
        .length ?? 0,
    command,
  );
}
...
async function readAgentShareCommands(
  ...
  return page.evaluate(
    () =>
      (
        window as Window & {
          __PUNKS_E2E_COMMAND_LOG__?: AgentShareCommand[];
        }
...
test("custom personas share with people and keep export separate", async ({
  ...
  await expect(
    exportDialog.getByRole("button", { name: "Send in Punks" }),
  ).toHaveCount(0);
  ...
  expect(copiedAgent?.html).toContain("data-punks-agent-snapshot");
  ...
  await page.evaluate(() => {
    ...
    (
      window as Window & {
        __PUNKS_RECIPIENT_POPOVER_OBSERVER__?: MutationObserver;
        __PUNKS_RECIPIENT_POPOVER_STATE_CHANGES__?: string[];
      }
    ).__PUNKS_RECIPIENT_POPOVER_OBSERVER__ = observer;
    (
      window as Window & {
        __PUNKS_RECIPIENT_POPOVER_STATE_CHANGES__?: string[];
      }
    ).__PUNKS_RECIPIENT_POPOVER_STATE_CHANGES__ = stateChanges;
  });
  await recipientSearch.click();
  await waitForAnimations(page);
  const recipientPopoverStateChanges = await page.evaluate(() => {
    ...
    trackedWindow.__PUNKS_RECIPIENT_POPOVER_OBSERVER__?.disconnect();
    return trackedWindow.__PUNKS_RECIPIENT_POPOVER_STATE_CHANGES__ ?? [];
  });
  ...
  await expect(bobChip).not.toHaveClass(/punks-poof-trigger/);
  await waitForAnimations(page);
  ...
  await expect(page.locator(".punks-poof-burst")).toHaveCount(0);
  await page.mouse.up();
...
test("people sharing blocks a timeout before encoding or upload", async ({
  ...
  await page.evaluate(() => {
    ...
    ).__PUNKS_E2E_ACTIVATE_TIMEOUT__?.(Date.now() + 60_000);
  });
...
test("people sharing rechecks destination eligibility after encoding", async ({
  ...
  await page.evaluate(() => {
    ...
    testWindow.__PUNKS_E2E_MUTATE_CHANNEL__?.({
      channelId: "d1ec7000-d000-4000-8000-000000000001",
      channelType: "forum",
    });
    return testWindow.__PUNKS_E2E_INVALIDATE_CHANNELS__?.();
  });
