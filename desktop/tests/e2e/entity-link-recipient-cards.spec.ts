...
// Regression coverage for Punks-native entity links after standalone cards were
...
test("agent-style Punks links stay chip-only with metadata tooltips", async ({
  page,
}) => {
  ...
  // punks:// URLs in a Markdown list and NO link-preview snapshot tags.
  await page.evaluate(
    ({ prId, issueId, alicePubkey, externalHref, cloneHref }) => {
      window.__PUNKS_E2E_EMIT_MOCK_MESSAGE__?.({
        ...
        content: [
          ...
          "",
          `- Pull request with enough leading context to wrap: <punks://pr?id=${prId}&owner=${alicePubkey}&d=relay-tools>`,
          `- Issue: <punks://issue?id=${issueId}&owner=${alicePubkey}&d=relay-tools>`,
          `- Repository: <punks://repo?owner=${alicePubkey}&d=relay-tools>`,
          `- Labeled issue: [triage this issue](punks://issue?id=${issueId}&owner=${alicePubkey}&d=relay-tools)`,
          `- Labeled clone: [clone relay-tools](${cloneHref})`,
          `- Missing repo: <punks://repo?owner=${alicePubkey}&d=missing-repo>`,
  ...
  await expect(row.locator('[data-link-preview^="punks-"]')).toHaveCount(0);
  ...
  const prContext = prTooltip.locator(
    '[data-punks-tooltip-metadata-content=""]',
  );
  ...
  const prFooter = prTooltip.locator('[data-punks-tooltip-metadata-type=""]');
  ...
  const issueChip = row.locator('[data-punks-link-kind="issue"]');
  ...
  await expect
    ...
    .toBe(true);
  await expect(
    issueTooltip.locator('[data-punks-tooltip-metadata-type=""]'),
  ...
  await expect(
    repoTooltip.locator('[data-punks-tooltip-metadata-content=""]'),
  ).toContainText("Operator tooling and admin CLI for relay deployments.");
  await expect(
    repoTooltip.locator('[data-punks-tooltip-metadata-type=""]'),
  ...
  await expect(
    cloneTooltip.locator('[data-punks-tooltip-metadata-content=""]'),
  ).toContainText("Operator tooling and admin CLI for relay deployments.");
  ...
  await expect(missingRepoChip).not.toHaveClass(/punks-link-unavailable/);
  ...
  await expect(
    missingRepoTooltip.locator('[data-punks-tooltip-metadata-content=""]'),
  ).toHaveCount(0);
  ...
  await page.evaluate(
    ({ issueId, alicePubkey }) => {
      window.__PUNKS_E2E_EMIT_MOCK_MESSAGE__?.({
        channelName: "general",
        pubkey: alicePubkey,
        content: `Issue link: punks://issue?id=${issueId}&owner=${alicePubkey}&d=relay-tools`,
      });
    },
...
test("entity tooltip uses project context while relay metadata is delayed", async ({
  ...
  await page.getByTestId("channel-general").click();
  await page.evaluate(() =>
    window.__PUNKS_E2E_ACTIVATE_RELAY_RATE_LIMIT__?.(300),
  );
  await page.waitForFunction(
    () => typeof window.__PUNKS_E2E_EMIT_MOCK_MESSAGE__ === "function",
  );
  await page.evaluate(
    ({ issueId, owner }) => {
      window.__PUNKS_E2E_EMIT_MOCK_MESSAGE__?.({
        channelName: "general",
        content: `Delayed issue: punks://issue?id=${issueId}&owner=${owner}&d=punks`,
      });
    },
  ...
  const issueChip = page.getByRole("button", {
    name: /Open issue .* in repository punks/,
  });
  await issueChip.hover();
  await expect(
    page
      .getByRole("tooltip")
      .locator('[data-punks-tooltip-metadata-content=""]'),
  ).toHaveText("punks · The complete Punks community platform.");
});

test("desktop composer and sent message keep Punks entities chip-only", async ({
  page,
}) => {
  await installMockBridge(page);
  await page.setViewportSize({ width: 900, height: 800 });
  ...
  const repoLink = `punks://repo?owner=${ALICE_PUBKEY}&d=relay-tools`;
  await page.getByTestId("message-input").fill(`Check out ${repoLink}`);

  // Punks-native links do not enter the standalone composer-preview surface.
...
test("composer classifies a same-relay clone URL as a repository chip, not a card", async ({
  ...
  const cloneHref = `${relayHttpUrl}/git/${DEFAULT_MOCK_PUBKEY}/punks.git`;
  ...
  const repoChip = row.getByRole("button", { name: "Open repository punks" });
  await expect(repoChip).toBeVisible();
  await repoChip.hover();
  await expect(
    page
  ...
  // canonical punks://repo target rather than being handed to the OS.
...
test("reopening the same entity link reapplies its workspace state", async ({
  ...
  const repoAddress = `30617:${DEFAULT_MOCK_PUBKEY}:punks`;
  await page.addInitScript(
  ...
  const repoLink = `punks://repo?owner=${DEFAULT_MOCK_PUBKEY}&d=punks&tab=prs`;
  const prLink = `punks://pr?id=${PR_ID}&owner=${DEFAULT_MOCK_PUBKEY}&d=punks`;
  const issueLink = `punks://issue?id=${ISSUE_ID}&owner=${DEFAULT_MOCK_PUBKEY}&d=punks`;
...
test("deleted reply links identify deletion and fall back to their thread root", async ({
  ...
  const link = `punks://message?channel=${channelId}&id=${deletedReplyId}&thread=${threadRootId}`;
...
test("deleted top-level message links identify deletion and fall back to channel navigation", async ({
  ...
  await page.addInitScript(() => {
    window.localStorage.setItem("punks-theme", "punks-dark");
    window.localStorage.setItem("punks-accent-color", "#c0a2f1");
  });
  const missingMessageId = "d".repeat(64);
  const channelId = "9dae0116-799b-5071-a0a8-fdd30a91a35d";
  const link = `punks://message?channel=${channelId}&id=${missingMessageId}`;
  ...
  await expect(deletedLink).toHaveClass(/punks-link-deleted/);
  await expect(deletedLink).not.toHaveClass(/punks-link-unavailable/);
  const unavailableColors = await linkMessage.evaluate((message) => {
    const probe = document.createElement("span");
    probe.className = "mention-chip punks-link-unavailable";
...
test("cold-start entity links drain after the React listener mounts", async ({
  ...
  const href = `punks://repo?owner=${DEFAULT_MOCK_PUBKEY}&d=punks&tab=prs`;
  await installMockBridge(page, {
