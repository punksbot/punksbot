...
async function getBadgeState(page: import("@playwright/test").Page) {
  return page.evaluate(() => {
    ...
    return {
      state: w.__PUNKS_E2E_APP_BADGE_STATE__ ?? "none",
      count: w.__PUNKS_E2E_APP_BADGE_COUNT__ ?? 0,
    };
  });
}
...
test("top-level @mention shows a red numeric badge on its channel", async ({
  ...
  await page.addInitScript(() => {
    window.localStorage.setItem("punks-theme", "slack-ochin");
  });
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  await expect(page.getByTestId("chat-title")).toHaveText("general");
...
test("remote read-state rollback is ignored while local mark-unread still increments badge", async ({
  ...
  await page.evaluate(
    ({ clientId, slotId, channelId, ts }) => {
      ...
      ).__PUNKS_E2E_EMIT_MOCK_READ_STATE__?.({
        clientId,
        slotId,
