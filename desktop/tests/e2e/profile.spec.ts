...
test("keeps the saved profile description after a community round trip", async ({
  ...
  await page.addInitScript((seed) => {
    window.localStorage.setItem("punks-communities", JSON.stringify(seed));
    window.localStorage.setItem("punks-active-community-id", seed[0].id);
  }, communities);
  await page.goto("/");
...
test("nests the avatar edit button in a clipped notch", async ({ page }) => {
  // Under the Punks default theme the settings nav overrides `--sidebar-active`
  ...
  // only holds outside the Punks theme.
  await page.addInitScript(() => {
    window.localStorage.setItem("punks-theme", "github-light");
  });
  await page.goto("/");
...
test("uploads local profile avatar files before saving", async ({ page }) => {
  ...
  const pastedAvatarUrl = await page.evaluate(
    () => new URL("/punks.svg", window.location.href).href,
  );
...
test("renders emoji avatars with a static background layer", async ({
  ...
  await expect(avatarPreview).not.toHaveClass(/punks-avatar-squish/);
      ...
      return {
        heroHeight: Number.parseFloat(
          getComputedStyle(layout).getPropertyValue(
            "--punks-profile-sticky-hero-height",
          ),
        ),
...
test("notification settings drive the Inbox badge and desktop alerts", async ({
  ...
  async function getAppBadgeCount() {
    return page.evaluate(() => {
      ...
      return win.__PUNKS_E2E_APP_BADGE_COUNT__ ?? 0;
    });
  }
  ...
  await page.evaluate(() => {
    ...
    win.__PUNKS_E2E_PUSH_MOCK_FEED_ITEM__?.({
      category: "mention",
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        ...
        return win.__PUNKS_E2E_NOTIFICATIONS__?.length ?? 0;
      }),
    )
    .toBe(1);
  ...
  const notifications = await page.evaluate(() => {
    ...
    return win.__PUNKS_E2E_NOTIFICATIONS__ ?? [];
  });
  ...
  const clickedNotification = await page.evaluate(() => {
    ...
    return win.__PUNKS_E2E_CLICK_NOTIFICATION__?.(0) ?? false;
  });
...
test("opens settings with the keyboard shortcut and updates theme", async ({
  ...
  // Default is Punks in System mode; Playwright's default color scheme is
  // light, so the app boots with the light Punks theme.
  await expect
    .poll(() =>
  ...
  await expect
    .poll(() => page.evaluate(() => localStorage.getItem("punks-theme")))
    .toBe("github-light");
...
test("supports webview zoom keyboard shortcuts", async ({ page }) => {
  ...
  const getTextScaleState = () =>
    page.evaluate(() => ({
      ...
      storedScale: localStorage.getItem("punks:text-scale"),
      webviewZoom: (window as Window & { __PUNKS_E2E_WEBVIEW_ZOOM__?: number })
        .__PUNKS_E2E_WEBVIEW_ZOOM__,
    }));
...
test("storage clear resets composed font size and keyboard zoom across windows", async ({
  ...
  const readTypographyState = () =>
    page.evaluate(() => {
      ...
      probe.style.fontSize = "var(--punks-type-rem)";
      ...
      return {
        ...
        rootFontSize: getComputedStyle(document.documentElement).fontSize,
        typeRemPx,
        textScale: localStorage.getItem("punks:text-scale"),
      };
    });
