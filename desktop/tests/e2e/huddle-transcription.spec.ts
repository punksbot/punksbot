...
async function setHuddleSnapshot(
  ...
  await page.evaluate(
    async ({ nextMembers, nextTranscriptionEnabled }) => {
      ...
      ).__PUNKS_E2E_SET_MOCK_HUDDLE_SNAPSHOT__;
      if (!setSnapshot) {
...
test("keeps the drawer open until the huddle is expanded", async ({ page }) => {
  ...
  const gradientUnderlay = page.locator(".punks-theme-gradient-underlay");
  ...
  const huddleShell = page.locator(".punks-huddle-shell");
  await expect(huddleShell).toHaveAttribute("data-huddle-open", "true");
  const huddleBackdrop = page.locator(".punks-huddle-drawer-backdrop");
  await expect(huddleBackdrop).toHaveClass(/punks-huddle-drawer-backdrop-open/);
  const [huddleBackdropColor, huddleDrawerColor] = await Promise.all([
    huddleBackdrop.evaluate(
      (element) => getComputedStyle(element).backgroundColor,
    ),
    page
      .locator(".punks-huddle-drawer")
      .first()
  ...
  await expect
    .poll(() =>
      huddleBackdrop.evaluate((element) => {
        ...
        const appSurface = shell?.querySelector(".punks-huddle-app-surface");
  ...
  const mainContentSurface = page.locator(
    "[data-punks-content-surface]:not([data-punks-content-unframed])",
  );
  ...
  await expect
    ...
    .toBe(1);
  await expect(page.locator(".punks-huddle-shell")).toHaveAttribute(
    "data-huddle-open",
    "false",
  );
...
test("floats the in-app huddle tray over the glass background", async ({
  ...
  await page.addInitScript(() => {
    window.localStorage.setItem("punks-theme", "punks-dark");
    window.localStorage.setItem("punks-glass-background", "true");
  ...
  const shell = page.locator('.punks-huddle-shell[data-huddle-window="false"]');
  const slot = shell.locator(".punks-huddle-drawer-slot");
  const drawer = slot.locator(":scope > .punks-huddle-drawer");
  const backdrop = shell.locator(".punks-huddle-drawer-backdrop");
...
test("keeps the popped-out huddle dock full-width over glass", async ({
  ...
  const root = page.locator("html");
  const shell = page.locator('.punks-huddle-shell[data-huddle-window="true"]');
  const slot = shell.locator(".punks-huddle-drawer-slot");
  const drawer = slot.locator(":scope > .punks-huddle-drawer");
...
test("shows speaker identity on every huddle chat message", async ({
  ...
  await page.evaluate(() => {
    const threadRoot = window.__PUNKS_E2E_EMIT_MOCK_MESSAGE__?.({
      channelName: "huddle",
...
test("ignores persisted community onboarding in the huddle room", async ({
  ...
  for (let load = 0; load < 2; load += 1) {
    ...
    await expect
      .poll(() =>
        page.evaluate(() =>
          JSON.parse(window.localStorage.getItem("punks-communities") ?? "[]"),
        ),
      )
      .not.toContainEqual(
...
test("keeps main-app shortcuts from navigating the huddle room", async ({
  ...
  await page.evaluate(async (channelId) => {
    await window.__PUNKS_E2E_EMIT_TAURI_EVENT__?.("deep-link-message", {
      channelId,
  ...
  await page.evaluate(async () => {
    await window.__PUNKS_E2E_EMIT_TAURI_EVENT__?.("deep-link-connect", null);
  });
  await expect
    .poll(() =>
      page.evaluate(async () =>
        window.__PUNKS_E2E_INVOKE_MOCK_COMMAND__?.(
...
test("speaks the first eligible agent reply with its participant identity", async ({
  ...
  await expect
    .poll(() =>
      page.evaluate(
        () =>
          (window.__PUNKS_E2E_COMMAND_LOG__ ?? []).find(
...
test("animates the responding agent with the shared speaker ring", async ({
  ...
  const agentAvatar = page
    ...
    .locator(".punks-huddle-speaking-avatar")
    .nth(1);
  await expect(agentAvatar).toBeVisible();
  await page.evaluate(async (pubkey) => {
    await window.__PUNKS_E2E_EMIT_MOCK_HUDDLE_TTS_SPEAKER__?.({
      pubkey,
      level: 0.8,
    });
  }, TEST_IDENTITIES.alice.pubkey);
  await expect
    .poll(() =>
      agentAvatar.evaluate((element) =>
        element.style.getPropertyValue("--punks-huddle-speaker-opacity"),
      ),
    )
    .toBe("0.890");
...
test("assigns distinct agent voices and exposes compact per-agent controls", async ({
  ...
  await expect
    .poll(async () => {
      const state = (await page.evaluate(() =>
        window.__PUNKS_E2E_INVOKE_MOCK_COMMAND__?.("get_huddle_state"),
      )) as {
  ...
  const assignedVoices = await page.evaluate(async () => {
    const state = (await window.__PUNKS_E2E_INVOKE_MOCK_COMMAND__?.(
      "get_huddle_state",
    )) as {
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        const updates = (window.__PUNKS_E2E_COMMAND_LOG__ ?? []).filter(
...
test("returns the companion transcript to the same channel in the main app", async ({
  ...
  await page.evaluate(async () => {
    await window.__PUNKS_E2E_INVOKE_MOCK_COMMAND__?.("close_huddle_companion");
  });
...
test("starts muted with Push to Talk while preserving manual microphone control", async ({
  ...
  await page.evaluate(async () => {
    await window.__PUNKS_E2E_EMIT_TAURI_EVENT__?.("ptt-state", true);
  });
  await expect(muteButton).toBeVisible();
  await muteButton.click();
  await expect
  ...
  await page.evaluate(async () => {
    await window.__PUNKS_E2E_EMIT_TAURI_EVENT__?.("ptt-state", false);
  });
  await expect(unmuteButton).toBeVisible();
