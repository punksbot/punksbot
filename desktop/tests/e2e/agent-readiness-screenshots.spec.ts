...
// pickers render for it just as they do for punks-agent.
...
/**
 * Wait for the LLM provider field to become visible (punks-agent
...
test.describe("agent readiness gate screenshots", () => {
  ...
  test("01-create-punksagent-uses-ai-defaults", async ({ page }) => {
    ...
    await page.getByRole("dialog").screenshot({
      path: `${SHOTS}/01-create-punksagent-uses-ai-defaults.png`,
    });
  });

  // Shot 02: customized Anthropic configuration uses Automatic model instead
  // of presenting an obsolete model-required error.
  test("02-create-punksagent-automatic-model", async ({ page }) => {
    await installMockBridge(page);
    await openCreateDialog(page);
    await selectProvider(page, "Punks shared compute");
    ...
    await page.getByRole("dialog").screenshot({
      path: `${SHOTS}/02-create-punksagent-automatic-model.png`,
    });
  });
  ...
  test("05-create-cli-login-runtime-no-provider-required", async ({ page }) => {
    ...
    // Switch the auto-selected punks-agent runtime to Claude Code.
    await selectDropdownOption(
      page,
  ...
  // Shot 08: goose runtime, provider empty + no global → save BLOCKED (same rule as punks-agent).
