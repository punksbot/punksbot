...
/** punks-agent is always available and has no auth step. */
const PUNKS_AGENT_AVAILABLE = {
  id: "punks-agent",
  label: "Punks Agent",
  avatar_url: "",
  availability: "available",
...
test.describe("Doctor panel state screenshots", () => {
  ...
  test("00-runtime-card-layout", async ({ page }) => {
    ...
    for (const runtimeId of ["goose", "claude", "punks-agent"]) {
      await expect(
    ...
    await expect(page.getByTestId("doctor-runtime-codex")).not.toContainText(
      "Punks talks to Codex through the Codex CLI.",
    );
        ...
        },
        {
          ...
          steps: [
            ...
            },
            {
              ...
              stdout: "",
              stderr:
                "The installer finished, but Punks still could not use codex (observed: NotInstalled).",
              exit_code: null,
              hint: "Punks requires the vendor CLI executable, not only its desktop app. If the CLI was installed while Punks was open, restart Punks and check again.",
            },
          ],
        },
      ],
  ...
  test("11-outdated-adapter-warning", async ({ page }) => {
    ...
    await expect(dialog).toContainText(
      "Older Punks releases using the legacy adapter may lose community access",
    );
  ...
  test("09-install-output-line-and-log-pointer", async ({ page }) => {
    await installMockBridge(page, {
      ...
      installAcpRuntimeResult: {
        ...
        log_path: "/tmp/punks-install-codex.log",
      },
    });
    ...
    await expect(installError).toContainText("/tmp/punks-install-codex.log");
