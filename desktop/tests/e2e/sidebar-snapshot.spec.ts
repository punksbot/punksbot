...
function snapshotKey(relayUrl: string, ownerPubkey = OWNER_PUBKEY) {
  return `punks-channels.v1:${relayUrl}:${ownerPubkey.toLowerCase()}`;
}
...
const SNAPSHOT_DIAGNOSTIC_MARK = "punks:sidebar:snapshot-diagnostic";
const FULL_SIDEBAR_PAINT_MARK = "punks:sidebar:full-list-painted";
const BOOT_TO_FULL_SIDEBAR_MEASURE = "punks:sidebar:boot-to-full-list-painted";
...
async function trackSnapshotRows(page: Page) {
  await page.addInitScript(() => {
    const testWindow = window as Window & {
      __PUNKS_E2E_SNAPSHOT_ROWS_SEEN__?: string[];
    };
    const seen: string[] = [];
    testWindow.__PUNKS_E2E_SNAPSHOT_ROWS_SEEN__ = seen;
...
async function getTrackedSnapshotRows(page: Page) {
  return page.evaluate(
    () =>
      ...
      ).__PUNKS_E2E_SNAPSHOT_ROWS_SEEN__ ?? [],
  );
}
