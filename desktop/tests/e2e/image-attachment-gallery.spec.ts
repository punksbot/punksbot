...
test("preview-first galleries retain Markdown image actions", async ({
  ...
  const previewUrl = "https://github.com/punksbot/punksbot/pull/6705";
  await installMockBridge(page, {
    uploadDescriptors: [
      {
        url: `http://localhost:3000/media/${IMAGE_SHAS[2]}.png`,
...
test("hidden spoiler images are excluded from gallery navigation until revealed", async ({
  ...
  const spoiler = row.locator(".punks-spoiler[data-spoiler]").first();
    ...
    .poll(() => {
      return page.evaluate(() => {
        return (
          typeof (
            window as Window & {
              __PUNKS_E2E_EMIT_MOCK_MESSAGE__?: unknown;
            }
          ).__PUNKS_E2E_EMIT_MOCK_MESSAGE__ === "function"
        );
      });
    })
    .toBe(true);
