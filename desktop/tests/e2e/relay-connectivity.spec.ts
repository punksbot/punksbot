...
// Self-profile cache key: punks-self-profile.v1:<relay>:<pubkey>
const MOCK_PUBKEY = "deadbeef".repeat(8);
const MOCK_RELAY_URL = "ws://localhost:3000";
const SELF_PROFILE_CACHE_KEY = `punks-self-profile.v1:${MOCK_RELAY_URL}:${MOCK_PUBKEY}`;
...
async function driveConnectionDegraded(
  ...
  if (state !== "connected") {
    ...
    await page.waitForFunction(() => {
      ...
      return (
        typeof win.__PUNKS_E2E_SET_RELAY_CONNECTION_STATE__ === "function" &&
        typeof win.__PUNKS_E2E_GET_RELAY_CONNECTION_STATE__ === "function" &&
        win.__PUNKS_E2E_GET_RELAY_CONNECTION_STATE__() === "connected"
      );
    });
  } else {
