...
// Live gate: boots a REAL punks-relay process, points the app at it, SIGTERMs
...
//
// Requires: PUNKS_E2E_RELAY_RESTART=1, PUNKS_E2E_RELAY_BIN, and
// PUNKS_E2E_DATABASE_URL (plus reachable Redis and media object store, same
// infra as the agents-everywhere live gate).
const enabled = process.env.PUNKS_E2E_RELAY_RESTART === "1";
...
async function runCli(args: string[], relayUrl: string, privateKey: string) {
  const binary = required("PUNKS_E2E_CLI_BIN", process.env.PUNKS_E2E_CLI_BIN);
...
async function connectionState(page: Page): Promise<string> {
  return page.evaluate(() => {
    ...
    return win.__PUNKS_E2E_GET_RELAY_CONNECTION_STATE__?.() ?? "uninstalled";
  });
}
...
test.describe("relay restart live gate", () => {
  test.skip(!enabled, "set PUNKS_E2E_RELAY_RESTART=1 to run live gate");
  ...
  test("client reconnects after the relay is SIGTERMed and restarted", async ({
    ...
    const spec: RelaySpec = {
      ...
      redisUrl:
        process.env.PUNKS_E2E_REDIS_RESTART ?? "redis://127.0.0.1:6379/13",
    };
