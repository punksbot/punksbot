...
/**
 * SCROLL-BACK latency profile for one channel (#punks-bugs) against a LIVE
 ...
 *   PUNKS_E2E_RELAY_URL=http://127.0.0.1:13000 \
 *   PUNKS_COMMUNITY_HOST=sprout-oss.stage.blox.sqprod.co \
 *   PUNKS_PERF_NSEC=nsec1... \
 *   npx playwright test --config=playwright.perf.config.ts scrollback-punksbugs.perf.ts
 */

const RELAY_HTTP =
  process.env.PUNKS_E2E_RELAY_URL ?? "https://sprout-oss.stage.blox.sqprod.co";
const NSEC = process.env.PUNKS_PERF_NSEC ?? "";
const COMMUNITY_HOST = process.env.PUNKS_COMMUNITY_HOST ?? "";
const TARGET_CHANNEL = process.env.PUNKS_PERF_CHANNEL ?? "punks-bugs";
const PAGES = Number(process.env.PUNKS_PERF_PAGES ?? 10);

const IDENTITY_OVERRIDE_KEY = "punks:e2e-identity-override.v1";
const ONBOARDING_PREFIX = "punks-onboarding-complete.v1:";
const WELCOME_PREFIX = "punks-welcome-channel-ensured.v2:";
...
function deriveIdentity(nsec: string) {
  ...
  if (decoded.type !== "nsec")
    throw new Error("PUNKS_PERF_NSEC is not an nsec");
...
test("MEASURE: scroll-back pagination latency in target channel", async ({
  ...
  if (!NSEC) throw new Error("Set PUNKS_PERF_NSEC to a real member nsec");
  ...
  await page.addInitScript(
    ({ ident, onboardingPrefix, welcomePrefix, relayUrl, overrideKey }) => {
      ...
      w.__PUNKS_E2E__ = { ...(w.__PUNKS_E2E__ ?? {}), identity: ident };
    },
    {
      ident: identity,
