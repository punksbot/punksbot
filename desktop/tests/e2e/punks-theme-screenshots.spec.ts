...
const SHOTS = "test-results/punks-theme";
const THEME_STORAGE_KEY = "punks-theme";
const GLASS_BACKGROUND_STORAGE_KEY = "punks-glass-background";
const GLASS_OPACITY_STORAGE_KEY = "punks-glass-opacity";
const PROMINENT_ACTIVE_TAB_STORAGE_KEY = "punks-prominent-active-tab";
const CONVERSATION_DENSITY_STORAGE_KEY = "punks.appearance.conversationDensity";
const FONT_SIZE_STORAGE_KEY = "punks.appearance.fontSize";
  ...
  await page.addInitScript(
    ({ channelId, pubkey }) => {
      window.localStorage.setItem(
        `punks-channel-sections.v1:${pubkey}`,
        JSON.stringify({
          version: 1,
          sections: [
            {
...
async function expectPunksSidebarPalette(page: Page, mode: "light" | "dark") {
  const mutedColor =
  ...
  await expect(
    page.locator("[data-punks-sidebar-secondary]").first(),
...
async function expectPunksContentShadow(page: Page, mode: "light" | "dark") {
  const effects = await page.evaluate(() => {
    const shell = document.querySelector(".punks-huddle-shell");
    const content = document.querySelector("[data-punks-content-surface]");
...
async function expectPunksGradientPaint(
  page: Page,
  mode: "light" | "dark",
): Promise<string> {
  const paint = await page.evaluate(() => {
    const root = document.documentElement;
    const appSurface = document.querySelector(".punks-huddle-app-surface");
    const lightLayer = document.querySelector('[data-punks-gradient="light"]');
    const darkLayer = document.querySelector('[data-punks-gradient="dark"]');
    ...
    return {
      ...
      theme: root.getAttribute("data-punks-theme"),
  ...
  expect(paint.theme).toBe(mode === "light" ? "punks" : "punks-dark");
...
async function expectPunksSettingsPalette(page: Page, mode: "light" | "dark") {
  const mutedColor =
    mode === "light" ? "rgba(0, 0, 0, 0.4)" : "rgba(255, 255, 255, 0.4)";
  ...
  await expectPunksGradientPaint(page, mode);
...
async function expectAppliedPunksTheme(
  page: Page,
  themeName: "punks" | "punks-dark",
  storedTheme: "punks" | "punks-dark" = themeName,
) {
  const isDark = themeName === "punks-dark";
  await expect
    .poll(() =>
      page.evaluate((storageKey) => {
        ...
        return {
          ...
          isDark: root.classList.contains("dark"),
          punksTheme: root.getAttribute("data-punks-theme"),
          gradientTop: styles.getPropertyValue("--punks-gradient-top").trim(),
          gradientBottom: styles
            .getPropertyValue("--punks-gradient-bottom")
            .trim(),
        };
      }, THEME_STORAGE_KEY),
    )
...
test("punks light sidebar gradient", async ({ page }) => {
  await seedTheme(page, "punks");
  await installMockBridge(page);
  await openChannel(page);
  await expectPunksGradientPaint(page, "light");
  await expectPunksSidebarPalette(page, "light");
  await expectPunksContentShadow(page, "light");
  await expectIconlessSectionTitleAligned(page, "stream-list");
  await expectIconlessSectionTitleAligned(page, "dm-list");
  await waitForAnimations(page);
  await page
    .getByTestId("app-sidebar")
    .screenshot({ path: `${SHOTS}/01-punks-light-sidebar.png` });
});

test("punks dark sidebar gradient", async ({ page }) => {
  await seedTheme(page, "punks-dark");
  await installMockBridge(page);
  await openChannel(page);
  await expectPunksGradientPaint(page, "dark");
  await expectPunksSidebarPalette(page, "dark");
  await expectPunksContentShadow(page, "dark");
  await expectIconlessSectionTitleAligned(page, "stream-list");
  await expectIconlessSectionTitleAligned(page, "dm-list");
  await expect(page.locator("[data-punks-content-surface]")).toHaveCSS(
    "background-color",
    "rgb(26, 26, 26)",
  );
  await waitForAnimations(page);
  await page
    .getByTestId("app-sidebar")
    .screenshot({ path: `${SHOTS}/02-punks-dark-sidebar.png` });
});
...
test("appearance groups theme and preferences into labeled rows", async ({
  ...
  await themeCard.getByTestId("theme-option-punks").click();
  await expect(themeStyleTrigger).toHaveAttribute("aria-expanded", "true");
  await expect(themeStyleOptions).toBeVisible();
  await expect(
    themeCard.getByTestId("prominent-active-tab-toggle"),
...
test("app font size and conversation density apply independently", async ({
  ...
  // type tokens ride on `--punks-type-rem`, which itself derives from the
  ...
  const readScale = () =>
    root.evaluate((element) => {
      const PROBE_ID = "punks-e2e-conversation-scale-probe";
      const tokens = {
  ...
  await expect(densityDescription).toHaveText(
    "Spacing in conversations and Markdown content across Punks",
  );
...
test("appearance picker — system tab (Punks follows OS)", async ({ page }) => {
  await seedTheme(page, "punks");
  await installMockBridge(page);
...
test("appearance picker — light tab (Punks)", async ({ page }) => {
  await seedTheme(page, "punks");
  await installMockBridge(page);
...
test("appearance picker — dark tab (Punks Dark)", async ({ page }) => {
  await seedTheme(page, "punks-dark");
  await installMockBridge(page);
...
test("settings nav uses Punks active pill + hover (light)", async ({
  page,
}) => {
  await seedTheme(page, "punks");
  await installMockBridge(page);
  ...
  // Appearance is the active section here; its nav row should carry the Punks
  ...
  await expectPunksSettingsPalette(page, "light");
...
test("settings nav uses Punks active pill + hover (dark)", async ({ page }) => {
  await seedTheme(page, "punks-dark");
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.getByTestId("open-settings").click();
  ...
  await expect(sidebar).toBeVisible({ timeout: 10_000 });
  await page.getByTestId("settings-nav-appearance").click();
  await expectPunksSettingsPalette(page, "dark");
...
for (const { mode, theme } of [
  ...
  test(`${theme} ignores the Punks prominent preference`, async ({ page }) => {
    await seedTheme(page, theme);
    await page.addInitScript(
      ({ key }) => window.localStorage.setItem(key, "true"),
      { key: PROMINENT_ACTIVE_TAB_STORAGE_KEY },
...
test("appearance hides accent picker under Punks", async ({ page }) => {
  await seedTheme(page, "punks");
  await installMockBridge(page);
  const panel = await openAppearance(page, "light");
  // The accent picker is hidden while a Punks theme is active. Its neutral
...
test("glass background keeps the content panel solid", async ({ page }) => {
  ...
  await expect(page.locator(".punks-theme-gradient-underlay")).not.toHaveCSS(
    "background-image",
    "none",
  );
  ...
  await expect(opacitySlider).toHaveClass(/punks-avatar-framing-slider/);
  ...
  await expect(
    opacitySlider.locator(".punks-avatar-framing-slider-handle"),
  ).toHaveCSS("opacity", "1");
  await expect(root).toHaveAttribute("data-glass-background", "");
  const punksSettingOrder = await page
  ...
  expect(punksSettingOrder).toEqual([
  ...
  await expect(page.locator(".punks-theme-gradient-underlay")).toHaveCSS(
    "background-image",
    "none",
  );
  ...
  await expect
    .poll(() =>
      page.evaluate(() =>
        (window.__PUNKS_E2E_COMMAND_LOG__ ?? []).some(
          (entry) =>
...
test("non-Punks glass preserves the selected theme sidebar tint", async ({
  page,
}) => {
  ...
  const tint = await page
    .locator(".punks-theme-gradient-layer")
    .evaluate((element) => {
...
test("accent picker reveals/hides when toggling Punks", async ({ page }) => {
  // Start on a non-Punks theme so the accent picker is present, then select the
  // Punks tile — the picker should animate out and unmount. Reselecting a
  // non-Punks tile brings it back. Asserts the presence toggle (the motion
  ...
  const nonPunksSettingOrder = await page
    .getByTestId("appearance-theme-card")
    .locator(
    ...
    .evaluateAll((rows) => rows.map((row) => row.getAttribute("data-testid")));
  expect(nonPunksSettingOrder).toEqual([
    "appearance-color-mode-row",
    "theme-style-row",
  ...
  // Switch to Punks — picker should leave (allow the exit animation to settle).
  await page.getByTestId("theme-style-trigger").click();
  await page.getByTestId("theme-option-punks").click();
  ...
  // Back to a non-Punks theme — picker returns.
...
test("Punks light and dark modes apply live without a reload", async ({
  page,
}) => {
  await seedTheme(page, "punks");
  await installMockBridge(page);
  await openAppearance(page, "light");
  await expectAppliedPunksTheme(page, "punks");
  const lightGradient = await expectPunksGradientPaint(page, "light");

  await page.getByTestId("appearance-mode-dark").click();
  await expectAppliedPunksTheme(page, "punks-dark");
  const darkGradient = await expectPunksGradientPaint(page, "dark");
...
test("Punks follows native system theme changes without a reload", async ({
  page,
}) => {
  await seedTheme(page, "punks");
  await page.addInitScript(() => {
  ...
  await expectAppliedPunksTheme(page, "punks-dark", "punks");
  await expectPunksGradientPaint(page, "dark");

  await emitNativeThemeChange(page, "light");
  await expectAppliedPunksTheme(page, "punks", "punks");
  await expectPunksGradientPaint(page, "light");
});
