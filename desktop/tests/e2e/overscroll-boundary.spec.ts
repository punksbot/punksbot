...
test("locks viewport rubber-band outside conversation scrollers", async ({
  ...
  // Punks Term consumes wheel gestures as custom scrollback rather than through
...
test("locks horizontal viewport pan everywhere", async ({ page }) => {
  ...
  // space locked even if a future layout places Punks content inside it.
  await page.evaluate(() => {
    const terminal = document.querySelector<HTMLElement>(
      '[data-testid="terminal-wheel-target"]',
    );
    const punksContent = document.createElement("div");
    punksContent.dataset.testid = "concealed-terminal-dead-space";
    terminal?.append(punksContent);
    if (terminal) terminal.dataset.terminalOwner = "punks";
  });
  await expect(
