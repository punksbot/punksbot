...
test("machine onboarding: landing, backup, setup docked CTAs", async ({
  ...
  await expect(
    page.getByTestId("backup-options").locator(".punks-card-textured"),
  ).toHaveCount(0);
  ...
  await expect(passwordPanel).not.toHaveClass(/punks-card-textured/);
  ...
  await expect(generatorPopover).not.toHaveClass(/punks-card-textured/);
  await waitForAnimations(page);
