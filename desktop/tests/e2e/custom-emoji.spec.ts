...
//
// The `:punks:` shortcode lives in a member-authored kind:30030 set
// (d=`punks:custom-emoji`) served by the mock bridge from two distinct
...
// real 127.0.0.1 URL rather than the punks-media:// fallback.
...
async function quickReactionStorageContains(
  ...
  return page.evaluate((selectedEmoji) => {
    for (let index = 0; index < window.localStorage.length; index += 1) {
      ...
      if (!key?.startsWith("punks.quick-reaction-emojis.v1")) continue;
...
test("editing a message with a custom emoji shows the image, not the shortcode (Bug 1)", async ({
  ...
  // node, NOT as literal `:punks:` text.
...
test("adding a custom emoji while editing keeps the image after save (Bug 2)", async ({
  ...
  // <img>, not a bare `:punks:`. The pre-fix edit path shipped no emoji tags,
