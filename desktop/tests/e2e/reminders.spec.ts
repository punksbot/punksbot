  ...
  await page.evaluate((seeded) => {
    window.__PUNKS_E2E_SEED_MOCK_REMINDERS__?.(
      seeded as Parameters<
        NonNullable<typeof window.__PUNKS_E2E_SEED_MOCK_REMINDERS__>
      >[0],
    );
    window.__PUNKS_E2E_QUERY_CLIENT__?.invalidateQueries({
      queryKey: ["reminders"],
    });
  }, events);
}
...
const NOTIFICATION_SETTINGS_KEY = `punks-notification-settings.v2:${MOCK_PUBKEY}`;
