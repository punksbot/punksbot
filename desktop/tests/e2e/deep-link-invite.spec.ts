...
const TRANSACTION_STORAGE_KEY = "punks-community-onboarding-transaction.v1";
...
const PENDING_ADD_COMMUNITY_LINK = {
  ...
  relayUrl: "wss://acme.communities.punks.xyz",
  code: null,
  name: "Acme Team",
};
...
const SECOND_PENDING_ADD_COMMUNITY_LINK = {
  ...
  kind: "add-community" as const,
  relayUrl: "wss://beta.communities.punks.xyz",
  code: null,
  name: "Beta Team",
};
...
test("deleted public starter channels do not strand community onboarding", async ({
  ...
  await page.getByRole("button", { name: "Take me to Punks" }).click();
...
test("required Welcome creation failure keeps community onboarding open", async ({
  ...
  await expect(
    page.getByRole("button", { name: "Take me to Punks" }),
  ).toBeEnabled();
