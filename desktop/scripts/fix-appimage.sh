...
APP_BIN="$WORKDIR/squashfs-root/usr/bin/punks-desktop"
if [[ ! -f "$APP_BIN" ]]; then
  echo "Error: app binary usr/bin/punks-desktop not found — bundler layout changed; update fix-appimage.sh" >&2
  exit 1
fi
if [[ -e "$APP_BIN.bin" ]]; then
  echo "Error: usr/bin/punks-desktop.bin already exists — shim already installed?" >&2
  exit 1
fi

# The real binary moves aside; punks-desktop becomes a shim AppRun.wrapped execs.
...
exec -a "punks-desktop" "$here/punks-desktop.bin" "$@"
SHIM
chmod +x "$APP_BIN"
