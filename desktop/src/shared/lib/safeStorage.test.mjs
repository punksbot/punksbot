...
test("getStorageItem returns the stored value when storage is healthy", () => {
  const restore = patchLocalStorage(
    createThrowingStorage({ "punks-theme": "punks" }),
  );
  try {
    assert.equal(getStorageItem("punks-theme"), "punks");
...
test("getStorageItem returns null when the key is absent", () => {
  ...
  try {
    assert.equal(getStorageItem("punks-theme"), null);
  } finally {
    restore();
  }
});
...
test("setStorageItem returns true on a healthy write", () => {
  ...
  try {
    assert.equal(setStorageItem("punks-theme", "punks-dark"), true);
    assert.equal(storage.values.get("punks-theme"), "punks-dark");
  } finally {
    restore();
  }
});
