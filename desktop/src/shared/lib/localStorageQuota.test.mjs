...
test("startup recovery removes disposable caches but preserves user state", () => {
  ...
  install(ls);
  ls.store.set("punks-channel-messages.v1:relay:chan", "big");
  ls.store.set("punks-channels.v1:relay", "big");
  ls.store.set("punks-timeline-skeleton-shape.v1:chan", "small");
  ls.store.set("punks-sidebar-skeleton-shape.v1:community:user", "small");
  ls.store.set("punks-user-labels.v1:relay", "small");
  ls.store.set("punks-communities", "keep");

  recoverLocalStorageQuotaOnStartup();

  assert.equal(ls.getItem("punks-channel-messages.v1:relay:chan"), null);
  assert.equal(ls.getItem("punks-channels.v1:relay"), null);
  assert.equal(ls.getItem("punks-timeline-skeleton-shape.v1:chan"), null);
  assert.equal(
    ls.getItem("punks-sidebar-skeleton-shape.v1:community:user"),
    null,
  );
  assert.equal(ls.getItem("punks-user-labels.v1:relay"), null);
  assert.equal(ls.getItem("punks-communities"), "keep");
  assert.equal(ls.getItem("punks-local-storage-quota-recovery.v1"), "1");
});

test("healthy startup preserves disposable caches", () => {
  const ls = makeQuotaLocalStorage({ maxEntries: 10 });
  install(ls);
  ls.store.set("punks-channel-messages.v1:relay:new", "snapshot");

  recoverLocalStorageQuotaOnStartup();

  assert.equal(ls.getItem("punks-channel-messages.v1:relay:new"), "snapshot");
  assert.equal(ls.getItem("punks-local-storage-quota-recovery.v1"), "1");
});
...
test("startup recovery does not remove namespace near misses", () => {
  ...
  install(ls);
  ls.store.set("punks-channels.v10:durable", "keep");
  ls.store.set("punks-channel-messages.v1-durable", "keep");

  recoverLocalStorageQuotaOnStartup();

  assert.equal(ls.getItem("punks-channels.v10:durable"), "keep");
  assert.equal(ls.getItem("punks-channel-messages.v1-durable"), "keep");
  assert.equal(ls.getItem("punks-local-storage-quota-recovery.v1"), null);
});
...
test("startup recovery runs only once", () => {
  ...
  recoverLocalStorageQuotaOnStartup();
  ls.store.set("punks-channel-messages.v1:relay:new", "new snapshot");
  recoverLocalStorageQuotaOnStartup();

  assert.equal(
    ls.getItem("punks-channel-messages.v1:relay:new"),
    "new snapshot",
  );
});
...
test("startup recovery retries after marker write fails", () => {
  ...
  ls.store.delete("punks-communities");
  ls.store.set("punks-channel-messages.v1:relay:chan", "big");
  recoverLocalStorageQuotaOnStartup();
...
test("global cache byte budget evicts only oldest entries needed", () => {
  ...
  const oldestKey = "punks-channel-messages.v1:relay:oldest";
  const newerKey = "punks-channels.v1:relay-newer";
  const newestKey = "punks-channel-messages.v1:relay:newest";
...
test("global cache byte budget spans relays and preserves durable state", () => {
  ...
  assert.equal(
    setLocalStorageItemWithRecovery(
      "punks-channel-messages.v1:relay-one:chan",
      largeSnapshot,
    ),
    true,
  );
  assert.equal(
    setLocalStorageItemWithRecovery(
      "punks-channel-messages.v1:relay-two:chan",
      largeSnapshot,
    ),
    true,
  );

  assert.equal(ls.getItem("punks-channel-messages.v1:relay-one:chan"), null);
  assert.equal(
    ls.getItem("punks-channel-messages.v1:relay-two:chan"),
    largeSnapshot,
  );
  assert.equal(ls.getItem("punks-communities"), "keep");
});
...
test("rejects a single cache entry larger than the global byte budget", () => {
  const ls = makeQuotaLocalStorage({ maxEntries: 10 });
  install(ls);
  const key = "punks-channel-messages.v1:relay:oversized";
...
test("returns false when eviction frees nothing", () => {
  const ls = makeQuotaLocalStorage({ maxEntries: 2 });
  install(ls);
  ls.store.set("punks-workspaces", "keep");
  ls.store.set("punks-active-workspace-id", "keep");

  assert.equal(setLocalStorageItemWithRecovery("k", "v"), false);
  assert.equal(ls.getItem("k"), null);
  assert.equal(ls.getItem("punks-workspaces"), "keep");
});

test("punks-observed-unread.v1: prefix participates in LRU eviction and durable state survives", () => {
  // Sentinel: fails if punks-observed-unread.v1: is removed from PURE_CACHE_KEY_PREFIXES —
  ...
  const snapshot = (updatedAt) =>
    JSON.stringify({ updatedAt, payload: "x".repeat(400_000) });
  const observedKey = "punks-observed-unread.v1:wss://relay.example.com:pk1";
  const olderKey = "punks-channel-messages.v1:relay:older";
  const newestKey = "punks-channel-messages.v1:relay:newest";
