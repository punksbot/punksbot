/**
 * Utilities for extracting and parsing the `punks:config-nudge` sentinel that
 * `punks-acp`'s setup-listener appends to its kind:9 nudge body.
 *
...
/**
 * The structured payload embedded in the `punks:config-nudge` sentinel block.
...
const FENCE_OPEN = "```punks:config-nudge";
const FENCE_CLOSE = "```";
...
/**
 * Strip the `punks:config-nudge` sentinel block (and any preceding blank line)
