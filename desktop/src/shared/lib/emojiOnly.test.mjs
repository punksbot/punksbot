...
test("detects known custom emoji-only shortcode messages", () => {
  assert.equal(isEmojiOnlyMessage(":punks:", CUSTOM_EMOJI), true);
  assert.equal(
    isEmojiOnlyMessage(":punks: :party_parrot:", CUSTOM_EMOJI),
    true,
  );
  assert.equal(isEmojiOnlyMessage(":Punks:", CUSTOM_EMOJI), true);
});
...
test("allows mixed unicode and custom emoji", () => {
  assert.equal(isEmojiOnlyMessage("😀 :punks: ❤️", CUSTOM_EMOJI), true);
});
