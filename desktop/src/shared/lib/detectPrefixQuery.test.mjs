...
const CHANNELS = ["punks-bugs", "punks dev", "general"];
...
test("triggers at start of string", () => {
  assert.deepEqual(at("#", "#punks", CHANNELS), {
    query: "punks",
    startIndex: 0,
  });
});
...
test("triggers after whitespace", () => {
  assert.deepEqual(at("#", "go to #punks", CHANNELS), {
    query: "punks",
    startIndex: 6,
  });
});
...
test("does NOT trigger when glued to a word character", () => {
  assert.equal(at("#", "foo#punks", CHANNELS), null);
...
test("multi-word channel name after ( resolves via multi-word path", () => {
  assert.deepEqual(at("#", "(#punks de", CHANNELS), {
    query: "punks de",
    startIndex: 1,
  });
});
...
test("multi-word: glued-to-word prefix still rejected", () => {
  // `x#punks de` — `#` preceded by `x`, no boundary → no query even though
  // "punks de" is a prefix of a known channel.
  assert.equal(at("#", "x#punks de", CHANNELS), null);
});
