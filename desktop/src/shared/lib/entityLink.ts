/**
 * `punks-local://` deep links for Punks-hosted git entities, mirroring
 * `features/messages/lib/messageLink.ts` for `punks-local://message`.
 *
 * Formats:
 *   punks-local://repo?owner=<owner-pubkey>&d=<repo-dtag>[&tab=<tab>][&commit=<git-hash>]
 *   punks-local://project?owner=<owner-pubkey>&d=<project-dtag>[&tab=<tab>]
 *   punks-local://pr?id=<event-id>&owner=<owner-pubkey>&d=<repo-dtag>
 *   punks-local://issue?id=<event-id>&owner=<owner-pubkey>&d=<repo-dtag>
 *
 ...
 * overview. The CLI builder in `crates/punks-cli/src/links.rs` emits the
 * same format — the two must stay compatible (see the golden-format tests
 * on both sides).
 */

const ENTITY_LINK_SCHEME = "punks-local:";
...
export function buildRepoLink(input: {
  ...
  return `punks-local://repo?owner=${input.owner.toLowerCase()}&d=${input.dtag}${tabSuffix(input.tab)}`;
}
...
export function buildCommitLink(input: {
  ...
  return `punks-local://repo?owner=${input.owner.toLowerCase()}&d=${input.dtag}&tab=commits&commit=${input.commitHash.toLowerCase()}`;
}
...
export function buildProjectLink(input: {
  ...
  checkCoordinate(input.owner, input.dtag);
  return `punks-local://project?owner=${input.owner.toLowerCase()}&d=${input.dtag}${tabSuffix(input.tab)}`;
}
...
export function buildPullRequestLink(input: {
  ...
  checkCoordinate(input.owner, input.dtag);
  return `punks-local://pr?id=${input.id.toLowerCase()}&owner=${input.owner.toLowerCase()}&d=${input.dtag}`;
}
...
export function buildIssueLink(input: {
  ...
  checkEventId(input.id);
  checkCoordinate(input.owner, input.dtag);
  return `punks-local://issue?id=${input.id.toLowerCase()}&owner=${input.owner.toLowerCase()}&d=${input.dtag}`;
}

/**
 * Cheap pre-check used by the markdown renderer and preview extraction
 * before parsing. `punks-local://message` is intentionally excluded — it has its
 * own pill rendering path.
 */
export function isEntityLink(href: string | undefined | null): boolean {
  if (!href) return false;
  return (
    href.startsWith("punks-local://pr?") ||
    href.startsWith("punks-local://issue?") ||
    href.startsWith("punks-local://repo?") ||
    href.startsWith("punks-local://project?")
  );
}

/**
 * Parse a `punks-local://pr|issue|repo?…` URL. Returns a discriminated result so
