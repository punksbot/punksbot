...
/**
 * Payload emitted by the Rust deep-link handler for `punks-local://message?…`.
...
/**
 * Payload emitted by the Rust deep-link handler for `punks-local://join?…` —
 ...
 *
 * When a `punks-local://connect?relay=<url>` link is opened, the handler
 ...
 *
 * When a `punks-local://join?relay=<url>&code=<invite>` link is opened (relay
 ...
 *
 * `punks-local://message?…` is handled separately by `listenForNavigationDeepLinks`,
...
/**
 * Register a listener for `deep-link-entity` events — the `punks-local://` share
