...
export const THEME_STORAGE_KEY = "punks-theme";
const CACHE_KEY = "punks-theme-cache";
export const ACCENT_STORAGE_KEY = "punks-accent-color";
export const GLASS_BACKGROUND_STORAGE_KEY = "punks-glass-background";
export const GLASS_OPACITY_STORAGE_KEY = "punks-glass-opacity";
export const PROMINENT_ACTIVE_TAB_STORAGE_KEY = "punks-prominent-active-tab";
...
const FOLLOW_SYSTEM_KEY = "punks-follow-system";
...
function applyAccentColor(value: string) {
  ...
  if (value === NEUTRAL_ACCENT) {
    ...
    root.style.setProperty("--punks-selected-accent", foreground);
    root.style.setProperty(
      "--punks-video-review-accent",
      VIDEO_REVIEW_NEUTRAL_ACCENT,
    );
    root.style.setProperty(
      "--punks-video-review-accent-foreground",
      VIDEO_REVIEW_NEUTRAL_ACCENT,
    );
  ...
  root.style.setProperty("--punks-selected-accent", accentHsl);
  root.style.setProperty("--punks-video-review-accent", accentHsl);
  root.style.setProperty(
    "--punks-video-review-accent-foreground",
...
/**
 * The Punks themes ship with a fixed neutral accent (the GitHub black/white
 * foreground) rather than a user-selectable accent color. When a Punks theme is
 ...
 */
export function isPunksTheme(themeName: string): boolean {
  return themeName === "punks" || themeName === "punks-dark";
}

/**
 * Resolve the accent to actually apply for a theme: Punks themes are pinned to
...
function resolveEffectiveAccent(
  ...
  return isPunksTheme(themeName) ? NEUTRAL_ACCENT : accentColor;
}

/** Toggle the Punks-specific gradient marker independently from glass. */
function applyPunksSidebar(themeName: string) {
  const root = document.documentElement;
  if (isPunksTheme(themeName)) {
    root.setAttribute("data-punks-sidebar", "");
    // Keep the concrete Punks variant on the root as well as the generic
    ...
    root.setAttribute("data-punks-theme", themeName);
  } else {
    root.removeAttribute("data-punks-sidebar");
    root.removeAttribute("data-punks-theme");
  }
}
...
function applyCachedVars(): string | null {
  try {
    ...
    applyPunksSidebar(themeName);
    glassThemeReady = true;

    const accent = getStorageItem(ACCENT_STORAGE_KEY) ?? DEFAULT_ACCENT;
    // Pin Punks themes to the neutral accent here too, matching applyTheme.
    // Otherwise a cached Punks theme + non-neutral stored accent flashes the
...
async function applyTheme(name: SyntaxThemeName): Promise<{
  ...
  // new theme for a frame — the flicker seen when switching to Punks. Punks
...
export function ThemeProvider({
  ...
  const [followSystem, setFollowSystemState] = useState<boolean>(() => {
    ...
    // Fresh profiles (no saved theme) default to System mode so the Punks
  ...
  // The stronger selected-row treatment belongs exclusively to Punks. Keep
  // the saved preference so it is restored when the user returns to Punks,
  // but remove the live marker for every other theme.
  useEffect(() => {
    setProminentActiveTabActive(
      prominentActiveTab && isPunksTheme(effectiveTheme),
    );
  ...
  // changes. applyTheme already applies the (Punks-neutral-aware) accent in the
