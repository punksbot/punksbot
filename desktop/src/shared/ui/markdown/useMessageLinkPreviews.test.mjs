...
const ENTITY_HREF = `punks-local://pr?id=${EVENT_ID}&owner=${OWNER}&d=punks-world`;
...
test("Punks entity links do not create message preview cards", () => {
  ...
  const snapshots = parseLinkPreviewSnapshots(
    [
      snapshotTag(ENTITY_HREF, "Forged sender title", "Definitely Real Punks"),
