...
function usePunksLinkContextMenu({
  href,
  interactive,
  onOpenLink,
}: {
  ...
  const contextMenu =
    position && href ? (
      <MediaContextMenu
        dataAttributes={["data-punks-link-context-menu"]}
        items={[
          {
            label: "Open link",
            onSelect: () => {
...
export function PunksLinkChip({
  ...
  const { contextMenu, onContextMenuCapture } = usePunksLinkContextMenu({
    href,
    interactive,
    onOpenLink,
  });
...
export function PunksInlineLink({
  children,
  href,
  interactive,
  onOpenLink,
