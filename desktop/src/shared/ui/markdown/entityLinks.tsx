...
import { PunksInlineLink, PunksLinkChip } from "./PunksLinkChip";
...
function EntityMetadataTooltip({
  ...
  React.useEffect(() => {
    let cancelled = false;
    void loadPunksEntityMetadata(href).then((value) => {
...
/**
 * Navigate to the project detail view for a `punks-local://pr|issue|repo` link.
...
/**
 * Resolve an anchor href to a canonical `punks-local://` entity link, accepting
 * both the deep-link scheme directly and HTTPS relay clone URLs (which the
 * preview parser normalizes onto `punks-local://repo` only when the URL origin
...
/**
 * Render an inline anchor for a Punks entity link (`punks-local://pr|issue|repo` or
...
export function renderEntityLinkAnchor({
  ...
  const chip = (metadata?: LinkPreviewMetadata | null) => {
    ...
    return (
      <PunksLinkChip
        data-punks-link-kind={parsed.value.type}
        href={href}
