...
import "./punks-logo-animation.css";
...
export type PunksLogoAnimationProps = {
  ariaLabel?: string;
  className?: string;
  fullScreen?: boolean;
  loop?: boolean;
...
export default function PunksLogoAnimation({
  ariaLabel = "Punks logo animation",
  className = "",
  fullScreen = true,
  loop = false,
  loopRestSeconds = 0,
  ...
  const maskId = `punks-logo-cutouts-${idSuffix}`;
  const textureId = `punks-logo-texture-${idSuffix}`;
  const texture = TEXTURES[config.texture ?? "soft"] ?? TEXTURES.soft;
  const classes = [
    "punks-logo",
    fullScreen && "punks-logo--screen",
    !fullScreen && "punks-logo--compact",
    showBackground && "punks-logo--background",
    className,
  ]
    .filter(Boolean)
    .join(" ");
