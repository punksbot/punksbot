...
import PunksLogoAnimation, {
  type PunksLogoAnimationProps,
} from "./PunksLogoAnimation";
...
export type FuzzyLogoProps = {
  ...
  pulse?: boolean;
  reverse?: boolean;
  variant?: PunksLogoAnimationProps["variant"];
};

/**
 * The fuzzy Punks mark. v8 ships a built-in animated texture (looping fractal-noise
...
export function FuzzyLogo({
  ...
  return (
    <PunksLogoAnimation
      ariaLabel={ariaLabel}
      className={cn(
        pulse && !fuzz && !hasRestWindow && "punks-logo--pulse",
        className,
      )}
      fullScreen={false}
      loop={loop}
