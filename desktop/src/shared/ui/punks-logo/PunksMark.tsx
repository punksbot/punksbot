...
/**
 * The finished Punks mark as a plain static SVG — no SMIL, no scripting, no
 * animation machinery. Geometry matches the final keyframe of the
 * PunksLogoAnimation morph (v8 variant), rendered in `currentColor`, so it
 * paints complete on the very first frame regardless of animation support.
 */
export function PunksMark({ className }: { className?: string }) {
  const maskId = `punks-mark-cutouts-${useId().replace(/[^a-zA-Z0-9_-]/g, "")}`;

  return (
    <svg
      aria-hidden="true"
      className={["punks-mark", className].filter(Boolean).join(" ")}
      viewBox="0 0 466 309"
      fill="currentColor"
    >
      <defs>
