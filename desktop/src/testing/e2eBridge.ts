...
function readSeededPunksEnvironment():
  | {
      distribution: "preparation" | "punks";
      mounted?: string[];
      compatibility?: { compatible: boolean; capabilities: string[] };
    }
  | undefined {
  try {
    const raw = window.localStorage.getItem("buzz-e2e-punks-environment");
    ...
    if (
      parsed.distribution !== "preparation" &&
      parsed.distribution !== "punks"
    ) {
      return undefined;
    }
    return {
      distribution: parsed.distribution,
      mounted: Array.isArray(parsed.mounted)
        ? parsed.mounted.filter((c): c is string => typeof c === "string")
        : undefined,
      compatibility: {
        compatible: parsed.compatibility?.compatible === true,
        capabilities: Array.isArray(parsed.compatibility?.capabilities)
          ? parsed.compatibility.capabilities.filter(
...
function applySeededPunksEnvironment() {
  const environment = readSeededPunksEnvironment();
  if (environment) {
    (
      window as typeof window & {
        __PUNKS_E2E_ENVIRONMENT__?: unknown;
      }
    ).__PUNKS_E2E_ENVIRONMENT__ = environment;
  }
}

export function maybeInstallE2eTauriMocks() {
  applySeededPunksEnvironment();
  if (installed) {
    return;
  }
  ...
  const handleMockCommand = async (
    ...
    switch (command) {
      case "punks_check_compatibility": {
        // Fail-closed : sans environnement Punks explicitement semé, la
        // compatibilité est refusée et aucune capacité n'est exposée.
        const environment = readSeededPunksEnvironment();
        return {
          contract: "desktop.compatibility-response@1",
          compatible: environment?.compatibility?.compatible ?? false,
          profile: "desktop-social-loop@1",
          registryVersion: 1,
          minimumClientVersion: "0.6.0",
          environment: environment?.compatibility ? "staging" : "local",
          origin: "https://punks.local",
          capabilities: environment?.compatibility?.capabilities ?? [],
        };
      }
