...
before(async () => {
  ...
  dom.window.localStorage.setItem("punks-follow-system", "false");
...
test("first-open splash waits for the first terminal frame", async () => {
  ...
  assert.equal(
    view.container.querySelector(".punks-terminal-welcome"),
    null,
  ...
  await waitFor(() =>
    assert.ok(view.container.querySelector(".punks-terminal-welcome")),
  );
  view.unmount();
});
...
test("opening a tab keeps terminal ownership while its attachment is pending", async () => {
  ...
  attachResolver = () => {};
  fireEvent.click(view.getByLabelText("New Punks Term tab"));
  await waitFor(() => assert.equal(typeof attachResolver, "function"));
  assert.equal(
    substrate.dataset.terminalOwner,
    "terminal",
    "an attaching session must not force the substrate back to Punks",
  );
...
// accumulates pixels into whole cells and `punks-terminal` already proves which
