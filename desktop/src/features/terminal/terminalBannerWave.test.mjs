 ...
 *
 * Tolerance is 1.2 dE00 over the theme's own static worst. Recalibrated for
 * the wider PUNKS geometry: min-dark is the bound at 1.1 while the remaining
 * themes stay below it. At the default wavelength the previous geometry's worst
 * excess across 62 themes was 0.47, and it stays under 0.62 through wavelength 2.0,
 ...
 * punks-dark before `cyclicRampPosition` folded the axis into a palindrome.
 *
...
test("the hue axis has no seam at the phase wrap", () => {
  ...
  for (const [name, p] of THEMES) {
    ...
    // seam measured 182.5 sRGB / 55.6 dE00 on punks-dark.
