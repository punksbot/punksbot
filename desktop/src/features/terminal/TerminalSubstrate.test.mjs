...
function gridDraws(view) {
  const grid = view.container.querySelector(
    ".punks-terminal-viewport > canvas:not(.punks-terminal-welcome)",
  );
...
function fixture(overrides = {}) {
  ...
  const renderTree = (nextProps) =>
    createElement(
      ...
      null,
      createElement("div", {
        className: "punks-huddle-app-surface",
        tabIndex: -1,
      }),
...
async function ready(view) {
  await waitFor(() =>
    assert.ok(view.container.querySelector(".punks-terminal-substrate")),
  );
}
...
test("mounted IME paths neither toggle nor emit preedit text", async () => {
  ...
  await ready(view);
  const substrate = view.container.querySelector(".punks-terminal-substrate");
  toggleChord(true);
  assert.equal(substrate.dataset.terminalOwner, "punks");
...
test("tab actions restore terminal input focus", async () => {
  ...
  const actions = [
    ...
    ["new", view.getByLabelText("New Punks Term tab")],
  ];
...
test("drag resize batches visual updates and commits state only on release", async () => {
  ...
  await ready(view);
  const substrate = view.container.querySelector(".punks-terminal-substrate");
  const handle = view.getByLabelText("Resize Punks Term");
  ...
  assert.equal(substrate.dataset.terminalResizing, "true");
  assert.equal(window.localStorage.getItem("punks-terminal-dock-height"), null);
...
test("unmount cancels a queued drag update", async () => {
  ...
  await ready(view);
  const handle = view.getByLabelText("Resize Punks Term");
  const previousHeight = handle.closest(".punks-terminal-substrate").style
    .height;
  ...
  assert.equal(window.localStorage.getItem("punks-terminal-dock-height"), null);
  assert.equal(
    handle.closest(".punks-terminal-substrate").style.height,
    previousHeight,
  );
});
...
async function expectWelcome(view, present) {
  ...
  await waitFor(() =>
    assert.equal(
      view.container.querySelector(".punks-terminal-welcome") !== null,
      present,
    ),
  );
}
...
test("canvas failure atomically restores Punks ownership", async () => {
  ...
  await waitFor(() => assert.equal(substrate.dataset.terminalOwner, "punks"));
  toggleChord();
  await waitFor(() =>
    assert.equal(substrate.dataset.terminalOwner, "terminal"),
  );
...
test("cursor blink runs only while the terminal owns input and resets on input", async () => {
  ...
  try {
    ...
    await ready(subject.view);
    assert.equal(callbacks.size, 0, "Punks ownership must pause blinking");
...
test("reduced motion keeps the terminal cursor solid", async () => {
  ...
  try {
    ...
    await waitFor(() =>
      assert.equal(
        subject.view.container.querySelector(".punks-terminal-substrate")
...
test("tab chords stay inert while Punks owns input", async () => {
  const subject = tabFixture();
  await ready(subject.view);
  // Deliberately not revealed: owner is "punks".
  const spawn = press({ code: "KeyT", metaKey: true });
  ...
  assert.equal(
    ...
    false,
    "Punks-mode ⌘T belongs to the rest of the app",
  );
});
...
test("mirrors the active canvas grid into a selectable plain-text layer", async () => {
  ...
  const selectionLayer = subject.view.container.querySelector(
    ".punks-terminal-selection-layer",
  );
  await waitFor(() =>
    assert.equal(
