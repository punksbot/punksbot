...
/** Punks navigation can use either its production tint or a stronger tab. */
...
/**
 * Build the sample thumbnail as an SVG data URL from the Punks gradient
 * tokens. Data-URL images cannot resolve CSS variables, so the token values
 * are read from the live stylesheet and baked in per render — if the Punks
 * gradient ever changes in `theme.css`, this preview follows automatically.
 */
function punksGradientSampleImage(isDark: boolean): string {
  ...
  const top = isDark
    ? readToken("--punks-gradient-dark-top", "#4a4616")
    : readToken("--punks-gradient-light-top", "#e6e6b6");
  const bottom = isDark
    ? readToken("--punks-gradient-dark-bottom", "#0a1423")
    : readToken("--punks-gradient-light-bottom", "#c4d0da");
...
function LinkPreviewSample({ style }: { style: LinkPreviewStyle }) {
  ...
  const preview = React.useMemo<ResolvedLinkPreview>(
    () => ({
      ...
      imageDataUrl: punksGradientSampleImage(isDark),
    }),
    [isDark],
  );
  return (
...
function ThreadLayoutDiagram({ mode }: { mode: ThreadViewMode }) {
  ...
  // Punks gradient tokens directly and follows theme.css automatically.
  const gradientTop = isDark
    ? "var(--punks-gradient-dark-top, #4a4616)"
    : "var(--punks-gradient-light-top, #e6e6b6)";
  const gradientBottom = isDark
    ? "var(--punks-gradient-dark-bottom, #0a1423)"
    : "var(--punks-gradient-light-bottom, #c4d0da)";
    ...
    <SettingsOptionRow className="items-start">
      <div className="min-w-0">
        ...
        <p
          ...
          data-settings-subcopy
        >
          Choose the highlight color used throughout Punks.
        </p>
      </div>
      <div
