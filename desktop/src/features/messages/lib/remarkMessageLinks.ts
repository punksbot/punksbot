/**
 * Remark plugin that detects bare `punks-local://message?…` URLs in text nodes and
 * replaces each with a custom `message-link` HAST element. Legacy
 * `punks-local://message?…` URLs are accepted during the rename. The `markdown.tsx`
 ...
 * and `www.`. Custom schemes like `punks-local://` only reach the `<a>` component
 * override when the user wrote an explicit `[label](punks-local://…)` link.
 *
...
const MESSAGE_URL_PATTERN = /punks-local:\/\/message\?[^\s<>"')\]]+/g;
