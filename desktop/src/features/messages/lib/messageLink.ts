/**
 * `punks-local://message` link encoding for "Copy link" / deep-link-to-message.
 *
 * Format: `punks-local://message?channel=<uuid>&id=<eventId>[&thread=<rootId>]`
 */

const MESSAGE_LINK_SCHEME = "punks-local:";
...
/**
 * Build a `punks-local://message` URL for a given channel + message.
 *
...
/**
 * Parse a `punks-local://message?…` URL. Returns a discriminated result so callers can
...
export function parseMessageLink(url: string): MessageLinkParseResult {
  ...
  // `new URL("punks-local://message?…")` puts "message" in `hostname`.
...
export function isMessageLink(href: string | undefined | null): boolean {
  ...
  return (
    href.startsWith("punks-local://message?") ||
    href === "punks-local://message"
  );
}
 ...
 * CommonMark autolinks (`<punks-local://message?...>`) and explicitly labeled links
