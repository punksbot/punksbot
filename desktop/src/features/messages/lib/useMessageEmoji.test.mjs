...
const EMOJI_TAGS = [["emoji", "punks", "https://relay/punks.png"]];
...
test("derives custom emoji and emoji-only flag from event tags", () => {
  ...
  assert.deepEqual(customEmoji, [
    { shortcode: "punks", url: "https://relay/punks.png" },
  ]);
  assert.equal(isEmojiOnlyMessage(":punks:", customEmoji), true);
  assert.equal(isEmojiOnlyMessage("hi :punks:", customEmoji), false);
});

test("messages without tags get no custom emoji and are never emoji-only", () => {
  const customEmoji = undefined;
  assert.equal(isEmojiOnlyMessage(":punks:", customEmoji), false);
});
