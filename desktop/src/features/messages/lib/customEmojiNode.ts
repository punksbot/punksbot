...
export function registerCustomEmojiMarkdownIt(
  ...
  const RULE_NAME = "punks_custom_emoji";
  const TOKEN_TYPE = "punks_custom_emoji";
  ...
  const rule = (state: any, silent: boolean): boolean => {
    ...
    // word char. This keeps prose and URLs intact — `not:punks:` and
    // `http://x:y:punks:` must NOT turn the inner `:punks:` into an image;
