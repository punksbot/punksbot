...
test("query builders emit the tokens FTS needs, nothing else", () => {
  ...
  assert.equal(
    repositoryDiscussionQuery({ owner: OWNER, dtag: "punks-world" }),
    `${OWNER} punks-world`,
  );
});
...
test("discussionSnippet strips entity links and coordinates", () => {
  assert.equal(
    discussionSnippet(
      `Can someone review punks-local://pr?id=${EVENT_ID}&owner=${OWNER}&d=punks before Friday?`,
    ),
  ...
  assert.equal(
    discussionSnippet(`punks-local://repo?owner=${OWNER}&d=punks`),
