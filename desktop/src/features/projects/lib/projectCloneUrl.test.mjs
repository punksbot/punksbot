...
test("projectRepoHost recognizes a canonical repository on the relay", () => {
  assert.deepEqual(projectRepoHost(`${ORIGIN}/git/${OWNER}/punks`, ORIGIN), {
    kind: "punks",
  });
});
...
test("projectRepoHost fails closed while either URL is unresolved", () => {
  ...
  assert.deepEqual(projectRepoHost(`${ORIGIN}/git/${OWNER}/punks`, null), {
    kind: "unresolved",
  });
...
test("projectRepoHostForProject recognizes an implicit relay repository", () => {
  assert.deepEqual(
    projectRepoHostForProject(
      { cloneUrls: [], dtag: "punks", owner: OWNER },
      ORIGIN,
    ),
    { kind: "punks" },
  );
...
test("repositoryDisplayPath fails closed without a resolvable clone URL", () => {
  assert.equal(
    repositoryDisplayPath({ cloneUrls: [], dtag: "punks", owner: OWNER }, null),
    null,
  );
  assert.equal(
    repositoryDisplayPath(
      { cloneUrls: ["not a URL"], dtag: "punks", owner: OWNER },
      ORIGIN,
    ),
    null,
  );
