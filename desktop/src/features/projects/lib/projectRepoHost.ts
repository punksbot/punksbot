...
export function projectRepoHost(
  ...
  try {
    ...
    const isPunksPath = /^\/git\/[0-9a-f]{64}\/[^/]+\/?$/i.test(clone.pathname);

    if (clone.origin === relay.origin && isPunksPath) {
      return { kind: "punks" };
    }
...
/**
 * Human-readable location of a repository's git data — "github.com/punksbot/punksbot"
 * for external repos (host + path, `.git` stripped), or "owner/repo" for
 * Punks-hosted ones (the relay host and full owner pubkey carry no signal;
...
export function repositoryDisplayPath(
  ...
  if (projectRepoHost(cloneUrl, relayOrigin).kind === "punks") {
