...
test("cache write evicts disposable entries and retries at quota", () => {
  const original = window.localStorage;
  const storage = new Map([
    ["punks-channel-messages.v1:relay:old", "big"],
    ["punks-timeline-skeleton-shape.v1:old", "small"],
  ]);
  window.localStorage = {
    get length() {
      return storage.size;
  ...
  try {
    ...
    assert.equal(storage.has("punks-channel-messages.v1:relay:old"), false);
    assert.equal(storage.has("punks-timeline-skeleton-shape.v1:old"), false);
  } finally {
    window.localStorage = original;
  }
});
