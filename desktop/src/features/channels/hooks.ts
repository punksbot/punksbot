...
export const CHANNELS_SNAPSHOT_DIAGNOSTIC_MARK =
  "punks:sidebar:snapshot-diagnostic";
export const CHANNELS_FULL_SIDEBAR_PAINT_MARK =
  "punks:sidebar:full-list-painted";
export const CHANNELS_BOOT_TO_FULL_SIDEBAR_MEASURE =
  "punks:sidebar:boot-to-full-list-painted";
...
export function useSetCanvasMutation(channelId: string | null) {
  ...
  return useMutation({
    mutationFn: (content: string) => {
      ...
      const current = queryClient.getQueryData<CanvasResponse>([
        "channel-canvas",
        channelId,
      ]);
      return setCanvas({
        channelId,
        content,
        expectedRevision: current?.eventId ?? null,
      });
    },
    onSuccess: () => {
      if (channelId) {
