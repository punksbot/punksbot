...
beforeEach(() => {
  ipc.reset();
  localStorage.clear();
  localStorage.setItem("punks-communities", JSON.stringify([COMMUNITY]));
  localStorage.setItem("punks-active-community-id", COMMUNITY.id);
});
...
test("markdown message links resolve private destinations without a directory scan", async () => {
  ...
  const link = `punks-local://message?channel=${channelId}&id=${messageId}`;
  const renderPaths = [
  ...
  for (const [path, content] of renderPaths) {
    ...
    assert.notEqual(
      mounted.container.querySelector(
        "span[data-message-link][data-punks-link]",
      ),
      null,
...
test("authored-label channel and message links respect the private-destination gate", async () => {
  ...
  //   - punks-local://channel/<uuid> and punks-local://channel/<uuid>/<event-id> reach the
  //     gate via ChannelDeepLinkAnchor's authored branch, and
  //   - the canonical punks-local://message?channel=&id= form (produced by
  ...
  const channelMessageLink = `punks-local://channel/${channelId}/${messageId}`;
  const canonicalMessageLink = `punks-local://message?channel=${channelId}&id=${messageId}`;
  const renderPaths = [
  ...
  for (const [path, content] of renderPaths) {
    ...
    assert.notEqual(
      mounted.container.querySelector("span[data-punks-link]"),
      null,
