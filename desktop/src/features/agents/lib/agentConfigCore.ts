...
import { PUNKS_AGENT_THINKING_EFFORT } from "../ui/punksAgentConfig";
...
export function deriveAgentConfigFieldModel({
  ...
  if (runtime?.thinkingEnvVar) {
    fields.push({
      ...
      render: "control",
      value: valueFromEnv(config, PUNKS_AGENT_THINKING_EFFORT),
    });
 ...
 * - Per-agent punks-agent: effort key + 3 numeric keys
 * - Per-agent Goose: 2 numeric keys only — Goose effort (PUNKS_AGENT_THINKING_EFFORT)
