...
test("mapping carries the runtime and definition fields", async () => {
  ...
  assert.equal(input.acpCommand, "punks-acp");
...
test("Punks shared compute definition carries native provider and auto model", async () => {
  const input = await buildInstanceInputForDefinition(
    persona({
      runtime: "punks-agent",
      provider: "relay-mesh",
    ...
    { ...gooseRuntime, id: "punks-agent", command: "punks-agent" },
  );
  assert.equal(input.agentCommand, "punks-agent");
...
// ── item-13 regression: punks-agent-first default runtime ─────────────────────
//
// Before this fix, resolveStartRuntimeForDefinition used runtimes[0] (catalog
// order: goose, claude, codex, punks-agent), so an installed goose would beat
// the bundled punks-agent sidecar as the default for runtime-less personas.
// The fix applies the preference order: punks-agent → goose → first available.

test("item-13: goose+punks-agent both available — persona with no runtime resolves punks-agent", () => {
  const { runtime, warnings } = resolveStartRuntimeForDefinition(
    persona({ runtime: undefined }),
    [gooseRuntime, claudeRuntime, punksAgentRuntime],
  );
  assert.equal(
    runtime.id,
    "punks-agent",
    "punks-agent must win over catalog-first goose for runtime-less personas",
  );
  assert.deepEqual(warnings, []);
});
