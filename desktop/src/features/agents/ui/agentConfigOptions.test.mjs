...
test("getPersonaProviderOptions returns databricks v1 and v2 when hideProviderIds is empty", () => {
  const options = getPersonaProviderOptions("", "punks-agent", "", new Set());
...
test("getPersonaProviderOptions with no hideProviderIds omits the tail for a known provider", () => {
  const options = getPersonaProviderOptions("anthropic", "punks-agent");
  const tail = options.at(-1);
...
test("getDefaultPersonaRuntime ignores an unavailable global preference", () => {
  ...
  assert.equal(getDefaultPersonaRuntime(runtimes, "claude")?.id, "punks-agent");
});

test("getDefaultPersonaRuntime returns punks-agent over goose when both are available", () => {
  const runtimes = [
    makeRuntime("goose"),
    makeRuntime("punks-agent"),
    makeRuntime("claude"),
  ...
  assert.equal(result?.id, "punks-agent");
});

test("getDefaultPersonaRuntime falls back to goose when punks-agent is unavailable", () => {
  const runtimes = [
    makeRuntime("punks-agent", "not_installed"),
    makeRuntime("goose"),
  ];
  const result = getDefaultPersonaRuntime(runtimes);
...
test("getDefaultPersonaRuntime returns first available when neither punks-agent nor goose is available", () => {
  const runtimes = [
    makeRuntime("punks-agent", "adapter_missing"),
...
test("runtimeSupportsLlmProviderSelection is true for punks-agent and goose", () => {
  assert.equal(runtimeSupportsLlmProviderSelection("punks-agent"), true);
...
test("resetConfigForHarnessChange clears harness-specific values", () => {
  const config = {
    env_vars: { PUNKS_AGENT_THINKING_EFFORT: "high", KEEP_ME: "yes" },
    model: "claude-opus",
...
test("getPersonaModelOptions for punks-agent with anthropic filters out zero-value default", () => {
  // anthropic requires explicit model — zero-value option is filtered out
  const options = getPersonaModelOptions("punks-agent", "anthropic");
...
test("getPersonaModelOptions for punks-agent with no provider returns default model", () => {
  const options = getPersonaModelOptions("punks-agent", "");
