...
function upsertTool(
  ...
  const existing = d.itemsById.get(id);
  const canonicalPunksToolName =
    punksToolName ?? findPunksToolName(toolName, true);
  if (existing?.type === "tool") {
    const updatedTitle = !isGenericToolTitle(title) ? title : existing.title;
    let updatedToolName = existing.toolName;
    let updatedPunksToolName = existing.punksToolName;
    if (canonicalPunksToolName) {
      updatedPunksToolName = canonicalPunksToolName;
      updatedToolName = canonicalPunksToolName;
    } else if (!existing.punksToolName && !isGenericToolTitle(toolName)) {
    ...
    const descriptor = classifyTool({
      ...
      punksToolName: updatedPunksToolName,
      args: updatedArgs,
  ...
  const resolvedToolName = canonicalPunksToolName ?? toolName;
  const descriptor = classifyTool({
    title,
