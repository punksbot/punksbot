...
test("inheritedRows_structured_keys_excluded_from_generic_rows", () => {
  // PUNKS_AGENT_PROVIDER, PUNKS_AGENT_MODEL, PUNKS_AGENT_THINKING_EFFORT must
  ...
  const allBaked = [
    ...
    { key: "PUNKS_AGENT_MODEL", value: "goose-claude-opus-4-8", masked: false },
    { key: "PUNKS_AGENT_THINKING_EFFORT", value: "medium", masked: false },
    {
      key: "DATABRICKS_HOST",
      value: "https://example.databricks.com/",
      masked: false,
...
// env vars (e.g. PUNKS_AGENT_MAX_ROUNDS) that are owned by first-class controls
...
test("buildRecord_hidden_tuning_key_unchanged_when_generic_row_edited", () => {
  // Structured field set PUNKS_AGENT_MAX_ROUNDS to "50"; it lives in value
  ...
  const value = { PUNKS_AGENT_MAX_ROUNDS: "50", MY_VAR: "old" };
...
test("buildRecord_runtime_switch_new_hiddenKeys_then_generic_edit", () => {
  ...
  //
  // Before switch: agent is punks-agent with PUNKS_AGENT_MAX_ROUNDS = "50" stored
  // in value (set via the numeric tuning control). After switching to Goose,
  // the punks-agent key is no longer hidden — it becomes a visible generic row.
  // The test verifies:
  //   (a) After the switch, the old punks-agent key appears as a generic row
  ...
  const punksAgentRuntime = {
    id: "punks-agent",
    label: "Punks Agent",
    avatarUrl: "",
    availability: "available",
    command: "punks-agent",
    binaryPath: "punks-agent",
    defaultArgs: [],
    mcpCommand: null,
    modelEnvVar: "PUNKS_AGENT_MODEL",
    providerEnvVar: "PUNKS_AGENT_PROVIDER",
    thinkingEnvVar: "PUNKS_AGENT_THINKING_EFFORT",
    maxTokensEnvVar: "PUNKS_AGENT_MAX_OUTPUT_TOKENS",
    contextLimitEnvVar: "PUNKS_AGENT_CONTEXT_LIMIT",
    maxRoundsEnvVar: "PUNKS_AGENT_MAX_ROUNDS",
    installHint: "",
  ...
  const punksDescriptors = deriveNumericDescriptors(punksAgentRuntime);
  const gooseDescriptors = deriveNumericDescriptors(gooseRuntime);
  const punksHiddenKeys = structuredEnvKeys(punksDescriptors);
  const gooseHiddenKeys = structuredEnvKeys(gooseDescriptors);

  // Sanity-check that PUNKS_AGENT_MAX_ROUNDS is hidden under punks-agent but not
  // under Goose — that contrast is what makes it become a generic row.
  assert.ok(
    punksHiddenKeys.includes("PUNKS_AGENT_MAX_ROUNDS"),
    "PUNKS_AGENT_MAX_ROUNDS must be hidden under punks-agent descriptors",
  );
  assert.equal(
    gooseHiddenKeys.includes("PUNKS_AGENT_MAX_ROUNDS"),
    false,
    "PUNKS_AGENT_MAX_ROUNDS must not be hidden under Goose descriptors",
  );

  // Pre-switch value: punks-agent max-rounds was set, GOOSE_MAX_TOKENS was
  ...
  const valueBeforeSwitch = {
    PUNKS_AGENT_MAX_ROUNDS: "50",
    GOOSE_MAX_TOKENS: "16384",
    USER_VAR: "original",
  };

  // After the switch to Goose, toRows is reproj with the new (Goose) hidden
  // set. PUNKS_AGENT_MAX_ROUNDS is no longer hidden → appears as a generic row.
  ...
  assert.ok(
    rowsAfterSwitch.some((r) => r.key === "PUNKS_AGENT_MAX_ROUNDS"),
...
test("filterBakedGenericRows_numeric_baked_key_excluded_and_placeholder_shown", () => {
  ...
  //
  // The global baked env contains PUNKS_AGENT_MAX_OUTPUT_TOKENS = "4096"
  ...
  const numericDescriptors = deriveNumericDescriptors(punksAgentRuntime);
  const numericStructuredKeys = structuredEnvKeys(numericDescriptors);

  assert.ok(
    numericStructuredKeys.includes("PUNKS_AGENT_MAX_OUTPUT_TOKENS"),
    "numeric key must appear in structured keys via production helpers",
  );

  // Simulate the baked env: PUNKS_AGENT_MAX_OUTPUT_TOKENS is baked, plus a
  // non-structured baked var.
  const bakedEnv = [
    { key: "PUNKS_AGENT_MAX_OUTPUT_TOKENS", value: "4096" },
  ...
  assert.equal(
    genericRows.some((r) => r.key === "PUNKS_AGENT_MAX_OUTPUT_TOKENS"),
    false,
...
test("buildRecord_clearing_structured_field_allows_placeholder_to_return", () => {
  ...
  //
  // Step 1: value has PUNKS_AGENT_MAX_ROUNDS = "50" (user set it via the
  //         structured field). PUNKS_AGENT_MAX_ROUNDS is in hiddenKeys.
  ...
  // After the clear, value no longer contains PUNKS_AGENT_MAX_ROUNDS.
  ...
  assert.equal(
    "PUNKS_AGENT_MAX_ROUNDS" in record,
    false,
