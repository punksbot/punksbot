...
import {
  ...
  PUNKS_AGENT_THINKING_EFFORT_VALUES,
  getProviderEffortConfig,
  isPunksAgentRuntime,
} from "./punksAgentConfig.ts";

// ---------------------------------------------------------------------------
// Thinking effort values
// ---------------------------------------------------------------------------

test("PUNKS_AGENT_THINKING_EFFORT_VALUES contains exactly the 7 accepted values", () => {
  assert.deepEqual(
    [...PUNKS_AGENT_THINKING_EFFORT_VALUES],
...
test("PUNKS_AGENT_THINKING_EFFORT_VALUES has no duplicates", () => {
  const set = new Set(PUNKS_AGENT_THINKING_EFFORT_VALUES);
  assert.equal(set.size, PUNKS_AGENT_THINKING_EFFORT_VALUES.length);
});
...
test("env var key constants match expected PUNKS_AGENT_* names", () => {
  assert.equal(PUNKS_AGENT_THINKING_EFFORT, "PUNKS_AGENT_THINKING_EFFORT");
  assert.equal(PUNKS_AGENT_MAX_OUTPUT_TOKENS, "PUNKS_AGENT_MAX_OUTPUT_TOKENS");
  assert.equal(
    PUNKS_AGENT_MAX_CONTEXT_TOKENS,
    "PUNKS_AGENT_MAX_CONTEXT_TOKENS",
  );
  assert.equal(PUNKS_AGENT_MAX_ROUNDS, "PUNKS_AGENT_MAX_ROUNDS");
});
...
test("isPunksAgentRuntime returns true only for punks-agent id", () => {
  assert.equal(isPunksAgentRuntime("punks-agent"), true);
});

test("isPunksAgentRuntime returns false for other runtimes", () => {
  assert.equal(isPunksAgentRuntime("goose"), false);
  assert.equal(isPunksAgentRuntime("custom"), false);
  assert.equal(isPunksAgentRuntime(""), false);
  assert.equal(isPunksAgentRuntime("punks-agent-v2"), false);
});
...
test("setting a thinking effort value writes the key into envVars", () => {
  ...
  assert.equal(result[PUNKS_AGENT_THINKING_EFFORT], "high");
});

test("clearing thinking effort removes the key so the agent inherits", () => {
  const initial = { [PUNKS_AGENT_THINKING_EFFORT]: "high" };
  const result = applyEnvVarChange(initial, PUNKS_AGENT_THINKING_EFFORT, "");
  assert.equal(Object.hasOwn(result, PUNKS_AGENT_THINKING_EFFORT), false);
});

test("setting max output tokens writes the exact PUNKS_AGENT_MAX_OUTPUT_TOKENS key", () => {
  const initial = {};
  const result = applyEnvVarChange(
    initial,
    PUNKS_AGENT_MAX_OUTPUT_TOKENS,
  ...
  assert.equal(result[PUNKS_AGENT_MAX_OUTPUT_TOKENS], "4096");
  // Must not affect other keys
  assert.equal(Object.keys(result).length, 1);
});

test("clearing max output tokens removes the key", () => {
  const initial = { [PUNKS_AGENT_MAX_OUTPUT_TOKENS]: "4096" };
  const result = applyEnvVarChange(initial, PUNKS_AGENT_MAX_OUTPUT_TOKENS, "");
  assert.equal(Object.hasOwn(result, PUNKS_AGENT_MAX_OUTPUT_TOKENS), false);
});

test("setting context limit writes the exact PUNKS_AGENT_MAX_CONTEXT_TOKENS key", () => {
  const initial = {};
  const result = applyEnvVarChange(
    initial,
    PUNKS_AGENT_MAX_CONTEXT_TOKENS,
  ...
  assert.equal(result[PUNKS_AGENT_MAX_CONTEXT_TOKENS], "100000");
});

test("clearing context limit removes the key", () => {
  const initial = { [PUNKS_AGENT_MAX_CONTEXT_TOKENS]: "100000" };
  const result = applyEnvVarChange(initial, PUNKS_AGENT_MAX_CONTEXT_TOKENS, "");
  assert.equal(Object.hasOwn(result, PUNKS_AGENT_MAX_CONTEXT_TOKENS), false);
});

test("setting max rounds writes the exact PUNKS_AGENT_MAX_ROUNDS key", () => {
  const initial = {};
  const result = applyEnvVarChange(initial, PUNKS_AGENT_MAX_ROUNDS, "50");
  assert.equal(result[PUNKS_AGENT_MAX_ROUNDS], "50");
});

test("clearing max rounds removes the key", () => {
  const initial = { [PUNKS_AGENT_MAX_ROUNDS]: "50" };
  const result = applyEnvVarChange(initial, PUNKS_AGENT_MAX_ROUNDS, "");
  assert.equal(Object.hasOwn(result, PUNKS_AGENT_MAX_ROUNDS), false);
});

test("changing one field does not disturb other env vars", () => {
  const initial = {
    SOME_OTHER_KEY: "value",
    [PUNKS_AGENT_MAX_OUTPUT_TOKENS]: "2048",
  };
  const result = applyEnvVarChange(initial, PUNKS_AGENT_MAX_ROUNDS, "20");
  assert.equal(result.SOME_OTHER_KEY, "value");
  assert.equal(result[PUNKS_AGENT_MAX_OUTPUT_TOKENS], "2048");
  assert.equal(result[PUNKS_AGENT_MAX_ROUNDS], "20");
});
...
test("thinking effort select is bounded: all 7 accepted values are present in the constant", () => {
  ...
  for (const v of expected) {
    assert.ok(
      PUNKS_AGENT_THINKING_EFFORT_VALUES.includes(v),
      `missing value: ${v}`,
    );
  }
  assert.equal(PUNKS_AGENT_THINKING_EFFORT_VALUES.length, expected.length);
});
...
test("non-numeric string is stored as-is (validation is at the backend)", () => {
  ...
  assert.equal(result[PUNKS_AGENT_MAX_OUTPUT_TOKENS], "not-a-number");
});
...
// Mirrors the `isPunksAgent` derivation in CreateAgentRuntimeFields.
...
test("isPunksAgentRuntime(prospectiveRuntimeId) shows fields when Edit resolves punks-agent even though selectedRuntimeId sentinel is 'inherit'", () => {
  // Simulates Edit dialog state: inheritHarness=true, persona is punks-agent.
  // selectedRuntimeId would be "inherit" (sentinel for custom-command hiding),
  // but prospectiveRuntimeId correctly resolves to "punks-agent".
  const selectedRuntimeIdSentinel = "inherit"; // what Edit passes to selectedRuntimeId
  const prospectiveRuntimeId = "punks-agent"; // what Edit passes to modelTuningRuntimeId

  assert.equal(
    isPunksAgentRuntime(selectedRuntimeIdSentinel),
    false,
    "sentinel 'inherit' must NOT trigger model-tuning fields",
  );
  assert.equal(
    isPunksAgentRuntime(prospectiveRuntimeId),
    true,
    "prospectiveRuntimeId 'punks-agent' MUST trigger model-tuning fields",
  );
});

test("isPunksAgentRuntime(prospectiveRuntimeId) shows fields when Edit has a pinned punks-agent (selectedRuntimeId sentinel is also 'inherit')", () => {
  // Simulates Edit dialog with a pinned non-custom runtime:
  // selectedRuntimeId sentinel = "inherit" (non-custom known runtime),
  // prospectiveRuntimeId = "punks-agent" (selectedRuntime?.id).
  const selectedRuntimeIdSentinel = "inherit";
  const prospectiveRuntimeId = "punks-agent";

  assert.equal(isPunksAgentRuntime(prospectiveRuntimeId), true);
  assert.equal(isPunksAgentRuntime(selectedRuntimeIdSentinel), false);
});

test("isPunksAgentRuntime(prospectiveRuntimeId) hides fields when Edit resolves to non-punks-agent", () => {
  // E.g. user switches from punks-agent to goose in Edit — prospectiveRuntimeId = "goose"
  const prospectiveRuntimeId = "goose";
  assert.equal(isPunksAgentRuntime(prospectiveRuntimeId), false);
});

test("isPunksAgentRuntime(prospectiveRuntimeId) hides fields when Edit has no resolved runtime (empty string)", () => {
...
// useEffect in PunksAgentModelTuningFields and AgentDefaultsSettingsCard
