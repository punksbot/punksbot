...
test("editAgent_globalProvider_globalApiKey_noPerAgentEnv_notMissing", () => {
  // Setup: punks-agent runtime, provider = anthropic (from global), key in globalEnvVars.
  ...
  assert.ok(
    ...
    "ANTHROPIC_API_KEY must appear in raw required keys for punks-agent/anthropic",
  );
...
test("editAgent_globalProvider_globalApiKeyEmpty_stillMissing", () => {
  // Setup: punks-agent runtime, provider = anthropic (from global), key NOT in globalEnvVars.
...
test("editAgent_globalSatisfiesOneKey_otherKeyStillMissing", () => {
  ...
  const allKeysOpenai = requiredCredentialEnvKeys("punks-agent", "openai");
