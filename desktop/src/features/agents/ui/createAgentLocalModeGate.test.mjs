...
test("localMode_punksAgent_supportsProviderSelection", () => {
  assert.equal(
    runtimeSupportsLlmProviderSelection("punks-agent"),
    true,
    "punks-agent must support LLM provider selection",
  );
});
...
test("localMode_punksAgent_emptyProvider_notSatisfied", () => {
  // Scenario: user selects punks-agent but leaves provider empty.
  // Rust readiness requires PUNKS_AGENT_PROVIDER — empty = NotReady.
...
test("localMode_punksAgent_emptyModel_notSatisfied", () => {
  // Scenario: punks-agent + anthropic + API key present, but model left empty.
  // Rust readiness requires PUNKS_AGENT_MODEL — empty = NotReady.
...
// ── Gate: punks-agent / anthropic with missing key → markers shown ─────────

test("localMode_punksAgent_anthropic_missingKey_notSatisfied", () => {
  // Scenario: user selects punks-agent/anthropic + fills model, but hasn't
...
test("localMode_punksAgent_anthropic_allRequired_present_allowed", () => {
...
test("localMode_requiredEnvKeys_surfaces_anthropicKey", () => {
  ...
  const allKeys = requiredCredentialEnvKeys("punks-agent", "anthropic");
  assert.ok(
    allKeys.includes("ANTHROPIC_API_KEY"),
    "requiredCredentialEnvKeys must include ANTHROPIC_API_KEY for punks-agent/anthropic",
  );
});
...
test("localMode_requiredEnvKeys_gate_and_envVarsEditor_share_same_key_set", () => {
  ...
  const fullKeys = requiredCredentialEnvKeys("punks-agent", "anthropic");
...
test("localMode_goose_providerSatisfiedByFileConfig_noNormalizedFieldRequired", () => {
  // Scenario: goose, no provider in Punks env but file config has provider + model.
...
test("providerDefaultLabel_noGlobal_returnsSelectAProvider", () => {
  ...
  const label = getDefaultLlmProviderLabel("punks-agent", undefined);
  assert.equal(
    label,
    "Select a provider\u2026",
...
test("providerDefaultLabel_emptyGlobal_returnsSelectAProvider", () => {
  ...
  const label = getDefaultLlmProviderLabel("punks-agent", "");
  assert.equal(
    label,
    "Select a provider\u2026",
...
test("providerDefaultLabel_globalSet_returnsInheritLabel", () => {
  ...
  const label = getDefaultLlmProviderLabel("punks-agent", "anthropic");
  assert.equal(
    label,
...
test("providerDefaultLabel_globalSetWithWhitespace_trimsAndReturnsInherit", () => {
  ...
  const label = getDefaultLlmProviderLabel("punks-agent", "  openai  ");
  assert.equal(
    label,
...
test("providerDefaultLabel_sharedCompute_neverLeaksInternalId", () => {
  assert.equal(
    getDefaultLlmProviderLabel("punks-agent", "relay-mesh"),
    "Use agent defaults (Punks shared compute)",
  );
});
...
test("bakedDefaults_emptyGlobal_usesBuildValuesForCreateAndEditLabels", () => {
  const bakedEnv = [
    { key: "PUNKS_AGENT_PROVIDER", value: "databricks_v2", masked: false },
    {
      key: "PUNKS_AGENT_MODEL",
      value: "goose-claude-opus-4-8",
      masked: false,
    },
    { key: "PUNKS_AGENT_THINKING_EFFORT", value: "high", masked: false },
  ];
  ...
  const model = resolveInheritedDefault(null, bakedEnv, "PUNKS_AGENT_MODEL");
  const effort = resolveInheritedDefault(
    null,
    bakedEnv,
    "PUNKS_AGENT_THINKING_EFFORT",
  );
  const providerOptions = getPersonaProviderOptions("", "punks-agent");
...
test("bakedDefaults_explicitGlobalsOverrideBuildValues", () => {
  const bakedEnv = [
    { key: "PUNKS_AGENT_PROVIDER", value: "databricks_v2", masked: false },
    { key: "PUNKS_AGENT_MODEL", value: "build-model", masked: false },
    { key: "PUNKS_AGENT_THINKING_EFFORT", value: "high", masked: false },
  ];

  assert.deepEqual(
    resolveInheritedDefault("anthropic", bakedEnv, "PUNKS_AGENT_PROVIDER"),
    { source: "global", value: "anthropic" },
  );
  assert.deepEqual(
    resolveInheritedDefault("global-model", bakedEnv, "PUNKS_AGENT_MODEL"),
    { source: "global", value: "global-model" },
  );
  assert.deepEqual(
    resolveInheritedDefault("low", bakedEnv, "PUNKS_AGENT_THINKING_EFFORT"),
...
test("f3b_buildTemplateModelDropdownOptions_anthropicGlobalModelSet_containsInheritOption", () => {
  ...
  const staticOptions = getPersonaModelOptions("punks-agent", "anthropic");
...
test("f3b_buildTemplateModelDropdownOptions_blankProviderGlobalModelSet_noDoubleSeed", () => {
  ...
  const staticOptions = getPersonaModelOptions("punks-agent", "");
...
// include ANTHROPIC_API_KEY for explicit punks-agent/anthropic so that the
...
test("providerConfig_explicitAnthropic_requiredKeysIncludesApiKey", () => {
  ...
  const required = requiredCredentialEnvKeys("punks-agent", "anthropic");
  assert.ok(
    required.includes("ANTHROPIC_API_KEY"),
    "punks-agent + explicit anthropic must list ANTHROPIC_API_KEY as required",
  );
});
...
test("providerConfig_inheritThenExplicitAnthropic_sameRequiredKeys", () => {
  ...
  const inheritKeys = requiredCredentialEnvKeys("punks-agent", "");
  const explicitKeys = requiredCredentialEnvKeys("punks-agent", "anthropic");
  assert.ok(
...
test("providerConfig_databricks_requiredKeyIsHost_noSecretClear", () => {
  ...
  const databricksKeys = requiredCredentialEnvKeys("punks-agent", "databricks");
  assert.ok(
    databricksKeys.includes("DATABRICKS_HOST"),
    "punks-agent + databricks must list DATABRICKS_HOST as required",
  );
