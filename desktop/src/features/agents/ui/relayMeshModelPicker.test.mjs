...
test("Punks shared compute maps persisted auto to Default and hides custom input", () => {
...
test("Punks shared compute fallback is Default auto while normal providers remain unchanged", () => {
...
test("Punks shared compute keeps Default auto when discovery is empty", () => {
  const state = relayMeshModelPickerState({
    discoveredOptions: [],
    fallbackOptions: fallback,
    isCustomEditing: false,
...
test("Punks shared compute dropdown contains Default plus live models and no custom escape hatch", () => {
