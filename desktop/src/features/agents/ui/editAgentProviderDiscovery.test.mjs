...
test("editAgent_providerFieldVisible_forPunksAgent", () => {
  assert.equal(
    runtimeSupportsLlmProviderSelection("punks-agent"),
    true,
    "punks-agent runtime must expose the provider picker",
  );
});
...
test("editAgent_providerOptions_includesDatabricksV2AndV1OnOSS", () => {
  ...
  const options = getPersonaProviderOptions("", "punks-agent");
...
test("editAgent_providerOptions_includesCurrentIfCustom", () => {
  const options = getPersonaProviderOptions("my-custom-llm", "punks-agent");
  const ids = options.map((o) => o.id);
  assert.ok(
...
//
// Switching to punks-agent runtime (which supports LLM provider selection)
// must make the provider field visible, enabling live discovery.

test("editAgent_runtimeSwitch_toPunksAgentEnablesProvider", () => {
  // Simulate: user switches from "claude" to "punks-agent"
  const previousRuntime = "claude";
  const nextRuntime = "punks-agent";
...
test("editAgent_modelClearedOnRuntimeChange", () => {
  const previousRuntime = "punks-agent";
...
test("editAgent_catalogArrival_rederivesRuntimeIdWhenNotTouched", () => {
  ...
  const agentCommand = "/usr/local/bin/punks-agent";
  const catalog = [
    { id: "punks-agent", command: agentCommand, defaultArgs: [] },
...
test("editAgent_noOpSavePreservesProvider_whenCatalogLate", () => {
  ...
  // If the catalog-arrival effect correctly sets selectedRuntimeId = "punks-agent",
  // then llmProviderFieldVisible = true and the provider is preserved on save.
  const selectedRuntimeId = "punks-agent"; // correctly derived after catalog arrival
  const savedProvider = "databricks_v2";
  ...
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field must be visible once catalog derives punks-agent runtime",
  );
  assert.equal(
    providerUpdate,
    undefined,
...
test("editAgent_runtimeDropdown_pinsHarnessWhenConcreteCatalogRuntimeSelected", () => {
  ...
  const _nextRuntimeId = "punks-agent";
  const catalogRuntime = {
    id: "punks-agent",
    command: "/usr/local/bin/punks-agent",
    defaultArgs: [],
  };
...
test("editAgent_runtimeDropdown_keepsInheritWhenCatalogEntryHasNoCommand", () => {
  ...
  const NO_RUNTIME_DROPDOWN_VALUE = "__none__";
  const nextValue = "punks-agent"; // catalog entry, but adapter missing
  const nextRuntimeId =
    nextValue === NO_RUNTIME_DROPDOWN_VALUE ? "" : nextValue;
  const resolvedRuntimeId = nextRuntimeId || "custom";
  const nextRuntime = { id: "punks-agent", command: null, defaultArgs: [] };
...
test("editAgent_inheritedAgentRuntimeSwitch_producesConsistentCommandProviderPair", () => {
  ...
  //
  // After fix: selecting punks-agent sets inheritHarness=false, so agentCommandUpdate
  // resolves to the punks-agent command, and provider persists consistently.

  // Initial state: inherited Claude agent
  const inheritHarness = false; // after fix: pinned by runtime switch
  const selectedRuntimeCommand = "/usr/local/bin/punks-agent";
  ...
  const selectedRuntimeId = "punks-agent";
  const llmProviderFieldVisible =
    runtimeSupportsLlmProviderSelection(selectedRuntimeId);
...
test("editAgent_inheritCheckboxRoundTrip_doesNotPersistProviderOnInheritedRuntime", () => {
  ...
  // → user picks punks-agent (inheritHarness→false, selectedRuntimeId='punks-agent')
  // → user picks databricks_v2
  // → user RE-CHECKS inherit (inheritHarness→true, selectedRuntimeId STAYS 'punks-agent')
  ...
  const selectedRuntimeId = "punks-agent"; // dropdown state (stale after re-check)
  ...
  const chosenProvider = "databricks_v2"; // chosen while dropdown was punks-agent

  // llmProviderFieldVisible is driven by the live dropdown (punks-agent → true).
  ...
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field is visible (dropdown shows punks-agent) — this is the UX state",
  );
...
test("editAgent_inheritCheckboxRoundTrip_clearsStaleSavedProviderWhenRevertingToInherit", () => {
  // Variant: agent previously had a provider saved (e.g. was pinned to punks-agent
  ...
  const selectedRuntimeId = "punks-agent"; // dropdown state
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
...
// persona's runtime is punks-agent/Goose legitimately carries a provider
...
test("editAgent_inheritedPunksAgentProvider_preservedOnNameOnlySave", () => {
  // Inherited punks-agent persona with databricks_v2 snapshot.
  // User makes a name-only edit (never touches runtime or provider).
  // The catalog-arrival effect correctly derived selectedRuntimeId="punks-agent".

  const inheritHarness = true; // agentCommandOverride == null → inheriting
  const agentCommand = "/usr/local/bin/punks-agent"; // inherited punks-agent command
  const runtimes = [
    {
      id: "punks-agent",
      command: "/usr/local/bin/punks-agent",
  ...
  const selectedRuntimeId = "punks-agent"; // correctly derived by catalog-arrival effect
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
  const savedProvider = "databricks_v2"; // valid provider snapshot
  const currentProvider = "databricks_v2"; // unchanged by user

  // llmProviderFieldVisible (UX) is true since dropdown shows punks-agent.
  const llmProviderFieldVisible =
    runtimeSupportsLlmProviderSelection(selectedRuntimeId);
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field must be visible for inherited punks-agent",
  );

  // The effective runtime for submit: inherited → match agentCommand in catalog → punks-agent.
  const llmProviderCanPersistAtSubmit = computeCanPersistAtSubmit({
    inheritHarness,
    agentCommand,
    runtimes,
  ...
  assert.equal(
    llmProviderCanPersistAtSubmit,
    true,
    "inherited punks-agent runtime IS provider-capable — provider must be persistable",
  );
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "name-only save on inherited punks-agent must omit provider (not send null to clear it)",
  );
});

test("editAgent_inheritedPunksAgentProvider_clearsWhenUserSwitchesToInheritedClaude", () => {
  // An agent inheriting punks-agent with databricks_v2, but the persona was
...
test("editAgent_findingE_emptyCatalog_providerOmittedNotCleared", () => {
  ...
  // Inherited punks-agent agent with saved databricks_v2.
  // A name-only save must omit provider (not send null).

  const inheritHarness = true;
  const agentCommand = "punks-agent"; // inherited command (short form)
...
test("editAgent_findingE_commandNullCatalogEntry_providerPreservedByIdMatch", () => {
  // Form 2: catalog loaded, but punks-agent's entry has command:null (adapter
  ...
  const inheritHarness = true;
  const agentCommand = "punks-agent"; // inherited command (short form = runtime id)
  const runtimes = [
    { id: "punks-agent", command: null, defaultArgs: [] }, // adapter missing
  ...
  assert.equal(
    capability,
    "capable",
    "command:null punks-agent entry must resolve to 'capable' via id-based fallback",
  );
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "name-only save on inherited punks-agent (command:null) must omit provider, not clear it",
  );
});
...
test("editAgent_findingE_lockedRuntimeStillClears", () => {
  ...
  const runtimes = [
    { id: "punks-agent", command: "punks-agent", defaultArgs: [] },
    { id: "claude", command: "claude-agent-acp", defaultArgs: [] },
  ];
  const selectedRuntimeId = "punks-agent"; // stale dropdown state (irrelevant)
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
...
test("editAgent_findingE_capablePunksAgentLoadedCatalog_preservedOnNoOpSave", () => {
  // Confirm loaded-catalog inherited punks-agent still preserves provider.
  // This is Finding D's good path — must not regress with the tri-state change.

  const inheritHarness = true;
  const agentCommand = "punks-agent";
  const runtimes = [
    { id: "punks-agent", command: "punks-agent", defaultArgs: [] },
    { id: "claude", command: "claude-agent-acp", defaultArgs: [] },
  ];
  ...
  assert.equal(capability, "capable", "loaded punks-agent must be 'capable'");
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "no-op save on inherited punks-agent (loaded catalog) must omit provider",
  );
});
...
//
// Problem: punks-agent stores agentCommand="punks-agent" (short name) while the
// catalog entry has command="/Applications/Punks.app/.../punks-agent" (resolved path).
...
test("editAgent_bugA_inheritedShortName_resolvesViaIdFallback", () => {
  // The core regression: agentCommand is the short name "punks-agent" but the
  // catalog's command is the resolved binary path. Command match fails; id
  // fallback must rescue it.

  const agentCommand = "punks-agent"; // short name stored by effective_agent_command
  const runtimes = [
    {
      id: "punks-agent",
      command: "/Applications/Punks.app/Contents/MacOS/punks-agent", // resolved path
  ...
  assert.equal(
    selectedRuntimeId,
    "punks-agent",
    "short-name agentCommand must resolve to punks-agent id via id-fallback when command path differs",
  );
});
...
test("editAgent_bugA_inheritedShortName_commandMatchStillWinsWhenPresent", () => {
  ...
  const agentCommand = "/usr/local/bin/punks-agent"; // full path (explicit pin)
  const runtimes = [
    {
      id: "punks-agent",
      command: "/usr/local/bin/punks-agent",
...
test("editAgent_bugA_inheritedShortName_discoveryGatePasses", () => {
  // Once selectedRuntimeId is correctly resolved to "punks-agent" via id-fallback,
  ...
  const agentCommand = "punks-agent"; // short name
  const runtimes = [
    {
      id: "punks-agent",
      command: "/Applications/Punks.app/Contents/MacOS/punks-agent",
      availability: "available",
      defaultArgs: [],
    },
  ];
  ...
  assert.equal(
    selectedRuntimeId,
    "punks-agent",
    "id-fallback must resolve selectedRuntimeId to punks-agent",
  );
  assert.ok(
  ...
  assert.equal(
    ...
    "/Applications/Punks.app/Contents/MacOS/punks-agent",
    "discoveryAgentCommand must be the resolved path from the catalog entry",
  );
  assert.equal(
    canDiscoverModelOptions,
    true,
    "discovery gate must pass for inherited punks-agent with databricks_v2 provider once id-fallback resolves the runtime",
  );
});
...
test("requiredCredentialEnvKeys: punks-agent + anthropic → ANTHROPIC_API_KEY", () => {
  const keys = requiredCredentialEnvKeys("punks-agent", "anthropic");
  assert.deepEqual(keys, ["ANTHROPIC_API_KEY"]);
});

test("requiredCredentialEnvKeys: punks-agent + openai → OPENAI_COMPAT_API_KEY", () => {
  const keys = requiredCredentialEnvKeys("punks-agent", "openai");
  assert.deepEqual(keys, ["OPENAI_COMPAT_API_KEY"]);
});

test("requiredCredentialEnvKeys: punks-agent + databricks → DATABRICKS_HOST only (no token)", () => {
  const keys = requiredCredentialEnvKeys("punks-agent", "databricks");
...
test("requiredCredentialEnvKeys: punks-agent + databricks_v2 → DATABRICKS_HOST only", () => {
  const keys = requiredCredentialEnvKeys("punks-agent", "databricks_v2");
  assert.deepEqual(keys, ["DATABRICKS_HOST"]);
});
...
test("requiredCredentialEnvKeys: punks-agent + no provider → empty (provider not yet selected)", () => {
  const keys = requiredCredentialEnvKeys("punks-agent", "");
...
test("blockSave_punksAgentAnthropicMissingKey_blocked", () => {
  // Will's exact case: punks-agent / anthropic / opus / no ANTHROPIC_API_KEY
  const requiredKeys = requiredCredentialEnvKeys("punks-agent", "anthropic");
...
test("blockSave_punksAgentAnthropicKeyProvided_allowed", () => {
  const requiredKeys = requiredCredentialEnvKeys("punks-agent", "anthropic");
...
test("blockSave_punksAgentAnthropicEmptyStringKey_blocked", () => {
...
test("blockSave_punksAgentDatabricksMissingHost_blocked", () => {
  const requiredKeys = requiredCredentialEnvKeys("punks-agent", "databricks");
  const envVars = {};
  assert.equal(
    hasRequiredEnvKeyMissing(requiredKeys, envVars),
    true,
...
test("blockSave_punksAgentDatabricksHostProvided_allowed", () => {
  const requiredKeys = requiredCredentialEnvKeys("punks-agent", "databricks");
...
//   FALSE-ALLOW: claude pin → inherit punks-agent persona → missing ANTHROPIC_API_KEY
//     → must be BLOCKED (prospective runtime is punks-agent/anthropic, key absent)
//   FALSE-BLOCK: punks-agent pin → inherit claude persona
//     → must NOT be blocked (claude has no dialog-fixable credential requirement)

test("blockSave_inheritTransition_claudePin_toPunksAgentPersona_missingKey_blocked", () => {
  ...
  // from persona" where the persona uses punks-agent/anthropic.
  // prospectiveRuntimeId resolves to "punks-agent"; providerForRequiredKeys must
  // use the PROSPECTIVE runtime's provider-field visibility (punks-agent supports
  // provider selection) rather than the current locked runtime's suppression.
  const prospectiveRuntimeId = "punks-agent"; // resolved from persona's agentCommand
  ...
  // providerForRequiredKeys must be "anthropic" (punks-agent supports selection)
  ...
  assert.equal(
    missing,
    true,
    "inheriting punks-agent/anthropic persona with no key must BLOCK save (false-allow prevented)",
  );
});

test("blockSave_inheritTransition_punksAgentPin_toClaudePersona_notBlocked", () => {
  // Scenario: agent is pinned to punks-agent/anthropic. The user checks
