/**
 * Tier-1 punks-agent model-tuning UI fields.
 *
...
/**
 * Shared effort-select dropdown for the `PUNKS_AGENT_THINKING_EFFORT` env var.
 *
 * Used by both `PunksAgentModelTuningFields` (per-agent/persona dialogs) and
...
export function EffortSelectField({
   ...
   * Defaults to `"Inherit"`.
   *
   * Per-agent callers (PunksAgentModelTuningFields) pass `"Inherit (agent default)"`
  ...
  const effortOptions: AgentDropdownOption[] = [
    ...
    ...PUNKS_AGENT_THINKING_EFFORT_VALUES.flatMap((v) => {
...
/**
 * Auto-clear hook: resets `PUNKS_AGENT_THINKING_EFFORT` to "" (Inherit) when
 ...
 *
 * - `maxOutputTokens` / `contextLimit`: minimum 1 — the punks-agent runtime
 *   rejects 0 for these fields (crates/punks-agent/src/config.rs:921-928).
...
export function PunksAgentModelTuningFields({
  envVars,
  inheritedEnvVars,
  model,
  onEnvVarChange,
  ...
  const currentEffort = envVars[PUNKS_AGENT_THINKING_EFFORT] ?? "";
  useEffortAutoClear({
    currentEffort,
    effortValid,
    onClear: () => onEnvVarChange(PUNKS_AGENT_THINKING_EFFORT, ""),
  });

  return (
    <div className="space-y-4">
      <p className="text-2xs font-semibold uppercase tracking-wide text-muted-foreground">
        punks-agent model tuning
      </p>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-1.5">
          <EffortSelectField
            ...
            inheritedEffort={inheritedEnvVars[PUNKS_AGENT_THINKING_EFFORT]}
            inheritFallbackLabel="Inherit (agent default)"
            label="Thinking / Effort"
            onChange={(value) =>
              onEnvVarChange(PUNKS_AGENT_THINKING_EFFORT, value)
            }
