...
export function parsePromptText(text: string): {
  ...
  const eventSection = sections.find((section) => {
    ...
    return title.startsWith("punks event");
  });
  ...
  return {
    ...
    userTitle: eventKind ? titleCase(eventKind) : "Punks event",
 ...
 *    `with_team()` in `punks-acp/src/pool.rs`. Same two cases (start-of-string
 ...
 *    (produced by the now-removed `compose_prompt()` in punks-persona), the body
...
export function parseSystemPromptSections(
  ...
  // with_team() in punks-acp/src/pool.rs appends "\n\n[Team Instructions]\n{instructions}"
...
export function extractToolIdentity(update: Record<string, unknown>): {
  ...
  const knownName = candidates
    .map((candidate) => findPunksToolName(candidate, true))
