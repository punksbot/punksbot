...
test("a saved kubernetes config renders labeled scalar rows", () => {
  const summary = summarizeRunOn({
    ...
    id: "kubernetes",
    config: {
      namespace: "punks-agents-x7k2mp",
      image: "ghcr.io/block/punks-sprig@sha256:17facfc7",
      cpu_request: "1",
  ...
  assert.equal(byKey.namespace.value, "punks-agents-x7k2mp");
  ...
  assert.equal(byKey.image.value, "ghcr.io/block/punks-sprig@sha256:17facfc7");
