 ...
 *
 * Mirrors the Rust `readiness::punks_agent_requirements` /
...
export const PERSONA_LLM_PROVIDER_OPTIONS: readonly PersonaModelOption[] = [
  ...
  { id: "relay-mesh", label: "Punks shared compute" },
...
> = {
  ...
  "punks-agent": [DEFAULT_MODEL_OPTION],
 ...
 *
 * punks-agent and goose use provider-specific credentials; claude and codex
...
export function requiredCredentialEnvKeys(
  ...
  if (normalizedRuntime !== "punks-agent" && normalizedRuntime !== "goose") {
    return [];
  }
...
export function runtimeSupportsLlmProviderSelection(runtimeId: string) {
  return runtimeId === "punks-agent" || runtimeId === "goose";
}
...
export function resetConfigForHarnessChange(
  ...
  delete nextEnvVars[PUNKS_AGENT_THINKING_EFFORT];
 ...
 *
 * Two classes of required field for provider-selection runtimes (punks-agent,
...
export function computeLocalModeGate({
  ...
  /** Env keys that are not set in Punks but are satisfied in the runtime's
