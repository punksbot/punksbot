 ...
 * persona runtime string (e.g. `punks-agent`). A catalog lookup then returns
 ...
 *
 * - punks-agent / goose → "capable" (`isProviderCapable`, id-based).
 ...
 * punks-agent/Anthropic). Persisting the local (empty) provider + credential-
 ...
 *
 * MODEL follows the same transition rule. punks-agent/goose readiness requires a
