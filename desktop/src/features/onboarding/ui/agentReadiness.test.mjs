...
test("resolveAgentReadiness_cli_ignores_punks_agent_runtime", () => {
  // punks-agent with availability=available and logged_in must NOT trigger the CLI path.
  const runtimes = [
    makeRuntime({
      id: "punks-agent",
      label: "punks-agent",
...
test("resolveAgentReadiness_punks_agent_ready_when_provider_model_and_key_set", () => {
  // anthropic requires ANTHROPIC_API_KEY
  const result = resolveAgentReadiness(
    [makeRuntime({ id: "punks-agent", label: "Punks Agent" })],
    makeConfig({
  ...
  assert.deepEqual(result, { ready: true, reason: "punks-agent" });
});

test("resolveAgentReadiness_punks_agent_not_ready_when_missing_required_credential_key", () => {
...
test("resolveAgentReadiness_punks_agent_not_ready_when_provider_missing", () => {
  const config = makeConfig({
    provider: null,
    model: "claude-3-5-sonnet-latest",
    env_vars: { ANTHROPIC_API_KEY: "sk-ant-test" },
...
test("resolveAgentReadiness_punks_agent_not_ready_when_model_missing", () => {
  const config = makeConfig({
    provider: "anthropic",
    model: null,
    env_vars: { ANTHROPIC_API_KEY: "sk-ant-test" },
...
test("resolveAgentReadiness_legacy_config_without_preference_uses_punks_agent_fields", () => {
  const runtimes = [makeRuntime({ id: "punks-agent", label: "Punks Agent" })];
  const result = resolveAgentReadiness(
    runtimes,
    makeConfig({
...
test("resolveAgentReadiness_preferred_goose_does_not_borrow_ready_punks_agent_config", () => {
  const runtimes = [
    makeRuntime({ id: "goose", label: "Goose" }),
    makeRuntime({ id: "punks-agent", label: "Punks Agent" }),
  ];
