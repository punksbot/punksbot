 ...
 * Provider path: the preferred Punks Agent or Goose runtime has provider and
...
export function resolveAgentReadiness(
  ...
  if (scope === "any") {
    for (const runtime of runtimes) {
      if (runtime.id === "punks-agent") continue;
      if (
  ...
  const preferredRuntime =
    scope === "preferred"
      ...
      : runtimes.find((runtime) => runtime.id === "punks-agent");
  ...
  if (
    preferredRuntime.id !== "punks-agent" &&
  ...
  if (provider.length > 0 && model.length > 0) {
    ...
    if (allKeysPresent) {
      return { ready: true, reason: "punks-agent" };
    }
  }
