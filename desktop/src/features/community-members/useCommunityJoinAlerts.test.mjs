...
function seedCommunities(activeId) {
  ...
  storage.set("punks-active-community-id", activeId);
}
...
function ledgerKeys() {
  return [...storage.keys()].filter((key) =>
    key.startsWith("punks-community-join-seen.v1"),
  );
}
...
describe("useCommunityJoinAlerts — mounted subscription behaviour", () => {
  ...
  it("is completely inert for a plain member: no subscription, no ledger key", async () => {
    ...
    assert.deepEqual(
      ledgerKeys(),
      [],
      "no punks-community-join-seen.v1 key may be created for a plain member",
    );
