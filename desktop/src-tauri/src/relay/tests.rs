...
#[test]
fn intercepted_proxy_html_returns_some() {
    let result = classify_intercepted_response("login.proxy.example", "text/html");
    ...
    assert!(
        msg.contains("HTML"),
        "should identify the intercepted response"
    );
}

#[test]
fn ordinary_json_host_is_not_intercepted() {
    let result = classify_intercepted_response("login.proxy.example", "application/json");
    assert!(result.is_none());
}

#[test]
fn intercepted_html_returns_some() {
    let result =
...
#[test]
fn arbitrary_json_host_is_not_intercepted() {
    let result = classify_intercepted_response("arbitrary.example", "application/json");
    assert!(
        result.is_none(),
        "host names alone must not imply interception"
    );
}
...
/// `punks_sdk_pkg::nip_oa::compute_auth_tag` expects nostr 0.36 types.
...
fn make_valid_auth_tag(agent_keys: &nostr::Keys) -> String {
    ...
    punks_sdk_pkg::nip_oa::compute_auth_tag(&owner_keys, &agent_compat_pubkey, "")
