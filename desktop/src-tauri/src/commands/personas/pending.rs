...
pub(in crate::commands) fn retain_persona_pending(
    ...
    if let Err(e) = prepare_persona_publication(app, state, persona, None) {
        eprintln!("punks-full-local: persona-retain: {e}");
    }
}
...
fn retained_persona_is_shared(row: Option<&RetainedEvent>) -> bool {
    use nostr::JsonUtil;
    use punks_core_pkg::kind::event_is_shared;
...
fn project_scoped_persona_sharing(
    ...
    if let Err(error) = projected {
        eprintln!("punks-full-local: persona-share-projection unavailable, reporting every agent as unshared: {error}");
...
pub(in crate::commands) fn tombstone_persona_pending(
    ...
    if let Err(e) = result {
        eprintln!("punks-full-local: persona-tombstone: {e}");
    }
}
