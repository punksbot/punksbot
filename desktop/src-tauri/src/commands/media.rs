...
pub(crate) fn sign_blossom_get_auth_header(
    ...
    let event = EventBuilder::new(Kind::from(24242), "Get punks-media")
        .tags(tags)
        .sign_with_keys(keys)
        .map_err(|e| e.to_string())?;
    Ok(format!(
...
pub(crate) fn mint_media_get_auth(state: &AppState, base_url: &str) -> Option<String> {
    let keys = match state.signing_keys() {
        Ok(k) => k,
        Err(e) => {
            eprintln!("punks-full-local: media get auth unavailable (unsigned request): {e}");
            return None;
        }
    };
    match sign_blossom_get_auth_header(&keys, base_url, MEDIA_GET_AUTH_EXPIRY_SECS) {
        ...
        Err(e) => {
            eprintln!("punks-full-local: media get auth signing failed (unsigned request): {e}");
            None
        }
    }
}
...
fn sign_blossom_upload_auth(
    ...
    EventBuilder::new(Kind::from(24242), "Upload punks-media")
        .tags(tags)
        .sign_with_keys(keys)
        .map_err(|e| e.to_string())
}
...
async fn process_picked_path(
    ...
    if let Some(poster) = poster_bytes {
        match do_upload(poster, "image/jpeg", state, None, None).await {
            ...
            Err(e) => eprintln!("punks-full-local: poster upload failed (non-fatal): {e}"),
        }
    }
