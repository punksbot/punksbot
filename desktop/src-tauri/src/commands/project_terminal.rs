...
fn merge_recovery_ref(expected_commit: &str) -> String {
    format!("refs/punks/merge-recovery/{expected_commit}")
}

fn merge_recovery_target_ref(target_commit: &str) -> String {
    format!("refs/punks/merge-recovery-target/{target_commit}")
}
...
mod tests {
    ...
    fn recovery_ref_is_namespaced_by_verified_commit() {
        ...
        assert_eq!(
            ...
            format!("refs/punks/merge-recovery/{commit}"),
        );
        assert_eq!(
            merge_recovery_target_ref(&commit),
            format!("refs/punks/merge-recovery-target/{commit}"),
        );
    }
}
