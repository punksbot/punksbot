...
const RELAY_MESH_RUNTIME_NO_TARGET: &str =
    "Punks shared compute requires a live serving member; start serving the selected model on a member, then try again";
...
fn restarting_share_status(config: &MeshSharingConfig) -> mesh_llm::MeshNodeStatus {
    mesh_llm::MeshNodeStatus {
        ...
        health: mesh_llm::MeshHealth {
            ...
            reason: Some("Punks is restarting to switch this machine to sharing".to_string()),
        },
        api_base_url: None,
        console_url: None,
...
fn punks_mesh_name_for_relay(relay_url: &str) -> String {
...
pub(super) fn punks_mesh_name(state: &AppState) -> String {
    punks_mesh_name_for_relay(&relay::relay_ws_url_with_override(state))
}
...
pub(crate) async fn resolve_trusted_owner_ids_or_self_only(state: &AppState) -> Vec<String> {
    match resolve_trusted_owner_ids(state).await {
        Ok(owners) => owners,
        Err(error) => {
            eprintln!("punks-mesh: roster query failed; allowing only this node: {error}");
            Vec::new()
        }
    }
}

/// Choose validated live endpoints from other runtimes in this Punks community.
...
/// existing Punks community mesh. `Ok(None)` means this machine is the first
/// live serving member (or is itself the shared bootstrap contact).
pub(crate) async fn resolve_punks_mesh_join_targets_at(
    state: &AppState,
    relay_url: &str,
...
async fn resolve_punks_mesh_startup_at(
    state: &AppState,
    relay_url: &str,
    ...
    match query_mesh_discovery_events_at(state, relay_url).await {
        ...
        Err(error) => {
            ...
            eprintln!(
                "punks-mesh: startup discovery failed; allowing only this node and starting isolated for now: {error}"
            );
            (Vec::new(), None)
        }
    }
...
pub(crate) async fn restore_mesh_sharing(app: &AppHandle, state: &AppState) -> CmdResult<()> {
    ...
    let (trusted_owner_ids, join_token) = resolve_punks_mesh_startup_at(state, &relay_url).await;
    ...
    if config.start_on_next_launch {
        ...
        // work. If Punks exits during that work, the next launch stays stopped.
    ...
    let request = mesh_llm::StartMeshNodeRequest {
        ...
        join_token,
        mesh_name: Some(punks_mesh_name_for_relay(&relay_url)),
    ...
    *runtime = Some(started);
    ...
    if let Err(error) = wait_for_mesh_inference(&config.model_id).await {
        eprintln!(
            "punks-mesh: restored node is not inference-ready yet ({error}); \
...
pub async fn mesh_start_node(
    ...
    if let Some(config) = sharing_config.as_ref() {
        ...
        // long `start()` below: if Punks exits before the runtime is installed
    ...
    let status = match started.status().await {
        ...
        Err(error) => {
            ...
            if let Err(cleanup_error) = &cleanup {
                eprintln!(
                    "punks-mesh: started node status failed and cleanup was incomplete: {cleanup_error:#}"
                );
            }
            ...
            app.request_restart();
            return Err(format!(
                "mesh node started but status probe failed: {error:#}; Punks is restarting to guarantee cleanup"
            ));
        }
    };
    ...
    *runtime = Some(started);
    ...
    if let Some(config) = sharing_config.as_ref() {
        ...
        if let Err(error) = wait_for_mesh_inference(&config.model_id).await {
            eprintln!(
                "punks-mesh: node started but inference is not ready yet ({error}); \
...
pub(crate) async fn ensure_client_node_for_model(
    ...
    let start = mesh_llm::StartMeshNodeRequest {
        ...
        mesh_name: Some(punks_mesh_name(state)),
...
/// encrypted iroh relay addresses, so no Punks relay connection coordination is
...
pub(crate) async fn ensure_relay_mesh_for_record(
    ...
    if state.mesh_llm_runtime.lock().await.is_some() {
        ...
        .await
        {
            ...
            mesh_llm::MeshRuntimeRecovery::Debouncing => {
                return Err(
                    "Punks shared compute ingress is temporarily unresponsive; recovery is already scheduled. Try again shortly."
                        .to_string(),
                );
            }
            mesh_llm::MeshRuntimeRecovery::ReleasePending => {
                return Err(
                    "Punks shared compute is still shutting down its previous local ingress. Try again shortly."
                        .to_string(),
                );
            }
            ...
            mesh_llm::MeshRuntimeRecovery::RestartRequired => {
                app.request_restart();
                return Err(
                    "Punks shared compute startup lost its local ingress before shutdown control became available. Punks is restarting to recover it."
                        .to_string(),
                );
            }
        }
    ...
    let target = match resolve_mesh_bootstrap_target(&state, model_id).await {
        ...
        Ok(None) => {
            return Err(
                "Punks shared compute cannot start because no live member is serving this model. Start serving it on a member, then try again."
                    .to_string(),
            );
        }
        Err(error) => {
            return Err(format!(
                "could not refresh Punks shared compute serving members: {error}"
            ));
        }
    };
