...
async fn resolve_public_addresses(host: &str) -> Result<Vec<IpAddr>, String> {
    ...
    if addresses.iter().any(punks_core_pkg::network::is_private_ip) {
...
async fn send_pinned_request(url: &Url, accept: &str) -> Result<reqwest::Response, String> {
    ...
    let request = client
        ...
        .header(USER_AGENT, "Punks Desktop link preview");
...
mod tests {
    ...
    fn metadata_prefers_open_graph_and_reads_site_name() {
        let html = r#"<meta content="Punks" property="og:site_name">
        ...
        assert_eq!(
            ...
            Some(LinkPreviewMetadata {
                ...
                site_name: Some("Punks".to_string()),
    ...
    fn favicon_metadata_prefers_a_supported_raster_candidate() {
        let page = Url::parse("https://github.com/punksbot/punksbot").unwrap();
