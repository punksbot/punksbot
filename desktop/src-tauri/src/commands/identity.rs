...
pub fn auto_connect_default_relay_enabled() -> bool {
    option_env!("PUNKS_DESKTOP_BUILD_AUTO_CONNECT_DEFAULT_RELAY").is_some()
}
...
mod auto_connect_default_relay_tests {
    ...
    fn compiled_flag_matches_expected() {
        let expected = std::env::var("PUNKS_TEST_EXPECTED_AUTO_CONNECT_DEFAULT_RELAY")
...
pub fn is_shared_identity() -> bool {
    std::env::var("PUNKS_SHARE_IDENTITY")
        .map(|v| v == "1")
        .unwrap_or(false)
        && std::env::var("PUNKS_PRIVATE_KEY")
            .ok()
...
pub async fn decrypt_observer_event(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        punks_core_pkg::observer::decrypt_observer_payload(&keys, &event)
...
pub fn build_observer_control_event(
    ...
    let encrypted =
        punks_core_pkg::observer::encrypt_observer_payload(&keys, &agent_pubkey, &payload)
            .map_err(|error| format!("encrypt observer control failed: {error}"))?;
    let builder = punks_sdk_pkg::build_agent_observer_frame(
        &agent_pubkey_hex,
        &agent_pubkey_hex,
        punks_core_pkg::observer::OBSERVER_FRAME_CONTROL,
        &encrypted,
    )
...
pub async fn import_identity(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        eprintln!("punks-full-local: imported identity pubkey {}", pubkey_hex);
...
pub(crate) fn commit_imported_identity(
    ...
    if let Err(e) = crate::key_backup::cleanup_stale_backup(&previous_pubkey, &pubkey, data_dir) {
        eprintln!(
            "punks-full-local: import committed, but stale key backup cleanup failed: {e}; \
...
/// open.
///
/// Not available in shared-identity mode (`PUNKS_SHARE_IDENTITY=1`): the key
...
pub async fn sign_out(app: tauri::AppHandle) -> Result<(), String> {
    if is_shared_identity() {
        return Err(
            "Sign out isn't available while PUNKS_SHARE_IDENTITY provides your identity. Unset PUNKS_SHARE_IDENTITY and PUNKS_PRIVATE_KEY, then relaunch to sign out."
                .to_string(),
        );
    }
    ...
    if let Err(e) = crate::shutdown::shutdown_managed_agents(&app) {
        eprintln!("punks-full-local sign-out: agent shutdown: {e}");
    }
...
mod nostr_identity_binding_tests {
    ...
    fn build_nostr_identity_binding_event_signs_exact_shape() {
        ...
        assert!(tags.contains(&vec!["audience".into(), "punks:nostr-identity".into()]));
        assert!(tags.contains(&vec!["action".into(), "bind_nostr_identity".into(),]));
        assert!(tags.contains(&vec!["protocol".into(), "punks-nostr-identity".into(),]));
