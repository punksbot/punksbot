...
fn resolve_config_surface(
    ...
    // model/provider/prompt so they can never masquerade as PunksExplicit and
...
/// the binary at compile time via `PUNKS_BUILD_AGENT_ENV`. The backend readiness
...
/// record; at spawn `runtime.rs` injects it as `PUNKS_ACP_EFFORT_LEVEL` and the
