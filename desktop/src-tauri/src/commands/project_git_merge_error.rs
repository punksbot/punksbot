...
impl From<String> for ProjectPullRequestMergeError {
    fn from(message: String) -> Self {
        ...
        if message.contains(punks_core_pkg::git_perms::GIT_NO_CHANNEL_BINDING_TOKEN) {
            return Self::new(
                punks_core_pkg::git_perms::GIT_NO_CHANNEL_BINDING_TOKEN,
                "This repository is not bound to a channel, so the relay cannot \
                 authorize pushes. Bind it with: punks repos bind --id <repo> \
...
mod tests {
    ...
    fn no_channel_binding_denial_converts_to_structured_code() {
        ...
        let remote_stderr = format!(
            ...
            punks_core_pkg::git_perms::GIT_NO_CHANNEL_BINDING_BODY
        );
        ...
        assert!(error.message.contains("punks repos bind"));
