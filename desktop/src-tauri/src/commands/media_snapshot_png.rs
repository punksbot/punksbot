//! Preservation of Punks snapshot tEXt chunks through the upload sanitizer.
//!
...
/// Extract the raw bytes of the first Punks snapshot tEXt chunk (length + type
...
mod tests {
    ...
    fn test_sanitizer_preserves_agent_snapshot_text_chunk() {
        ...
        for keyword in ["punks_agent_snapshot", "punks_team_snapshot"] {
    ...
    fn test_agent_snapshot_survives_full_share_pipeline() {
        ...
        let snapshot = AgentSnapshot {
            format: "punks-agent-snapshot".to_string(),
            version: 1,
        ...
        // Relay ingest path.
        let relay_config = punks_media_pkg::MediaConfig {
            s3_endpoint: String::new(),
            s3_access_key: String::new(),
            s3_secret_key: String::new(),
            s3_bucket: String::new(),
            s3_region: "us-east-1".to_string(),
            s3_addressing_style: punks_media_pkg::S3AddressingStyle::Path,
        ...
        assert_eq!(
            punks_media_pkg::validation::validate_content(&sanitized, &relay_config)
