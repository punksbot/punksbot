...
fn managed_node_unsupported_step() -> InstallStepResult {
    InstallStepResult {
        ...
        stdout: String::new(),
        stderr: format!(
            "Punks does not provide a managed Node.js runtime for {}-{} yet",
            std::env::consts::OS,
            std::env::consts::ARCH
        ),
        exit_code: None,
        hint: Some(
            "Install Node.js from https://nodejs.org, restart Punks, then click Install again."
                .to_string(),
        ),
    }
}
...
fn managed_node_install_hint() -> String {
    "Punks could not install its private Node.js runtime. Check your network and app-data directory permissions, then click Install again.".to_string()
}
...
///
/// Only a Punks-managed shim (path under `managed_prefix`) with an orphaned
...
pub(super) fn managed_node_runtime_supported() -> bool {
    MANAGED_NODE_ARTIFACT.is_some() && crate::managed_agents::punks_managed_node_bin_dir().is_some()
}
...
pub(super) fn ensure_managed_node_runtime_blocking() -> Result<(), Box<InstallStepResult>> {
    ...
    let Some(root) = crate::managed_agents::punks_managed_node_root() else {
        return Err(Box::new(managed_node_failed_step(
            "failed to resolve Punks app-data directory for private Node.js runtime".to_string(),
        )));
    };
...
/// Guidance text shown when the Punks-private npm prefix is not available.
fn managed_npm_prefix_hint() -> String {
    "Punks could not create its private Node tools directory. Check app-data directory permissions, restart Punks, then click Install again.".to_string()
}
...
pub(super) fn managed_npm_command(command: &str) -> Result<Option<String>, Box<InstallStepResult>> {
    ...
    let Some(prefix) = crate::managed_agents::punks_managed_npm_prefix() else {
        return Err(Box::new(InstallStepResult {
            step: "adapter".to_string(),
            command: command.to_string(),
            success: false,
            stdout: String::new(),
            stderr: "failed to resolve Punks app-data directory for private npm prefix".to_string(),
            exit_code: None,
    ...
    if let Err(error) = std::fs::create_dir_all(&prefix) {
        return Err(Box::new(InstallStepResult {
            ...
            stdout: String::new(),
            stderr: format!(
                "failed to create Punks private npm prefix '{}': {error}",
                prefix.display()
            ),
            exit_code: None,
            hint: Some(managed_npm_prefix_hint()),
...
pub(super) fn npm_eacces_hint(stderr: &str, _command: &str) -> Option<String> {
    if stderr.contains("EACCES: permission denied") || stderr.contains("npm error EACCES") {
        Some(
            "npm could not write to Punks's private Node tools directory. Check app-data directory permissions, restart Punks, then click Install again."
                .to_string(),
        )
    } else {
        None
