...
//!
//! Punks publishes a client-signed, replaceable discovery note containing the
...
//! The Punks relay is only a generic Nostr store for membership and discovery;
...
/// NIP-51 bookmark-set kind with a reserved d-tag so existing Punks relays accept
/// and store it through their generic user-state path. The relay needs no mesh
/// handler or kind-registry change.
pub const KIND_PUNKS_MESH_MEMBER_STATUS: u16 = punks_core_pkg::kind::KIND_BOOKMARK_SET as u16;
const STATUS_D_TAG_PREFIX: &str = "punks-mesh-member-status";
...
/// the relay. Recheck promptly so simultaneous starts converge into one Punks
...
pub async fn start_coordinator(app: AppHandle) {
    ...
    let roster_watcher = tokio::spawn(async move {
        ...
        loop {
            ...
            if let Err(error) = reconcile_roster(&roster_app, &mut pending_shrink).await {
                eprintln!("punks-mesh: roster reconcile failed: {error}");
            }
        }
    });
    ...
    let mesh_join_watcher = tokio::spawn(async move {
        ...
        loop {
            ...
            match reconcile_punks_mesh_join(&join_app).await {
                Ok(()) => sleep_for = MESH_JOIN_POLL_INTERVAL,
                Err(error) => {
                    eprintln!("punks-mesh: community mesh join reconcile failed: {error}");
    ...
    let ingress_watchdog = tokio::spawn(async move {
        ...
        loop {
            ...
            match crate::mesh_llm::rearm_relay_mesh_for_running_agents(&ingress_app).await {
                ...
                Err(error) => {
                    eprintln!("punks-mesh: ingress re-arm watchdog: {error}");
...
/// Join an isolated runtime to the existing Punks community mesh. The relay is
...
async fn reconcile_punks_mesh_join(app: &AppHandle) -> Result<(), String> {
    let state = app.state::<AppState>();
    ...
    let targets =
        crate::commands::mesh_llm::resolve_punks_mesh_join_targets_at(&state, &relay_url).await?;
...
enum RosterReconcileAction {
    ...
    Keep,
    /// Restart Punks so MeshLLM is rebuilt with a freshly resolved roster.
    ///
    /// MeshLLM's native listeners are process-owned in practice: stopping and
    /// starting the embedded runtime in one process can terminate Punks or race
...
fn roster_reconcile_action(
    ...
    let fresh = match query {
        Err(error) => {
            eprintln!(
                "punks-mesh: roster reconcile query failed; keeping current allowlist: {error}"
            );
...
async fn reconcile_roster(
    ...
    match roster_reconcile_action(current_owners, pending_shrink.as_deref(), query) {
        ...
        RosterReconcileAction::AwaitConfirm(reduced) => {
            eprintln!("punks-mesh: roster shrink observed; awaiting confirmation before restart");
    ...
    if startup_pending {
        eprintln!(
            "punks-mesh: membership roster changed while client management startup is pending; deferring restart"
        );
        return Ok(());
    }
    if guard
    ...
    drop(guard);
    eprintln!(
        "punks-mesh: membership roster changed; restarting Punks to rebuild MeshLLM with the fresh community allowlist"
    );
    app.request_restart();
    Ok(())
}
...
pub(crate) async fn publish_current_status_once(app: &AppHandle, reason: &str) {
    ...
    {
        Ok(Ok(())) => {}
        Ok(Err(error)) => eprintln!("punks-mesh: status report after {reason} failed: {error}"),
        Err(_) => eprintln!("punks-mesh: status report after {reason} timed out"),
    }
}
...
pub(crate) async fn publish_stopped_status_once_at(
    ...
    .await
    {
        Ok(Ok(())) => {}
        Ok(Err(error)) => {
            eprintln!("punks-mesh: stopped status report after {reason} failed: {error}");
        }
        Err(_) => eprintln!("punks-mesh: stopped status report after {reason} timed out"),
    }
}
...
pub(crate) fn build_status_report_event(
    ...
    let k = Tag::parse(["k", "punks-mesh-status"]).map_err(|error| error.to_string())?;
    Ok(nostr::EventBuilder::new(
        nostr::Kind::Custom(KIND_PUNKS_MESH_MEMBER_STATUS),
        payload.to_string(),
    )
    .tags([d, k]))
}
