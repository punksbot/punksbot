...
//! Punks/Nostr identity). The node presents a signed ownership attestation
...
//! has one owner identity whether mesh runs embedded in Punks or standalone.
...
impl OwnerIdentity {
    /// Sign a Punks-to-MeshLLM ownership binding. The member's Nostr signature
