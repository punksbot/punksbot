...
impl PendingNavigationDeepLinks {
    fn lock(&self) -> std::sync::MutexGuard<'_, VecDeque<PendingNavigationDeepLink>> {
        self.0.lock().unwrap_or_else(|poisoned| {
            eprintln!("punks-full-local: recovering poisoned pending navigation deep-link queue");
            poisoned.into_inner()
        })
    }
...
fn activate_main_window(app: &tauri::AppHandle) {
    ...
    if let Err(error) = window.unminimize() {
        eprintln!("punks-full-local: failed to unminimize main window for deep link: {error}");
    }
    if let Err(error) = window.show() {
        eprintln!("punks-full-local: failed to show main window for deep link: {error}");
    }
    if let Err(error) = window.set_focus() {
        eprintln!("punks-full-local: failed to focus main window for deep link: {error}");
    }
}
...
pub(crate) fn install_deep_link_handlers(app: &mut tauri::App) {
    ...
    match app.deep_link().get_current() {
        ...
        Ok(None) => {}
        Err(error) => eprintln!("punks-full-local: failed to read launch deep link: {error}"),
    }
}

/// Parse the query string of a `punks-local://message?…` URL into the JSON
...
/// Parse the query string of a `punks-local://join?…` URL into the JSON payload
...
/// Hosts of the `punks-local://` git-entity links built by
/// `desktop/src/shared/lib/entityLink.ts` and `crates/punks-cli/src/links.rs`.
...
/// Validate a `punks-local://repo|project|pr|issue?…` link and return it verbatim
...
///
/// Workspace tabs addressable by `punks-local://repo|project` links — mirrors
...
/// Handle an incoming `punks-local://` deep link URL.
///
/// Currently supports:
/// - `punks-local://connect?relay=<ws(s)://...>` — emits `deep-link-connect` to the frontend
/// - `punks-local://repo|project|pr|issue?…` — emits `deep-link-entity` to the frontend
pub(crate) fn handle_deep_link_url(app: &tauri::AppHandle, url_str: &str) {
    let url = match Url::parse(url_str) {
        Ok(u) => u,
        Err(e) => {
            eprintln!("punks-full-local: invalid deep link URL {url_str:?}: {e}");
            return;
        }
    };

    if url.scheme() != "punks-local" {
        eprintln!("punks-full-local: ignoring unsupported deep link scheme: {url_str}");
        return;
    }
    ...
    match url.host_str() {
        Some("connect") => {
            let Some(relay_url) = parse_websocket_relay_param(&url) else {
                eprintln!("punks-full-local: connect deep link missing/invalid relay: {url_str}");
                return;
            };
        ...
        Some("join") => {
            // `punks-local://join?relay=<ws(s)://...>&code=<invite code>` — fired by
            ...
            let Some(payload) = parse_join_deep_link(&url) else {
                eprintln!(
                    "punks-full-local: join deep link missing/invalid relay or code: {url_str}"
                );
                return;
            };
            activate_main_window(app);
        ...
        Some("add-community") => {
            let Some(payload) = parse_add_community_deep_link(&url) else {
                eprintln!(
                    "punks-full-local: add-community deep link missing/invalid relay: {url_str}"
                );
                return;
            };
            activate_main_window(app);
        ...
        Some("channel") => {
            let Some(payload) = parse_channel_deep_link(&url) else {
                eprintln!("punks-full-local: channel deep link missing/invalid channel: {url_str}");
                return;
            };
            activate_main_window(app);
        ...
        Some("message") => {
            // `punks-local://message?channel=<uuid>&id=<eventId>[&thread=<rootId>]`
            //
            ...
            let Some(payload) = parse_message_deep_link(&url) else {
                eprintln!("punks-full-local: message deep link missing channel or id: {url_str}");
                return;
            };
            activate_main_window(app);
            queue_navigation_deep_link(app, "message", &payload);
        ...
        Some("repo" | "project" | "pr" | "issue") => {
            // `punks-local://repo|project?owner=<pubkey>&d=<dtag>` and
            // `punks-local://pr|issue?id=<eventId>&owner=<pubkey>&d=<dtag>` — the
            ...
            if parse_entity_deep_link(&url).is_none() {
                eprintln!("punks-full-local: malformed entity deep link: {url_str}");
                return;
            }
            activate_main_window(app);
        ...
        Some("nostr-bind") => match parse_nostr_bind_deep_link(&url) {
            ...
            Err(error) => {
                eprintln!("punks-full-local: rejecting nostr-bind deep link: {error}: {url_str}");
            }
        },
        Some(action) => {
            eprintln!("punks-full-local: unknown deep link action: {action}");
        }
        None => {
            eprintln!("punks-full-local: deep link missing action: {url_str}");
        }
    }
}
