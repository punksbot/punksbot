...
pub fn relay_ws_url() -> String {
    configured_env_var("PUNKS_RELAY_URL")
        .or_else(|| option_env!("PUNKS_DESKTOP_BUILD_RELAY_URL").map(str::to_string))
...
pub fn relay_api_base_url() -> String {
    if let Some(base) = configured_env_var("PUNKS_RELAY_HTTP") {
        return base.trim_end_matches('/').to_string();
    }

    if let Some(base) = option_env!("PUNKS_DESKTOP_BUILD_RELAY_HTTP") {
...
/// - Any HTML response (proxy login page, captive portal, etc.)
///
...
fn classify_intercepted_response(_final_host: &str, content_type: &str) -> Option<String> {
    let ct = content_type.to_lowercase();

    // HTML cannot be a relay JSON response; report the intercepted transport
    // without coupling the local product to any particular proxy vendor.
...
///
/// Punks's command kinds (e.g. 41010, 30620, 46020) acknowledge writes via
...
fn build_profile_event(
    ...
    let builder = if let Some(tag_json) = auth_tag_json {
        ...
        punks_sdk_pkg::nip_oa::verify_auth_tag(tag_json, &compat_pubkey)
        ...
        let compat_tag = punks_sdk_pkg::nip_oa::parse_auth_tag(tag_json)
