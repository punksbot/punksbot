//! Rust-owned PTY sessions and the typed Tauri transport for Punks Substrate.
...
use punks_terminal::context::{context_vars, GuiContext};
use punks_terminal::damage::{Frame, Style};
use punks_terminal::{Fences, SharedTerminal, Size, Terminal, Viewport};
...
impl punks_terminal::lifecycle::DrainingReader for ReaderThread {
...
struct Session {
    ...
    reader: Option<Box<dyn punks_terminal::lifecycle::DrainingReader + Send>>,
...
impl Session {
    ...
    fn shutdown(mut self) {
        ...
        if let Some(reader) = self.reader.take() {
            #[cfg(unix)]
            {
                let _ = punks_terminal::lifecycle::shutdown_draining(&mut self.child, reader);
            }
            #[cfg(not(unix))]
            {
...
pub(crate) fn terminal_attach(
    ...
    if let Some(existing_id) = request.session_id.as_deref() {
        ...
        let mut encoder = punks_terminal::damage::Encoder::new();
    ...
    let shell = {
        #[cfg(unix)]
        {
            punks_terminal::shell::resolve_shell(std::env::var("SHELL").ok().as_deref())
        }
        #[cfg(windows)]
        {
            punks_terminal::shell::resolve_shell(std::env::var("ComSpec").ok().as_deref())
        }
    };
    let mut command = CommandBuilder::new_default_prog();
    punks_terminal::env_fence::fence_env(
        &mut command,
        &punks_terminal::path::user_shell_path(),
        &shell,
    );
    ...
    std::thread::spawn(move || {
        while let Ok(action) = actions.recv() {
            let presentation = match &action {
                punks_terminal::Action::Title(title) => Some(TerminalMessage::Title(title.clone())),
                punks_terminal::Action::ResetTitle => Some(TerminalMessage::ResetTitle),
                punks_terminal::Action::Bell => Some(TerminalMessage::Bell),
                _ => None,
            };
            ...
            if let Some(reply) = punks_terminal::listener::reply(
                action,
                size.cols,
                size.rows,
                size.pixel_width,
    ...
    let reader_handle = std::thread::spawn(move || {
        ...
        let mut encoder = punks_terminal::damage::Encoder::new();
        let mut snapshot_encoder = punks_terminal::damage::Encoder::new();
        loop {
...
mod tests {
    use super::*;
    use punks_terminal::damage::{CursorFrame, RowFrame, Span};

    fn publication(spans: Vec<Span>) -> Publication {
        Publication {
            subscription_id: SubscriptionId::new(),
            sequence: 7,
            frame: punks_terminal::damage::Frame {
                rows: vec![RowFrame {
                    line: 3,
                    wrapped: true,
                    spans,
