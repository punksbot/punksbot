...
pub struct AppState {
    ...
    /// In-process mesh-llm node started by Punks Desktop.
...
/// Parse the `PUNKS_PRIVATE_KEY` env var into identity keys. `Some` means the
...
fn identity_from_env() -> Option<Keys> {
    match std::env::var("PUNKS_PRIVATE_KEY") {
        Ok(nsec) => match Keys::parse(nsec.trim()) {
            Ok(keys) => Some(keys),
            Err(error) => {
                eprintln!("punks: invalid PUNKS_PRIVATE_KEY: {error}");
                None
            }
        },
        Err(std::env::VarError::NotUnicode(_)) => {
            eprintln!("punks: PUNKS_PRIVATE_KEY contains invalid UTF-8");
            None
        }
...
pub fn build_app_state() -> AppState {
    ...
    let (keys, identity_storage) = match identity_from_env() {
        Some(keys) => {
            eprintln!(
                "punks: configured identity pubkey {}",
    ...
    pub fn signing_keys(&self) -> Result<Keys, String> {
                ...
                .load(std::sync::atomic::Ordering::Acquire)
        {
            return Err("identity is in recovery mode; event signing is disabled \
                 until the identity is restored and Punks is relaunched"
                .to_string());
        }
        self.keys
            .lock()
...
///
/// Priority: `PUNKS_PRIVATE_KEY` env var (already handled in `build_app_state`)
    ...
    match store.probe(IDENTITY_KEY_NAME) {
        KeyringProbe::Present => {
            if let Some(nsec) = store.load(IDENTITY_KEY_NAME)? {
                match Keys::parse(nsec.trim()) {
                    Ok(keyring_keys) => {
                        eprintln!(
                            "punks: persisted identity pubkey {}",
                        ...
                        if legacy_path.exists() {
                            match load_key_file(legacy_path) {
                                ...
                                {
                                    eprintln!(
                                        "punks: identity.key differs from keyring; \
                                    ...
                                    let storage = if let Err(e) = persist_identity_to_keyring(
                                        ...
                                        eprintln!(
                                            "punks: keyring adoption of identity.key \
                                ...
                                Err(e) => {
                                    eprintln!(
                                        "punks: leftover identity.key is corrupt ({e}); \
                        ...
                        if !legacy_path.exists() && !migration_marker_path(data_dir).exists() {
                            ...
                            {
                                eprintln!(
                                    "punks: keyring present but marker missing; \
        ...
        KeyringProbe::ReachableButEmpty => {
            ...
            if legacy_path.exists() {
                ...
                eprintln!(
                    "punks: identity lost — keyring was empty despite migration marker; \
        ...
        KeyringProbe::Unreachable => {
            ...
            if !legacy_path.exists() && migration_marker_path(data_dir).exists() {
                let ephemeral = Keys::generate();
                eprintln!(
                    "punks: keyring unreachable but migration marker present; \
...
fn recover_from_keyring(
    ...
    eprintln!("punks: corrupt nsec in keyring ({error}), clearing and recovering from file");
    if let Err(e) = store.delete(IDENTITY_KEY_NAME) {
        eprintln!("punks: failed to clear corrupt keyring value: {e}");
    }
    if legacy_path.exists() {
        if let Some(keys) = migrate_identity_file(store, legacy_path, data_dir)? {
            return Ok(ResolvedIdentity {
    ...
    if migration_marker_path(data_dir).exists() {
        let ephemeral = Keys::generate();
        eprintln!(
            "punks: identity lost — keyring had corrupt data and no valid identity.key \
...
fn load_file_or_generate(
    ...
    eprintln!(
        "punks: generated and saved identity pubkey {}",
        keys.public_key().to_hex()
    );
    Ok(keys)
}
...
fn migrate_identity_file(
    ...
    let keys = match load_key_file(legacy_path) {
        Ok(keys) => keys,
        Err(error) => {
            eprintln!("punks: corrupt identity.key during migration ({error}), skipping");
            return Ok(None);
        }
    };
    let nsec = keys
    ...
    if let Err(e) = write_migration_marker(&marker_path) {
        eprintln!(
            "punks: keyring import ok but failed to write migration marker ({e}); \
             keeping identity.key so the key is not stranded"
        );
        return Ok(Some(keys));
    }
    if let Err(e) = std::fs::remove_file(legacy_path) {
        eprintln!("punks: keyring import ok but failed to delete identity.key: {e}");
    } else {
        eprintln!("punks: migrated identity key into OS keyring");
    }
    Ok(Some(keys))
}
...
fn persist_identity_to_keyring(
    ...
    if let Err(e) = write_migration_marker(&marker_path) {
        ...
        if !legacy_path.exists() {
            if let Err(write_err) = save_key_file(legacy_path, keys) {
                eprintln!(
                    "punks: keyring ok but marker write failed ({e}) and \
                ...
                eprintln!(
                    "punks: keyring ok but marker write failed ({e}); \
    ...
    if legacy_path.exists() {
        if let Err(e) = std::fs::remove_file(legacy_path) {
            eprintln!("punks: keyring write ok but failed to delete identity.key: {e}");
        }
    }
...
fn persist_imported_identity_impl(
    ...
    match persist_identity_to_keyring(store, keys, legacy_path, data_dir) {
        ...
        Err(e) => {
            eprintln!(
                "punks: keyring write failed during import ({e}), \
...
fn generate_and_persist(
    ...
    if storage == IdentityStorage::SystemKeyring {
        ...
        if let Err(e) = write_migration_marker(&marker_path) {
            eprintln!(
                "punks: stored identity in keyring but failed to write migration marker \
...
fn store_key_preferring_keyring(
    ...
    match store.store(IDENTITY_KEY_NAME, &nsec) {
        Ok(()) => Ok(IdentityStorage::SystemKeyring),
        Err(keyring_err) => {
            eprintln!("punks: keyring write failed ({keyring_err}), using file fallback");
            save_key_file(legacy_path, keys)?;
            Ok(IdentityStorage::LocalFile)
        }
    }
...
fn cleanup_leftover_identity_file(legacy_path: &std::path::Path) {
    ...
    match std::fs::remove_file(legacy_path) {
        Ok(()) => eprintln!("punks: removed leftover identity.key (key is in keyring)"),
        Err(e) => eprintln!("punks: failed to remove leftover identity.key: {e}"),
    }
}
...
fn quarantine_corrupt_key(key_path: &std::path::Path, data_dir: &std::path::Path, error: &str) {
    ...
    eprintln!("punks: corrupt identity.key ({error}), quarantining to {bad_name}");
