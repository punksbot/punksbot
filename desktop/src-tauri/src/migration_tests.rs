...
fn legacy_mcp_server_command() -> String {
    String::from_utf8(
        [98, 117, 122, 122]
            .into_iter()
            .chain(b"-mcp-server".iter().copied())
            .collect(),
    )
    .expect("legacy command bytes are UTF-8")
}

#[test]
fn canonical_dev_data_dir_replaces_last_component() {
    let current =
        PathBuf::from("/Users/me/Library/Application Support/xyz.block.punks.app.dev.my-branch");
    let canonical = canonical_dev_data_dir(&current).unwrap();
    assert_eq!(
        canonical,
        PathBuf::from("/Users/me/Library/Application Support/xyz.block.punks.app.dev")
    );
}
...
fn legacy_app_data_dir_maps_release_identifier() {
    let current = PathBuf::from("/Users/me/Library/Application Support/xyz.block.punks.app");
...
fn setup_sync_layout() -> (tempfile::TempDir, PathBuf, PathBuf) {
    ...
    let worktree = parent.path().join("xyz.block.punks.app.dev.my-branch");
    let main_instance = parent.path().join("xyz.block.punks.app.dev.main");
...
fn seed_up_migrates_sibling_file_to_canonical_then_symlinks() {
    ...
    let sibling = canonical
        .parent()
        .unwrap()
        .join("xyz.block.punks.app.dev.main");
...
fn canonical_dev_data_dir_returns_self_for_canonical_instance() {
    ...
    // testing only.
    let current = PathBuf::from("/Users/me/Library/Application Support/xyz.block.punks.app.dev");
...
#[test]
fn reconcile_mcp_commands_clears_previous_product_mcp_server() {
    let dir = tempfile::tempdir().unwrap();
    write_agents_json(
        dir.path(),
        &serde_json::json!([{
            "name": "Fizz",
            "agent_command": "goose",
            "mcp_command": legacy_mcp_server_command()
        }]),
    );
...
fn reconcile_mcp_commands_handles_mixed_agents() {
    ...
    write_agents_json(
        dir.path(),
        &serde_json::json!([
            {"name": "Stale Goose", "agent_command": "goose", "mcp_command": legacy_mcp_server_command()},
            {"name": "Clean Goose", "agent_command": "goose", "mcp_command": ""},
            {"name": "Custom Agent", "agent_command": "goose", "mcp_command": "my-custom-mcp"},
            {"name": "Stale Punks", "agent_command": "punks-agent", "mcp_command": legacy_mcp_server_command()}
        ]),
    );
    reconcile_mcp_commands_in_file(&dir.path().join("agents/managed-agents.json"));
    let records = read_agents_json(dir.path());
...
fn reconcile_mcp_commands_resolves_persona_runtime_over_stale_snapshot() {
    // The frozen snapshot is punks-agent (wants punks-dev-mcp), but the linked
    // persona's runtime is goose (wants no mcp). The reconcile must follow the
    // EFFECTIVE harness (persona-wins) and clear the stale legacy MCP command.
    let dir = tempfile::tempdir().unwrap();
    write_agents_json(
        dir.path(),
        &serde_json::json!([{
...
fn reconcile_mcp_commands_sees_team_dir_runtime_edit_same_launch() {
    ...
    // (goose → punks-agent) into personas.json. The reader runs AFTER, so it
    // must derive the NEW punks-agent mcp_command without a second launch.
    write_personas_json(
        dir.path(),
...
fn reconcile_mcp_commands_honors_explicit_override_over_persona() {
    ...
    // runtime: persona is goose (no mcp) but the pin is punks-agent, so the
    // reconcile sets the punks-agent mcp_command.
    let dir = tempfile::tempdir().unwrap();
    write_agents_json(
        dir.path(),
        &serde_json::json!([{
...
fn migrate_legacy_nest_carries_knowledge_and_skips_repos() {
    ...
    let current = dir.path().join(".punks");
    ...
    std::fs::create_dir_all(legacy.join("REPOS/punks")).unwrap();
    std::fs::write(legacy.join("REPOS/punks/huge.bin"), "checkout").unwrap();
...
fn migrate_legacy_nest_overwrites_generated_default_agents_md() {
    ...
    // ~/.punks/AGENTS.md, then migration runs.
...
fn punks_local_never_runs_legacy_boot_migrations() {
    assert!(
        ...
        "Punks Full Local must not inspect or import any previous Punks product data"
    );
}
