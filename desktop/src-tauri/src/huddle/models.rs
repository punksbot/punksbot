...
//!
//! Mental model:
//!   app launch → start_stt_download (background) → ~/.punks/models/parakeet-tdt-ctc-110m-en/
//!   app launch → start_tts_download (background) → ~/.punks/models/pocket-tts/
...
//! `~/.punks/models/moonshine-tiny/` is removed best-effort once the new STT
...
const MANIFEST_FILENAME: &str = ".punks-model-manifest";
...
(https://github.com/k2-fsa/sherpa-onnx); Punks ships this conversion
unmodified.
...
#[derive(Clone)]
struct ModelSlot {
    dir_name: &'static str,                  // subdir under ~/.punks/models/
    ...
    fn recover_interrupted_install(&self, models_dir: &Path) {
        ...
        if self.is_ready(models_dir) {
            if let Err(error) = std::fs::remove_dir_all(&backup_dir) {
                eprintln!(
                    "punks-full-local: could not remove stale {} backup: {error}",
                    self.dir_name
                );
            }
            return;
        ...
        if final_dir.exists() {
            if let Err(error) = std::fs::remove_dir_all(&final_dir) {
                eprintln!(
                    "punks-full-local: could not remove incomplete {} install: {error}",
                    self.dir_name
                );
                return;
            }
        ...
        if let Err(error) = std::fs::rename(&backup_dir, &final_dir) {
            eprintln!(
                "punks-full-local: could not restore interrupted {} install: {error}",
                self.dir_name
            );
        }
    }
        ...
        Fut: std::future::Future<Output = Result<(), String>> + Send,
    {
        ...
        tauri::async_runtime::spawn(async move {
            if let Err(e) = download_fn(http_client).await {
                eprintln!("punks-full-local: {name} download failed: {e}");
...
impl ModelManager {
    /// Create a new `ModelManager` rooted at `~/.punks/models/`.
    ///
    ...
    pub fn start_tts_download(&self, http_client: reqwest::Client) {
        if let Err(error) = voice_upgrade::install_vctk_presets_into_v4_model(&self.models_dir) {
            eprintln!(
                "punks-full-local: could not upgrade existing Pocket voices in place: {error}"
            );
        }
        let manager = self.clone();
    ...
    async fn download_stt_model(&self, http_client: reqwest::Client) -> Result<(), String> {
        ...
        eprintln!("punks-full-local: downloading STT model from {STT_DOWNLOAD_URL}");
        ...
        .await?;
        eprintln!("punks-full-local: downloaded {bytes} bytes, wrote to disk");
        ...
        eprintln!("punks-full-local: extracting STT archive…");
        ...
        eprintln!(
            "punks-full-local: STT model ready at {}",
    ...
    async fn download_tts_model(&self, http_client: reqwest::Client) -> Result<(), String> {
        ...
        for (i, (url, artifact)) in downloads.iter().enumerate() {
            ...
            eprintln!("punks-full-local: downloading Pocket TTS {filename} from {url}");
            ...
            eprintln!("punks-full-local: downloaded {bytes} bytes ({filename}), wrote to disk");
        ...
        eprintln!(
            "punks-full-local: Pocket TTS model ready at {}",
...
///
/// Removes `~/.punks/models/moonshine-tiny/` if present (~70 MB on disk).
...
async fn cleanup_legacy_moonshine_dir(models_dir: &Path) {
    ...
    match tokio::fs::remove_dir_all(&legacy).await {
        Ok(()) => eprintln!(
            "punks-full-local: removed legacy STT model dir {}",
            legacy.display()
        ),
        Err(e) => eprintln!(
            "punks-full-local: could not remove legacy STT model dir {}: {e} \
