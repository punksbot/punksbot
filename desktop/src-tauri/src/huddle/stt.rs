...
/// env override (`PUNKS_STT_FLUSH_MS`) let it be lowered to 150 ms, which split
...
/// `PUNKS_STT_THREADS`. Default preserves the production single thread.
fn stt_num_threads() -> i32 {
    std::env::var("PUNKS_STT_THREADS")
        .ok()
...
/// EXPERIMENTAL (latency bench): `PUNKS_STT_SPECULATIVE=1` starts the Parakeet
...
fn stt_speculative_decode() -> bool {
    std::env::var("PUNKS_STT_SPECULATIVE").is_ok_and(|v| v == "1")
}
...
fn stt_worker(
    ...
    if !tokens_path.exists() || !model_path.exists() {
        eprintln!(
            "punks-full-local: STT model not found at {} — STT disabled",
            model_dir.display()
        );
    ...
    let recognizer = match OfflineRecognizer::create(&cfg) {
        Some(r) => r,
        None => {
            eprintln!("punks-full-local: OfflineRecognizer::create returned None — STT disabled");
            drain_until_shutdown(audio_rx, &shutdown);
            return;
        }
    };
    ...
    let mut local_stream = match SttStreamState::new() {
        Ok(stream) => stream,
        Err(error) => {
            eprintln!("punks-full-local: {error}");
            return;
        }
    };
...
fn resample_chunk(resampler: &mut rubato::Fft<f32>, chunk_48k: &[f32]) -> Vec<f32> {
    ...
    let input = match InterleavedSlice::new(chunk_48k, 1, chunk_48k.len()) {
        Ok(a) => a,
        Err(e) => {
            eprintln!("punks-full-local: STT resample input error: {e}");
            return Vec::new();
        }
    };
    ...
    match resampler.process(&input, 0, None) {
        ...
        Err(e) => {
            eprintln!("punks-full-local: STT resample error: {e}");
            Vec::new()
        }
    }
}
...
fn flush_to_stt(
    ...
    if !has_enough_voiced_audio(voiced_frames) {
        eprintln!(
            "punks-full-local: STT dropped short VAD segment ({voiced_frames}/{MIN_VOICED_FRAMES} voiced frames)"
        );
        return;
    }
...
fn send_transcript(text: String, text_tx: &tokio_mpsc::Sender<String>) {
    if !text.is_empty() {
        if let Err(e) = text_tx.blocking_send(text) {
            eprintln!("punks-full-local: STT text channel closed: {e}");
        }
    }
}
