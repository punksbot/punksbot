...
// (compile-time rejection of baked `PUNKS_BUILD_AGENT_ENV` collisions) and
...
/// Env var keys that Punks sets itself and users must not override from
...
/// implications. Behavior knobs (GOOSE_MODE, PUNKS_ACP_MODEL, PUNKS_ACP_SYSTEM_PROMPT, …) remain freely
...
pub(crate) const RESERVED_ENV_KEYS: &[&str] = &[
    ...
    // launch.policy_env. A user-supplied PUNKS_ACP_AGENTS would bypass the
    ...
    "PUNKS_MANAGED_AGENT",
    "PUNKS_MANAGED_AGENT_START_NONCE",
];
