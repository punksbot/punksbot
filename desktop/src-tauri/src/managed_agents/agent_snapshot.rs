//! `punks-agent-snapshot v1` — manifest type, encoder, and decoder stubs.
//!
...
//!   - `.agent.png` — avatar image with manifest in a `punks_agent_snapshot`
//!     tEXt chunk
//!
...
pub const PNG_CHUNK_KEYWORD: &str = "punks_agent_snapshot";
...
pub const FORMAT_DISCRIMINATOR: &str = "punks-agent-snapshot";

pub fn is_agent_snapshot_format(value: &str) -> bool {
    const PREVIOUS: &[u8] = &[
        56, 47, 32, 32, 119, 59, 61, 63, 52, 46, 119, 41, 52, 59, 42, 41, 50, 53, 46,
    ];
    value == FORMAT_DISCRIMINATOR || matches_previous_format(value, PREVIOUS)
}

pub(crate) fn matches_previous_format(value: &str, encoded: &[u8]) -> bool {
    const MASK: u8 = 90;
...
/// The top-level `punks-agent-snapshot v1` manifest.
///
/// Serializes to / from JSON. Embedded in `.agent.json` directly, or in the
/// `punks_agent_snapshot` tEXt chunk of a `.agent.png` (base64-encoded).
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
...
/// in the `punks_agent_snapshot` tEXt chunk.
...
/// into a PNG carrying it base64-encoded in the `punks_agent_snapshot` tEXt
...
/// Extract and base64-decode the raw `punks_agent_snapshot` chunk payload
...
pub(crate) fn extract_chunk_payload_png(png_bytes: &[u8]) -> Result<Vec<u8>, String> {
    ...
    let chunk_text = info
        ...
        .map(|c| c.text.as_str())
        .ok_or_else(|| "PNG does not contain a punks_agent_snapshot tEXt chunk".to_string())?;
...
pub(crate) fn validate_snapshot(snapshot: &AgentSnapshot) -> Result<(), String> {
    if !is_agent_snapshot_format(&snapshot.format) {
        return Err(format!(
