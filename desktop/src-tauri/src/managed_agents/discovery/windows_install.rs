...
//! # Exit-code fidelity
//!
//! [#2892](https://github.com/punksbot/punksbot/pull/2892) established that an install
...
///
/// `slug` names the downloaded script (`punks-install-<slug>.ps1`) so concurrent
...
macro_rules! windows_install_command {
    ...
    ($slug:literal, $url:literal, $env_prefix:literal) => {
        concat!(
            ...
            "$installer=Join-Path $env:TEMP 'punks-install-",
            $slug,
            ".ps1'; ",
            "Invoke-RestMethod ",
            $url,
...
mod tests {
    ...
    fn test_every_windows_install_command_downloads_to_a_file_then_executes_it() {
        ...
        for (id, command) in commands {
            ...
            assert!(
                command.contains(&format!("punks-install-{id}.ps1")),
