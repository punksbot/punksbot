...
pub(crate) struct KnownAcpRuntime {
    ...
    /// `Some(dir)` → Punks creates a symlink at `<nest>/<dir>/punks-cli`
    /// pointing to the canonical `.agents/skills/punks-cli`. `None` → this
    ...
    /// authenticated. `None` for runtimes that have no login step (goose, punks-agent).
