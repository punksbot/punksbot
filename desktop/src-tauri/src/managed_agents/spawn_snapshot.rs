...
pub(crate) struct SpawnConfigSnapshot {
    ...
    /// `None` when a user env override shadows `PUNKS_ACP_SESSION_TITLE`: spawn
    ...
    /// `PUNKS_ACP_RESPOND_TO_ALLOWLIST` only there, so edits to a dormant list
    ...
    /// present, else the user-seeded `PUNKS_ACP_EFFORT_LEVEL` from the layered
...
/// is absent does a user-supplied `PUNKS_ACP_EFFORT_LEVEL` from the layered env
...
impl SpawnConfigSnapshot {
    ...
    pub(crate) fn from_inputs(inputs: SpawnConfigInputs<'_>) -> Self {
        ...
        Self {
            ...
            // value is a no-op (no phantom `env.PUNKS_ACP_EFFORT_LEVEL` add or
