...
fn validate_rejects_reserved_key() {
    let config = config_with_env(&[("PUNKS_PRIVATE_KEY", "should-not-be-settable")]);
...
#[test]
fn validate_rejects_derived_key_punks_agent_provider() {
    let config = config_with_env(&[("PUNKS_AGENT_PROVIDER", "anthropic")]);
    let err = validate_global_config(&config).unwrap_err();
    assert!(
        err.contains("structured provider/model fields"),
        "got: {err}"
...
fn validate_ignores_empty_values_for_reserved_key_check() {
    ...
    let config = config_with_env(&[("PUNKS_PRIVATE_KEY", "")]);
