...
pub struct AgentDefinition {
    ...
    /// Punks spawns. When deploying from this persona, this runtime is pre-selected in the UI.
    ...
    /// direct). Punks stores and passes through without interpretation.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Opaque to Punks — keys and values are runtime-specific.
    ///
...
pub struct ManagedAgentRecord {
    ...
    /// Deprecated: `PUNKS_ACP_TURN_TIMEOUT` is ignored by the harness and the
    ...
    /// Idle timeout in seconds (`PUNKS_ACP_IDLE_TIMEOUT`): how long the agent
    ...
    /// Inbound author gate mode. Translates to `PUNKS_ACP_RESPOND_TO`.
    #[serde(default)]
    ...
    /// inference through Punks's relay-mesh local endpoint; the `model_ref` is
    ...
    /// Canonical Claude Code effort level. Injected as `PUNKS_ACP_EFFORT_LEVEL` at spawn
...
pub struct ManagedAgentProcess {
    ...
    /// `PUNKS_ACP_SETUP_PAYLOAD` was set at launch because the agent was
...
pub struct InstallStepResult {
    ...
    /// recognized condition (e.g. EACCES writing Punks's private npm prefix).
...
pub const DEFAULT_ACP_COMMAND: &str = "punks-acp";
/// ~5 min (320s) — matches the CLI harness default (PUNKS_ACP_IDLE_TIMEOUT).
