...
//! expected `punks-acp` path from the running executable and kills any process
...
///    intermediates run in their own process groups (e.g. punks-acp ->
///    node shim -> codex-acp).
#[cfg(target_os = "macos")]
...
///
/// On Linux, `read_link("/proc/<pid>/exe")` returns `…/punks-acp (deleted)`
...
/// binary name (`punks-acp`) proceed to the full exe-path fetch.  This mirrors
...
/// processes but at most a handful of `punks-acp` instances).
#[cfg(target_os = "macos")]
...
/// Derive the expected path of the `punks-acp` harness binary next to the
/// current executable. Returns `None` if `current_exe()` fails or has no
/// parent directory.
///
/// In a `.app` bundle: `.../Contents/MacOS/punks-acp`.
/// In a dev checkout: `<target-dir>/debug/punks-acp` or similar.
...
/// Sweep and kill harness processes that share this bundle's exact `punks-acp`
...
/// where possible) equals `<this bundle>/punks-acp` are candidates. Dev builds
...
mod tests {
    ...
    fn strip_deleted_suffix_removes_kernel_suffix() {
        ...
        let p = PathBuf::from("/Applications/Punks.app/Contents/MacOS/punks-acp (deleted)");
        assert_eq!(
            strip_deleted_suffix(p),
            PathBuf::from("/Applications/Punks.app/Contents/MacOS/punks-acp")
        );
    }

    #[test]
    fn strip_deleted_suffix_leaves_normal_path_unchanged() {
        let p = PathBuf::from("/Applications/Punks.app/Contents/MacOS/punks-acp");
    ...
    fn strip_deleted_suffix_does_not_strip_partial_match() {
        ...
        let p = PathBuf::from("/some/path/punks-acp(deleted)");
        assert_eq!(strip_deleted_suffix(p.clone()), p,);
    }

    // ── select_untracked_bundle_harnesses ────────────────────────────────

    const BUNDLE_HARNESS: &str = "/Applications/Punks.app/Contents/MacOS/punks-acp";
    const DEV_HARNESS: &str = "/Users/dev/punks/.worktrees/main/target/debug/punks-acp";
    ...
    fn deleted_suffix_stripped_path_matches_expected() {
        ...
        let raw = PathBuf::from("/Applications/Punks.app/Contents/MacOS/punks-acp (deleted)");
