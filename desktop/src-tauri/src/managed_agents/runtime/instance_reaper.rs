...
/// Binary names for the Punks desktop/Tauri process. Used by dead-instance
/// detection to confirm the owning desktop is still alive.
const DESKTOP_BINARY_NAMES: &[&str] = &["Punks", "punks-full-local", "punks-full-loca"];

/// Check if a process name matches a known Punks desktop binary.
...
/// JSON as `"identifier":"xyz.block.punks.app.dev"` and in environment entries
...
/// `xyz.block.punks.app` from matching inside `xyz.block.punks.app.dev`.
...
/// Extract the `PUNKS_MANAGED_AGENT` value from a process's environment.
/// Returns `None` if the process doesn't have the marker or can't be read.
#[cfg(target_os = "macos")]
fn extract_punks_marker_value(pid: u32) -> Option<String> {
    let prefix = b"PUNKS_MANAGED_AGENT=";
...
#[cfg(not(unix))]
fn extract_punks_marker_value(_pid: u32) -> Option<String> {
    None
}

/// Check if a Punks desktop process is still alive for the given instance ID.
/// Scans all user-owned Punks processes and checks
...
/// Reap agent processes belonging to dead Punks desktop instances.
///
/// Scans all user processes for `PUNKS_MANAGED_AGENT=*`, groups them by
/// instance ID, and for each foreign instance (≠ `our_instance_id`) checks
/// whether a Punks desktop binary is still alive for that instance. If not,
...
pub(crate) fn reap_dead_instance_agents(our_instance_id: &str, skip_pids: &[u32]) {
    ...
    for &pid in &pids {
        ...
        // arbitrary binary names and PUNKS_MANAGED_AGENT is the authoritative
        // ownership proof.
        let Some(agent_instance_id) = extract_punks_marker_value(upid) else {
            continue;
        };
    ...
    for (instance_id, agent_pids) in &foreign_agents {
        ...
        eprintln!(
            "punks-full-local: reaping {} orphaned agent(s) from dead instance '{instance_id}'",
            agent_pids.len()
        );
