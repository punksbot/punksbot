...
/// Binary name fragments for all known agent/harness processes that Punks
...
/// carry `PUNKS_MANAGED_AGENT` in its environment (checked by the caller via
/// `process_has_punks_marker()`). This avoids sweeping unrelated node processes.
...
/// Callers must additionally verify `PUNKS_MANAGED_AGENT` ownership.
...
pub(crate) fn process_belongs_to_us(pid: u32) -> bool {
    ...
    // the caller's `process_has_punks_marker()` check decides true ownership.
...
/// build's agents, and vice versa). This is what lets two Punkss coexist on
/// one machine without one's cleanup nuking the other's agents.
pub(crate) fn current_instance_id(app: &AppHandle) -> String {
    app.config().identifier.clone()
}

/// Build the full `PUNKS_MANAGED_AGENT=<instance-id>` env entry we match
/// against when scanning processes. Kept here so the spawn stamp and the sweep
/// matcher can never drift apart.
pub(super) fn punks_marker_entry(instance_id: &str) -> Vec<u8> {
    format!("PUNKS_MANAGED_AGENT={instance_id}").into_bytes()
}

/// Check if a running process is one of *our* managed agents: it must carry
/// `PUNKS_MANAGED_AGENT=<instance_id>` in its environment, where `instance_id`
/// is this desktop instance's id. A process stamped with a *different* instance
/// id belongs to another live Punks app and must never be reaped here.
#[cfg(target_os = "macos")]
pub(crate) fn process_has_punks_marker(pid: u32, instance_id: &str) -> bool {
    let marker = punks_marker_entry(instance_id);
...
#[cfg(not(unix))]
pub(crate) fn process_has_punks_marker(_pid: u32, _instance_id: &str) -> bool {
    false
}
...
pub(super) fn resolve_pgids_and_kill(candidate_pids: &[i32]) {
    ...
    if pgids.is_empty() && candidate_groups > 0 {
        eprintln!(
            "punks-full-local: orphan sweep: skipped all {candidate_groups} candidate group(s) (live foreign group leader or candidate already exited); nothing signalled"
        );
    }
...
pub(crate) fn valid_agent_runtime_receipt_with(
    ...
    canonical == receipt.key
        ...
        // Punks-owned by construction. Marker-only ownership: custom-harness
