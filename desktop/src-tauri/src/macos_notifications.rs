...
const TARGET_USER_INFO_KEY: &str = "punksNotificationTarget";
...
define_class!(
    ...
    #[name = "PunksNotificationCenterDelegate"]
        ...
        fn did_receive_notification_response(
            ...
            if &*response.actionIdentifier() == unsafe { UNNotificationDefaultActionIdentifier } {
                if let Some(target) = target_from_response(response) {
                    ...
                    {
                        eprintln!(
                            "punks-full-local: failed to emit macOS notification activation: {error}"
                        );
                    }
                }
            }
            ...
            // malformed notifications that Punks intentionally ignores.
...
pub(crate) fn init(app: &AppHandle) -> tauri::Result<()> {
    if !is_bundled_application() {
        ...
        eprintln!(
            "punks-full-local: macOS notifications disabled because the process is not running from an app bundle"
        );
        return Ok(());
    }
...
fn ensure_bundled_application() -> Result<(), String> {
    if is_bundled_application() {
        ...
        Err(
            "macOS notifications are unavailable when Punks is not running from an app bundle"
                .to_string(),
        )
    }
}
...
fn queue_activation(target: serde_json::Value) {
    ...
    let Ok(mut queue) = queue.lock() else {
        eprintln!("punks-full-local: macOS notification activation queue is unavailable");
        return;
    };
