...
//! GPU/driver/compositor combinations, so Punks comes up with no window at all
//! and the user has no way to fix it (#2338, upstream tauri#9394).
//!
//! Historically Punks set `WEBKIT_DISABLE_DMABUF_RENDERER=1`, which used to fall
...
/// process is still single threaded, which it is nowhere else in Punks.
///
...
pub fn apply() -> Result<(), String> {
    match plan(
        ...
        Plan::Apply { vars, why } => {
            ...
            eprintln!("punks-full-local: {} — {why}", applied.join(" "));
            Ok(())
        }
        Plan::Leave { why } => {
            eprintln!("punks-full-local: WebKit rendering left as-is — {why}");
            Ok(())
        }
