...
/// Secrets Punks's own process holds. Sourced from the desktop crate's
...
/// Uniquely-named executable seeded into Punks's own PATH; the child must not
/// be able to run it.
const CANARY_BIN: &str = "punks-hermit-canary-tool";
...
/// canary value they set themselves, so a real `PUNKS_PRIVATE_KEY` in the
...
fn fence_still_delivers_the_terminal_contract() {
    ...
    for (key, value) in [("TERM", "xterm-256color"), ("TERM_PROGRAM", "Punks")] {
        assert!(
...
fn fence_excludes_keys_it_has_never_heard_of() {
    std::env::set_var("PUNKS_SOME_FUTURE_CREDENTIAL", CANARY);
    let out = fenced_child_environment();

    assert!(
        !out.contains("PUNKS_SOME_FUTURE_CREDENTIAL"),
        "an unknown key reached the child:\n{out}"
    );
}

/// `PATH` is constructed, not inherited, so Punks's Hermit build toolchain
...
#[test]
fn child_path_is_free_of_punks_toolchain() {
    let dir = std::env::temp_dir().join("punks-terminal-path-canary");
    ...
    // Stand in for Hermit activation: Punks's own PATH leads with a directory
    ...
    assert!(
        !path_line.contains("punks-terminal-path-canary"),
        "Punks's toolchain leaked into the child PATH: {path_line}"
    );
    ...
    assert!(
        ...
        "a Punks-only executable was reachable from the child shell: {resolved}"
    );
}
...
fn resolve_shell_rejects_a_file_the_user_cannot_execute() {
    ...
    let path = fixture_file("punks-terminal-group-only-exec", "#!/bin/sh\ntrue\n", 0o010);
...
fn resolve_shell_rejects_a_non_executable_file() {
    let path = fixture_file("punks-terminal-not-a-shell", "not a shell", 0o644);
    assert_ne!(resolve_shell(path.to_str()), path.to_str().unwrap());
}
...
/// The child must be told the shell we actually spawned, not the one Punks
