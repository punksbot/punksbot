...
//!
//! Punks's own process holds `PUNKS_PRIVATE_KEY` (an nsec), `PUNKS_AUTH_TAG`, and
...
/// Keys the child is allowed to inherit from Punks's own environment.
///
...
/// Values Punks sets on the child unconditionally, overriding any inherited
...
/// it is injected rather than inherited. Punks's own `SHELL` and the shell we
...
pub fn fence_env(cmd: &mut CommandBuilder, path: &str, shell: &str) {
    ...
    // 3. Apply Punks's own terminal identity.
