...
Punks has three independent release lanes. Desktop and relay use release PRs.
...
| Relay | `just release-relay` | `ghcr.io/punksbot/punksbot` container image |
...
exact candidate tag. The mobile handoff to the private `punks-releases` pipeline
...
1. **`just release-relay`** runs locally on `main`, creates or updates a
   `relay-release/<version>` PR, bumps `crates/punks-relay/Cargo.toml`,
...
1. **Publish a candidate.** From a clean checkout whose `origin` is the
   canonical `punksbot/punksbot` repository, run
   ...
   `mobile-vX.Y.Z-rc.N` tag there through the dedicated `punks-release-bot`
...
2. **Build the exact tag.** Enter the candidate tag as `mobile_ref` in the
   private Punks mobile Buildkite pipeline. OSS CI deliberately cannot trigger
...
| Relay | `crates/punks-relay/Cargo.toml` |
...
```sh
gh workflow run signed-macos-canary.yml --repo punksbot/punksbot --ref main
```
...
create or move tags, and cannot update `punks-desktop-latest` or `latest.json`.

Download the artifact from the completed run:

```sh
gh run download <run-id> --repo punksbot/punksbot --name <artifact-name>
```
...
(or use `gh run rerun <run-id> --failed --repo punksbot/punksbot`). A rerun
...
[Release Mobile pipeline](https://buildkite.com/runway/punks-mobile-releases) with
...
[punks-releases README](https://github.com/punksbot/punksbot#cutting-a-release)
...
2. **`punks-desktop-latest`**: the rolling auto-updater release. Its
...
`punks-desktop-latest/latest.json`. Same-version retries succeed only when the
...
- **Write access** to the `punksbot/punksbot` GitHub repository
- An `origin` remote whose configured URL is the canonical `punksbot/punksbot`
  repository
...
- Release tag ruleset [`14378754`](https://github.com/punksbot/punksbot/rules/14378754)
  active for `desktop-v*` and `mobile-v*`, with creation, update, deletion, and
  non-fast-forward protections and `punks-release-bot` as its sole always-bypass
  actor
- The `punks-release-bot` App credentials configured for GitHub Actions
- The following **GitHub Actions variables and secrets** configured for the
  desktop release lane:

  | Name | Kind | Purpose |
  |------|------|---------|
  | `PUNKS_RELEASE_TAGGER_CLIENT_ID` | Variable | GitHub App client ID used to create protected release tags |
  | `PUNKS_RELEASE_TAGGER_PRIVATE_KEY` | Secret | GitHub App private key |
  ...
  | `PUNKS_UPDATER_PUBLIC_KEY` or `SPROUT_UPDATER_PUBLIC_KEY` | Secret | Tauri updater public key |
...
must be installed on `punksbot/punksbot`, have Contents write and Metadata read, and
...
Confirm `punks-release-bot` remains the sole always-bypass actor for the active
...
Verify that the `punks-desktop-latest` release exists and contains a
