...
/// startup budget (see punksbot/punksbot#3355). Skip that unrelated global startup
...
mod tests {
    ...
    fn test_mentions_mode_default_kinds() {
        ...
        for ch in &channels {
            ...
            assert!(kinds.contains(&punks_core::kind::KIND_STREAM_MESSAGE));
            assert!(kinds.contains(&punks_core::kind::KIND_WORKFLOW_APPROVAL_REQUESTED));
            assert!(kinds.contains(&punks_core::kind::KIND_STREAM_REMINDER));
        }
    }
...
"#,
    ...
    fn compose_session_title_qualifies_the_agent_name_with_the_channel() {
        assert_eq!(
            compose_session_title("Fizz", Some("punks-dev")),
            "Fizz · #punks-dev"
        );
    }
    ...
    fn compose_session_title_drops_the_channel_when_the_agent_name_fills_the_cap() {
        ...
        assert_eq!(compose_session_title(&agent, Some("punks-dev")), agent);
    }
