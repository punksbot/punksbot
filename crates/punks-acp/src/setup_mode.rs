...
pub(crate) async fn run_setup_listener(config: Config, payload: SetupPayload) -> Result<()> {
    ...
    loop {
        let Some(punks_event) = relay.next_event().await else {
        ...
        {
            handle_setup_membership(&mut relay, &punks_event, &config, &rules, &channel_ids).await;
            continue;
        }
        ...
        let is_dm = crate::is_dm_channel(punks_event.channel_id, &channel_info).await;
...
async fn handle_setup_membership(
    ...
    punks_event: &crate::relay::PunksEvent,
    config: &Config,
    rules: &[filter::SubscriptionRule],
    _initial_channel_ids: &[Uuid],
) {
    let kind_u32 = punks_event.event.kind.as_u16() as u32;
    let channel_id = punks_event.channel_id;
...
async fn publish_setup_nudge(
    ...
    let event_builder = punks_sdk::build_message(
        channel_id,
        &body,
        thread_ref.as_ref(),
...
mod tests {
    ...
    #[test]
    fn nudge_body_all_punks_managed_retains_original_footer() {
