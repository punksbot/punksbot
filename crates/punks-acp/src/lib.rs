...
fn resolve_agent_owner(config: &Config) -> Option<String> {
    ...
    if let Ok(auth_tag) = std::env::var("PUNKS_AUTH_TAG") {
        if !auth_tag.is_empty() {
            ...
            match punks_sdk::nip_oa::verify_auth_tag(&auth_tag, &agent_pk) {
                Ok(owner_pk) => {
...
async fn check_sibling_via_profile(
    ...
    for tag in tags {
        ...
        match punks_sdk::nip_oa::verify_auth_tag(&tag_json, &agent_pk) {
            Ok(_) => {
...
/// keyed to `OBSERVER_MAX_PLAINTEXT_LEN` in punks-core/observer.rs:25) is what actually
...
/// already-small frame. Reusing that string would mean changing punks-core's
/// `encrypt_observer_payload` signature or adding a parallel encrypt path; both
/// are out of this change's scope (punks-core stays untouched). The clean `&mut`
...
async fn publish_relay_observer_event(
    ...
    let builder = match punks_sdk::build_agent_observer_frame(
        owner_pubkey_hex,
        agent_pubkey_hex,
...
fn handle_relay_observer_control_event(
    ...
    if let Err(e) = punks_core::verify_event(&event) {
...
async fn tokio_main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("punks_acp=info")),
        )
        .compact()
        .init();
    ...
    let relay_auth_tag: Option<nostr::Tag> = std::env::var("PUNKS_AUTH_TAG")
        .ok()
        .filter(|s| !s.is_empty())
        .and_then(|s| punks_sdk::nip_oa::parse_auth_tag(&s).ok());
    ...
    loop {
        ...
        let pool_event: Option<PoolEvent> = {
            ...
            tokio::select! {
                ...
                punks_event = relay.next_event() => {
                    let _ = result_rx; // end split borrow before relay handling
                    match punks_event {
                        Some(punks_event) => {
                            let kind_u32 = punks_event.event.kind.as_u16() as u32;

                            if kind_u32 == KIND_MEMBER_ADDED_NOTIFICATION
                                || kind_u32 == KIND_MEMBER_REMOVED_NOTIFICATION
                            {
                                let ch = punks_event.channel_id;
                                let ts = punks_event.event.created_at.as_secs();
                                let eid = punks_event.event.id.to_hex();
                            ...
                            if config.ignore_self && punks_event.event.pubkey.to_hex() == pubkey_hex {
                                tracing::debug!(channel_id = %punks_event.channel_id, "dropping self-authored event");
                                continue;
                            }
                            ...
                            if is_shutdown {
                                ...
                                if let Some(owner) = owner {
                                    if punks_event.event.pubkey.to_hex() == *owner {
                                        tracing::info!(
                                            channel_id = %punks_event.channel_id,
                                            sender = %punks_event.event.pubkey.to_hex(),
                                ...
                                if let Some(owner) = owner_cache.get() {
                                    if punks_event.event.pubkey.to_hex() == *owner {
                                        ...
                                        if fired {
                                            ...
                                            let invalidated = pool.invalidate_channel_sessions(punks_event.channel_id);
                                            tracing::info!(
                                                channel_id = %punks_event.channel_id,
                                                invalidated,
                            ...
                            {
                                let author = punks_event.event.pubkey.to_hex();
                                // DM hardening: resolve channel type (fail-closed
                                // to DM) so allowlist/anyone modes cannot be
                                // exercised by non-owner authors inside DMs.
                                let is_dm =
                                    is_dm_channel(punks_event.channel_id, &ctx.channel_info).await;
                                ...
                                if !allowed {
                                    tracing::debug!(
                                        channel_id = %punks_event.channel_id,
                                        author = %punks_event.event.pubkey.to_hex(),
                            ...
                            let matched = filter::match_event(&punks_event.event, punks_event.channel_id, &rules, &pubkey_hex).await;
                            let prompt_tag = match matched {
                                Some(m) => m.prompt_tag,
                                None => {
                                    tracing::debug!(channel_id = %punks_event.channel_id, kind = punks_event.event.kind.as_u16(), "event matched no rule — dropping");
                                    continue;
                                }
                            };
                            // Capture author pubkey before queue.push() moves
                            // punks_event.event (needed for mode gate below).
                            let author_hex = punks_event.event.pubkey.to_hex();
                            let event_id_hex = punks_event.event.id.to_hex();
                            ...
                            let event_for_steer = punks_event.event.clone();
                            let prompt_tag_for_steer = prompt_tag.clone();
                            let accepted = queue.push(QueuedEvent {
                                channel_id: punks_event.channel_id,
                                event: punks_event.event,
                            ...
                            if accepted && queue.is_channel_in_flight(punks_event.channel_id) {
...
mod build_mcp_servers_tests {
    ...
    #[test]
    fn session_new_mcp_server_forwards_punks_auth_tag() {
    ...
    #[test]
    fn session_new_mcp_server_skips_empty_punks_auth_tag() {
        let _guard = ENV_LOCK.lock().unwrap();
