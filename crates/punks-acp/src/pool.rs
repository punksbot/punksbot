                ...
                mode = rx => {
                    ...
                    if agent.acp.has_in_flight_prompt() {
                            ...
                            .await
                        {
                            Ok(stop_reason) => {
                                ...
                                publish_agent_turn_metric(
                                    ...
                                    &session_id,
                                    &turn_id,
                                    Some(punks_core::agent_turn_metric::StopReason::Cancelled),
                                )
                                .await;
                                send_prompt_result(
                                    &result_tx,
                            ...
                            Err(error) => {
                                ...
                                publish_agent_turn_metric(
                                    ...
                                    &session_id,
                                    &turn_id,
                                    Some(punks_core::agent_turn_metric::StopReason::Error),
                                )
                                .await;
                                send_prompt_result(
                                    &result_tx,
                        ...
                        publish_agent_turn_metric(
                            ...
                            &session_id,
                            &turn_id,
                            Some(punks_core::agent_turn_metric::StopReason::EndTurn),
                        )
                        .await;
                        send_prompt_result(
                            &result_tx,
...
pub(crate) async fn fetch_channel_info(
    ...
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(
            punks_core::kind::KIND_NIP29_GROUP_METADATA as u16,
        ))
...
async fn fetch_huddle_instructions(
    ...
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(
            punks_core::kind::KIND_HUDDLE_GUIDELINES as u16,
        ))
        .author(*owner)
...
fn huddle_instructions_from_query_response(
    ...
    if event.pubkey != *owner
        || event.kind.as_u16() as u32 != punks_core::kind::KIND_HUDDLE_GUIDELINES
        || !event
            .tags
            .iter()
...
async fn fetch_canvas_section(channel_id: Uuid, rest: &RestClient) -> Option<String> {
    ...
    let h_tag = SingleLetterTag::lowercase(Alphabet::H);
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(punks_core::kind::KIND_CANVAS as u16))
        .custom_tags(h_tag, [channel_id.to_string()])
        .limit(1);
...
pub(crate) fn canvas_section_from_query_response(
    ...
    if event.kind != nostr::Kind::Custom(punks_core::kind::KIND_CANVAS as u16) {
        tracing::warn!(
            target: "canvas::fetch",
            channel = %channel_uuid,
    ...
    CountFut: std::future::Future<Output = Result<serde_json::Value, crate::relay::RelayError>>,
{
    ...
    let replies_filter = nostr::Filter::new()
        .kinds([
            nostr::Kind::Custom(punks_core::kind::KIND_STREAM_MESSAGE as u16),
            nostr::Kind::Custom(punks_core::kind::KIND_STREAM_MESSAGE_V2 as u16),
        ])
...
fn acp_stop_to_core(r: &StopReason) -> punks_core::agent_turn_metric::StopReason {
    use punks_core::agent_turn_metric::StopReason as CoreStop;
    match r {
...
pub(crate) fn build_turn_metric_counts(
    ...
    Option<punks_core::agent_turn_metric::TokenCounts>,
    Option<punks_core::agent_turn_metric::TokenCounts>,
) {
    use punks_core::agent_turn_metric::TokenCounts;
...
async fn publish_agent_turn_metric(
    ...
    session_id: &str,
    turn_id: &str,
    stop_reason: Option<punks_core::agent_turn_metric::StopReason>,
) {
    use punks_core::agent_turn_metric::AgentTurnMetricPayload;
    ...
    let ciphertext = match punks_core::agent_turn_metric::encrypt_agent_turn_metric(
        &ctx.agent_keys,
        owner_pk,
        &payload,
    ) {
    ...
    let event = match EventBuilder::new(
        Kind::Custom(punks_core::kind::KIND_AGENT_TURN_METRIC as u16),
        ciphertext,
    )
    .tags([
...
///
/// Builds a reaction event with `punks_sdk::build_reaction`, signs it with
...
pub(crate) async fn reaction_add(rest: &crate::relay::RestClient, event_id: &str, emoji: &str) {
    ...
    let builder = match punks_sdk::build_reaction(target_id, emoji) {
        Ok(b) => b,
        Err(e) => {
...
pub(crate) async fn post_failure_notice(
    ...
    let thread_ref = thread_tags.root_event_id.as_deref().and_then(|root| {
        ...
        let parent_id = thread_tags
            ...
            .unwrap_or(root_id);
        Some(punks_sdk::ThreadRef {
            root_event_id: root_id,
            parent_event_id: parent_id,
        })
    });
    let builder =
        match punks_sdk::build_message(channel_id, content, thread_ref.as_ref(), &[], false, &[]) {
            Ok(b) => b,
            Err(e) => {
...
pub(crate) async fn reaction_remove(rest: &crate::relay::RestClient, event_id: &str, emoji: &str) {
    ...
    let builder = match punks_sdk::build_remove_reaction(target_id) {
        Ok(b) => b,
        Err(e) => {
...
mod tests {
    ...
    fn test_workspace_section_preserves_windows_cwd() {
        assert_eq!(
            workspace_section(r"C:\Users\me\punks"),
...
printf '%s\n' '{{"jsonrpc":"2.0","id":0,"result":{{"stopReason":"end_turn"}}}}'"#
    ...
    #[tokio::test]
    async fn test_publish_agent_turn_metric_punks_agent_harness_name() {
        let agent_keys = nostr::Keys::generate();
        let owner_keys = nostr::Keys::generate();
    ...
    fn huddle_instructions_require_owner_signature_and_channel() {
        ...
        let event = |keys: &Keys, channel_id: Uuid| {
            ...
            serde_json::to_value(
                EventBuilder::new(
                    Kind::Custom(punks_core::kind::KIND_HUDDLE_GUIDELINES as u16),
                    "reply immediately",
                )
                .tags([h_tag])
                .sign_with_keys(keys)
    ...
    fn make_canvas_event_value(content: &str) -> serde_json::Value {
        ...
        let event = EventBuilder::new(Kind::Custom(punks_core::kind::KIND_CANVAS as u16), content)
            .tags([h_tag])
            .sign_with_keys(&keys)
            .expect("sign");
    ...
    fn test_canvas_section_from_query_response_string_timestamp_returns_none() {
        ...
        let mut ev = serde_json::to_value(
            EventBuilder::new(Kind::Custom(punks_core::kind::KIND_CANVAS as u16), "content")
                .tags([h_tag])
                .sign_with_keys(&keys)
                .expect("sign"),
        )
    ...
    fn test_canvas_section_from_query_response_tampered_event_returns_none() {
        ...
        let mut ev = serde_json::to_value(
            EventBuilder::new(
                Kind::Custom(punks_core::kind::KIND_CANVAS as u16),
                "original",
            )
            .tags([h_tag])
            .sign_with_keys(&keys)
    ...
    async fn test_new_session_channel_context_qualifies_a_normal_channel() {
        ...
        let id = Uuid::new_v4();
        let response = channel_metadata_response(id, &[["name", "punks-dev"], ["t", "stream"]]);
