...
fn build_bootstrap(cwd: &Path, shell_hint: &str) -> String {
    ...
    let punks_hint =
        if std::env::var("PUNKS_RELAY_URL").is_ok() && std::env::var("PUNKS_PRIVATE_KEY").is_ok() {
            "\nPunks relay configured. Run `punks --help` to see available commands.\n"
        } else {
            ""
        };
    format!(
        "Working directory: {}\n\
         Detected stack: {}\n\
         Shell: {shell_hint} (set PUNKS_SHELL to override) — write command strings in that shell's syntax.\n\
...
pub async fn run(
    ...
    // PUNKS_PRIVATE_KEY is intentionally inherited — the punks CLI needs it.
...
///
/// The resolver supports `PUNKS_SHELL=cmd`/`pwsh` (explicit operator overrides
...
/// broken, so the resolver only needs to honor PUNKS_SHELL. The probe logic and
...
fn resolve_bash(_path_env: &str) -> Result<(PathBuf, String), String> {
    // Honor PUNKS_SHELL on Unix so power users can opt into zsh or another shell.
    if let Some(raw) = std::env::var_os("PUNKS_SHELL") {
...
///   1. `PUNKS_SHELL` env override — explicit operator choice, any shell (cmd,
...
fn resolve_bash(path_env: &str) -> Result<(PathBuf, String), String> {
    ...
    Err(
        "Git for Windows (Git Bash) is required but was not found. Checked \\
         PUNKS_SHELL, GIT_BASH, bash.exe and git.exe on PATH, the standard Git install locations, \\
         and HKLM/HKCU\\\\SOFTWARE\\\\GitForWindows. Git's \"Cmd\" PATH option adds \\
         Git\\\\cmd\\\\git.exe but not Git\\\\bin\\\\bash.exe; Punks normally derives Git Bash from that git.exe. \\
         Install it from https://git-scm.com/download/win and select \"Git from the command line \\
         and also from 3rd-party software\", then relaunch Punks. You can also set \\
         PUNKS_SHELL to a shell executable."
            .into(),
    )
}
...
mod windows_resolver_tests {
    ...
    // Process-global env mutation guard: tests that mutate PUNKS_SHELL,
    ...
    #[test]
    fn punks_shell_override_wins_over_everything() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        // PUNKS_SHELL pointing at a real file must be returned without probing
        // the standard Git-for-Windows locations or PATH.
        let dir = tempdir().expect("tempdir");
        let fake_bash = dir.path().join("my-bash.exe");
        touch(&fake_bash);
        // Temporarily set PUNKS_SHELL; clean up after the test.
        env::set_var("PUNKS_SHELL", &fake_bash);
        let result = resolve_bash("");
        env::remove_var("PUNKS_SHELL");
        let (resolved, _name) = result.expect("PUNKS_SHELL override should resolve");
        assert_eq!(resolved, fake_bash);
    }

    #[test]
    fn punks_shell_override_skipped_when_path_absent() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        // If PUNKS_SHELL points at a non-existent path the resolver must fall
        // through rather than returning a dead path.
        env::set_var("PUNKS_SHELL", r"C:\does\not\exist\bash.exe");
        ...
        if let Ok((resolved, _)) = result {
            assert_ne!(
                ...
                r"C:\does\not\exist\bash.exe",
                "non-existent PUNKS_SHELL must not be returned"
            );
        }
        // An Err is also acceptable (no Git installed on test host).
    }

    /// Explicit PUNKS_SHELL bare name resolves through PATH and uses NO System32
    /// exclusion — cmd/pwsh live in System32 legitimately.
    #[test]
    fn punks_shell_explicit_bare_name_resolves_from_system32() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        // Simulate cmd.exe living in a dir that would be excluded by the WSL guard.
        // The explicit PUNKS_SHELL branch must NOT skip System32.
        ...
        // for bash.exe, but must NOT skip it for an explicit PUNKS_SHELL).
        let path_env = env::join_paths([sys32.path().to_path_buf()]).expect("join");
        env::set_var("PUNKS_SHELL", "cmd");
        ...
        let (resolved, name) =
            result.expect("explicit PUNKS_SHELL=cmd should resolve even from System32-like dir");
    ...
    fn implicit_bash_scan_still_skips_system32() {
        ...
        // Without an explicit PUNKS_SHELL, the fallback scan must skip it.
        let sys32 = tempdir().expect("sys32");
        touch(&sys32.path().join("bash.exe"));

        let path_env = env::join_paths([sys32.path().to_path_buf()]).expect("join");
        // No PUNKS_SHELL — trigger the implicit bash fallback scan.
        env::remove_var("PUNKS_SHELL");
    ...
    /// Verify that constructing SharedState with PUNKS_SHELL set produces a
    ...
    fn shared_state_bootstrap_hint_matches_resolved_shell() {
        ...
        touch(&fake_pwsh);
        env::set_var("PUNKS_SHELL", &fake_pwsh);
    ...
    /// F3: PUNKS_SHELL bare command name (e.g. "pwsh") resolved through PATH.
    /// When pwsh.exe is on PATH, resolve_bash must return it and report "pwsh".
    #[test]
    fn punks_shell_bare_name_resolved_through_path_when_present() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        let dir = tempdir().expect("tempdir");
        let fake_pwsh = dir.path().join("pwsh.exe");
        touch(&fake_pwsh);
        ...
        env::set_var("PUNKS_SHELL", "pwsh");
        let result = resolve_bash(path_env.to_str().expect("utf8"));
        env::remove_var("PUNKS_SHELL");

        let (resolved, name) = result.expect("bare PUNKS_SHELL=pwsh should resolve from PATH");
    ...
    /// F3: PUNKS_SHELL bare command name absent from PATH → fall through, do not
    /// report pwsh as the active shell.
    #[test]
    fn punks_shell_bare_name_absent_from_path_falls_through() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        // Set PUNKS_SHELL to a command that won't be on any real PATH.
        env::set_var("PUNKS_SHELL", "punks-shell-does-not-exist-xyz");
        let result = resolve_bash("");
        env::remove_var("PUNKS_SHELL");
        if let Ok((resolved, name)) = result {
            assert_ne!(
                resolved.file_name().and_then(|n| n.to_str()).unwrap_or(""),
                "punks-shell-does-not-exist-xyz.exe",
                "absent PUNKS_SHELL must not be returned as the resolved path"
            );
            assert_ne!(
                name, "punks-shell-does-not-exist-xyz",
                "absent PUNKS_SHELL must not be reported as the shell name"
            );
        }
    ...
    fn git_cmd_on_path_resolves_sibling_git_bash() {
        ...
        let old_punks_shell = env::var_os("PUNKS_SHELL");
        ...
        match old_punks_shell {
            Some(value) => env::set_var("PUNKS_SHELL", value),
            None => env::remove_var("PUNKS_SHELL"),
        }
        match old_git_bash {
