...
pub async fn dispatch_action(
    ...
    let serving_write =
        punks_deletion::acquire_serving_write(&engine.db, community_id, "workflow_action")
            .await
            .map_err(|error| {
...
// is_private_ip is provided by punks_core::network::is_private_ip
...
async fn check_ssrf(host: &str, port: u16) -> Result<std::net::IpAddr, WorkflowError> {
    ...
    for ip in &addrs {
        if punks_core::network::is_private_ip(ip) {
...
async fn add_reaction_impl(message_id: &str, emoji: &str) -> Result<JsonValue, WorkflowError> {
    let base_url = std::env::var("PUNKS_RELAY_BASE_URL")
    ...
    if let Ok(token) = std::env::var("PUNKS_API_TOKEN") {
        req = req.header("Authorization", format!("Bearer {token}"));
    } else if let Ok(pubkey) = std::env::var("PUNKS_RELAY_PUBKEY") {
...
pub async fn execute_run(
    ...
    engine
        ...
        .update_workflow_run(
            community_id,
            run_id,
            punks_db::workflow::RunStatus::Running,
            0,
            &serde_json::json!([]),
            None,
        )
...
mod tests {
    ...
    fn trigger_context_get_field_webhook_field() {
        let mut ctx = make_trigger();
        ctx.webhook_fields
            .insert("repo".to_owned(), "punks".to_owned());
        assert_eq!(ctx.get_field("repo"), Some("punks"));
    }
