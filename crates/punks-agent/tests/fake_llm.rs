//! Integration test: fake LLM HTTP server + punks-agent subprocess.
//!
...
impl Harness {
    async fn spawn(base_url: &str) -> Self {
        let bin = env!("CARGO_BIN_EXE_punks-agent");
        ...
        cmd.env("PUNKS_AGENT_PROVIDER", "openai")
            ...
            .kill_on_drop(true);
        let mut child = cmd.spawn().expect("spawn punks-agent");
...
/// Wait for the `activeRunId` advert punks-agent emits at prompt start and
...
/// completion_tokens). punks-agent maps these to `accumulatedInputTokens` /
...
/// punks-agent must treat this turn's total as Unknown and poison the cumulative.
...
/// punks-agent must emit `_goose/unstable/session/update` with `sessionUpdate:
...
/// When the provider returns a response with no `usage` block, punks-agent must
...
/// (so token counts are observed), punks-agent must still emit the usage
