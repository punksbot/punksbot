//! Shared subprocess test harness for the punks-agent ACP integration suites.
//!
...
//!
//! It drives a real `punks-agent` child over the ACP wire against a fake LLM
...
impl Harness {
    ...
    pub async fn recv(&mut self) -> Value {
        let mut line = String::new();
        let n =
            match tokio::time::timeout(Duration::from_secs(15), self.stdout.read_line(&mut line))
                .await
            {
                Ok(result) => result.expect("read line"),
                Err(_) => panic!("recv timeout; stderr={}", self.stderr_text()),
            };
...
/// `session/request_permission` response. Mirrors punks-acp's answering side,
