...
//! exercised manually via the `punks-agent auth databricks` subcommand
//! (see `lib.rs::auth_subcommand`).
//!
//! The second test module (further down) is an ACP-level envelope
//! regression: it spawns the real `punks-agent` binary with
...
use punks_agent::auth::{PkceOAuthConfig, PkceOAuthTokenSource, TokenSource};
...
//
// Boots the real punks-agent binary with `DATABRICKS_TOKEN` set (so the
...
async fn model_discovery_surfaces_rejected_static_token_as_auth_failure() {
    ...
    use punks_agent::config::{Config, Provider};
    use punks_agent::discover_databricks_models;
