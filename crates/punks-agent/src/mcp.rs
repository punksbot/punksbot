...
const PASSTHROUGH_ENV: &[&str] = &[
    ...
    "SSL_CERT_FILE",
    "SSL_CERT_DIR",
    // Punks identity — dev-mcp writes NOSTR_PRIVATE_KEY to a keyfile then
    // removes it from its own env (children never see it). PUNKS_PRIVATE_KEY
    // and PUNKS_RELAY_URL are kept for the punks CLI. PUNKS_AUTH_TAG is a
    ...
    // spawn punks-agent without declaring it.
...
mod content_tests {
    ...
    #[test]
    fn passthrough_includes_punks_owner_attestation() {
        assert!(PASSTHROUGH_ENV.contains(&"PUNKS_AUTH_TAG"));
    }
