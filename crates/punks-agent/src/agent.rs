...
fn is_punks_reply_call(call: &ToolCall, mcp: &McpRegistry) -> bool {
...
///
/// Split from [`is_punks_reply_call`] only so the matcher is testable without a
...
impl RunCtx<'_> {
    ...
    pub async fn run(&mut self, prompt: Vec<ContentBlock>) -> Result<StopReason, AgentError> {
        ...
        *self.turn_total_state = TurnTotalState::Unseen;
        ...
        // successful publish. See `is_punks_reply_call`.
        let mut punks_reply_call_seen = false;
        ...
        loop {
            ...
            if self.cfg.require_reply && !punks_reply_call_seen {
                punks_reply_call_seen = calls.iter().any(|c| is_punks_reply_call(c, self.mcp));
            }
            self.history.push(HistoryItem::Assistant {
                text: response.text,
...
fn synthetic_hook_id(hook: &str, server: &str, ordinal: u64) -> String {
    format!("punks_hook_{hook}_{server}_{ordinal}")
}
