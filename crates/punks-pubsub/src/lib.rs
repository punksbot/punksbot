#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `punks-pubsub` — Redis pub/sub fan-out, presence tracking, and typing indicators.
//!
//! # Architecture
//!
//! ```text
...
//!         └── dynamic SUBSCRIBE punks:{community}:channel:{id} / punks:{community}:global
...
pub struct ChannelEvent {
    ...
    pub community_id: punks_core::CommunityId,
...
/// Central pub/sub manager for a Punks relay instance.
...
impl PubSubManager {
    ...
    /// `punks:{community}:channel:{id}` / `punks:{community}:global`, and
