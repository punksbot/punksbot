...
/// Redis key prefix for Punks-scoped pub/sub topics and keys.
pub const PUNKS_PREFIX: &str = "punks";
    ...
    pub fn redis_channel(&self) -> String {
        match self.topic {
            EventTopic::Channel(channel_id) => {
                format!("{PUNKS_PREFIX}:{}:channel:{channel_id}", self.community_id)
            }
            EventTopic::Global => format!("{PUNKS_PREFIX}:{}:global", self.community_id),
        }
    }
...
mod tests {
    ...
    fn channel_key_includes_community_and_channel() {
        ...
        assert_eq!(
            ...
            format!("punks:{}:channel:{channel_id}", ctx.community())
        );
    }
    ...
    fn global_key_includes_community() {
        ...
        assert_eq!(global_key(&ctx), format!("punks:{}:global", ctx.community()));
    }
    ...
    fn parses_channel_topic() {
        ...
        let channel_id = Uuid::from_u128(0xbbbb);
        let raw = format!("punks:{community_id}:channel:{channel_id}");
    ...
    fn parses_global_topic() {
        let community_id = CommunityId::from_uuid(Uuid::from_u128(0xaaaa));
        let raw = format!("punks:{community_id}:global");
    ...
    fn rejects_malformed_or_wrong_prefix_topics() {
        for raw in [
            "",
            "not-punks:00000000-0000-0000-0000-00000000aaaa:global",
            "punks:not-a-uuid:global",
            "punks:00000000-0000-0000-0000-00000000aaaa",
            "punks:00000000-0000-0000-0000-00000000aaaa:global:extra",
            "punks:00000000-0000-0000-0000-00000000aaaa:channel",
            "punks:00000000-0000-0000-0000-00000000aaaa:channel:not-a-uuid",
            "punks:00000000-0000-0000-0000-00000000aaaa:presence:abc",
        ] {
            assert!(
