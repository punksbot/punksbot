...
/// Subscribes to `punks:*:conn-control` and forwards scoped commands to the
...
mod tests {
    ...
    fn conn_control_channel_is_community_scoped() {
        ...
        assert_eq!(
            ...
            format!("punks:{}:conn-control", a.community())
        );
    ...
    fn parse_rejects_foreign_channels() {
        assert_eq!(
            parse_conn_control_channel("punks:not-a-uuid:conn-control"),
            None
        );
        assert_eq!(parse_conn_control_channel("punks:*:cache-invalidate"), None);
        let a = ctx(0x1234, "a.example");
