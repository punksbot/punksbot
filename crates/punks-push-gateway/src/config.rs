...
impl Config {
    ...
    pub fn from_map(e: &HashMap<String, String>) -> Result<Self, ConfigError> {
        ...
        let grant_keys = parse_keyring(e, "PUNKS_PUSH_GRANT_KEYS")?;
        let token_keys = parse_keyring(e, "PUNKS_PUSH_TOKEN_KEYS")?;
        if grant_keys.iter().any(|grant| {
            token_keys
                .iter()
                .any(|token| grant.id == token.id || grant.key == token.key)
        }) {
            return Err(ConfigError::Invalid("PUNKS_PUSH_TOKEN_KEYS"));
        }
        let public_delivery_url = req(e, "PUNKS_PUSH_PUBLIC_DELIVERY_URL")?
            .parse::<url::Url>()
            .map_err(|_| ConfigError::Invalid("PUNKS_PUSH_PUBLIC_DELIVERY_URL"))?;
        if public_delivery_url.scheme() != "https"
            || public_delivery_url.host_str() != Some("push.punks.xyz")
        ...
        {
            return Err(ConfigError::Invalid("PUNKS_PUSH_PUBLIC_DELIVERY_URL"));
        }
        let max_grant_lifetime_seconds = req(e, "PUNKS_PUSH_MAX_GRANT_LIFETIME_SECONDS")?
            .parse::<i64>()
            .ok()
            .filter(|seconds| (1..=31_536_000).contains(seconds))
            .ok_or(ConfigError::Invalid("PUNKS_PUSH_MAX_GRANT_LIFETIME_SECONDS"))?;
        let max_installation_lifetime_seconds = e
            .get("PUNKS_PUSH_MAX_INSTALLATION_LIFETIME_SECONDS")
            .map(String::as_str)
            .unwrap_or("7776000")
            .parse::<i64>()
            .ok()
            ...
            .ok_or(ConfigError::Invalid(
                "PUNKS_PUSH_MAX_INSTALLATION_LIFETIME_SECONDS",
            ))?;
        ...
        let endpoint_quota_window_seconds =
            bounded_positive("PUNKS_PUSH_ENDPOINT_QUOTA_WINDOW_SECONDS", 10, 86_400)?;
        let endpoint_quota_max_deliveries =
            bounded_positive("PUNKS_PUSH_ENDPOINT_QUOTA_MAX_DELIVERIES", 10, 10_000)?;
        let enabled_profiles = req(e, "PUNKS_PUSH_ENABLED_PROFILES")?
            .split(',')
            .map(|profile| match profile {
                "punks-ios-production" => Ok(crate::model::AppProfile::PunksIosProduction),
                "punks-ios-sandbox" => Ok(crate::model::AppProfile::PunksIosSandbox),
                _ => Err(ConfigError::Invalid("PUNKS_PUSH_ENABLED_PROFILES")),
            })
            .collect::<Result<HashSet<_>, _>>()?;
        if enabled_profiles.is_empty() {
            return Err(ConfigError::Invalid("PUNKS_PUSH_ENABLED_PROFILES"));
        }
        Ok(Self {
            bind_addr: e
                .get("PUNKS_PUSH_BIND_ADDR")
                .map(String::as_str)
                .unwrap_or("0.0.0.0:8080")
                .parse()
                .map_err(|_| ConfigError::Invalid("PUNKS_PUSH_BIND_ADDR"))?,
            health_addr: e
                .get("PUNKS_PUSH_HEALTH_ADDR")
                .map(String::as_str)
                .unwrap_or("0.0.0.0:8081")
                .parse()
                .map_err(|_| ConfigError::Invalid("PUNKS_PUSH_HEALTH_ADDR"))?,
            public_delivery_url,
            ...
            app_attest_app_id: req(e, "PUNKS_PUSH_APP_ATTEST_APP_ID")?.to_owned(),
            app_attest_root_cert_path: req(e, "PUNKS_PUSH_APP_ATTEST_ROOT_CERT_PATH")?.into(),
            grant_keys,
            token_keys,
            apns_key_path: req(e, "PUNKS_PUSH_APNS_KEY_PATH")?.into(),
            apns_key_id: req(e, "PUNKS_PUSH_APNS_KEY_ID")?.to_owned(),
            apns_team_id: req(e, "PUNKS_PUSH_APNS_TEAM_ID")?.to_owned(),
            apns_topic: req(e, "PUNKS_PUSH_APNS_TOPIC")?.to_owned(),
        })
    }
}
...
mod tests {
    ...
    fn base() -> HashMap<String, String> {
        HashMap::from([
            ...
            (
                "PUNKS_PUSH_PUBLIC_DELIVERY_URL".into(),
                "https://push.punks.xyz/v1/deliveries/apns".into(),
            ),
            (
                "PUNKS_PUSH_MAX_GRANT_LIFETIME_SECONDS".into(),
                "2592000".into(),
            ),
            (
                "PUNKS_PUSH_ENABLED_PROFILES".into(),
                "punks-ios-production".into(),
            ),
            (
                "DATABASE_URL".into(),
                "postgres://punks:test@localhost/punks".into(),
            ),
            ("PUNKS_PUSH_APP_ATTEST_APP_ID".into(), "TEAM.app".into()),
            (
                "PUNKS_PUSH_APP_ATTEST_ROOT_CERT_PATH".into(),
                "/apple-root.pem".into(),
            ),
            ("PUNKS_PUSH_APNS_KEY_PATH".into(), "/key.p8".into()),
            ("PUNKS_PUSH_APNS_KEY_ID".into(), "key".into()),
            ("PUNKS_PUSH_APNS_TEAM_ID".into(), "team".into()),
            ("PUNKS_PUSH_APNS_TOPIC".into(), "app".into()),
        ])
    }
    ...
    fn malformed_security_configuration_fails_startup() {
        for (key, value) in [
            (
                "PUNKS_PUSH_PUBLIC_DELIVERY_URL",
            ...
            ("PUNKS_PUSH_APP_ATTEST_APP_ID", ""),
            ("PUNKS_PUSH_ENABLED_PROFILES", "unknown-profile"),
            ("PUNKS_PUSH_MAX_GRANT_LIFETIME_SECONDS", "0"),
            ("PUNKS_PUSH_MAX_GRANT_LIFETIME_SECONDS", "31536001"),
            ("PUNKS_PUSH_MAX_INSTALLATION_LIFETIME_SECONDS", "0"),
        ] {
            let mut env = base();
    ...
    fn cross_keyring_id_or_material_reuse_fails_startup() {
        for token_keys in [
            ...
            let mut env = base();
            env.insert("PUNKS_PUSH_TOKEN_KEYS".into(), token_keys);
    ...
    fn malformed_or_empty_keyrings_fail_startup() {
        for (variable, value) in [
            ("PUNKS_PUSH_GRANT_KEYS", ""),
            ("PUNKS_PUSH_GRANT_KEYS", "missing_separator"),
            ("PUNKS_PUSH_GRANT_KEYS", "id:bad-base64"),
            ("PUNKS_PUSH_TOKEN_KEYS", ""),
            ("PUNKS_PUSH_TOKEN_KEYS", "missing_separator"),
            ("PUNKS_PUSH_TOKEN_KEYS", "id:bad-base64"),
        ] {
            let mut env = base();
