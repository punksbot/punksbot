//! Punks V2 kind number registry.
//!
//! This module is the authoritative source for Punks kind numbers.
...
/// NIP-01: Channel metadata (replaceable). Not used by Punks today.
...
/// Punks custom one-time identity binding proof (ephemeral, not stored).
...
// NEW: Punks command kinds (Pure Nostr plan)
...
/// `crates/punks-search/tests/fts_integration.rs`).
///
...
/// visibility pushdown in `punks-db`.
///
...
/// Punks product feedback submission. Accepted at ingest, sidecarred to the
...
// Punks community moderation commands (mod-signed, processed like 9030-series:
...
/// Punks: Set the workspace profile (icon). Admin/owner-signed command.
...
/// Restore a previously retracted message while retaining its audit history.
pub const KIND_STREAM_MESSAGE_RESTORED: u32 = 40009;
/// Permanently erase message content while retaining an audited tombstone.
pub const KIND_STREAM_MESSAGE_ERASED: u32 = 40010;
...
// Not using NIP-90 kinds (5000–6999) — Punks requires auth chains (depth ≤ 3, breadth ≤ 10).
...
/// or the Punks workspace-profile admin command (9033).
...
/// Returns `true` if `kind` is a Punks command kind that requires transactional execution.
...
/// Safe: all Punks kinds fit in i32 (max 65535 < i32::MAX).
...
// Compile-time: all Punks kind constants fit in nostr's u16-backed Kind.
