...
//! `punks-relay` / `punks-pubsub`. Fixed-window counter algorithm.
//!
...
///
/// The Redis-backed production implementation lives in `punks-relay` / `punks-pubsub`.
...
/// key is community-prefixed (`punks:{community}:ratelimit:{pubkey}:{suffix}`). The
...
/// `punks:{community}:ratelimit:{pubkey_hex}:{suffix}`.
///
...
/// Redis key for IP-based rate limit: `punks:ratelimit:ip:{ip}:conn`.
///
/// Operator-global by design — see [`RateLimiter`] docs.
pub fn ip_rate_limit_key(ip: &IpAddr) -> String {
    format!("punks:ratelimit:ip:{}:conn", ip)
}
...
mod tests {
    ...
    fn rate_limit_key_includes_community_prefix() {
        ...
        let expected_prefix = format!("punks:{}:ratelimit:", ctx.community());
        assert!(
    ...
    fn ip_rate_limit_key_format() {
        ...
        assert_eq!(ip_rate_limit_key(&ip), "punks:ratelimit:ip:192.168.1.1:conn");
    }
