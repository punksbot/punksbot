#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `punks-auth` — Authentication and authorization for the Punks relay.
//!
//! ## Auth paths
//!
...
///
/// Uses `SHA-256("punks-test-key:{username}")` as the secret key material.
...
pub fn derive_pubkey_from_username(username: &str) -> Result<nostr::PublicKey, AuthError> {
    ...
    let seed = format!("punks-test-key:{username}");
