...
//!
//! Defines [`ChannelAccessChecker`] so `punks-auth` can enforce access
//! without depending on `punks-db` directly.
...
///
/// Implemented by the database layer (`punks-db`) in production. The `punks-auth`
