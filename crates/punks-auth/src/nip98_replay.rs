...
//!
//! ```ignore
//! let pubkey = punks_auth::verify_nip98_event(json, url, method, body)?;
//! if !replay.try_mark(&ctx, &event_id, punks_auth::DEFAULT_REPLAY_TTL_SECS).await? {
...
///
/// The production implementation lives in `punks-pubsub` (Redis `SET NX EX`).
...
/// `punks:{community}:nip98:{event_id_hex}`.
///
...
pub fn nip98_replay_key_for_scope(scope: &str, event_id: &EventId) -> String {
    format!("punks:{scope}:nip98:{}", event_id.to_hex())
}
...
mod tests {
    ...
    fn key_includes_community_prefix() {
        ...
        let expected_prefix = format!("punks:{}:nip98:", ctx.community());
        assert!(
