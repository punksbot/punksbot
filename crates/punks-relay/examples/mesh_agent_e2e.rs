...
//!   P4 agentic tool use     — agent + punks-dev-mcp writes a file on disk.
//!
...
//! `punks-agent` binary with the exact env vars the relay-mesh preset ships.
//!
...
async fn run() -> anyhow::Result<()> {
    ...
    // mode that broke relay-mesh agents when punks-agent's default 32768
    // budget met a 32k-context model). 150k output: passes punks-agent's own
    ...
    // P4: agentic tool use via punks-dev-mcp — write a real file inside the
    ...
    let prompt = format!(
        "Use your developer tools to create {marker_name} in the current working directory containing exactly the text PUNKS_OK (no quotes, no newline commentary). Then confirm."
    );
    let mcp = vec![("dev".to_string(), repo_bin("punks-dev-mcp")?)];
    let (r, marker) =
        agent_chat_with_marker(&base, "mesh", None, &prompt, &mcp, &marker_name).await;
    let file_ok = std::fs::read_to_string(&marker)
        .map(|c| c.contains("PUNKS_OK"))
        .unwrap_or(false);
    match r {
        Ok(text) => record(
            "P4 agentic tool use",
...
/// Spawn the real punks-agent with relay-mesh preset env and drive one ACP
...
async fn agent_chat_in_isolated_home(
    ...
    let agent = match repo_bin("punks-agent") {
        Ok(agent) => agent,
    ...
    command
        ...
        // model, key, and chat API. Not PUNKS_AGENT_REQUIRE_REPLY, which needs
        // Punks's publish tools to mean anything.
        .env("PUNKS_AGENT_PROVIDER", "openai")
        .env("PUNKS_AGENT_MODEL", model)
        .env("OPENAI_COMPAT_BASE_URL", base)
        .env("OPENAI_COMPAT_MODEL", model)
        .env("OPENAI_COMPAT_API_KEY", "punks-mesh-local")
        .env("OPENAI_COMPAT_API", "chat")
        .env("PUNKS_AGENT_MAX_OUTPUT_TOKENS", "4096")
        // No PUNKS_AGENT_THINKING_EFFORT: apply_relay_mesh_env() deliberately
    ...
    if let Some(value) = max_output_tokens {
        command.env("PUNKS_AGENT_MAX_OUTPUT_TOKENS", value);
    }
