//! Relay-driven mesh lifecycle smoke — the full Punks join story, CI-shaped.
//!
...
//!      punks-relay (kind:13534 roster via punks-admin); a third is not.
...
//! cargo build --profile ci -p punks-admin
//! PUNKS_ADMIN_BIN=target/ci/punks-admin \
//!   cargo run --profile ci -p punks-relay --example mesh_relay_lifecycle_smoke
//! ```
...
/// (`KIND_PUNKS_MESH_MEMBER_STATUS` in the desktop coordinator).
...
const STATUS_K_TAG: &str = "punks-mesh-status";
...
fn member_endpoint_binding_bytes(member_pubkey: &str, endpoint_tokens: &[String]) -> Vec<u8> {
    ...
    format!(
        "punks-mesh-owner-endpoint-binding-v1:{}:{}",
        member_pubkey.trim().to_ascii_lowercase(),
...
async fn role_serve() -> anyhow::Result<()> {
    ...
    let keys = Keys::parse(&env("PUNKS_MEMBER_NSEC")?)?;
    ...
    let mut relay = PunksTestClient::connect(&relay_ws_url(), &keys)
        .await
    ...
    let cfg = serve::EmbeddedServeConfig::builder()
        ...
        // auto-discover. The Punks relay is the only discovery surface.
        .publish(false)
        .auto_join(false)
...
async fn role_stranger() -> anyhow::Result<()> {
    ...
    match PunksTestClient::connect(&relay_ws_url(), &keys).await {
        Err(error) => {
...
fn orchestrate() -> anyhow::Result<()> {
    ...
    let admin =
        std::env::var("PUNKS_ADMIN_BIN").unwrap_or_else(|_| "target/ci/punks-admin".to_string());
    anyhow::ensure!(
        std::path::Path::new(&admin).exists(),
        "punks-admin binary not found at {admin} (set PUNKS_ADMIN_BIN)"
    );

    let scratch = std::env::temp_dir().join(format!("punks-mesh-lifecycle-{}", std::process::id()));
    ...
    // MEMBERSHIP: A and B become relay members via punks-admin (publishes the
    ...
    for (label, keys) in [("A", &member_a), ("B", &member_b)] {
        let status = Command::new(&admin)
            ...
            .status()?;
        anyhow::ensure!(status.success(), "punks-admin add-member {label} failed");
        eprintln!(
    ...
    let mut serve_child = Command::new(&exe)
        ...
        .env("PUNKS_MEMBER_NSEC", secret_hex(&member_a))
    ...
    let mut client_child = Command::new(&exe)
        ...
        .env("PUNKS_MEMBER_NSEC", secret_hex(&member_b))
    ...
    let mut stranger_child = Command::new(&exe)
        ...
        .env("PUNKS_MEMBER_NSEC", secret_hex(&stranger))
