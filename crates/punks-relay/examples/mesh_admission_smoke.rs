...
//!
//! Proves the security claim of Punks's membership-derived mesh admission:
...
//!      same builder calls Punks desktop makes with a resolved member roster.
...
//! corrupt each other — exactly how Punks runs it in production anyway (one
//! desktop = one node).
//!
...
fn orchestrate() -> anyhow::Result<()> {
    ...
    let scratch = std::env::temp_dir().join(format!("punks-mesh-admission-{}", std::process::id()));
