...
use punks_core::kind::KIND_STREAM_MESSAGE;
use punks_core::tenant::CommunityId;
use punks_workflow::action_sink::{ActionSink, ActionSinkError};
use chrono::Utc;
...
impl ActionSink for RelayActionSink {
    fn send_message(
        ...
        Box::pin(async move {
            ...
            let tenant = punks_core::tenant::TenantContext::resolved(community_id, host);
            ...
            let channel = state
                ...
                .await
                .map_err(|e| match &e {
                    punks_db::DbError::ChannelNotFound(_) | punks_db::DbError::NotFound(_) => {
            ...
            //    - `punks:workflow` tag prevents recursive workflow triggering
            ...
            // the `root` + `reply` pair — matching `punks_sdk::builders::thread_tags`
            ...
            let thread_meta = Some(match &thread_meta_owned {
                ...
                None => punks_db::event::ThreadMetadataParams {
...
mod integration_tests {
    ...
    //!   `cargo test -p punks-relay --lib workflow_sink -- --ignored`
    use super::*;
    use punks_core::channel::{ChannelType, ChannelVisibility, MemberRole};
    use punks_db::CreateCommunityWithOwnerResult;
    use std::sync::Arc;
