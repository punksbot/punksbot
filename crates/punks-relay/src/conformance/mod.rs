...
//! from `punks-conformance`. See `crates/punks-conformance/src/lib.rs` for
...
///   [`punks_db::Punks::communities_of_channels`]). On a hit, return the
...
fn project_row_community(
    ...
    channel_communities: &std::collections::HashMap<Uuid, punks_core::CommunityId>,
...
/// [`punks_db::Punks::communities_of_channels`] over the distinct
/// channel ids in `rows`.
