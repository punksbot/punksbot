...
use punks_core::filter::{filters_match, reader_authorized_for_event};
...
async fn process_match_batch(state: &AppState, batch: punks_db::push::ClaimedMatchBatch) {
    ...
    let mut wakes: Vec<punks_db::push::WakeRequest> = Vec::new();
    for job in &batch.jobs {
        ...
        match match_job(job, &context) {
            ...
            Err(e) => {
                ...
                if job.attempt >= punks_db::push::MAX_MATCH_ATTEMPTS {
    ...
    match state.db.enqueue_push_wakes(community, &wakes).await {
        ...
        Err(e) => {
            ...
            for (event_id, attempt) in pending {
                if attempt >= punks_db::push::MAX_MATCH_ATTEMPTS {
                    completed.push(event_id);
                } else {
                    retry.push(event_id);
                }
...
fn match_job(
    job: &punks_db::push::BatchedMatch,
    context: &MatchContext,
) -> anyhow::Result<Vec<punks_db::push::WakeRequest>> {
    ...
    for lease in &context.leases {
        ...
        wakes.push(punks_db::push::WakeRequest {
...
fn push_filter_authorized_for_event(
    ...
    if punks_core::kind::event_kind_u32(event) != punks_core::kind::KIND_GIFT_WRAP {
        return true;
    }
        ...
        match state.db.usage_community_hosts().await {
            Ok(communities) => {
                for community in communities {
                    let community = punks_core::CommunityId::from_uuid(community.id);
                    let until = Utc::now() + TimeDelta::seconds(CLAIM_SECS);
...
async fn deliver_one(
    state: &AppState,
    http: &reqwest::Client,
    claimed: punks_db::push::ClaimedWake,
) {
    let outcome = match state
        ...
        .await
    {
        Ok(punks_db::push::RevalidateWakeOutcome::Deliver(wake)) => wake,
        Ok(punks_db::push::RevalidateWakeOutcome::Suppressed) => {
            let _ = state
                .db
...
async fn retry_or_fail(state: &AppState, wake: &punks_db::push::ClaimedWake, delay: i64) {
