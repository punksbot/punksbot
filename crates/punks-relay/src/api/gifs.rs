...
//! GIF bytes are never downloaded, cached, or stored by Punks.
//!
...
async fn enforce_search_admission(
    ...
    .await
    {
        ...
        Err(crate::admission::AdmissionError::Exceeded { reset_in_secs }) => {
            metrics::counter!("punks_gif_search_rejections_total", "reason" => "quota").increment(1);
            Err(api_error(
...
mod tests {
    ...
    async fn unconfigured_test_state() -> Arc<AppState> {
        ...
        let pool = sqlx::postgres::PgPoolOptions::new()
            .connect_lazy("postgres://punks:punks_dev@127.0.0.1:1/punks") // sadscan:disable np.postgres.1
        ...
        let media_storage =
            punks_media::MediaStorage::new(&config.media).expect("test media storage config");
        let (state, _audit_shutdown) = AppState::new(
            config,
            db,
            redis_pool,
            None::<punks_audit::AuditService>,
            pubsub,
            auth,
            search,
            workflow_engine,
