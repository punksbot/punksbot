...
/// - `PUNKS_HOOK_URL` — internal policy endpoint (http://127.0.0.1:{port}/internal/git/policy)
/// - `PUNKS_HOOK_SECRET` — per-push HMAC secret
/// - `PUNKS_REPO_ID` — repo identifier (d-tag)
/// - `PUNKS_COMMUNITY_ID` — server-resolved community UUID for the git HTTP request
/// - `PUNKS_PUSHER_PUBKEY` — authenticated pusher's hex pubkey
///
...
# Punks pre-receive hook — FAIL-CLOSED
...
: "${PUNKS_REPO_ID:?error: PUNKS_REPO_ID not set}"
: "${PUNKS_REPO_OWNER:?error: PUNKS_REPO_OWNER not set}"
: "${PUNKS_COMMUNITY_ID:?error: PUNKS_COMMUNITY_ID not set}"
: "${PUNKS_PUSHER_PUBKEY:?error: PUNKS_PUSHER_PUBKEY not set}"
: "${PUNKS_HOOK_URL:?error: PUNKS_HOOK_URL not set}"
: "${PUNKS_HOOK_SECRET:?error: PUNKS_HOOK_SECRET not set}"
...
SIGNATURE=$(printf '%s' "$HMAC_INPUT" | openssl dgst -sha256 -hmac "$PUNKS_HOOK_SECRET" -hex 2>/dev/null | sed 's/.*= //')
if [ -z "$SIGNATURE" ]; then
...
SAFE_REPO_ID=$(printf '%s' "$PUNKS_REPO_ID" | sed 's/\\/\\\\/g; s/"/\\"/g')
BODY="{\"repo_id\":\"${SAFE_REPO_ID}\",\"repo_owner\":\"${PUNKS_REPO_OWNER}\",\"community_id\":\"${PUNKS_COMMUNITY_ID}\",\"pusher_pubkey\":\"${PUNKS_PUSHER_PUBKEY}\",\"ref_updates\":[${REFS}],\"timestamp\":${TIMESTAMP},\"signature\":\"${SIGNATURE}\"}"
