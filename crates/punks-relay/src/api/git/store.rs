...
//! ## The 412 sharp edge
//!
//! `rust-s3 = "0.37"` is shared across the workspace with `punks-media`. The
...
impl GitStore {
    ...
    ///
    /// Credential selection mirrors [`punks_media::MediaStorage::new`]:
    ...
    pub fn new(
        ...
        bucket_name: &str,
        region: &str,
        addressing_style: punks_media::config::S3AddressingStyle,
        ...
        let bucket = match addressing_style {
            punks_media::config::S3AddressingStyle::Path => bucket.with_path_style(),
            punks_media::config::S3AddressingStyle::Virtual => bucket,
        };
        Ok(Self {
...
mod tests {
    ...
    fn constructor_applies_both_addressing_styles() {
        for (style, expected_url, path_style) in [
            (
                punks_media::config::S3AddressingStyle::Path,
                "https://storage.example/punks-git",
                true,
            ),
            (
                punks_media::config::S3AddressingStyle::Virtual,
                "https://punks-git.storage.example",
                false,
            ),
        ] {
            let store = GitStore::new(
    ...
    fn partial_static_keys_are_rejected() {
        for (access, secret) in [("punks_dev", ""), ("", "punks_dev_secret")] {
...
mod probe {
    ...
    //!
    //! Run manually:
    //!   PUNKS_GIT_S3_PROBE=1 cargo test -p punks-relay --lib \
    //!     api::git::store::probe -- --nocapture --test-threads=1
    //!
    //! Pre-req: `docker compose up minio` and the `punks-git` bucket exists.

    use super::*;

    fn probe_enabled() -> bool {
        std::env::var("PUNKS_GIT_S3_PROBE").as_deref() == Ok("1")
    }
    ...
    fn store() -> GitStore {
        ...
        let endpoint =
            std::env::var("PUNKS_S3_ENDPOINT").unwrap_or_else(|_| "http://localhost:9000".into());
        let access_key = std::env::var("PUNKS_S3_ACCESS_KEY").unwrap_or_else(|_| "punks_dev".into());
        let secret_key =
            std::env::var("PUNKS_S3_SECRET_KEY").unwrap_or_else(|_| "punks_dev_secret".into());
        let bucket = std::env::var("PUNKS_S3_BUCKET").unwrap_or_else(|_| "punks-git".into());
        let region = std::env::var("PUNKS_S3_REGION").unwrap_or_else(|_| "us-east-1".into());
        let addressing_style = std::env::var("PUNKS_S3_ADDRESSING_STYLE")
    ...
    async fn probe_412_surfacing() {
        if !probe_enabled() {
            eprintln!("skipping: set PUNKS_GIT_S3_PROBE=1 to run against live MinIO");
            return;
        }
        let st = store();
