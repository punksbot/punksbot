//! Relay startup wiring for the inter-relay mesh (`PUNKS_MESH` seam).
//!
//! [`boot_mesh`] is the ONLY place the relay constructs mesh machinery. It
//! returns `None` — and touches nothing — when `PUNKS_MESH=off`, so mesh-off
//! deployments stay byte-identical to a relay built before this module
//! existed. When enabled, it:
//!
//! 1. binds the iroh endpoint on `PUNKS_MESH_BIND_ADDR` (boot-unique keypair =
...
use punks_relay_mesh::endpoint::MeshEndpoint;
use punks_relay_mesh::gossip::GossipRecord;
use punks_relay_mesh::registry::{ReadyRecord, ReadyRegistry};
use punks_relay_mesh::{
...
///   either the `PUNKS_MESH_DEMO_ECHO` consumer (testbed evidence runs: echo
...
/// `PUNKS_MESH_DEMO_ECHO` consumer: echo every validated `Data` frame back to
...
const PROTO_VERSION: u16 = punks_relay_mesh::WIRE_VERSION as u16;
...
/// `PUNKS_MESH_ADVERTISE_ADDR` (explicit, classic-LB shapes) →
...
fn advertise_addrs(endpoint: &MeshEndpoint) -> Vec<String> {
    if let Ok(addr) = std::env::var("PUNKS_MESH_ADVERTISE_ADDR") {
...
/// Boot the mesh, or return `None` when `PUNKS_MESH=off`.
///
...
/// sets `PUNKS_MESH=on` wants the mesh or wants to know why not; silently
...
pub async fn boot_mesh(
    ...
    if !config.mesh.enabled {
        tracing::info!("mesh disabled (PUNKS_MESH is not 'on') — single-instance behavior");
        return Ok(None);
    }
    ...
    punks_relay_mesh::runtime::spawn_registry_heartbeat(
        registry.clone(),
        ready_record,
...
mod tests {
    ...
    /// PUNKS_MESH=off must be a hard no-op: no endpoint bind, no Redis write,
    ...
    /// Blocker fix (Wren review of 8b077fdb): absent `PUNKS_MESH`, the mesh is
    /// OFF — an env-untouched image upgrade must not bind or write Redis.
    #[test]
    fn mesh_defaults_off_when_env_absent() {
        // `Config::from_env` in the test env has no PUNKS_MESH set unless a
        // caller exported it; assert the fail-safe reading.
        if std::env::var("PUNKS_MESH").is_ok() {
        ...
        assert!(!config.mesh.enabled, "PUNKS_MESH absent must mean mesh off");
    }
    ...
    fn session_hello(sender: RuntimeId, profile: Profile) -> StreamHello {
        StreamHello {
            sender,
            role: punks_relay_mesh::StreamRole::Session {
                fenced: FencedHeader {
    ...
    async fn wired_datagram_consumer_shares_the_handle_fence() {
        ...
        impl punks_relay_mesh::RelayPeerTransport for NoopTransport {
