...
//! - `OTEL_SERVICE_NAME` (default: `punks-relay`; read explicitly — not via SDK detector)
...
/// `PUNKS_OTEL_FILTER` to override the default targets.
pub fn otel_env_filter(configured: Option<&str>) -> EnvFilter {
    EnvFilter::new(configured.unwrap_or("punks_relay=info,punks_datastore=info"))
}
...
/// 3. Hard-coded fallback `punks-relay`.
///
...
/// `punks-relay` default.  We therefore read `OTEL_SERVICE_NAME` explicitly
...
pub fn service_resource() -> Resource {
    // Honor OTEL_SERVICE_NAME when set+non-empty; otherwise use punks-relay.
    ...
    let service_name = std::env::var("OTEL_SERVICE_NAME")
        .ok()
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| "punks-relay".to_string());
...
mod tests {
    ...
    fn test_service_resource_default_when_env_unset() {
        ...
        assert_eq!(
            ...
            Some("punks-relay"),
            "expected punks-relay fallback when OTEL_SERVICE_NAME is unset"
        );
    }
    ...
    fn test_service_resource_empty_string_falls_back_to_default() {
        ...
        assert_eq!(
            service_name_from(&r).as_deref(),
            Some("punks-relay"),
            "expected punks-relay fallback when OTEL_SERVICE_NAME is empty"
        );
    }
