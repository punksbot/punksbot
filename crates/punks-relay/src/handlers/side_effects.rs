...
use punks_db::channel::{MemberRecord, MemberRole};
...
/// The per-fire authority gate in `punks-workflow` is the fail-closed backstop;
...
async fn evict_conn_channel_subscriptions(
    ...
    for update in removed {
        state
            .pubsub
            .release_topic(tenant, punks_pubsub::EventTopic::Channel(channel_id))
            .await;
        if update.removed {
...
///
/// Punks accepts standard deletions for self-authored events, plus the owning
...
async fn actor_owns_any_owner_agent(
    state: &Arc<AppState>,
    community_id: punks_core::CommunityId,
    members: &[punks_db::channel::MemberRecord],
    actor_bytes: &[u8],
) -> anyhow::Result<bool> {
    ...
    match kind {
        9000 => {
            ...
            let requested_role = match role_str {
                Some(ref s) => match s.parse::<punks_db::channel::MemberRole>() {
                    Ok(r) => Some(r),
            ...
            let members = state.db.get_members(tenant.community(), channel_id).await?;
            let actor_role: Option<punks_db::channel::MemberRole> = members
                .iter()
                ...
                .filter(|(m, role)| m.role != role.as_str())
            {
                ...
                if target.role == "owner"
                    && role != punks_db::channel::MemberRole::Owner
        ...
        9002 => {
            ...
            for t in event.tags.iter() {
                if t.kind().to_string() == "name" {
                    match t.content() {
                        Some(v)
                            if !punks_core::channel::canonical_channel_name(v)
                                .trim()
                                .is_empty() => {}
                        _ => {
...
async fn emit_addressable_discovery_event(
    ...
    let min_ts = {
        let existing = state
            .db
            .query_events(&punks_db::event::EventQuery {
                kinds: Some(vec![kind as i32]),
                channel_id: Some(channel_id),
                limit: Some(1),
                ..punks_db::event::EventQuery::for_community(tenant.community())
            })
            .await
            .unwrap_or_default();
        existing
...
async fn store_group_members_event(
    ...
    state: &Arc<AppState>,
    channel_id: Uuid,
    member_snapshot: &mut punks_db::channel::LockedMemberSnapshot,
) -> anyhow::Result<Option<punks_core::StoredEvent>> {
...
async fn dispatch_group_members_event(
    tenant: &TenantContext,
    state: &Arc<AppState>,
    stored: Option<punks_core::StoredEvent>,
    relay_pubkey_hex: &str,
) {
...
pub async fn emit_group_discovery_events(
...

    {
        ...
        // Punks channels always require explicit membership
                ...
                "name" => {
                    state
                        ...
                        .update_channel(
                            tenant.community(),
                            channel_id,
                            punks_db::channel::ChannelUpdate {
...
async fn handle_create_group(
    ...
    let channel_type: punks_db::channel::ChannelType = channel_type_str
        .parse()
    ...
    if visibility == punks_db::channel::ChannelVisibility::Open {
        state.invalidate_all_accessible_channels(tenant);
    }
...
async fn handle_join_request(
    ...
    state
        ...
        .add_member(
            ...
            channel_id,
            &actor_bytes,
            punks_db::channel::MemberRole::Member,
            None,
        )
        .await?;
    ...
    if let Err(e) = emit_membership_notification(
        ...
        &actor_bytes,
        &actor_bytes,
        punks_core::kind::KIND_MEMBER_ADDED_NOTIFICATION,
    )
    .await
    {
...
async fn handle_a_tag_deletion(
    ...
    match kind_num {
        ...
        punks_core::kind::KIND_WORKFLOW_DEF => {
...
pub(crate) async fn handle_git_repo_announcement_inner(
    ...
    use punks_db::git_repo::ReserveOutcome;
    ...
    let serving_write = punks_deletion::acquire_serving_write(
        &state.db,
        tenant.community(),
...
/// carrying the announcer's pubkey in the `p` tag (punks extension).
...
pub async fn reconcile_nip43_membership_snapshots(state: &Arc<AppState>) -> anyhow::Result<usize> {
    ...
    for community in communities {
        let community_id = punks_core::CommunityId::from_uuid(community.id);
        ...
        match result {
            ...
            Ok(false) => {}
            Err(error) => {
                metrics::counter!("punks_nip43_membership_reconciliation_failures_total")
                    .increment(1);
    ...
    metrics::counter!("punks_nip43_membership_reconciliations_total").increment(reconciled as u64);
    Ok(reconciled)
}
...
pub async fn publish_nip43_membership_list(
    ...
    metrics::counter!("punks_nip43_membership_publications_total", "result" => "attempted")
        .increment(1);
    let result = publish_nip43_membership_list_inner(tenant, state).await;
    metrics::histogram!("punks_nip43_membership_publication_seconds")
        .record(started_at.elapsed().as_secs_f64());
    metrics::counter!(
        "punks_nip43_membership_publications_total",
...
pub async fn reconcile_large_channel_member_snapshots(
    ...
    for candidate in candidates {
        ...
        match result {
            ...
            Ok(false) => {}
            Err(error) => {
                metrics::counter!("punks_channel_roster_reconciliation_failures_total").increment(1);
                warn!(
    ...
    metrics::counter!("punks_channel_roster_reconciliations_total").increment(reconciled as u64);
    Ok(reconciled)
}
...
pub(crate) mod publish_test_hooks {
    use punks_core::tenant::CommunityId;
