...
//!    participant-hash-idempotent DM model (`punks-db/src/dm.rs`).
...
use punks_core::kind::{event_kind_u32, KIND_STREAM_MESSAGE};
...
mod tests {
    ...
    fn tenant() -> TenantContext {
        TenantContext::resolved(
            punks_core::CommunityId::from_uuid(Uuid::new_v4()),
            "example.org",
        )
    }
