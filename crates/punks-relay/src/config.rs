...
pub enum ConfigError {
    /// The `PUNKS_BIND_ADDR` environment variable could not be parsed as a socket address.
    #[error("invalid PUNKS_BIND_ADDR: {0}")]
...
pub struct Config {
    ...
    /// Replica read budget `B` in milliseconds (`PUNKS_REPLICA_READ_MAX_AGE_MS`).
    /// `0` (the default) disables bounded-staleness replica routing; see
    /// [`punks_db::DbConfig::replica_read_max_age_ms`].
    ...
    /// shutdown (`PUNKS_DRAIN_JITTER_MS`). Each live connection is closed after
    ...
    ///
    /// The `punks-db` default of 20 was sized for a handful of pods against
    ...
    /// (`PUNKS_DB_READ_POOL_SIZE`). Defaults to `db_pool_size`. Sized
    ...
    pub auth: punks_auth::AuthConfig,
    ...
    /// `PUNKS_HUDDLE_AUDIO_AVAILABLE=false` until the out-of-relay media/SFU
    /// service lands.
    pub huddle_audio_available: bool,

    /// Inter-relay mesh configuration (`PUNKS_MESH`, `PUNKS_MESH_BIND_ADDR`).
    /// Opt-in: mesh forms only when `PUNKS_MESH=on` is explicit. The default
    ...
    pub mesh: punks_relay_mesh::MeshConfig,

    /// Testbed-only reliable-stream echo consumer (`PUNKS_MESH_DEMO_ECHO`).
    ...
    /// Same strict opt-in as `PUNKS_MESH`; default off means inbound reliable
    ...
    ///
    /// Default: `false`. Set via `PUNKS_ALLOW_NIP_OA_AUTH=true`.
    ...
    pub media: punks_media::MediaConfig,
    ...
    /// Set `PUNKS_AUDIT_ENABLED=false` for deployments that do not require it.
    ...
    /// Example: `PUNKS_EPHEMERAL_TTL_OVERRIDE=60` → all ephemeral channels expire
...
const DEFAULT_PUSH_GATEWAY_DELIVERY_URL: &str = "https://push.punks.xyz/v1/deliveries/apns";

fn parse_push_gateway_delivery_url(raw: &str) -> Result<url::Url, ConfigError> {
    let url = url::Url::parse(raw.trim()).map_err(|e| {
        ConfigError::InvalidValue(format!(
            "PUNKS_PUSH_GATEWAY_DELIVERY_URL is not a valid URL: {e}"
        ))
    })?;
    if url.scheme() != "https"
        || url.host().is_none()
    ...
    {
        return Err(ConfigError::InvalidValue(
            "PUNKS_PUSH_GATEWAY_DELIVERY_URL must be an exact HTTPS /v1/deliveries/apns URL without credentials, query, or fragment"
                .to_string(),
        ));
    }
    Ok(url)
...
fn ensure_git_repo_path(
    ...
    ensure_git_path("PUNKS_GIT_REPO_PATH", raw)
}
...
///
/// `PUNKS_REQUIRE_MEDIA_GET_AUTH` was the real flag; `PUNKS_REQUIRE_MEDIA_READ_AUTH`
...
const INERT_MEDIA_READ_AUTH_VARS: [&str; 2] = [
    "PUNKS_REQUIRE_MEDIA_GET_AUTH",
    "PUNKS_REQUIRE_MEDIA_READ_AUTH",
];
...
impl Config {
    ...
    pub fn from_env() -> Result<Self, ConfigError> {
        let bind_addr_raw =
            std::env::var("PUNKS_BIND_ADDR").unwrap_or_else(|_| "0.0.0.0:3000".to_string());
        let bind_addr = parse_bind_addr(&bind_addr_raw)?;

        let database_url = std::env::var("DATABASE_URL")
            .unwrap_or_else(|_| "postgres://punks:punks_dev@localhost:5432/punks".to_string()); // sadscan:disable np.postgres.1
        ...
        if std::env::var("PUNKS_REPLICA_HEAD_MAX_AGE_SECS").is_ok() {
            return Err(ConfigError::InvalidValue(
                "PUNKS_REPLICA_HEAD_MAX_AGE_SECS was renamed to PUNKS_REPLICA_READ_MAX_AGE_MS \
        ...
        let replica_read_max_age_ms = match std::env::var("PUNKS_REPLICA_READ_MAX_AGE_MS") {
            Ok(raw) => raw.trim().parse::<u64>().map_err(|_| {
                ConfigError::InvalidValue(
                    "PUNKS_REPLICA_READ_MAX_AGE_MS must be a non-negative integer".to_string(),
                )
            })?,
            Err(_) => 0,
        };
        ...
        let drain_jitter_ms = match std::env::var("PUNKS_DRAIN_JITTER_MS") {
            ...
            Ok(raw) => raw
                ...
                .map_err(|_| {
                    ConfigError::InvalidValue(
                        "PUNKS_DRAIN_JITTER_MS must be a non-negative integer".to_string(),
                    )
                })?
        ...
        let redis_pool_size = std::env::var("PUNKS_REDIS_POOL_SIZE")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .filter(|&v| v > 0)
            .unwrap_or(16);

        let db_pool_size = std::env::var("PUNKS_DB_POOL_SIZE")
            .ok()
            .and_then(|v| v.parse::<u32>().ok())
            .filter(|&v| v > 0)
            .unwrap_or(50);

        let db_read_pool_size = std::env::var("PUNKS_DB_READ_POOL_SIZE")
            .ok()
            .and_then(|v| v.parse::<u32>().ok())
            .filter(|&v| v > 0);
        ...
        let pairing_relay_url = std::env::var("PUNKS_PAIRING_RELAY_URL")
            .ok()
            ...
            .map(|value| {
                let parsed = url::Url::parse(&value).map_err(|e| {
                    ConfigError::InvalidValue(format!(
                        "PUNKS_PAIRING_RELAY_URL must be a valid ws:// or wss:// URL: {e}"
                    ))
                })?;
                if !matches!(parsed.scheme(), "ws" | "wss") || parsed.host_str().is_none() {
                    return Err(ConfigError::InvalidValue(
                        "PUNKS_PAIRING_RELAY_URL must be a valid ws:// or wss:// URL".to_string(),
                    ));
                }
                Ok(value)
            })
        ...
        let max_connections = std::env::var("PUNKS_MAX_CONNECTIONS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(10_000);

        let max_concurrent_handlers = std::env::var("PUNKS_MAX_CONCURRENT_HANDLERS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(1024);

        let send_buffer_size = std::env::var("PUNKS_SEND_BUFFER")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(1_000);

        let max_frame_bytes = std::env::var("PUNKS_MAX_FRAME_BYTES")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .filter(|&v| v > 0)
            .unwrap_or(DEFAULT_MAX_FRAME_BYTES);

        let slow_client_grace_limit = std::env::var("PUNKS_SLOW_CLIENT_GRACE_LIMIT")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(15);

        let require_auth_token = std::env::var("PUNKS_REQUIRE_AUTH_TOKEN")
            .map(|v| v == "true" || v == "1")
            .unwrap_or(false);

        let pubkey_allowlist_enabled = std::env::var("PUNKS_PUBKEY_ALLOWLIST")
            .map(|v| v == "true" || v == "1")
            .unwrap_or(false);

        let require_relay_membership = std::env::var("PUNKS_REQUIRE_RELAY_MEMBERSHIP")
            .map(|v| v == "true" || v == "1")
            .unwrap_or(false);
        ...
        let huddle_audio_available = std::env::var("PUNKS_HUDDLE_AUDIO_AVAILABLE")
            .map(|v| !(v == "false" || v == "0"))
            .unwrap_or(true);
        ...
        // `PUNKS_MESH=on`; anything else (absent, `off`, other values) keeps
        // exact single-instance behavior.
        let mesh_enabled = std::env::var("PUNKS_MESH")
            .map(|v| v.eq_ignore_ascii_case("on") || v == "true" || v == "1")
            .unwrap_or(false);
        let mesh_bind_addr = std::env::var("PUNKS_MESH_BIND_ADDR")
            .map(|raw| {
                raw.parse::<SocketAddr>().map_err(|e| {
                    ConfigError::InvalidValue(format!("invalid PUNKS_MESH_BIND_ADDR: {e}"))
                })
            })
            .unwrap_or_else(|_| Ok("0.0.0.0:3478".parse().expect("static default parses")))?;
        let mesh = punks_relay_mesh::MeshConfig {
            enabled: mesh_enabled,
            bind_addr: mesh_bind_addr,
            registry_refresh: std::time::Duration::from_secs(15),
        };

        // Demo echo opt-in: same strict pattern as PUNKS_MESH — explicit
        // `on`/`true`/`1` only, anything else (absent, `off`, typos) is off.
        let mesh_demo_echo = std::env::var("PUNKS_MESH_DEMO_ECHO")
            .map(|v| v.eq_ignore_ascii_case("on") || v == "true" || v == "1")
            .unwrap_or(false);

        let allow_nip_oa_auth = std::env::var("PUNKS_ALLOW_NIP_OA_AUTH")
            .map(|v| v == "true" || v == "1")
            .unwrap_or(false);

        let klipy = std::env::var("PUNKS_KLIPY_API_KEY")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
            .map(|api_key| KlipyConfig { api_key });

        // Note: intentionally not prefixed with PUNKS_ — this is a relay-identity
        ...
        // Note: intentionally not prefixed with PUNKS_ — same relay-identity
        ...
        let auth = punks_auth::AuthConfig {
            rate_limits: rate_limit_config_from_env()?,
        };

        if !require_auth_token {
            warn!(
                "PUNKS_REQUIRE_AUTH_TOKEN is false — REST API requests bypass token auth. \
                 WebSocket protocol auth is unaffected. Set to true for production."
            );
        }

        let cors_origins = std::env::var("PUNKS_CORS_ORIGINS")
            .unwrap_or_default()
            .split(',')
        ...
        let relay_private_key = std::env::var("PUNKS_RELAY_PRIVATE_KEY").ok();

        let uds_path = std::env::var("PUNKS_UDS_PATH")
            .ok()
            .map(|s| s.trim().to_string())
            .filter(|s| !s.is_empty());

        let health_port = std::env::var("PUNKS_HEALTH_PORT")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(8080);

        let metrics_port = std::env::var("PUNKS_METRICS_PORT")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(9102);

        let s3_addressing_style = match std::env::var("PUNKS_S3_ADDRESSING_STYLE") {
            Ok(value) => value.parse().map_err(ConfigError::InvalidValue)?,
            Err(std::env::VarError::NotPresent) => punks_media::config::S3AddressingStyle::default(),
            Err(std::env::VarError::NotUnicode(_)) => {
                return Err(ConfigError::InvalidValue(
                    "PUNKS_S3_ADDRESSING_STYLE must be valid Unicode and one of 'path' or 'virtual'"
                        .to_string(),
                ));
            }
        };
        let media = punks_media::MediaConfig {
            ...
            s3_region: std::env::var("PUNKS_S3_REGION")
                .or_else(|_| std::env::var("AWS_REGION"))
                .unwrap_or_else(|_| "us-east-1".to_string()),
            s3_addressing_style,
            max_image_bytes: std::env::var("PUNKS_MAX_IMAGE_BYTES")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(50 * 1024 * 1024),
            max_gif_bytes: std::env::var("PUNKS_MAX_GIF_BYTES")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(10 * 1024 * 1024),
            max_video_bytes: std::env::var("PUNKS_MAX_VIDEO_BYTES")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(500 * 1024 * 1024),
            max_file_bytes: std::env::var("PUNKS_MAX_FILE_BYTES")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(100 * 1024 * 1024),
            public_base_url: std::env::var("PUNKS_MEDIA_BASE_URL")
            ...
            upload_records_enabled: std::env::var("PUNKS_MEDIA_UPLOAD_RECORDS")
                .map(|v| v == "true" || v == "1")
                .unwrap_or(false),
            upload_ip_header: std::env::var("PUNKS_MEDIA_UPLOAD_IP_HEADER")
                .ok()
                .map(|s| s.trim().to_lowercase())
                .filter(|s| !s.is_empty()),
            upload_port_header: std::env::var("PUNKS_MEDIA_UPLOAD_PORT_HEADER")
                .ok()
                .map(|s| s.trim().to_lowercase())
                .filter(|s| !s.is_empty()),
        };
        let media_max_concurrent_uploads: usize =
            std::env::var("PUNKS_MEDIA_MAX_CONCURRENT_UPLOADS")
                .ok()
                .and_then(|v| v.parse().ok())
                .filter(|&v| v > 0)
                .unwrap_or(8);
        let media_max_concurrent_uploads_per_pubkey: u32 =
            std::env::var("PUNKS_MEDIA_MAX_CONCURRENT_UPLOADS_PER_PUBKEY")
                .ok()
                .and_then(|v| v.parse().ok())
                .filter(|&v| v > 0)
                .unwrap_or(2)
                .min(u32::try_from(media_max_concurrent_uploads).unwrap_or(u32::MAX));
        let media_uploads_per_minute: u32 = std::env::var("PUNKS_MEDIA_UPLOADS_PER_MINUTE")
            .ok()
            .and_then(|v| v.parse().ok())
            .filter(|&v| v > 0)
            .unwrap_or(30);
        ...
        let ephemeral_ttl_override = std::env::var("PUNKS_EPHEMERAL_TTL_OVERRIDE")
            .ok()
        ...
        if let Some(ttl) = ephemeral_ttl_override {
            warn!(
                "PUNKS_EPHEMERAL_TTL_OVERRIDE={ttl}s — all ephemeral channels will use \
        ...
        let git_repo_path = ensure_git_repo_path(
            std::env::var("PUNKS_GIT_REPO_PATH").unwrap_or_else(|_| "./repos".to_string()),
        )?;
        let git_pack_cache_path = ensure_git_path(
            "PUNKS_GIT_PACK_CACHE_PATH",
            std::env::var("PUNKS_GIT_PACK_CACHE_PATH")
                .map(std::path::PathBuf::from)
                .unwrap_or_else(|_| git_repo_path.join(".pack-cache")),
        )?;
        let git_max_pack_bytes: u64 = std::env::var("PUNKS_GIT_MAX_PACK_BYTES")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(500 * 1024 * 1024); // 500 MB
        let git_max_repo_bytes: u64 = std::env::var("PUNKS_GIT_MAX_REPO_BYTES")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or_else(|| git_max_pack_bytes.saturating_mul(2)); // 1 GB at defaults
        let git_pack_cache_max_bytes: u64 = std::env::var("PUNKS_GIT_PACK_CACHE_MAX_BYTES")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or_else(|| git_max_repo_bytes.saturating_mul(5)); // 5 GB at defaults
        let git_pack_cache_max_concurrent_populations: usize =
            std::env::var("PUNKS_GIT_PACK_CACHE_MAX_CONCURRENT_POPULATIONS")
                .ok()
                .and_then(|v| v.parse().ok())
                .filter(|value| *value > 0)
                .unwrap_or(2);
        let git_max_repos_per_pubkey: u32 = std::env::var("PUNKS_GIT_MAX_REPOS_PER_PUBKEY")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(100);
        let git_max_concurrent_ops: usize = std::env::var("PUNKS_GIT_MAX_CONCURRENT_OPS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(20);
        let git_hook_hmac_secret: String = std::env::var("PUNKS_GIT_HOOK_HMAC_SECRET")
            .unwrap_or_else(|_| {
        ...
        let push_executor_key_id =
            std::env::var("PUNKS_PUSH_EXECUTOR_KEY_ID").unwrap_or_else(|_| "relay-v1".to_string());
        if push_executor_key_id.is_empty() || push_executor_key_id.len() > 64 {
            return Err(ConfigError::InvalidValue(
                "PUNKS_PUSH_EXECUTOR_KEY_ID must contain 1..=64 bytes".to_string(),
            ));
        }
        let push_gateway_delivery_url = match std::env::var("PUNKS_PUSH_GATEWAY_DELIVERY_URL") {
        ...
        let push_gateway_timeout_millis = match std::env::var("PUNKS_PUSH_GATEWAY_TIMEOUT_MS") {
            Ok(raw) => raw
                .parse::<u64>()
                .ok()
                ...
                .ok_or_else(|| {
                    ConfigError::InvalidValue(
                        "PUNKS_PUSH_GATEWAY_TIMEOUT_MS must be an integer in 100..=10000"
                            .to_string(),
                    )
                })?,
            Err(_) => 2_000,
        ...
        let terms_markdown = read_policy_markdown("PUNKS_TERMS_OF_SERVICE_MARKDOWN")?;
        let privacy_markdown = read_policy_markdown("PUNKS_PRIVACY_POLICY_MARKDOWN")?;
        let age_attestation_required = parse_optional_bool("PUNKS_AGE_ATTESTATION_REQUIRED")?;
        let audit_enabled = parse_bool("PUNKS_AUDIT_ENABLED", true)?;
        ...
        let admin = match std::env::var("PUNKS_ADMIN_HOST")
            .ok()
            ...
            .filter(|value| !value.is_empty())
        {
            ...
            Some(host) => {
                if host.contains(['/', '\\', '@']) {
                    return Err(ConfigError::InvalidValue(
                        "PUNKS_ADMIN_HOST must be an exact authority".to_string(),
                    ));
                }
                let web_dir = std::env::var("PUNKS_ADMIN_WEB_DIR")
                    .ok()
                ...
                if let Some(ref dir) = web_dir {
                    if !dir.join("index.html").is_file() {
                        return Err(ConfigError::InvalidValue(format!(
                            "PUNKS_ADMIN_WEB_DIR={} does not contain index.html",
                            dir.display()
                        )));
                    }
                }
        ...
        let web_dir = std::env::var("PUNKS_WEB_DIR")
            .ok()
            .map(|s| s.trim().to_string())
            .filter(|s| !s.is_empty())
            .map(std::path::PathBuf::from);
        let serve_git_web_gui = std::env::var("PUNKS_SERVE_GIT_WEB_GUI")
        ...
        if let Some(ref dir) = web_dir {
            if !dir.join("index.html").is_file() {
                return Err(ConfigError::InvalidValue(format!(
                    "PUNKS_WEB_DIR={} does not contain index.html",
                    dir.display()
                )));
            }
            tracing::info!("PUNKS_WEB_DIR={} — serving web UI from relay", dir.display());
        }
        ...
        // only fires when someone sets PUNKS_GIT_HOOK_HMAC_SECRET to a weak value.
        if std::env::var("PUNKS_GIT_HOOK_HMAC_SECRET").is_ok() && git_hook_hmac_secret.len() < 32 {
            return Err(ConfigError::InvalidValue(
                "PUNKS_GIT_HOOK_HMAC_SECRET must be at least 32 characters (16 bytes hex)"
                    .to_string(),
            ));
        }
...
mod tests {
    ...
    fn inert_media_read_auth_vars_are_reported_even_when_false() {
        let found = inert_env_vars(
            ...
            env_of(&[("PUNKS_REQUIRE_MEDIA_GET_AUTH", "false")]),
        );

        assert_eq!(found, vec!["PUNKS_REQUIRE_MEDIA_GET_AUTH"]);
    }

    /// `PUNKS_REQUIRE_MEDIA_READ_AUTH` was advertised in `.env.example` as an
    ...
    fn inert_media_read_auth_vars_include_the_documented_alias() {
        let found = inert_env_vars(
            &INERT_MEDIA_READ_AUTH_VARS,
            env_of(&[
                ("PUNKS_REQUIRE_MEDIA_GET_AUTH", "true"),
                ("PUNKS_REQUIRE_MEDIA_READ_AUTH", "false"),
            ]),
        );
    ...
    fn inert_media_read_auth_vars_stay_quiet_when_unset() {
        let found = inert_env_vars(
            &INERT_MEDIA_READ_AUTH_VARS,
            env_of(&[("PUNKS_REQUIRE_RELAY_MEMBERSHIP", "true")]),
        );
    ...
    fn defaults_are_valid() {
        ...
        assert_eq!(
            ...
            punks_media::config::S3AddressingStyle::Path,
    ...
    fn s3_addressing_style_env_accepts_virtual_and_rejects_invalid_values() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("PUNKS_S3_ADDRESSING_STYLE");

        std::env::set_var("PUNKS_S3_ADDRESSING_STYLE", "virtual");
        let configured = Config::from_env()
            .expect("virtual style config")
            .media
            .s3_addressing_style;

        std::env::set_var("PUNKS_S3_ADDRESSING_STYLE", "auto");
        let invalid = Config::from_env();

        if let Some(value) = previous {
            std::env::set_var("PUNKS_S3_ADDRESSING_STYLE", value);
        } else {
            std::env::remove_var("PUNKS_S3_ADDRESSING_STYLE");
        }

        assert_eq!(configured, punks_media::config::S3AddressingStyle::Virtual);
        assert!(matches!(
            invalid,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("PUNKS_S3_ADDRESSING_STYLE must be 'path' or 'virtual'")
        ));
    }
    ...
    fn redis_pool_size_env_override_and_invalid_fallback() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("PUNKS_REDIS_POOL_SIZE");

        std::env::set_var("PUNKS_REDIS_POOL_SIZE", "32");
        let overridden = Config::from_env().expect("config").redis_pool_size;

        std::env::set_var("PUNKS_REDIS_POOL_SIZE", "0");
        let zero = Config::from_env().expect("config").redis_pool_size;

        std::env::set_var("PUNKS_REDIS_POOL_SIZE", "not-a-number");
        let junk = Config::from_env().expect("config").redis_pool_size;

        if let Some(value) = previous {
            std::env::set_var("PUNKS_REDIS_POOL_SIZE", value);
        } else {
            std::env::remove_var("PUNKS_REDIS_POOL_SIZE");
        }
    ...
    fn db_pool_size_env_override_and_invalid_fallback() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("PUNKS_DB_POOL_SIZE");

        std::env::set_var("PUNKS_DB_POOL_SIZE", "80");
        let overridden = Config::from_env().expect("config").db_pool_size;

        std::env::set_var("PUNKS_DB_POOL_SIZE", "0");
        let zero = Config::from_env().expect("config").db_pool_size;

        std::env::set_var("PUNKS_DB_POOL_SIZE", "not-a-number");
        let junk = Config::from_env().expect("config").db_pool_size;

        if let Some(value) = previous {
            std::env::set_var("PUNKS_DB_POOL_SIZE", value);
        } else {
            std::env::remove_var("PUNKS_DB_POOL_SIZE");
        }
    ...
    fn db_read_pool_size_env_override_and_invalid_fallback() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("PUNKS_DB_READ_POOL_SIZE");

        std::env::remove_var("PUNKS_DB_READ_POOL_SIZE");
        let unset = Config::from_env().expect("config").db_read_pool_size;

        std::env::set_var("PUNKS_DB_READ_POOL_SIZE", "40");
        let overridden = Config::from_env().expect("config").db_read_pool_size;

        std::env::set_var("PUNKS_DB_READ_POOL_SIZE", "0");
        let zero = Config::from_env().expect("config").db_read_pool_size;

        std::env::set_var("PUNKS_DB_READ_POOL_SIZE", "not-a-number");
        let junk = Config::from_env().expect("config").db_read_pool_size;

        if let Some(value) = previous {
            std::env::set_var("PUNKS_DB_READ_POOL_SIZE", value);
        } else {
            std::env::remove_var("PUNKS_DB_READ_POOL_SIZE");
        }
    ...
    fn read_database_url_unset_or_blank_is_none() {
        ...
        std::env::set_var("READ_DATABASE_URL", "postgres://punks:pw@replica:5432/punks"); // sadscan:disable np.postgres.1
        ...
        assert_eq!(
            set.as_deref(),
            Some("postgres://punks:pw@replica:5432/punks") // sadscan:disable np.postgres.1
        );
    }
    ...
    fn replica_read_max_age_defaults_off_and_rejects_junk() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("PUNKS_REPLICA_READ_MAX_AGE_MS");
        let previous_old = std::env::var_os("PUNKS_REPLICA_HEAD_MAX_AGE_SECS");
        std::env::remove_var("PUNKS_REPLICA_HEAD_MAX_AGE_SECS");

        std::env::remove_var("PUNKS_REPLICA_READ_MAX_AGE_MS");
        let unset = Config::from_env().expect("config").replica_read_max_age_ms;

        std::env::set_var("PUNKS_REPLICA_READ_MAX_AGE_MS", "1000");
        let set = Config::from_env().expect("config").replica_read_max_age_ms;

        std::env::set_var("PUNKS_REPLICA_READ_MAX_AGE_MS", "0");
        let zero = Config::from_env().expect("config").replica_read_max_age_ms;

        std::env::set_var("PUNKS_REPLICA_READ_MAX_AGE_MS", "soon");
        ...
        std::env::set_var("PUNKS_REPLICA_READ_MAX_AGE_MS", "1000");
        std::env::set_var("PUNKS_REPLICA_HEAD_MAX_AGE_SECS", "5");
        let old_name = Config::from_env();

        std::env::remove_var("PUNKS_REPLICA_HEAD_MAX_AGE_SECS");
        if let Some(value) = previous {
            std::env::set_var("PUNKS_REPLICA_READ_MAX_AGE_MS", value);
        } else {
            std::env::remove_var("PUNKS_REPLICA_READ_MAX_AGE_MS");
        }
        if let Some(value) = previous_old {
            std::env::set_var("PUNKS_REPLICA_HEAD_MAX_AGE_SECS", value);
        }
        ...
        match old_name {
            Err(ConfigError::InvalidValue(message)) => assert!(
                message.contains("PUNKS_REPLICA_READ_MAX_AGE_MS"),
    ...
    fn drain_jitter_defaults_off_and_rejects_junk() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("PUNKS_DRAIN_JITTER_MS");

        std::env::remove_var("PUNKS_DRAIN_JITTER_MS");
        let unset = Config::from_env().expect("config").drain_jitter_ms;

        std::env::set_var("PUNKS_DRAIN_JITTER_MS", "20000");
        let set = Config::from_env().expect("config").drain_jitter_ms;

        std::env::set_var("PUNKS_DRAIN_JITTER_MS", "60000");
        let capped = Config::from_env().expect("config").drain_jitter_ms;

        std::env::set_var("PUNKS_DRAIN_JITTER_MS", "0");
        let zero = Config::from_env().expect("config").drain_jitter_ms;

        std::env::set_var("PUNKS_DRAIN_JITTER_MS", "soon");
        let junk = Config::from_env();

        std::env::set_var("PUNKS_DRAIN_JITTER_MS", "");
        ...
        if let Some(value) = previous {
            std::env::set_var("PUNKS_DRAIN_JITTER_MS", value);
        } else {
            std::env::remove_var("PUNKS_DRAIN_JITTER_MS");
        }
    ...
    fn audit_logging_defaults_on_and_accepts_explicit_off() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("PUNKS_AUDIT_ENABLED");
        std::env::remove_var("PUNKS_AUDIT_ENABLED");
        assert!(parse_bool("PUNKS_AUDIT_ENABLED", true).unwrap());
        std::env::set_var("PUNKS_AUDIT_ENABLED", "false");
        assert!(!parse_bool("PUNKS_AUDIT_ENABLED", true).unwrap());
        if let Some(value) = previous {
            std::env::set_var("PUNKS_AUDIT_ENABLED", value);
        } else {
            std::env::remove_var("PUNKS_AUDIT_ENABLED");
        }
    }
    ...
    fn audit_logging_rejects_invalid_boolean() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("PUNKS_AUDIT_ENABLED");
        std::env::set_var("PUNKS_AUDIT_ENABLED", "sometimes");
        let result = parse_bool("PUNKS_AUDIT_ENABLED", true);
        if let Some(value) = previous {
            std::env::set_var("PUNKS_AUDIT_ENABLED", value);
        } else {
            std::env::remove_var("PUNKS_AUDIT_ENABLED");
        ...
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("PUNKS_AUDIT_ENABLED")
        ));
    }
    ...
    fn join_policy_age_attestation_rejects_invalid_boolean() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("PUNKS_AGE_ATTESTATION_REQUIRED");
        std::env::set_var("PUNKS_AGE_ATTESTATION_REQUIRED", "sometimes");
        let result = parse_optional_bool("PUNKS_AGE_ATTESTATION_REQUIRED");
        if let Some(value) = previous {
            std::env::set_var("PUNKS_AGE_ATTESTATION_REQUIRED", value);
        } else {
            std::env::remove_var("PUNKS_AGE_ATTESTATION_REQUIRED");
        }
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("PUNKS_AGE_ATTESTATION_REQUIRED")
        ));
    }
    ...
    fn rate_limits_can_be_overridden() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("PUNKS_RATE_LIMIT_HUMAN_MESSAGES_PER_MIN", "1001");
        std::env::set_var("PUNKS_RATE_LIMIT_GIF_SEARCHES_PER_MIN", "1004");
        std::env::set_var("PUNKS_RATE_LIMIT_HUMAN_API_CALLS_PER_MIN", "1002");
        std::env::set_var("PUNKS_RATE_LIMIT_HUMAN_WS_EVENTS_PER_SEC", "1003");

        let config = Config::from_env().expect("config");

        std::env::remove_var("PUNKS_RATE_LIMIT_HUMAN_MESSAGES_PER_MIN");
        std::env::remove_var("PUNKS_RATE_LIMIT_GIF_SEARCHES_PER_MIN");
        std::env::remove_var("PUNKS_RATE_LIMIT_HUMAN_API_CALLS_PER_MIN");
        std::env::remove_var("PUNKS_RATE_LIMIT_HUMAN_WS_EVENTS_PER_SEC");
    ...
    fn rate_limit_overrides_reject_zero() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("PUNKS_RATE_LIMIT_HUMAN_WS_EVENTS_PER_SEC", "0");
        ...
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("PUNKS_RATE_LIMIT_HUMAN_WS_EVENTS_PER_SEC")
        ));
    }
    ...
    fn relay_operator_pubkeys_parse_dedupe_and_normalize() {
        ...
        std::env::set_var(
            "RELAY_OPERATOR_API_ORIGIN",
            "http://punks.mesh.bb-production.com",
        );
        let config = Config::from_env().expect("config");
    ...
    fn relay_operator_api_origin_rejects_paths() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("RELAY_OPERATOR_API_ORIGIN", "https://punks.example/operator");
        let result = Config::from_env();
        std::env::remove_var("RELAY_OPERATOR_API_ORIGIN");
    ...
    #[test]
    fn push_gateway_defaults_to_punks_and_can_be_disabled() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("PUNKS_PUSH_GATEWAY_DELIVERY_URL");
        std::env::remove_var("PUNKS_PUSH_GATEWAY_DELIVERY_URL");
        let config = Config::from_env().expect("default config");
        assert_eq!(
            config
                .push_gateway_delivery_url
    ...
    fn invalid_push_gateway_timeout_is_not_silently_defaulted() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("PUNKS_PUSH_GATEWAY_TIMEOUT_MS", "99");
        let result = Config::from_env();
        std::env::remove_var("PUNKS_PUSH_GATEWAY_TIMEOUT_MS");
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("PUNKS_PUSH_GATEWAY_TIMEOUT_MS")
        ));
    }
    ...
    fn invalid_push_executor_key_id_is_rejected() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("PUNKS_PUSH_EXECUTOR_KEY_ID", "");
        let result = Config::from_env();
        std::env::remove_var("PUNKS_PUSH_EXECUTOR_KEY_ID");
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("PUNKS_PUSH_EXECUTOR_KEY_ID")
        ));
    }
    ...
    fn huddle_audio_available_can_be_disabled_for_horizontal_scaling() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("PUNKS_HUDDLE_AUDIO_AVAILABLE", "false");
        let config = Config::from_env().expect("config");
        std::env::remove_var("PUNKS_HUDDLE_AUDIO_AVAILABLE");
        assert!(
            !config.huddle_audio_available,
            "PUNKS_HUDDLE_AUDIO_AVAILABLE=false must disable huddle audio (multi-pod deployments)"
        );
    }
    ...
    fn pairing_relay_url_accepts_websocket_urls_and_rejects_http() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("PUNKS_PAIRING_RELAY_URL", "wss://pairing.punks.xyz");
        let config = Config::from_env().expect("config");
        assert_eq!(
            config.pairing_relay_url.as_deref(),
            Some("wss://pairing.punks.xyz")
        );

        std::env::set_var("PUNKS_PAIRING_RELAY_URL", "https://pairing.punks.xyz");
        let result = Config::from_env();
        std::env::remove_var("PUNKS_PAIRING_RELAY_URL");
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref msg)) if msg.contains("PUNKS_PAIRING_RELAY_URL")
        ));
    }
    ...
    fn max_frame_bytes_can_be_configured() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("PUNKS_MAX_FRAME_BYTES", "262144");
        let config = Config::from_env().expect("config");
        std::env::remove_var("PUNKS_MAX_FRAME_BYTES");
    ...
    fn git_repo_path_is_created_if_missing() {
        ...
        std::env::set_var("PUNKS_GIT_REPO_PATH", &nested);
        let result = Config::from_env();
        std::env::remove_var("PUNKS_GIT_REPO_PATH");
    ...
    fn git_repo_path_unwritable_returns_error() {
        ...
        assert!(
            matches!(result, Err(ConfigError::InvalidValue(ref msg)) if msg.contains("PUNKS_GIT_REPO_PATH")),
            "expected InvalidValue mentioning PUNKS_GIT_REPO_PATH, got {result:?}"
        );
    }
}
