...
fn relay_server_tag(relay_url: &str) -> Option<String> {
    let authority = punks_core::tenant::relay_url_authority(relay_url);
...
pub struct PunksClient {
    http: reqwest::Client,
    relay_url: String, // base URL, no trailing slash, e.g. "https://relay.punks.place"
    keys: Keys,
...
impl PunksClient {
    ...
    /// `punks_ws_client::publish_event` which handles connect, NIP-42 auth,
    ...
    pub async fn publish_ephemeral_event(&self, event: nostr::Event) -> Result<String, CliError> {
        ...
        // See punks_ws_client::{AUTH_CHALLENGE_TIMEOUT_SECS, AUTH_OK_TIMEOUT_SECS,
        // PUBLISH_OK_TIMEOUT_SECS} for the inner ceilings.
        let ok =
            punks_ws_client::publish_event(&ws_url, event, &self.keys, self.auth_tag.as_ref(), 75)
                .await
                .map_err(|e| CliError::Other(e.to_string()))?;
