...
///
/// ```ignore
/// let code = punks_cli::run_from_args(std::env::args()).await;
...
pub enum ProjectsCmd {
    ...
    Create {
        ...
        slug: String,
        /// Member repository coordinate: bare Punks repo id (e.g. `punks`) or full
...
async fn run(cli: Cli) -> Result<(), CliError> {
    ...
    let (auth_tag, auth_tag_json) = match cli.auth_tag {
        Some(ref input) if !input.is_empty() => {
            ...
            let tag = punks_sdk::nip_oa::parse_auth_tag(&json)
                .map_err(|e| CliError::Auth(format!("PUNKS_AUTH_TAG is malformed: {e}")))?;
            punks_sdk::nip_oa::verify_auth_tag(&json, &keys.public_key()).map_err(|e| {
...
mod tests {
    ...
    fn messages_thread_accepts_link_or_explicit_identifiers() {
        ...
        assert!(
            Cli::try_parse_from(["punks", "messages", "thread", "--link", link.as_str(),]).is_ok()
        );
    ...
    fn messages_thread_rejects_partial_or_mixed_targets() {
        ...
        assert!(Cli::try_parse_from(["punks", "messages", "thread"]).is_err());
        assert!(Cli::try_parse_from(["punks", "messages", "thread", "--channel", channel]).is_err());
        assert!(Cli::try_parse_from([
            "punks",
            "messages",
            "thread",
    ...
    fn set_status_clear_rejects_text_and_emoji() {
        for extra in [["--text", "busy"], ["--emoji", "🎶"]] {
            let args = ["punks", "users", "set-status", "--clear"]
                .into_iter()
                .chain(extra);
            assert!(
    ...
    fn set_status_requires_text_or_clear() {
        assert!(Cli::try_parse_from(["punks", "users", "set-status"]).is_err());
        assert!(
            Cli::try_parse_from(["punks", "users", "set-status", "--emoji", "🎶"]).is_err(),
            "--emoji alone must not imply a status"
        );
        assert!(Cli::try_parse_from(["punks", "users", "set-status", "--clear"]).is_ok());
    }
    ...
    fn projects_update_no_mutation_is_rejected_by_clap() {
        ...
        assert!(
            Cli::try_parse_from(["punks", "projects", "update", "my-slug"]).is_err(),
