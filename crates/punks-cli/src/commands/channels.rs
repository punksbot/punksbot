...
use punks_core::kind::{KIND_MANAGED_AGENT, KIND_TEAM};
...
pub async fn cmd_create_channel(
    ...
    let vis = match visibility {
        "open" => punks_sdk::Visibility::Open,
        "private" => punks_sdk::Visibility::Private,
        _ => unreachable!(),
    };
    let ct = match channel_type {
        "stream" => punks_sdk::ChannelKind::Stream,
        "forum" => punks_sdk::ChannelKind::Forum,
        _ => unreachable!(),
    };
    let builder =
        punks_sdk::build_create_channel(channel_uuid, name, Some(vis), Some(ct), description, ttl)
...
pub async fn cmd_create_channel_from_template(
    ...
    let builder = punks_sdk::build_create_channel(
        channel_uuid,
        name,
        Some(vis),
        Some(ct),
    ...
    if let Some(canvas_template) = template.canvas_template.as_deref() {
        ...
        let canvas_result: Result<(), CliError> = async {
            let builder = punks_sdk::build_set_canvas(channel_uuid, &content)
    ...
    for agent in &resolved.agents {
        let outcome: Result<(), CliError> = async {
            let builder = punks_sdk::build_add_member(
                channel_uuid,
                &agent.pubkey,
                Some(punks_sdk::MemberRole::Bot),
            )
...
pub async fn cmd_update_channel(
    ...
    let builder =
        punks_sdk::build_update_channel(channel_uuid, name, description, visibility, ttl_change)
...
pub async fn cmd_set_channel_topic(
    ...
    let builder = punks_sdk::build_set_topic(channel_uuid, topic)
...
pub async fn cmd_set_channel_purpose(
    ...
    let builder = punks_sdk::build_set_purpose(channel_uuid, purpose)
...
pub async fn cmd_join_channel(client: &PunksClient, channel_id: &str) -> Result<(), CliError> {
    ...
    let builder = punks_sdk::build_join(channel_uuid)
...
pub async fn cmd_leave_channel(client: &PunksClient, channel_id: &str) -> Result<(), CliError> {
    ...
    let builder = punks_sdk::build_leave(channel_uuid)
...
pub async fn cmd_archive_channel(client: &PunksClient, channel_id: &str) -> Result<(), CliError> {
    ...
    let builder = punks_sdk::build_archive(channel_uuid)
...
pub async fn cmd_unarchive_channel(client: &PunksClient, channel_id: &str) -> Result<(), CliError> {
    ...
    let builder = punks_sdk::build_unarchive(channel_uuid)
...
pub async fn cmd_delete_channel(client: &PunksClient, channel_id: &str) -> Result<(), CliError> {
    ...
    let builder = punks_sdk::build_delete_channel(channel_uuid)
...
pub async fn cmd_add_channel_member(
    ...
    let typed_role = match role {
        None => None,
        Some("owner") => Some(punks_sdk::MemberRole::Owner),
        Some("admin") => Some(punks_sdk::MemberRole::Admin),
        Some("member") => Some(punks_sdk::MemberRole::Member),
        Some("guest") => Some(punks_sdk::MemberRole::Guest),
        Some("bot") => Some(punks_sdk::MemberRole::Bot),
        Some(other) => {
            return Err(CliError::Usage(format!(
    ...
    let builder = punks_sdk::build_add_member(channel_uuid, pubkey, typed_role)
        .map_err(|e| CliError::Other(format!("build_add_member failed: {e}")))?;
...
pub async fn cmd_remove_channel_member(
    ...
    let builder = punks_sdk::build_remove_member(channel_uuid, pubkey)
...
pub async fn cmd_set_add_policy(client: &PunksClient, policy: &str) -> Result<(), CliError> {
    ...
    let builder = EventBuilder::new(
        Kind::Custom(punks_sdk::kind::KIND_AGENT_PROFILE as u16),
        &content,
    )
    .tags([]);
    let event = client.sign_event(builder)?;
...
mod tests {
    ...
    fn from_event_extracts_known_tags() {
        ...
        assert_eq!(s.name, "punks-chat-composer");
