use punks_core::kind::KIND_MANAGED_AGENT;
use nostr::PublicKey;
...
// TODO(phase-4): Replace raw nostr::EventBuilder usage in cmd_set_presence with punks-sdk builder
...
fn owner_verification(event: &serde_json::Value, expected_owner: &str) -> &'static str {
    ...
    match punks_sdk::nip_oa::verify_auth_tag(&auth_tag_json, &agent_pubkey) {
...
pub async fn cmd_set_profile(
    ...
    let builder = punks_sdk::build_profile(
...
pub async fn cmd_set_presence(client: &PunksClient, status: &str) -> Result<(), CliError> {
    let builder = punks_sdk::build_presence_update(status).map_err(crate::validate::sdk_err)?;
    let event = client.sign_event(builder)?;
...
pub async fn cmd_set_status(
    ...
    let builder = punks_sdk::build_user_status(text, emoji).map_err(crate::validate::sdk_err)?;
    let event = client.sign_event(builder)?;
    let resp = client.submit_event(event).await?;
    println!("{}", normalize_write_response(&resp));
    Ok(())
...
mod tests {
    ...
    fn owner_verification_requires_one_valid_auth_tag_for_requested_owner() {
        ...
        let valid_tag: serde_json::Value = serde_json::from_str(
            &punks_sdk::nip_oa::compute_auth_tag(&owner_keys, &agent_keys.public_key(), "kind=0")
                .unwrap(),
        )
        .unwrap();
        let foreign_tag: serde_json::Value = serde_json::from_str(
            &punks_sdk::nip_oa::compute_auth_tag(
                &foreign_owner_keys,
