//! `punks notes` — NIP-23 long-form editable notes (kind:30023).
//!
...
pub async fn fetch_own_note(client: &PunksClient, slug: &str) -> Result<Option<Event>, CliError> {
...
pub async fn fetch_by_slug(client: &PunksClient, slug: &str) -> Result<Vec<Event>, CliError> {
    let filter = serde_json::json!({
        "kinds": [KIND_LONG_FORM],
        "#d": [slug],
        "limit": 50,
...
pub async fn cmd_rm(client: &PunksClient, slug: &str) -> Result<(), CliError> {
...
pub async fn dispatch(cmd: crate::NotesCmd, client: &PunksClient) -> Result<(), CliError> {
    use crate::NotesCmd;
    match cmd {
        NotesCmd::Set {
            name,
