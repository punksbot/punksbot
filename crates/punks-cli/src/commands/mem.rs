//! `punks mem` — agent-side engram management (NIP-AE).
//!
//! Subcommands:
//! - `punks mem ls`                   — list non-tombstoned memories
//! - `punks mem get <slug>`            — print the value to stdout
//! - `punks mem hash <slug>`           — print sha256(value) hex
//! - `punks mem set <slug> <value|-> ` — write a value (use `-` for stdin)
//! - `punks mem patch <slug>`          — apply a unified diff to the current value
//! - `punks mem rm <slug>`             — publish a tombstone
//!
//! By default, the caller's `PUNKS_PRIVATE_KEY` is the agent's nsec. The
//! agent's owner pubkey is resolved from `PUNKS_AUTH_TAG` (NIP-OA attestation)
...
fn resolve_owner(client: &PunksClient, owner_flag: Option<&str>) -> Result<PublicKey, CliError> {
    ...
    let tag = client.auth_tag_owner_hex().ok_or_else(|| {
        CliError::Usage(
            "owner pubkey required (set PUNKS_AUTH_TAG with a NIP-OA attestation or pass --owner)"
                .into(),
        )
    })?;
...
/// owner from `--owner` / PUNKS_AUTH_TAG. Owner-side recovery passes
...
async fn submit_engram(client: &PunksClient, event: nostr::Event) -> Result<(), CliError> {
...
/// `punks mem ls` — list non-tombstoned memory entries.
pub async fn cmd_ls(
    client: &PunksClient,
    owner_flag: Option<&str>,
    agent_flag: Option<&str>,
...
/// `punks mem get <slug>` — print value (memory) or profile (core) to stdout.
///
...
pub async fn cmd_get(
    ...
    match body {
        ...
        Some(Body::Memory { value: Some(v), .. }) => {
            // Raw stdout, no trailing newline — round-trips with `punks mem set foo -`.
            std::io::stdout()
...
/// `punks mem set <slug> <value|->` — write a value or core profile.
///
        ...
        if buf.is_empty() && !allow_empty {
            return Err(CliError::Usage(
                "refusing to write empty value from stdin (an upstream pipeline step likely \
                 failed). Pass --allow-empty to confirm, or use `punks mem rm <slug>` to \
                 tombstone."
                    .into(),
            ));
        }
...
/// `punks mem hash <slug>` — print sha256(value) in hex to stdout.
///
...
/// then pass it to `punks mem patch --base-hash <hex>` to make the edit
...
pub async fn cmd_patch(
    ...
    match (base_hash, no_base_hash) {
        ...
        (None, false) => {
            return Err(CliError::Usage(
                "missing --base-hash <hex> (run `punks mem hash <slug>` to get it). \
    ...
    if new_value.is_empty() && !allow_empty {
        return Err(CliError::Usage(
            "refusing to write empty value (patch result is empty). \
             Pass --allow-empty to confirm, or use `punks mem rm <slug>` to tombstone."
                .into(),
        ));
    }
...
/// `punks mem rm <slug>` — publish a tombstone (`value: null`).
///
...
pub async fn cmd_rm(
    ...
    if slug == engram::CORE_SLUG {
        return Err(CliError::Usage(
            "core cannot be tombstoned; overwrite it with `punks mem set core ''` instead".into(),
        ));
    }
    let owner = resolve_owner(client, owner_flag)?;
...
pub async fn dispatch(cmd: crate::MemCmd, client: &PunksClient) -> Result<(), CliError> {
    use crate::MemCmd;
    match cmd {
...
mod tests {
    ...
    fn test_client(keys: nostr::Keys) -> PunksClient {
        PunksClient::new("http://127.0.0.1:9".into(), keys, None, None).unwrap()
    }
