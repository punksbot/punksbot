...
use punks_core::kind::KIND_PROJECT;
use punks_sdk::{
...
#[cfg(test)]
mod tests {
    use punks_sdk::{validate_project_envelope, PROJECT_MEMBER_CAP};
    use nostr::Tag;
    ...
    fn expand_repo_coord_rejects_uppercase_owner() {
        let upper = "30617:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA:punks";
    ...
    fn rebuild_project_preserves_hinted_member_tags() {
        ...
        let coord = format!("30617:{OWNER_HEX}:punks");
    ...
    fn project_head_at(created_at: u64) -> Event {
        ...
        let tags = vec![
            make_test_tag(&["d", "platform"]),
            make_test_tag(&["a", &format!("30617:{OWNER_HEX}:punks")]),
        ];
