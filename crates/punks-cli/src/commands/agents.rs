use punks_core::kind::KIND_IA_ARCHIVED_LIST;
use punks_sdk::builders::{build_archive_identity_request, build_unarchive_identity_request};
use nostr::PublicKey;
use serde_json::json;
...
mod tests {
    ...
    fn archive_admin_flag_is_parsed() {
        use crate::Cli;
        use clap::Parser;
        let cli = Cli::try_parse_from(["punks", "agents", "archive", &hex64('a'), "--admin"])
    ...
    fn unarchive_admin_flag_is_parsed() {
        use crate::Cli;
        use clap::Parser;
        let cli = Cli::try_parse_from(["punks", "agents", "unarchive", &hex64('a'), "--admin"])
