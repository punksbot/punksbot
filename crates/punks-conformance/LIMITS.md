...
boundary** in `crates/punks-relay/src/handlers/{ingest,req,event}.rs`.
...
cargo test -p punks-conformance --lib

# 2. Replay fixtures (5 tests). Three JSONL traces in
#    crates/punks-conformance/tests/fixtures/ are committed for reviewer
...
#    PUNKS_CONFORMANCE_UPDATE=1 cargo test -p punks-conformance --test replay_fixtures
cargo test -p punks-conformance --test replay_fixtures

# 3. EmitGuard coverage-breach self-test (2 tests in
#    crates/punks-relay/src/conformance/mod.rs). Proves the Drop guard
#    records `ImplBug` when no emit reaches the tracer, and stays
#    silent when an emit did. The seam-name string flows through.
cargo test -p punks-relay --lib conformance::
