# Trace Schema (`punks-conformance`)
...
- **`sanitized_error { reason }`** where `reason ∈ { restricted,
  ...
  `crates/punks-relay/src/conformance/mod.rs`) goes non-exhaustive and
  CI catches it.
...
| File | What it emits |
|------|---------------|
| `crates/punks-relay/src/conformance/mod.rs` | helpers + `EmitGuard` + `sanitized_reason_for` |
| `crates/punks-relay/src/conformance/tracers.rs` | `NoopTracer` (prod default), `JsonlTracer` |
| `crates/punks-relay/src/handlers/ingest.rs` | `AuthCheck`, `WriteInsert`, `WriteInsertGlobal`, `WriteDuplicate`, outer-wrapper `SanitizedError` |
| `crates/punks-relay/src/handlers/req.rs` | **held back** — additive patch for integration onto Max's req.rs work |

## Where the checker lives

| File | What it does |
|------|--------------|
| `crates/punks-conformance/src/lib.rs` | schema + `Tracer` trait |
| `crates/punks-conformance/src/transitions.rs` | spec `Next` re-implementation |
| `crates/punks-conformance/src/checker.rs` | replay engine: `IllegalTransition` / `StateMismatch` / `NonInterference` / `CoverageBreach` |
...
`crates/punks-conformance/src/checker.rs::tests` proving the gate bites
