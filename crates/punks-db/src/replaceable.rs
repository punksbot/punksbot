...
async fn replace_parameterized_event_in_transaction_impl(
    ...
    let read_state_t_tag_count = event
        ...
        .count();
    let is_nip_rs = kind_i32 == punks_core::kind::KIND_READ_STATE as i32
        && d_tag_count == 1
        && has_exact_d_tag
    ...
    let is_punks_mesh_status = kind_i32 == punks_core::kind::KIND_BOOKMARK_SET as i32
        && d_tag.starts_with("punks-mesh-member-status:")
        && event.tags.iter().any(|tag| {
            let parts = tag.as_slice();
            parts.len() == 2 && parts[0] == "k" && parts[1] == "punks-mesh-status"
        });
    let hard_delete_superseded = is_nip_rs || is_punks_mesh_status;
    ...
    if existing.is_some() {
        ...
        if is_nip_rs {
            ...
            sqlx::query("SELECT set_config('punks.nip_rs_hard_delete', $1, true)")
                .bind(previous_value)
                .execute(&mut *savepoint)
                .await?;
        }
