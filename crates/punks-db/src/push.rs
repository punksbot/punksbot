...
const PUSH_GATE_LOCK_NAMESPACE: &str = "punks_push_gate:";
...
pub struct BatchedMatch {
    ...
    pub event: punks_core::StoredEvent,
    /// Attempt number, starting at one for the first claim.
    pub attempt: i32,
}
...
where
    ...
    Fut: std::future::Future<Output = Result<Vec<punks_core::StoredEvent>>>,
{
...
mod tests {
    ...
    async fn setup_pool() -> PgPool {
        let database_url = std::env::var("PUNKS_TEST_DATABASE_URL")
            ...
            .unwrap_or_else(|_| "postgres://punks:punks_dev@localhost:5432/punks".into()); // sadscan:disable np.postgres.1 -- local test-only credentials
