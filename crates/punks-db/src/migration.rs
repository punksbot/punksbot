//! Embedded SQLx migrations for Punks.
//!
...
/// Run all pending Punks database migrations.
///
...
mod tests {
    ...
    fn embedded_migrator_contains_consolidated_initial_schema() {
        ...
        assert!(mesh_retention.contains("punks-mesh-member-status:%"));
        assert!(mesh_retention.contains("punks-mesh-status"));
        assert!(mesh_retention
            .contains("CREATE TRIGGER trg_events_purge_soft_deleted_punks_mesh_status"));
        assert!(!migrations[0]
            .sql
            .as_str()
            .contains("purge_soft_deleted_punks_mesh_status"));
        ...
        assert!(push_gate.contains("'punks_push_gate:' || NEW.community_id::text"));
        ...
        assert!(ttl_shared.contains("'punks_channel_ttl:' || NEW.community_id::text"));
        ...
        assert!(roster_fence.contains("'punks_channel_membership:'"));
    ...
    fn migration_execution_cannot_bypass_schema_destruction_lock() {
        ...
        // exemption honest.
        let push_gateway_exception = crates_dir.join("punks-push-gateway/src/postgres.rs");
        let push_gateway_migrations = crates_dir.join("punks-push-gateway/migrations");
        for entry in
            std::fs::read_dir(&push_gateway_migrations).expect("read push gateway migrations")
        {
            ...
            assert!(
                ...
                "{} defines community-scoped data; its migrator would bypass the \
                 schema/destruction lock and must move under punks-db migrations",
                path.display()
            );
        }
            ...
            if *path == this_file {
                ...
                assert_eq!(
                    ...
                    "{} embeds or runs a SQLx migrator outside the schema/destruction \
                     ...
                     punks_db migration::run_migrations",
                    path.display()
                );
            }
        }
    ...
    async fn cancelled_migration_cannot_expose_shared_lock_while_ddl_backend_lives() {
        ...
        let probe_db = format!("punks_lock_cancel_{}", uuid::Uuid::new_v4().simple());
