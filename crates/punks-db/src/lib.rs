#![deny(unsafe_code)]
#![warn(missing_docs)]
//! punks-db — Postgres event store for Punks.
//!
//! ## Design invariants
...
//! This crate intentionally keeps database runtime and Punks domain persistence
...
pub mod partition;
/// Punks product-feedback sidecar persistence.
...
use punks_datastore_tracing::datastore_span;
...
use punks_core::{CommunityId, StoredEvent};
            ...
            ReadSessionInner::Replica { tx, writer } => {
                match event::query_events_on(tx, q).await {
                    ...
                    Err(e) => {
                        ...
                        // Deliberately not a `punks_db_route_decision` event:
                        // the page's route was already recorded, and the
                        // offload metric must stay one-event-per-request.
                        metrics::counter!("punks_db_read_session_degraded").increment(1);
                        writer.clone()
                    }
                }
            }
...
enum RouteDecision {
    ...
    /// failure emits exactly one `punks_db_route_decision` event per request
...
impl RoutePredicate {
    ...
    ///
    /// `routing_enabled` is whether `PUNKS_REPLICA_READ_MAX_AGE_MS` is set
...
/// Map the configured read budget (`PUNKS_REPLICA_READ_MAX_AGE_MS`) to the
...
pub struct DbConfig {
    ...
    /// `PUNKS_DB_READ_POOL_SIZE`). `None` inherits [`Self::max_connections`].
    ...
    /// `PUNKS_REPLICA_READ_MAX_AGE_MS`). `0` disables bounded-staleness
...
impl Default for DbConfig {
    ...
    fn default() -> Self {
        Self {
            database_url: "postgres://punks:punks_dev@localhost:5432/punks".to_string(), // sadscan:disable np.postgres.1
...
impl Db {
    ...
    /// `punks.created_at_floor` GUC — this is what makes the replica fence
        ...
        let options = PgPoolOptions::new()
            ...
            .after_connect(|conn, _meta| {
                Box::pin(async move {
                    ...
                    sqlx::query("SELECT set_config('punks.created_at_floor', $1, false)")
    ...
    /// the migration decision. On a relay with `PUNKS_AUTO_MIGRATE` off, the
    ...
    > {
        ...
        .await
        {
            ...
            // `punks_db_route_decision{decision="writer",reason="reader_acquire_timeout"}`
            ...
            // Runbook: correlate with `punks_db_read_pool_active` / `_max`
            ...
            // sample (PUNKS_POOL_METRICS_INTERVAL_SECS, default 10s) while
    ...
    /// the writer's: `punks_db_read_pool_active / punks_db_read_pool_max` is
    /// the operator's utilisation signal for tuning `PUNKS_DB_READ_POOL_SIZE`,
    ...
    pub async fn insert_event_with_serving_write_guard(
        ...
        if punks_core::kind::is_ephemeral(kind_u32) {
    ...
    /// seam is gated on `PUNKS_REPLICA_READ_MAX_AGE_MS` (default off): when
    ...
    ///   `PUNKS_REPLICA_HEAD_MAX_AGE_SECS` until it has, proven by a
    ...
    pub async fn replace_addressable_event(
        ...
        let kind_i32 = punks_core::kind::event_kind_i32(event);
    ...
    pub async fn nip43_membership_snapshot_needs_reconciliation(
        ...
        let snapshot = self
            .query_events(&crate::event::EventQuery {
                kinds: Some(vec![punks_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32]),
    ...
    pub async fn publish_nip43_membership_locked(
        ...
        let kind_i32 = punks_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32;
...
mod tests {
    ...
    const TEST_DB_URL: &str = "postgres://punks:punks_dev@localhost:5432/punks";
    ...
    async fn nip_rs_replacement_hard_deletes_payload_and_watermark_rejects_replay() {
        ...
        let base = Timestamp::now().as_secs();
        let old = EventBuilder::new(Kind::Custom(punks_core::kind::KIND_READ_STATE as u16), "old")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign old");
        let new = EventBuilder::new(Kind::Custom(punks_core::kind::KIND_READ_STATE as u16), "new")
            .tags(tags)
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
            .expect("sign new");
    ...
    async fn nip_rs_transaction_operation_restores_hard_delete_opt_in() {
        ...
        let event = |d_tag: &str, content: &str, timestamp: u64| {
            EventBuilder::new(
                Kind::Custom(punks_core::kind::KIND_READ_STATE as u16),
                content,
            )
            .tags(vec![
        ...
        let leaked: Option<String> = sqlx::query_scalar(
            "SELECT NULLIF(current_setting('punks.nip_rs_hard_delete', true), '')",
        )
        .fetch_one(&mut *tx)
        .await
    ...
    async fn parameterized_replacement_in_existing_transaction_honors_revision_and_rollback() {
        ...
        let event = |content: &str, timestamp: u64| {
            EventBuilder::new(Kind::Custom(punks_core::kind::KIND_PROJECT as u16), content)
        ...
        .bind(community.as_uuid())
        .bind(punks_core::kind::KIND_PROJECT as i32)
        .bind(keys.public_key().to_bytes())
        .bind(&d_tag)
        .fetch_one(&db.pool)
        .await
        ...
        let missing = EventBuilder::new(
            Kind::Custom(punks_core::kind::KIND_PROJECT as u16),
            "missing",
        )
        .tags(vec![
    ...
    async fn parameterized_replacement_rolls_back_when_mention_indexing_fails() {
        ...
        let base = Timestamp::now().as_secs();
        let old = EventBuilder::new(Kind::Custom(punks_core::kind::KIND_PROJECT as u16), "old")
            .tags(tags())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign old project");
        let new = EventBuilder::new(Kind::Custom(punks_core::kind::KIND_PROJECT as u16), "new")
            .tags(tags())
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
    ...
    async fn mesh_status_replacement_keeps_one_physical_row() {
        ...
        let community = CommunityId::from_uuid(make_community(&db.pool).await);
        let keys = Keys::generate();
        let d_tag = "punks-mesh-member-status:owner-test";
        let tags = vec![
            Tag::parse(["d", d_tag]).expect("d tag"),
            Tag::parse(["k", "punks-mesh-status"]).expect("k tag"),
        ];
        let base = Timestamp::now().as_secs();
        for (offset, content) in [(0, "running"), (1, "running-again"), (2, "stopped")] {
            let event = EventBuilder::new(
                Kind::Custom(punks_core::kind::KIND_BOOKMARK_SET as u16),
                content,
            )
            .tags(tags.clone())
    ...
    async fn coordinate_delete_spares_head_newer_than_the_deletion() {
        ...
        let community = CommunityId::from_uuid(make_community(&db.pool).await);
        let keys = Keys::generate();
        let kind = punks_core::kind::KIND_PROJECT as i32;
    ...
    async fn nip_rs_hard_delete_fence_fails_closed_and_scopes_opt_in_to_transaction() {
        ...
        for commit in [true, false] {
            ...
            let value: String =
                sqlx::query_scalar("SELECT set_config('punks.nip_rs_hard_delete', 'on', true)")
                    .fetch_one(&mut *tx)
                    .await
    ...
    async fn database_guard_covers_legacy_writer_and_nip09_deletion() {
        ...
        let base = Timestamp::now().as_secs();
        let a = EventBuilder::new(Kind::Custom(punks_core::kind::KIND_READ_STATE as u16), "A")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign A");
        let x = EventBuilder::new(Kind::Custom(punks_core::kind::KIND_READ_STATE as u16), "X")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
            .expect("sign X");
        let b = EventBuilder::new(Kind::Custom(punks_core::kind::KIND_READ_STATE as u16), "B")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base + 2))
            .sign_with_keys(&keys)
            .expect("sign B");
        let c = EventBuilder::new(Kind::Custom(punks_core::kind::KIND_READ_STATE as u16), "C")
            .tags(tags)
        ...
        async fn legacy_insert(
            ...
            .bind(punks_core::kind::KIND_READ_STATE as i32)
    ...
    async fn test_usage_metrics_lock_has_single_owner_and_releases_on_drop() {
        ...
        // races any live punks-relay on the same database (see #3619).
        ...
        // Same key as production (`punks-relay` USAGE_METRICS_LOCK_KEY) — safe here
    ...
    /// `routing_enabled = false` (PUNKS_REPLICA_READ_MAX_AGE_MS unset)
    ...
    /// ceiling, not the writer's — `punks_db_read_pool_active / _max` is the
    ...
    /// directly, because `punks_db_route_decision` is emitted by `route_read`
    ...
    async fn routed_fallback_spends_one_acquire_budget_when_aurora_cache_is_cold() {
        ...
        let reasons: std::collections::HashMap<(String, String), u64> = snapshotter
            ...
            .into_vec()
            .into_iter()
            .filter(|(key, ..)| key.key().name() == "punks_db_route_decision")
            .map(|(key, _, _, value)| {
                let metrics_util::debugging::DebugValue::Counter(n) = value else {
                    panic!("punks_db_route_decision must be a counter");
                };
    ...
    async fn lazy_reader_pool_still_spawns_fence_probe() {
        ...
        // `punks.created_at_floor` GUC — `spawn_fence_probe` verifies the
    ...
    async fn created_at_floor_guard_aborts_old_channel_rows_at_commit() {
        ...
        let insert_raw = |ev: nostr::Event, channel_id: Option<Uuid>| {
            ...
            async move {
                ...
                sqlx::query("SELECT set_config('punks.created_at_floor', $1, true)")
    ...
    fn writer_pool_safety_hook_is_single_and_composed() {
        ...
        assert!(connect_pool.contains("punks.created_at_floor"));
    ...
    async fn armed_pool_rejects_old_channel_inserts_through_public_api() {
        ...
        let effective: String = sqlx::query_scalar("SHOW punks.created_at_floor")
            .fetch_one(&db.pool)
            .await
    ...
    /// `PUNKS_AUTO_MIGRATE` off: an armed GUC with no enforcing trigger must
    ...
    async fn fence_probe_refuses_to_start_without_verified_floor_guard() {
        ...
        // Sabotage B: trigger dropped entirely (the PUNKS_AUTO_MIGRATE=off /
