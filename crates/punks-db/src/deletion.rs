...
/// the instant it is checked. The value is arbitrary (ASCII `punksdel1`) but
...
impl DeletionStore {
    ...
    ///
    /// Probe the deployed catalog rather than SQLx's migration ledger. Punks also
    ...
    pub async fn purge_postgres(&self, token: &LeaseToken) -> Result<BTreeMap<String, u64>> {
        ...
        sqlx::query("SELECT set_config('punks.nip_rs_hard_delete', 'on', true)")
            .execute(&mut *tx)
            .await?;
    ...
    pub async fn guard_transaction_with_serving_lease(
        ...
        sqlx::query(
            "SELECT set_config('punks.serving_write_community', $1, true), \
                    set_config('punks.serving_write_lease_id', $2, true), \
                    set_config('punks.serving_write_owner', $3, true), \
                    set_config('punks.serving_write_generation', $4, true), \
                    set_config('punks.serving_write_fence_generation', $5, true)",
        )
...
async fn set_executor_gucs(
    ...
    sqlx::query(
        "SELECT set_config('punks.deletion_executor_community', $1, true), \
                set_config('punks.deletion_fence_generation', $2, true)",
    )
...
mod postgres_tests {
    ...
    async fn store() -> (Db, DeletionStore) {
        let database_url = std::env::var("PUNKS_TEST_DATABASE_URL")
            ...
            .unwrap_or_else(|_| "postgres://punks:punks_dev@localhost:5432/punks".to_string()); // sadscan:disable np.postgres.1 -- local test-only credentials
    ...
    async fn preclaim_setup_failure_is_durable_without_a_lease() {
        ...
        assert_eq!(
            ...
            Some("PUNKS_S3_ENDPOINT is required")
        );
        assert_eq!(
    ...
    async fn write_assertion_rejects_pinned_snapshot_isolation_before_authorization() {
        ...
        for isolation in ["REPEATABLE READ", "SERIALIZABLE"] {
            ...
            sqlx::query(
                "SELECT set_config('punks.deletion_executor_community', $1, true), \
                        set_config('punks.deletion_fence_generation', '0', true)",
            )
            .bind(community.to_string())
            .execute(&mut *tx)
            .await
    ...
    async fn serving_lease_reaper_remains_global_across_tombstoned_tenant() {
        ...
        sqlx::query(
            "SELECT set_config('punks.deletion_executor_community', $1, true), \
                    set_config('punks.deletion_fence_generation', '1', true)",
        )
    ...
    async fn desired_state_schema_bootstrap_progresses_beyond_fencing() {
        ...
        let admin = PgPool::connect(&base_url)
            .await
            .expect("connect admin database");
        let probe_db = format!("punks_desired_state_{}", Uuid::new_v4().simple());
        sqlx::query(AssertSqlSafe(format!("CREATE DATABASE {probe_db}")))
            .execute(&admin)
            .await
            .expect("create probe database");
