...
//! - Punks-agent personas emit PUNKS_AGENT_MODEL, PUNKS_AGENT_PROVIDER
...
use punks_persona::resolve::resolve_pack;
...
#[test]
fn resolve_pack_punks_agent_persona_emits_punks_agent_vars() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path();
...
"#,
    ...
    assert!(
        ...
        "punks-agent runtime must not emit GOOSE_MODEL"
    );
    assert!(
        !env.contains_key("GOOSE_PROVIDER"),
        "punks-agent runtime must not emit GOOSE_PROVIDER"
    );
}
...
"#,
    ...
    // Punks-agent persona
    fs::write(
        root.join("agents/punks-bot.persona.md"),
        r#"---
name: "punks-bot"
display_name: "Punks Bot"
description: "A punks-agent runtime bot"
runtime: "punks-agent"
model: "openai:gpt-4o"
---
You are a punks bot.
"#,
    ...
    let punks = pack
        .personas
        .iter()
        .find(|p| p.name == "punks-bot")
        .expect("punks-bot should exist");
    ...
    // Punks-agent persona gets PUNKS_AGENT_* env vars
    let punks_env: std::collections::HashMap<_, _> = punks
        .runtime_env_vars
        .iter()
        .map(|(k, v)| (k.as_str(), v.as_str()))
        .collect();
    assert_eq!(punks_env.get("PUNKS_AGENT_MODEL"), Some(&"gpt-4o"));
    assert_eq!(punks_env.get("PUNKS_AGENT_PROVIDER"), Some(&"openai"));
    assert!(
        !punks_env.contains_key("GOOSE_MODEL"),
        "punks-agent persona must not emit GOOSE_MODEL"
    );
    assert!(
        !punks_env.contains_key("GOOSE_PROVIDER"),
        "punks-agent persona must not emit GOOSE_PROVIDER"
    );
}
