//! `punks-test-cli` — Manual testing CLI for the Punks relay.
//!
//! # Usage
//!
//! ```text
//! punks-test-cli [OPTIONS]
//!
//! Options:
...
//! Send a message:
//! ```text
//! punks-test-cli --channel my-channel --send "Hello, Punks!"
//! ```
//!
//! Subscribe and watch events:
//! ```text
//! punks-test-cli --channel my-channel --subscribe
//! ```
...
async fn main() {
    tracing_subscriber::fmt()
        .with_env_filter(
            std::env::var("RUST_LOG")
                .unwrap_or_else(|_| "punks_test_client=debug".to_string())
                .as_str(),
        )
        .init();
    ...
    let keys = match std::env::var("PUNKS_PRIVATE_KEY") {
        Ok(sk) => Keys::parse(&sk).expect("invalid PUNKS_PRIVATE_KEY"),
        Err(_) => Keys::generate(),
    };
...
async fn run_send(url: &str, keys: &Keys, channel: &str, message: &str, kind: u16) {
    ...
    let mut client = match PunksTestClient::connect(url, keys).await {
        Ok(c) => c,
        Err(e) => {
...
async fn run_subscribe(url: &str, keys: &Keys, channel: &str, kind: u16) {
    ...
    loop {
        match client.recv_event(Duration::from_secs(30)).await {
            ...
            Ok(_) => {}
            Err(punks_test_client::TestClientError::Timeout) => {
                // Keep waiting.
            }
            Err(e) => {
                eprintln!("Error: {e}");
...
fn print_help() {
    println!(
        r#"punks-test-cli — Manual testing CLI for the Punks relay
...
EXAMPLES:
    ...
    punks-test-cli --url ws://relay.example.com --channel test --subscribe
"#
    );
}
