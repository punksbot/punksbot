...
//!   cargo test -p punks-test-client --test e2e_git -- --ignored --nocapture
//! ```
...
use punks_media::S3AddressingStyle;
...
/// named by the announcement's `punks-channel` tag, so every repo these tests
...
/// Run `git` with the Punks credential helper and isolated config.
...
/// Run `git` with the Punks credential helper and isolated config. Asserts the
...
impl GitS3Probe {
    ...
    fn from_env() -> Self {
        ...
        // relay. Unit/live MinIO probes in punks-relay keep explicit local
        // fixtures and do not need provider overrides.
        let endpoint = std::env::var("PUNKS_S3_ENDPOINT")
            .unwrap_or_else(|_| "http://localhost:9000".to_string());
        let access_key =
            std::env::var("PUNKS_S3_ACCESS_KEY").unwrap_or_else(|_| "punks_dev".to_string());
        let secret_key =
            std::env::var("PUNKS_S3_SECRET_KEY").unwrap_or_else(|_| "punks_dev_secret".to_string());
        let bucket_name =
            std::env::var("PUNKS_S3_BUCKET").unwrap_or_else(|_| "punks-media".to_string());
        let region_name =
            std::env::var("PUNKS_S3_REGION").unwrap_or_else(|_| "us-east-1".to_string());
    ...
    fn pointer_key(owner: &str, repo: &str) -> String {
        ...
        if let Ok(community) = std::env::var("PUNKS_E2E_GIT_COMMUNITY_ID") {
...
fn git_s3_probe_builds_both_addressing_styles() {
    ...
    assert_eq!(path.url(), "https://storage.example/punks-media");
    ...
    assert_eq!(virtual_hosted.url(), "https://punks-media.storage.example");
}
...
async fn git_clone_push_fetch_force_roundtrip() {
    ...
    // The `punks-channel` binding is the repo's ACL: without it the read gate
    ...
    let announce = EventBuilder::new(Kind::from(30617), "")
        .tags(vec![
            ...
            Tag::parse(["punks-channel", &channel]).unwrap(),
        ])
...
async fn git_concurrent_push_one_wins_and_repo_recovers() {
    ...
    let tmp = tempdir_named("punks-e2e-git-concurrent");
    let url = format!("{}/git/{}/{}", relay_http_url(), owner_hex, repo);
