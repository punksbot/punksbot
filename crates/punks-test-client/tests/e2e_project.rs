...
//!
//! The ingest unit tests in `punks-relay` pin the envelope contract in isolation.
...
//!
//! ```text
//! RELAY_URL=ws://localhost:3000 cargo test -p punks-test-client --test e2e_project -- --ignored
//! ```
...
async fn query(client: &mut PunksTestClient, name: &str, filter: Filter) -> Vec<nostr::Event> {
    let sid = sub_id(name);
    client
...
async fn test_project_publish_and_query_returns_cross_owner_members() {
    ...
    let members = vec![
        member_coord(&owner, "punks"),
        member_coord(&other, "punks-infra"),
    ];

    let mut client = PunksTestClient::connect(&url, &owner)
        .await
        .expect("connect");
...
async fn test_project_replacement_keeps_only_newest_for_same_author_and_d() {
    ...
    let members = vec![member_coord(&owner, "punks")];
...
async fn test_project_same_d_under_two_authors_are_independent() {
    ...
    let mut alice_client = PunksTestClient::connect(&url, &alice)
        .await
        .expect("connect");
    let ok = alice_client
    ...
    let mut bob_client = PunksTestClient::connect(&url, &bob).await.expect("connect");
    let ok = bob_client
...
async fn test_project_malformed_envelope_rejected_by_relay() {
    ...
    let duplicate = member_coord(&owner, "punks");
    ...
    let cases: Vec<(&str, nostr::Event, &str)> = vec![
        ...
        (
            ...
            project_event(
                ...
                "Bad kind",
                &[format!("30618:{}:punks", owner.public_key().to_hex())],
                None,
            ),
        ...
        (
            ...
            project_event(
                ...
                "Uppercase",
                &[format!("{REPO_ANNOUNCEMENT_KIND}:{}:punks", "A".repeat(64))],
                None,
            ),
            "member `a` tag must be",
        ),
