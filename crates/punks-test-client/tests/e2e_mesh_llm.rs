//! End-to-end acceptance tests for Punks shared compute.
//!
//! These tests require a membership-gated punks-relay and a mesh-enabled desktop
...
//! | 5 | dropped member → typed auth failure reaches lastError | runbook (desktop harness) | punks-agent `-32001` unit |
...
const KIND_PUNKS_MESH_MEMBER_STATUS: u16 = 30003;
const MESH_STATUS_D_TAG_PREFIX: &str = "punks-mesh-member-status:";
const MESH_STATUS_TYPE: &str = "punks-mesh-status";
...
fn mesh_status_filter() -> Filter {
    Filter::new()
        .kind(Kind::Custom(KIND_PUNKS_MESH_MEMBER_STATUS))
...
async fn trust_member_reads_mesh_status() {
    ...
    let mut client = PunksTestClient::connect(&url, &member)
        .await
    ...
    let status = events
        .iter()
        .find(|e| {
            e.kind == Kind::Custom(KIND_PUNKS_MESH_MEMBER_STATUS) && e.pubkey == member.public_key()
        })
...
///
/// Requires a relay with `PUNKS_REQUIRE_RELAY_MEMBERSHIP=true` and a published
...
async fn trust_nonmember_read_denied() {
    ...
    let mut client = match PunksTestClient::connect(&url, &stranger).await {
        Ok(c) => c,
    ...
    let leaked = events
        .iter()
        .any(|e| e.kind == Kind::Custom(KIND_PUNKS_MESH_MEMBER_STATUS));
    assert!(
        !leaked,
...
/// B running a mesh client + a launched punks-agent pointed at B's local
...
/// split route. Auto-split is mesh runtime behavior (no Punks code); this row
