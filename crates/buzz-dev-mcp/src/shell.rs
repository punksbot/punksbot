...
fn build_bootstrap(cwd: &Path, shell_hint: &str) -> String {
    ...
    let buzz_hint =
        if std::env::var("PUNKS_RELAY_URL").is_ok() && std::env::var("PUNKS_PRIVATE_KEY").is_ok() {
            "\nPunks relay configured. Run `buzz --help` to see available commands.\n"
        } else {
            ""
        };
    format!(
...
pub async fn run(
    ...
    // PUNKS_PRIVATE_KEY is intentionally inherited — the buzz CLI needs it.
...
mod windows_resolver_tests {
    ...
    fn buzz_shell_bare_name_absent_from_path_falls_through() {
        ...
        env::set_var("PUNKS_SHELL", "buzz-shell-does-not-exist-xyz");
        let result = resolve_bash("");
        env::remove_var("PUNKS_SHELL");
    ...
    fn git_cmd_on_path_resolves_sibling_git_bash() {
        ...
        let old_buzz_shell = env::var_os("PUNKS_SHELL");
