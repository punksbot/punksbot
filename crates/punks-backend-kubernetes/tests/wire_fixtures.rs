...
fn run(request: &str) -> (String, i32) {
    let mut child = Command::new(env!("CARGO_BIN_EXE_punks-backend-kubernetes"))
...
fn info_response_carries_the_contract_fields() {
    ...
    assert!(
        default.starts_with("punks-agents-"),
    ...
    assert!(
        image_default.starts_with("ghcr.io/punksbot/punksbot-sprig:")
