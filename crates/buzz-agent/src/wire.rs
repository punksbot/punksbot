...
pub enum Inbound {
    ...
    /// to a request punks-agent issued. Today the only such request is
...
/// one. `expected_run_id` must match the run id punks-agent advertised via
...
/// session use `model_id` instead of the configured `PUNKS_AGENT_MODEL`.
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub fn classify(msg: &Value) -> Inbound {
    ...
    match (method, id) {
        ...
        // Bare responses (id present, no method) answer a request punks-agent
...
/// `session/request_permission`. punks-acp's answering side selects the option
...
/// is applied.
///
/// - **v2** (what punks-agent negotiates with current punks-acp): tool context
...
/// method goose uses for custom usage and status events.  Used by punks-agent
/// to emit the `usage_update` payload so punks-acp's `UsageTracker` can treat
/// punks-agent and goose symmetrically.
...
/// exists to prevent.
///
/// All counts are SESSION-cumulative, matching goose, so punks-acp's
...
/// | `contextLimit` | `u64` | `0` — punks-agent has no context limit tracking |
...
pub fn usage_update_payload(
    ...
    let mut update = json!({
        ...
        // contextLimit: 0 (punks-agent has no context limit tracking).
...
mod tests {
    ...
    /// v2 (what punks-agent negotiates with current punks-acp): tool context is
    ...
    /// `optionId == kind` so punks-acp's `kind`-based selector and this side's
    ...
    /// request punks-agent issued; it routes to the broker with its `result`.
    #[test]
    ...
    /// fails closed. punks-agent never leaves the waiter hanging on an error.
    #[test]
