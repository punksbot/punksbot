...
//! cargo test -p punks-media --test versioned_minio -- --ignored --nocapture
//! ```
//!
//! The test creates and removes its own bucket. The MinIO container name is
//! overridable with `PUNKS_MINIO_CONTAINER`; credentials/endpoint/region/addressing
//! use the same `PUNKS_S3_*` env vars as `static_creds_minio`.

use std::process::Command;

use punks_media::config::MediaConfig;
use punks_media::storage::{MediaStorage, ObjectVersionKind, ObjectVersionRef};
...
fn minio_config(bucket: String) -> MediaConfig {
    MediaConfig {
        s3_endpoint: env_or("PUNKS_S3_ENDPOINT", "http://localhost:9000"),
        s3_access_key: env_or("PUNKS_S3_ACCESS_KEY", "punks_dev"),
        s3_secret_key: env_or("PUNKS_S3_SECRET_KEY", "punks_dev_secret"),
        s3_bucket: bucket,
        s3_region: env_or("PUNKS_S3_REGION", "us-east-1"),
        s3_addressing_style: env_or("PUNKS_S3_ADDRESSING_STYLE", "path")
            .parse()
...
fn run_mc(args: &[String]) -> Result<(), String> {
    let container = env_or("PUNKS_MINIO_CONTAINER", "punks-minio");
...
async fn list_all_versions(
    ...
    prefix: &str,
    max_keys: usize,
) -> Vec<punks_media::storage::ObjectVersionEntry> {
...
fn refs_from(entries: &[punks_media::storage::ObjectVersionEntry]) -> Vec<ObjectVersionRef> {
    entries
        .iter()
...
async fn never_versioned_bucket_lists_null_versions_and_exact_delete_empties_listing() {
    let bucket = format!("punks-media-never-versioned-{}", std::process::id());
...
async fn versioned_bucket_exact_version_delete_reaches_final_list_versions_emptiness() {
    let bucket = format!("punks-media-versioned-{}", std::process::id());
    let bucket_path = format!("local/{bucket}");
    let config = minio_config(bucket.clone());
    mc_alias(&config.s3_access_key, &config.s3_secret_key).expect("configure mc alias");
    run_mc(&[
