//! `punks-pair` — NIP-AB device pairing interop testing CLI.
//!
//! # Usage
//!
//! ```text
//! punks-pair source --relay wss://relay.example.com [--nsec nsec1...]
//! punks-pair target [--relay wss://relay.example.com]
...
use punks_core::kind::KIND_PAIRING;
use punks_core::pairing::session::PairingSession;
use punks_core::pairing::{
...
async fn cmd_source(relay_url: String, nsec: Option<String>) -> Result<(), CliError> {
    ...
    if !confirmed {
        ...
        if let Some(abort_event) =
            session.abort(punks_core::pairing::types::AbortReason::SasMismatch)?
        {
