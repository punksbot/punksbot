...
CLI tool for testing the [NIP-AB device pairing protocol](../punks-core/src/pairing/NIP-AB.md) end-to-end. Exercises the full protocol over a live Nostr relay — designed for interop testing and NIP submission, not production use.

## Quick Start

```bash
cargo build --release -p punks-pairing-cli

# Terminal 1 — source (holds the secret)
./target/release/punks-pair source --relay wss://relay.damus.io

# Terminal 2 — target (receives the secret)
./target/release/punks-pair target --show-secret
...
```
punks-pair source --relay <RELAY_URL> [--nsec <BECH32_NSEC>]
```
...
```
punks-pair target [--relay <OVERRIDE_URL>] [--show-secret]
```
...
```
punks-pair test-vectors
```

## Testing Against a Local Punks Relay

The CLI supports NIP-42 authentication, so it works with Punks relays out of the box.

### Prerequisites

- Docker running (for Postgres, Redis, etc.)
- Punks relay built: `cargo build --release -p punks-relay`
...
screen -dmS relay bash -c "./target/release/punks-relay 2>&1 | tee /tmp/punks-relay.log"
...
```bash
# Terminal 1
./target/release/punks-pair source --relay ws://localhost:3000
