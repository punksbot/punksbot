#![deny(unsafe_code)]
#![warn(missing_docs)]
//! Punks search — community-scoped Postgres full-text search over Punks events.
//!
...
//! by `punks-db`. The relay refetches canonical events through `punks-db`'s
...
pub use punks_core::CommunityId;
