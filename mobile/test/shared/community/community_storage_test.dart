...
import 'package:punks/shared/community/community_storage.dart';
  ...
  group('CommunityStorage', () {
    ...
    group('migration', () {
      test('migrates saved workspaces to communities', () async {
        ...
        fakeSecure['punks_workspaces'] = jsonEncode([legacy.toJson()]);
        fakeSecure['punks_active_workspace_id'] = legacy.id;

        final loaded = await storage.loadAll();

        expect(loaded.single.id, legacy.id);
        expect(await storage.loadActiveId(), legacy.id);
        expect(fakeSecure['punks_communities'], isNotNull);
        expect(fakeSecure['punks_workspaces'], isNull);
        expect(fakeSecure['punks_active_workspace_id'], isNull);
      });

      test('migrates legacy keys to community on first load', () async {
        fakeSecure['punks_relay_url'] = 'https://legacy.example.com';
        fakeSecure['punks_token'] = 'legacy_token';
        fakeSecure['punks_pubkey'] = 'legacy_pub';
        fakeSecure['punks_nsec'] = 'legacy_nsec';
        ...
        expect(fakeSecure['punks_relay_url'], isNull);
        expect(fakeSecure['punks_token'], isNull);
        expect(fakeSecure['punks_pubkey'], isNull);
        expect(fakeSecure['punks_nsec'], isNull);
      ...
      test('migration generates name from localhost URL', () async {
        fakeSecure['punks_relay_url'] = 'http://localhost:3000';
        fakeSecure['punks_token'] = 'tok';
