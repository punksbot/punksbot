...
test("report rows render the relay response contract", async ({ page }) => {
      ...
      body: JSON.stringify([
        {
          ...
          communityHost: "design.punks.xyz",
  ...
  await page.goto("/reports");
  await expect(page.getByText("design.punks.xyz")).toBeVisible();
...
test("feedback can be searched and filtered by community and time", async ({
    ...
    route.fulfill({
      ...
      body: JSON.stringify([
        ...
        },
        {
          id: "old",
          communityId: "two",
          communityHost: "engineering.punks.xyz",
  ...
  await page.getByLabel("Community").selectOption("design.punks.xyz");
...
test("feedback attachments render from imeta without raw markdown", async ({
  ...
  const imageUrl = `https://design.punks.xyz/media/${"a".repeat(64)}.png`;
  const fileUrl = `https://design.punks.xyz/media/${"b".repeat(64)}.txt`;
  await page.route(`**/api/admin/v1/feedback/${id}`, (route) =>
    route.fulfill({
      contentType: "application/json",
      body: JSON.stringify({
