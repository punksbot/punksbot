export const PUNKS_RELEASES_URL = "https://github.com/punksbot/punksbot/releases";
const PUNKS_RELEASES_API_URL =
  "https://api.github.com/repos/punksbot/punksbot/releases?per_page=10";
const CACHE_KEY = "punks.latestDownload.v1";
const CACHE_TTL_MS = 60 * 60 * 1000;

export type PunksDownloadPlatform = {
...
): PunksDownloadPlatform["operatingSystem"] {
...
function normalizeArchitecture(
  value: string,
): PunksDownloadPlatform["architecture"] {
...
export async function detectPunksDownloadPlatform(
  navigatorValue: Navigator,
): Promise<PunksDownloadPlatform> {
  const userAgentData = (
...
function assetPattern(platform: PunksDownloadPlatform): RegExp | undefined {
...
export function selectPunksDownloadUrl(
  releases: GitHubRelease[],
  platform: PunksDownloadPlatform,
): string | undefined {
...
export async function resolvePunksDownloadUrlForPlatform(
  platform: PunksDownloadPlatform,
): Promise<string> {
  try {
  ...
  try {
    const response = await fetch(PUNKS_RELEASES_API_URL, {
      headers: { Accept: "application/vnd.github+json" },
    });
    if (!response.ok) return PUNKS_RELEASES_URL;
    const url = selectPunksDownloadUrl(
      (await response.json()) as GitHubRelease[],
      platform,
    );
    if (!url) return PUNKS_RELEASES_URL;
    try {
    ...
    return PUNKS_RELEASES_URL;
  }
}

export async function resolvePunksDownloadUrl(): Promise<string> {
  return resolvePunksDownloadUrlForPlatform(
    await detectPunksDownloadPlatform(navigator),
  );
}
