...
test("home page loads with Punks branding", async ({ page }) => {
  await page.goto("/");
  await expect(
    page.getByRole("main").getByRole("img", { name: "Punks" }),
  ).toBeVisible();
});
...
test("invite requires age and legal consent before opening Punks", async ({
  page,
}) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify([
        { draft: false, prerelease: false, assets: [] },
        {
          ...
          assets: [
            {
              name: "Punks_0.4.9_aarch64.dmg",
              browser_download_url:
                "https://github.com/punksbot/punksbot/releases/download/v0.4.9/Punks_0.4.9_aarch64.dmg",
            },
            {
              name: "Punks_0.4.9_x64.dmg",
              browser_download_url:
                "https://github.com/punksbot/punksbot/releases/download/v0.4.9/Punks_0.4.9_x64.dmg",
            },
            {
              name: "Punks_0.4.9_amd64.AppImage",
              browser_download_url:
                "https://github.com/punksbot/punksbot/releases/download/v0.4.9/Punks_0.4.9_amd64.AppImage",
            },
            {
              name: "Punks_0.4.9_x64-setup_alpha-unsigned.exe",
              browser_download_url:
                "https://github.com/punksbot/punksbot/releases/download/v0.4.9/Punks_0.4.9_x64-setup_alpha-unsigned.exe",
            },
          ],
        },
      ]),
  ...
  await page
    .locator("label")
    .filter({
      hasText: "I agree to the Punks Terms of Service and Privacy Policy.",
    })
...
test("invite asks Safari users to choose their Mac download", async ({
  ...
  await expect(openedPage).toHaveURL("https://github.com/punksbot/punksbot/releases");
...
test("invite download falls back for mobile and non-desktop devices", async ({
  ...
  for (const device of unsupportedDevices) {
    ...
    await expect(
      page.getByRole("link", { name: "Download it now" }),
      device.name,
    ).toHaveAttribute("href", "https://github.com/punksbot/punksbot/releases");
    await context.close();
  }
});
