# Punks relay bus scaling harness
...
- **new/community-scoped bus:** each pod retains only the server-resolved community topics for which it has local subscribers (`punks:{community_id}:global` or `punks:{community_id}:channel:{channel_id}`).
...
The scoped Redis channel format corresponds to `punks_pubsub::EventTopicKey::redis_channel()` in `crates/punks-pubsub/src/topic.rs`:

- global: `punks:{community_id}:global`
- channel: `punks:{community_id}:channel:{channel_id}`
