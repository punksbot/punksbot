#!/usr/bin/env python3
"""Reproducible Punks relay Redis bus scaling harness.
...
PUNKS_PREFIX = "punks"
OLD_GLOBAL_CHANNEL = f"{PUNKS_PREFIX}:global"
...
def scoped_global_channel(community: str) -> str:
    # Mirrors crates/punks-pubsub/src/topic.rs EventTopic::Global:
    # format!("punks:{}:global", self.community_id)
    return f"{PUNKS_PREFIX}:{community}:global"
...
def print_rows(args: argparse.Namespace, rows: list[Measurement]) -> None:
    print("Punks relay Redis bus scaling harness")
    print("====================================")
    print(
        "scenario: "
