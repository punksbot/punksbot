# Punks Bot — Instructions propres au fork

## Frontière d’exécution prioritaire

Ces règles Punks remplacent toute instruction Punks contradictoire plus bas dans
ce fichier :

- le backend et ses tests utilisent uniquement les services Cloudflare Workers
  gérés et les implémentations `workerd` ;
- ne jamais démarrer le relay Punks, Docker, PostgreSQL, Redis, MinIO, Helm,
  Kubernetes ou un runtime serveur historique, ni en développement ni en CI ;
- ne jamais exécuter `just setup`, `just relay`, les tests d’intégration relay ou
  un workflow archivé de `.github/legacy-workflows` ;
- utiliser `pnpm cloudflare:check` comme gate backend Punks ;
- le guide Punks conservé ci-dessous est une source historique pour la parité et
  les clients importés, pas une autorisation d’exécuter son infrastructure.
...
This guide is for AI agents contributing to the Punks codebase. It covers
...
Punks spans five repos. This one (`punksbot/punksbot`) is the OSS source for the relay, desktop, mobile, and CLI. The others handle internal builds and deployment:

| Repo | Purpose |
|------|---------|
| [punksbot/punksbot](https://github.com/punksbot/punksbot) | OSS source — relay, desktop app, mobile app, CLI, agent harness |
| [punksbot/punksbot](https://github.com/punksbot/punksbot) | Buildkite pipelines producing Block-signed macOS + iOS builds with `-block` desktop version suffix |
...
```
punksbot/punksbot (source)
  ├─► punks-releases      (desktop + mobile builds → Artifactory, GitHub, Mobile Releases)
...
crates/
  # Relay + core
  punks-relay          # WebSocket relay server — main entry point; also hosts git + huddle audio
  punks-core           # Core types, event verification, filter matching, kind registry
  punks-db             # Postgres event store and data access layer
  punks-auth           # Authentication and authorization
  punks-pubsub         # Redis pub/sub fan-out, presence, typing indicators
  punks-search         # Postgres FTS full-text search
  punks-audit          # Hash-chain audit log
  punks-media          # Blossom/S3 media storage
  # Agent surface
  punks-acp            # ACP harness bridging Punks events to AI agents
  punks-agent          # Minimal ACP-compliant agent (non-streaming, tool-calls-as-output)
  punks-dev-mcp        # Developer MCP server — shell + file-edit tools
  punks-persona        # Agent persona packs
  punks-workflow       # YAML-as-code workflow engine (evalexpr conditions)
  # Clients + interop
  punks-pair-relay     # Ephemeral sidecar relay for NIP-AB device pairing
  punks-pairing-cli    # CLI for NIP-AB device pairing interop testing
  ...
  punks-sdk            # Typed Nostr event builders
  punks-admin          # Operator CLI for relay administration
  punks-ws-client      # Shared NIP-42 WebSocket client (connect, auth, publish)
  punks-test-client    # Integration test client and E2E test suite
...
Run `just test` for integration tests if you touched `punks-relay`,
`punks-db`, or `punks-auth` — these require a running Postgres and Redis.
...
**Commit with `git commit -s`.** The required **DCO Check** fails any PR with a commit missing a `Signed-off-by` trailer, and `just hooks` installs a `commit-msg` hook that adds it to commits you create locally (`git rebase` and `git cherry-pick` still need `--signoff`) — if you build commit commands programmatically, include `-s` every time. To repair a branch that already has unsigned commits: `git rebase --signoff prod`, then force-push.
...
- `POST /query` — Nostr REQ filters over HTTP. NIP-50 `search` filters
  are routed to `punks-search` (Postgres FTS) automatically.
...
`punks-core/src/kind.rs`. New features get new kind integers — add them here
...
**Agent-facing operations go in `punks-cli`**: New agent-facing features belong in `punks-cli` — add a subcommand there first, then wire the REST/WebSocket call in `client.rs`. `punks-dev-mcp` (shell + file tools for `punks-agent`) is separate.

**Workflow conditions**: `punks-workflow` uses
...
`punks` is the agent-first CLI. Auth env vars
(`PUNKS_RELAY_URL`, `PUNKS_PRIVATE_KEY`, `PUNKS_AUTH_TAG`) are auto-injected
by the ACP harness into managed agent subprocesses. In development, set
`PUNKS_PRIVATE_KEY` and `PUNKS_RELAY_URL` in your environment manually.

### Building the CLI

```bash
cargo build --release -p punks-cli
```

Binary location: `./target/release/punks`. Add `./target/release` to `PATH`
or invoke with the full path.

### Deep Links

`punks://message?channel=<uuid>&id=<hex>` links reference a specific message
thread. To read the linked thread:

```bash
punks --format compact messages thread --channel <uuid> --event <hex>
```
...
`punks --format compact channels list`, NOT `punks channels list --format compact`.

See `crates/punks-cli/TESTING.md` for the full live-testing runbook.
...
E2E tests live in `crates/punks-test-client/tests/`:
...
> **Do NOT use `punks upload`, the relay media endpoint, or any third-party
...
fields are optional and passed through to `__PUNKS_E2E_EMIT_MOCK_MESSAGE__`:
...
gh pr view <pr> --repo punksbot/punksbot --json comments \
  --jq '.comments[] | select(.body | test("pr-<pr>--")) | {id, url}'
gh api -X DELETE repos/punksbot/punksbot/issues/comments/<stale-comment-id>
```
...
`__PUNKS_E2E_EMIT_MOCK_MESSAGE__` — messages are silently dropped without a
...
description. See [PR #803](https://github.com/punksbot/punksbot/pull/803).

---

## Common Gotchas

1. **Kind `39000` for channel metadata, not `41`** — kind 41 is NIP-01 (unused). All kinds defined in `punks-core/src/kind.rs`.
