...
Gated by PUNKS_TESTBED_LIVE=1 with stack coordinates in the environment:
  PUNKS_TESTBED_RELAY_HTTP   (default http://localhost:3000)
  PUNKS_TESTBED_RELAY_WS     (default ws://host.docker.internal:3000)
  PUNKS_TESTBED_OWNER_KEY    relay owner secret key (hex)
  PUNKS_TESTBED_PG_DSN       benchmark Postgres DSN
"""
...
pytestmark = pytest.mark.skipif(
    ...
    reason="live testbed suite; set PUNKS_TESTBED_LIVE=1 with a running stack",
)


@pytest.fixture()
def provisioner() -> PunksTrialProvisioner:
    owner_key = os.environ.get("PUNKS_TESTBED_OWNER_KEY")
    dsn = os.environ.get("PUNKS_TESTBED_PG_DSN")
    if not owner_key or not dsn:
        pytest.fail("PUNKS_TESTBED_OWNER_KEY and PUNKS_TESTBED_PG_DSN are required")
    return PunksTrialProvisioner(
        TestbedConfig(
            relay_http_url=os.environ.get(
                "PUNKS_TESTBED_RELAY_HTTP", "http://localhost:3000"
            ),
            relay_ws_url=os.environ.get(
                "PUNKS_TESTBED_RELAY_WS", "ws://host.docker.internal:3000"
            ),
...
def cli_for(provisioner: PunksTrialProvisioner, credential) -> PunksCli:
...
def test_create_is_idempotent_and_isolated(provisioner, manifest):
    ...
    try:
        ...
        with pytest.raises(PunksCliError, match="private"):
    ...
    provisioner.teardown(handle_a)
    with psycopg.connect(os.environ["PUNKS_TESTBED_PG_DSN"]) as conn:
        row = conn.execute(
