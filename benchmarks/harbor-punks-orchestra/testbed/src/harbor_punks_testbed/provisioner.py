"""Channel-per-trial Punks provisioning against a local benchmark stack."""
...
from .punks_cli import PunksCli
...
def provisioner_from_dict(config: dict[str, object]) -> PunksTrialProvisioner:
    """Harbor CLI factory for a JSON-decoded testbed configuration."""
    return PunksTrialProvisioner(TestbedConfig(**config))


class PunksTrialProvisioner:
    """Implements the TrialHandle v1.1 contract against a live Punks relay.

    Guarantees (contract PLANS/HARBOR_PUNKS_TRIALHANDLE_CONTRACT.md):
    ...
    def _provision(
        ...
        # mirroring production Punks, where a human owns the channel their
        # agents work in.
    ...
    def _cli_for(self, credential: AgentCredential) -> PunksCli:
        return PunksCli(
