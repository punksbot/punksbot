"""Run the production Punks agent stack inside the Harbor task container.

Each provisioned identity is a full ``punks-acp`` → ``punks-agent`` →
``punks-dev-mcp`` process tree launched *inside* the task container — the same
binaries and the same MCP toolset (shell, file tools, the ``punks`` CLI on
PATH) that the desktop app gives a Punks agent. The harness stays outside:
...
DEFAULT_MAX_AGENT_ROUNDS = 0  # 0 = unbounded (PUNKS_AGENT_MAX_ROUNDS=0); the trial budget is the clock
# Container-side layout for the uploaded Punks stack.
REMOTE_ROOT = "/opt/punks"
...
class RuntimeLaunchError(RuntimeError):
    """Raised when a Punks agent process cannot be launched or exits early."""
...
class PunksContainerRuntime:
    """Launch one production Punks agent stack per identity in the container."""
    ...
    def __init__(
        ...
        *,
        ...
        punks_acp_binary: str = "punks-acp",
        punks_agent_binary: str = "punks-agent",
        punks_dev_mcp_binary: str = "punks-dev-mcp",
        punks_cli_binary: str = "punks",
        relay_gateway: str = "",
        ...
        self.punks_acp_binary = punks_acp_binary
        self.punks_agent_binary = punks_agent_binary
        self.punks_dev_mcp_binary = punks_dev_mcp_binary
        # Host build used for user/provisioning operations only:
        self.punks_cli_binary = punks_cli_binary
    ...
    async def run(
        ...
        *,
        ...
        if not workers:
            raise RuntimeLaunchError("Punks orchestration requires at least one worker")
        trial_dir = self.logs_dir / "punks"
        ...
        try:
            ...
            # The task arrives exactly as it would in production Punks: a
    ...
    async def _install_stack(self, environment: BaseEnvironment) -> None:
        ...
        uploads = {
            f"{REMOTE_BIN}/punks-acp": self.punks_acp_binary,
            f"{REMOTE_BIN}/punks-agent": self.punks_agent_binary,
            f"{REMOTE_BIN}/punks-dev-mcp": self.punks_dev_mcp_binary,
        }
    ...
    async def _launch_agent(
        ...
        *,
        ...
        command = (
            f"{shlex.quote(f'{REMOTE_BIN}/punks-acp')} </dev/null "
    ...
    def _agent_env(
        ...
        *,
        ...
        return {
            **endpoint.env,
            "PUNKS_RELAY_URL": trial.relay_ws_url,
            "PUNKS_PRIVATE_KEY": credential.nostr_secret_key,
            # Desktop parity: the GUI also sets NOSTR_PRIVATE_KEY on punks-acp
            # so punks-dev-mcp's shim can wire git auth/signing for the agent.
            "NOSTR_PRIVATE_KEY": credential.nostr_secret_key,
            "PUNKS_AUTH_TAG": credential.nostr_auth_tag,
            "PUNKS_ACP_AGENT_COMMAND": f"{REMOTE_BIN}/punks-agent",
            "PUNKS_ACP_AGENT_ARGS": "",
            "PUNKS_ACP_MCP_COMMAND": f"{REMOTE_BIN}/punks-dev-mcp",
            "PUNKS_ACP_CHANNELS": trial.channel_id,
            "PUNKS_ACP_SUBSCRIBE": "mentions",
            "PUNKS_ACP_RESPOND_TO": "anyone",
            "PUNKS_ACP_NO_MEMORY": "true",
            "PUNKS_ACP_SYSTEM_PROMPT_FILE": remote_prompt,
            "PUNKS_AGENT_PROVIDER": endpoint.provider,
            "PUNKS_AGENT_MODEL": credential.llm_endpoint,
            "PUNKS_AGENT_MAX_OUTPUT_TOKENS": str(
                agent_class.generation.max_output_tokens
            ),
            "PUNKS_AGENT_MAX_CONTEXT_TOKENS": str(
                agent_class.generation.context_window_tokens
            ),
            "PUNKS_AGENT_MAX_ROUNDS": str(
    ...
    async def _wait_for_done(
        ...
        while True:
            ...
            polls += 1
            messages = await self._punks_json(
                trial.user,
                trial,
                "messages",
                "get",
    ...
    # -- Punks CLI as the trial user / provisioning identities -------------------
    ...
    async def _send(
        ...
        *,
        ...
        await self._punks_json(credential, trial, *args)

    async def _punks_json(
        self, credential: AgentCredential, trial: TrialHandle, *args: str
    ) -> Any:
        process = await asyncio.create_subprocess_exec(
            self.punks_cli_binary,
            *args,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
            env={
                **os.environ,
                "PUNKS_RELAY_URL": self._user_relay_url(trial),
                "PUNKS_PRIVATE_KEY": credential.nostr_secret_key,
                "PUNKS_AUTH_TAG": credential.nostr_auth_tag,
            },
        )
        ...
        if process.returncode != 0:
            raise RuntimeLaunchError(
                f"punks {shlex.join(args)} exited {process.returncode}: "
                f"{stderr.decode(errors='replace').strip()}"
            )
        try:
            return json.loads(stdout)
        except json.JSONDecodeError as error:
            raise RuntimeLaunchError("punks returned invalid JSON") from error
    ...
    def _user_relay_url(trial: TrialHandle) -> str:
        ...
        return PunksContainerRuntime._cli_relay_url(trial.relay_ws_url)
    ...
    def _compose_system_prompt(
        ...
        *,
        ...
        The analogue of a production Punks workspace's team context: each agent
        ...
        lines = [
            ...
            f"The team coordinates in Punks channel `{trial.channel_id}`.",
            (
