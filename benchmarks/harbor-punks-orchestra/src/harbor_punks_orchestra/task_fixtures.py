"""Public setup declarations for Punks-native benchmark tasks."""
...
_CREATE_CHANNEL_FIXTURE = PunksTaskFixture(
    directory=tuple(
        [
...
_USER_MENTION_FIXTURE = PunksTaskFixture(
    user_display_name=USER_MENTION_DISPLAY_NAME,
    requires_evidence=True,
)

_NARRATIVE_AGENT_NAMES_FIXTURE = PunksTaskFixture(
    directory=(
...
_INTERLEAVED_AGENT_REPORTS_FIXTURE = PunksTaskFixture(
    directory=(
...
_CROSS_THREAD_REQUESTS_FIXTURE = PunksTaskFixture(
    scripted_messages=(
        ScriptedMessage(
            label="beta-request",
            actor="user",
...
_AMBIGUOUS_USER_MENTION_FIXTURE = PunksTaskFixture(
    directory=(
        DirectoryEntry(
            "Taylor Morgan Lee",
            "user",
...
_FIXTURES = {
    ...
    REPLY_TO_THREAD_TASK: PunksTaskFixture(requires_evidence=True),
    READ_NAMED_PATH_TASK: PunksTaskFixture(requires_evidence=True),
    MULTILINE_MESSAGE_TASK: PunksTaskFixture(
...
def fixture_for(task_name: str | None) -> PunksTaskFixture:
    """Return the declared setup for a task, or an empty setup."""
    return _FIXTURES.get(task_name or "", PunksTaskFixture())
