...
from harbor_punks_orchestra.provisioning import AgentCredential, TrialHandle
...
def test_prompt_hash_and_identity_override_are_fail_closed(tmp_path):
    ...
    endpoint = EndpointLaunchConfig(
        "anthropic", "ANTHROPIC_API_KEY", {"PUNKS_ACP_MCP_COMMAND": "evil"}
    )
...
async def test_install_stack_uploads_the_pinned_stack(tmp_path):
    binaries = {}
    for name in ("punks-acp", "punks-agent", "punks-dev-mcp"):
        path = tmp_path / name
        path.write_text("#!binary")
        binaries[name] = str(path)
    rt = runtime(
        tmp_path,
        punks_acp_binary=binaries["punks-acp"],
        punks_agent_binary=binaries["punks-agent"],
        punks_dev_mcp_binary=binaries["punks-dev-mcp"],
    )
    environment = Environment()
    await rt._install_stack(environment)
    assert {target for _, target in environment.uploads} == {
        f"{REMOTE_BIN}/punks-acp",
        f"{REMOTE_BIN}/punks-agent",
        f"{REMOTE_BIN}/punks-dev-mcp",
    }
...
async def test_install_stack_requires_binaries_on_disk(tmp_path):
    rt = runtime(tmp_path, punks_acp_binary=str(tmp_path / "missing"))
...
async def test_forwarder_bridges_the_canonical_relay_address(tmp_path):
    from harbor_punks_orchestra.container_runtime import FORWARDER
...
async def test_launch_wires_the_desktop_environment(tmp_path, configured, expected):
    ...
    environment = Environment(
        responses={"punks-acp": ExecResult(stdout="4242\n", stderr="", return_code=0)}
    )
    ...
    assert f"{REMOTE_BIN}/punks-acp" in command
    # The real product wiring: acp spawns punks-agent, which gets punks-dev-mcp.
    assert env["PUNKS_ACP_AGENT_COMMAND"] == f"{REMOTE_BIN}/punks-agent"
    assert env["PUNKS_ACP_MCP_COMMAND"] == f"{REMOTE_BIN}/punks-dev-mcp"
    assert env["PUNKS_RELAY_URL"] == trial.relay_ws_url
    assert env["PUNKS_PRIVATE_KEY"] == orch.nostr_secret_key
    assert env["NOSTR_PRIVATE_KEY"] == orch.nostr_secret_key
    assert env["PUNKS_AGENT_NO_HINTS"] == "1"
    assert env["PUNKS_AGENT_MAX_ROUNDS"] == expected
    assert env["PUNKS_ACP_SYSTEM_PROMPT_FILE"].endswith("orch-1.system-prompt.md")
    # The composed prompt was uploaded into the container.
    assert any(
        target == env["PUNKS_ACP_SYSTEM_PROMPT_FILE"]
        for _, target in environment.uploads
    )


def test_runtime_validates_construction_bounds(tmp_path):
    # 0 is legal and means unbounded (PUNKS_AGENT_MAX_ROUNDS=0); the trial
...
async def test_wait_for_agents_ready_requires_every_channel_subscription(tmp_path):
    ...
    from harbor_punks_orchestra.container_runtime import _Agent
...
async def test_send_mentions_by_pubkey_so_task_text_stays_inert(
    ...
    async def punks_json(credential, trial, *args):
        calls.append(args)
        return {}

    monkeypatch.setattr(rt, "_punks_json", punks_json)
...
async def test_wait_for_done_requires_orchestrator_authorship(tmp_path, monkeypatch):
    ...
    async def punks_json(credential, *args, **kwargs):
...
async def test_stop_agents_sweeps_the_uploaded_stack(tmp_path):
    ...
    await PunksContainerRuntime._stop_agents(environment, agents)
