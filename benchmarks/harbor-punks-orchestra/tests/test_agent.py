...
from harbor_punks_orchestra import (
    AgentCredential,
    PunksOrchestraAgent,
    RuntimeResult,
    TrialHandle,
...
async def test_missing_integrations_fail_explicitly(tmp_path, manifest_data):
    agent = PunksOrchestraAgent(logs_dir=tmp_path, manifest=manifest_data)
...
async def test_cli_runtime_construction_from_json(tmp_path, manifest_data):
    ...
    agent = PunksOrchestraAgent(
        ...
        punks_acp_binary="/pinned/punks-acp",
        punks_agent_binary="/pinned/punks-agent",
        punks_dev_mcp_binary="/pinned/punks-dev-mcp",
        punks_cli_binary="/pinned/punks",
    )
    assert agent.runtime.artifact_root == tmp_path
    assert agent.runtime.endpoints["frontier/rev"].provider == "anthropic"
    assert agent.runtime.punks_acp_binary == "/pinned/punks-acp"
    assert agent.runtime.punks_agent_binary == "/pinned/punks-agent"
    assert agent.runtime.punks_dev_mcp_binary == "/pinned/punks-dev-mcp"
    assert agent.runtime.punks_cli_binary == "/pinned/punks"
