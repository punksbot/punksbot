# Harbor Punks Orchestra

A stock-Harbor custom agent that runs a manifest-defined team through the real
Punks stack. Harbor sees one `PunksOrchestraAgent`; behind that adapter, one
...
`punks-acp` → `punks-agent` → `punks-dev-mcp` process tree the desktop app
launches: the production MCP toolset (shell, file tools, todo) with the
`punks` CLI on the shell's PATH. No Harbor fork or patch is required.
...
```bash
uv run --project benchmarks/harbor-punks-orchestra/testbed harbor run --yes -p <TASK_OR_DIRECTORY> --agent harbor_punks_orchestra:PunksOrchestraAgent --agent-kwarg manifest=<CONDITION.yaml> --agent-kwarg provisioner_factory=harbor_punks_testbed:provisioner_from_dict --agent-kwarg provisioner_config=<PROVISIONER.json> --agent-kwarg endpoint_config=<ENDPOINTS.json> --agent-kwarg artifact_root=benchmarks/harbor-punks-orchestra --agent-kwarg punks_acp_binary=<LINUX_BIN>/punks-acp --agent-kwarg punks_agent_binary=<LINUX_BIN>/punks-agent --agent-kwarg punks_dev_mcp_binary=<LINUX_BIN>/punks-dev-mcp --agent-kwarg punks_cli_binary=target/debug/punks --agent-kwarg run_id="bench-$(date -u +%Y%m%dT%H%M%SZ)" --agent-timeout-multiplier 15 --n-concurrent 1
```

`punks_acp_binary`/`punks_agent_binary`/`punks_dev_mcp_binary` must be **Linux**
...
any Linux base image works). `punks_cli_binary` is the **host** CLI the harness
...
Each trial gets fresh keys and a private Punks channel. The provisioner archives
rather than deletes that channel, leaving the relay/Postgres event timeline
and the per-agent acp/agent logs (downloaded into the trial's `punks/`
artifacts) available for analysis.

## Leaderboard runs

`just benchmark` is the one-command path: it stands up a dedicated Docker
stack (`punks-benchmark` compose project — relay :3600, Postgres :5633, secrets
...
relay membership list and opens the Punks desktop app logged in as them, so
...
uv run --project benchmarks/harbor-punks-orchestra/testbed \
    benchmarks/harbor-punks-orchestra/scripts/run_leaderboard.py \
    --dataset terminal-bench/terminal-bench-2-1 \
    --attempts 5 \
    --manifest benchmarks/harbor-punks-orchestra/manifests/<TEAM>.yaml \
    --endpoint-config benchmarks/harbor-punks-orchestra/testbed/endpoints/<ENDPOINTS>.json \
...
```bash
cd benchmarks/harbor-punks-orchestra
