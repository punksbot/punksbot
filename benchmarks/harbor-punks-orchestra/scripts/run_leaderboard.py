...
importable:
    ...
    uv run --project benchmarks/harbor-punks-orchestra/testbed \
        ...
        --agent-bin-dir <DIR with Linux punks-acp/punks-agent/punks-dev-mcp>
"""
...
AGENT_IMPORT = "harbor_punks_orchestra:PunksOrchestraAgent"
PROVISIONER_FACTORY = "harbor_punks_testbed:provisioner_from_dict"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="JSON config for the Punks relay/Postgres provisioner",
    )
    parser.add_argument(
        "--punks-bin-dir",
        type=Path,
        default=None,
        help="Directory with the host punks CLI (default: repo target/release, then target/debug)",
    )
    parser.add_argument(
        "--agent-bin-dir",
        type=Path,
        required=True,
        help="Directory with Linux builds of punks-acp/punks-agent/punks-dev-mcp "
...
def find_binaries(bin_dir: Path | None) -> dict[str, Path]:
    ...
    raise SystemExit(
        f"punks binaries not found (need {', '.join(BINARIES)}; searched {searched}). "
        "Build them with `cargo build` or pass --punks-bin-dir."
    )
...
def build_command(
    ...
    kwargs = {
        ...
        "punks_acp_binary": agent_binaries["punks-acp"],
        "punks_agent_binary": agent_binaries["punks-agent"],
        "punks_dev_mcp_binary": agent_binaries["punks-dev-mcp"],
        "punks_cli_binary": binaries["punks"],
...
def write_metadata_template(args: argparse.Namespace, job_dir: Path) -> Path:
    ...
    metadata = {
        "agent_url": "https://github.com/punksbot/punksbot",
        "agent_display_name": f"Punks Orchestra ({manifest.get('condition', 'team')})",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        bin_dir = args.punks_bin_dir or PACKAGE_ROOT.parents[1] / "target" / "release"
        ...
        return 0
    binaries = find_binaries(args.punks_bin_dir)
