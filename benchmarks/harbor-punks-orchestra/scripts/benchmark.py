#!/usr/bin/env python3
"""One-command benchmark: bring up the Punks stack in Docker and run it.
...
- A dedicated ``punks-benchmark`` compose project reusing the production
...
- ``--gui`` adds that user to the relay membership list and opens the Punks
...
Run inside the testbed environment (the just recipe does this):
    ...
    uv run --project benchmarks/harbor-punks-orchestra/testbed \
        benchmarks/harbor-punks-orchestra/scripts/benchmark.py [--gui] [...]
"""
...
COMPOSE_PROJECT = "punks-benchmark"
COMPOSE_FILES = (
...
GUI_BUNDLE_IDENTIFIER = "xyz.block.punks.app.benchmark"
...
AGENT_BINARIES = ("punks-acp", "punks-agent", "punks-dev-mcp")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--gui",
        action="store_true",
        help="Open the Punks desktop app as the benchmark user to watch the run live",
    )
    parser.add_argument(
        "--fresh",
        action="store_true",
...
def load_state() -> dict[str, str]:
    ...
    from harbor_punks_testbed.keys import generate_keypair, keypair_from_secret
...
def print_user_identity(state: dict[str, str]) -> None:
    ...
    nothing beyond it)."""
    from harbor_punks_testbed.keys import encode_nsec
...
def write_env_file(state: dict[str, str]) -> Path:
    ...
    lines = {
        "PUNKS_IMAGE": os.environ.get("PUNKS_IMAGE", "ghcr.io/punksbot/punksbot:main"),
        "PUNKS_DOMAIN": "localhost",
        "RELAY_URL": f"ws://localhost:{RELAY_HTTP_PORT}",
        "PUNKS_MEDIA_BASE_URL": f"http://localhost:{RELAY_HTTP_PORT}/media",
        "PUNKS_MEDIA_SERVER_DOMAIN": "localhost",
        "PUNKS_CORS_ORIGINS": f"http://localhost:{RELAY_HTTP_PORT}",
        "PUNKS_REQUIRE_AUTH_TOKEN": "true",
        "PUNKS_REQUIRE_RELAY_MEMBERSHIP": "true",
        "PUNKS_ALLOW_NIP_OA_AUTH": "true",
        "PUNKS_AUTO_MIGRATE": "true",
        "PUNKS_GIT_CONFORMANCE_PROBE": "true",
        "RUST_LOG": "punks_relay=info,punks_db=info,punks_auth=info",
        "RELAY_OWNER_PUBKEY": state["owner_pubkey"],
        "PUNKS_RELAY_PRIVATE_KEY": state["relay_private_key"],
        "PUNKS_GIT_HOOK_HMAC_SECRET": state["git_hook_hmac_secret"],
        "POSTGRES_DB": "punks",
        "POSTGRES_USER": "punks",
        "POSTGRES_PASSWORD": state["postgres_password"],
        "REDIS_PASSWORD": state["redis_password"],
        "PUNKS_S3_ACCESS_KEY": state["s3_access_key"],
        "PUNKS_S3_SECRET_KEY": state["s3_secret_key"],
        "PUNKS_S3_BUCKET": "punks-media",
        "PUNKS_HTTP_PORT": str(RELAY_HTTP_PORT),
        "PUNKS_PG_HOST_PORT": str(PG_HOST_PORT),
        "PUNKS_METRICS_HOST_PORT": str(METRICS_HOST_PORT),
    }
...
def postgres_dsn(state: dict[str, str]) -> str:
    return (
        f"postgresql://punks:{state['postgres_password']}@127.0.0.1:{PG_HOST_PORT}/punks"
    )
...
def ensure_binaries() -> dict[str, Path]:
    """Find the host punks CLI, building it once if missing."""
    try:
        return run_leaderboard.find_binaries(None)
    except SystemExit:
        print("host punks CLI missing — building (cargo build, first run only)...")
    cargo = REPO_ROOT / "bin" / "cargo"
    subprocess.run(
        [str(cargo), "build", "-p", "punks-cli"],
        cwd=REPO_ROOT,
        check=True,
    )
    return run_leaderboard.find_binaries(None)
...
def ensure_agent_binaries() -> Path:
    ...
    punks-acp → punks-agent → punks-dev-mcp stack, so the binaries must be
...
def launch_gui(state: dict[str, str]) -> subprocess.Popen:
    """Open the Punks desktop app logged in as the benchmark user.

    The relay runs closed (membership required), so the user pubkey is first
    added to the relay membership list via punks-admin inside the container —
    ...
    for name in (
        ...
        if not stub.exists():
            stub.touch()
    real_cli = sidecar_dir / f"punks-{triple}"
    real_cli.write_bytes(binaries["punks"].read_bytes())
    real_cli.chmod(0o755)

    print(
        f"Opening Punks GUI as the benchmark user ({state['user_pubkey'][:16]}…).\n"
    ...
    # workspace's relay URL overrides PUNKS_RELAY_URL by design. Reusing the
    ...
    tauri_config = json.dumps(
        {"identifier": GUI_BUNDLE_IDENTIFIER, "productName": "Punks Benchmark"}
    )
    return subprocess.Popen(
        ...
        env={
            **os.environ,
            "PUNKS_RELAY_URL": f"ws://localhost:{RELAY_HTTP_PORT}",
            "PUNKS_PRIVATE_KEY": state["user_secret_key"],
        },
    )
...
def leaderboard_argv(
    ...
    argv += [
        ...
        # PUNKS_BENCHMARK_DOCKER_HOST if your engine exposes the host
        # differently.
        "--relay-gateway",
        (
            f"{os.environ.get('PUNKS_BENCHMARK_DOCKER_HOST', 'host.docker.internal')}"
            f":{RELAY_HTTP_PORT}"
        ),
        "--n-concurrent",
