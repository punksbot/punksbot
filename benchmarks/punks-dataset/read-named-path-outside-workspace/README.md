...
reading it. See punksbot/punksbot#6261.

## Environment

`python:3.12-slim-bookworm` with `HOME=/home/punks`, so `~` in the instruction
...
Reads the post-agent `/logs/artifacts/punks-evidence.json` snapshot plus the
...
```bash
just benchmark \
  --path benchmarks/punks-dataset/read-named-path-outside-workspace \
  --attempts 1 \
...
`../harbor-punks-orchestra/tests/test_read_named_path_outside_workspace_verifier.py`.
