...
`punks-agent` stack against a dedicated relay, and the agent does all its work
through `punks channels create` / `channels invite`. Agent timeout 300s.

Directory identities are derived deterministically from the owner key
(`PunksTrialProvisioner._stable_credential`) without persisting any secret, and
`_seed_directory` skips profiles already published — so reruns are idempotent
and pubkeys are stable across trials.

## Verifier

Reads the post-agent `/logs/artifacts/punks-evidence.json` snapshot. The
...
```bash
just benchmark \
  --path benchmarks/punks-dataset/create-channel-invite-users \
  --attempts 1 \
...
`../harbor-punks-orchestra/tests/test_create_channel_invite_users_verifier.py`.
