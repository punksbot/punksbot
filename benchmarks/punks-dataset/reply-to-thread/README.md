...
> the behavior under test, and it must come from `punks-acp`'s production base
...
container's shell. `PunksOrchestraAgent` launches the real `punks-acp` /
`punks-agent` stack against a dedicated relay, and the agent works entirely
through Punks. Agent timeout 300s; the manifest's `trial_budget` is the
effective clock.

## Verifier

Reads the post-agent `/logs/artifacts/punks-evidence.json` snapshot (written by
`PunksContainerRuntime._collect_evidence` after the agent stops, so the agent
...
```bash
just benchmark \
  --path benchmarks/punks-dataset/reply-to-thread \
  --attempts 1 \
  --manifest benchmarks/harbor-punks-orchestra/manifests/punks-native-solo-luna.yaml \
  --endpoint-config benchmarks/harbor-punks-orchestra/testbed/endpoints/openai-live.json \
  --n-concurrent 1
```

`harbor run -a oracle` does **not** work here, and no `solution/solve.sh` is
shipped: the Oracle agent replaces `PunksOrchestraAgent`, so no relay trial is
...
`../harbor-punks-orchestra/tests/test_reply_to_thread_verifier.py` (run from
`benchmarks/harbor-punks-orchestra`: `uv run --extra dev pytest -q`).
