...
gets a real Punks notification instead of a message they have to notice.

> **The instruction deliberately says nothing about mentioning anyone.** The
> mention is the behavior under test and must come from `punks-acp`'s production
...
`punks-agent` stack against a dedicated relay. Agent timeout 300s.

## Verifier

Reads the post-agent `/logs/artifacts/punks-evidence.json` snapshot. Every
...
```bash
just benchmark \
  --path benchmarks/punks-dataset/user-mention \
  --attempts 1 \
...
`../harbor-punks-orchestra/tests/test_user_mention_verifier.py`.
