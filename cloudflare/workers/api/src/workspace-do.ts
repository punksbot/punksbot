...
export class WorkspaceDO extends DurableObject<ApiEnv> {
  ...
  async execute(input: unknown): Promise<WorkspaceExecuteResult> {
        ...
        : contract === "workspace.rename@1"
          ...
          : contract === "workspace.member-set-role@1"
            ...
            : contract === "workspace.member-remove@1"
              ...
              : null;
    if (contractId === null || !validateContract(contractId, input).valid) {
    ...
    const command = input as WorkspaceCommand;
      ...
      this.ctx.storage.sql.exec(
        `INSERT INTO pending_command
          (singleton, command_id, payload_hash, command_json, unsigned_json,
           next_state_json, chunks_json, reduction_overlay, attempts, created_at)
         VALUES (1, ?, ?, ?, ?, ?, ?, ?, 0, ?)`,
        command.commandId,
        payloadHash,
        commandJson,
    ...
    return this.ctx.storage.sql
      .exec<PendingRow>(
        `SELECT command_id, payload_hash, command_json, unsigned_json, next_state_json,
                chunks_json, reduction_overlay, attempts
...
async function validPendingWorkspaceCommand(
  ...
  try {
    ...
    if (chunks.length === 0) {
      const legacy = decideWorkspaceCommand(current, command, context);
      return (
        canonicalJson(legacy.nextState) === canonicalJson(workspace) &&
        canonicalJson(legacy.event) === canonicalJson(unsigned)
      );
    }
    const decision = await decideWorkspaceCommandV2(current, command, context);
