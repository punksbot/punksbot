...
export class EmailClaimDO extends DurableObject<AuthEnv> {
    ...
    if (row === undefined) {
      this.ctx.storage.sql.exec(
        `INSERT INTO email_claim
          (singleton, email_hash, punk_id, transaction_id, status, attempts, created_at, updated_at)
         VALUES (1, ?, ?, ?, 'pending', 0, ?, ?)`,
        input.emailHash,
        input.punkId,
        input.transactionId,
  ...
  activate(input: { punkId: string; transactionId: string }): boolean {
    ...
    this.ctx.storage.sql.exec(
      "UPDATE email_claim SET status = 'active', updated_at = ? WHERE singleton = 1",
  ...
  release(input: { punkId: string; transactionId: string }): boolean {
    ...
    this.ctx.storage.sql.exec("DELETE FROM email_claim WHERE singleton = 1");
    return true;
  }
  ...
  override async alarm(): Promise<void> {
    ...
    if (await punk.hasVerifiedEmail(String(row.email_hash))) {
      this.activate({
        punkId: String(row.punk_id),
    ...
    this.ctx.storage.sql.exec(
      "UPDATE email_claim SET attempts = ?, updated_at = ? WHERE singleton = 1",
      attempts,
      new Date().toISOString(),
    );
  ...
  private row(): ClaimRow | undefined {
    return this.ctx.storage.sql
      .exec<ClaimRow>("SELECT * FROM email_claim WHERE singleton = 1")
      .toArray()[0];
  }
