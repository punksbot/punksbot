...
| Punk authentication | Google/GitHub/passkey backend plus the recoverable native desktop ceremony, quarantine, confirmed rotation and revoke-only queue are implemented and tested in source, not deployed | Add explicit two-account merge, abuse controls and operator key/secret rotation; then produce the signed installed-app and exact staging evidence before promotion. |
