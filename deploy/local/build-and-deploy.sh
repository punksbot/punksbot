#!/usr/bin/env bash
# Local docker-desktop k8s testbed for the Punks relay mesh.
#
...
NS="${NS:-punks-mesh}"
RELEASE="${RELEASE:-punks}"
IMAGE_REPO="${IMAGE_REPO:-punks-relay}"
IMAGE_TAG="${IMAGE_TAG:-mesh-local}"
CHART="${REPO_ROOT}/deploy/charts/punks"
...
if [ "$helm_rc" != 0 ]; then
  ...
  kubectl -n "$NS" describe pods -l app.kubernetes.io/name=punks | tee "$EVID/describe-onfail.txt"
  kubectl -n "$NS" logs -l app.kubernetes.io/name=punks --tail=100 --all-containers | tee "$EVID/logs-onfail.txt"
  die "helm install returned rc=$helm_rc"
fi

# ── 4. verify 3/3 Ready ───────────────────────────────────────────────────────
# Find the relay Deployment: everything under this release named "punks" except
...
# chart name, so the name isn't always "<release>-punks".)
DEPLOY=""
...
# The bundled MinIO + init pods share app.kubernetes.io/name=punks, so select by
...
RELAY_PODS=$(kubectl -n "$NS" get pods \
  -l "app.kubernetes.io/name=punks,app.kubernetes.io/instance=$RELEASE" \
