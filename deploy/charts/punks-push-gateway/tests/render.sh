...
helm lint deploy/charts/punks-push-gateway >/dev/null
helm template push deploy/charts/punks-push-gateway >"$out"
# Production values must attach push.punks.xyz to an explicit Gateway.
production_args=(
  -f deploy/charts/punks-push-gateway/values-production.yaml
  --set 'image.digest=sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
  --set 'appAttestAppId=REALTEAM.xyz.punks'
...
helm lint deploy/charts/punks-push-gateway "${production_args[@]}" >/dev/null
helm template push deploy/charts/punks-push-gateway "${production_args[@]}" >"$production_out"
...
runtime={'app.kubernetes.io/name':'punks-push-gateway','app.kubernetes.io/instance':'push','app.kubernetes.io/component':'runtime'}
...
assert jenv['PUNKS_PUSH_RUNTIME_DATABASE_ROLE']['value']=='punks_push_gateway_runtime'
...
required={'DATABASE_URL','PUNKS_PUSH_APNS_KEY_ID','PUNKS_PUSH_APNS_TEAM_ID','PUNKS_PUSH_APNS_TOPIC','PUNKS_PUSH_GRANT_KEYS','PUNKS_PUSH_TOKEN_KEYS','PUNKS_PUSH_MAX_GRANT_LIFETIME_SECONDS'}
assert required <= env
...
np=next(x for x in nps if x['metadata']['name']=='push-punks-push-gateway')
migration_np=next(x for x in nps if x['metadata']['name']=='push-punks-push-gateway-migration')
...
assert 'push.punks.xyz' in route['spec']['hostnames']
PY

# Enabling a route without a Gateway attachment must fail schema validation.
if helm template push deploy/charts/punks-push-gateway --set httpRoute.enabled=true >/dev/null 2>&1; then
...
if helm template push deploy/charts/punks-push-gateway -f deploy/charts/punks-push-gateway/values-production.yaml >/dev/null 2>&1; then
...
helm template push deploy/charts/punks-push-gateway \
...
np=next(x for x in xs if x and x.get('kind')=='NetworkPolicy' and x['metadata']['name']=='push-punks-push-gateway')
...
if helm template push deploy/charts/punks-push-gateway \
  --set podMonitor.enabled=true \
